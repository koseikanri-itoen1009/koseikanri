...
#  ${CMD_JP1PFM} "*" | sed 1,3d |  sed "s/ ¥{2,¥}/ /g" | awk '{if($7 != ""){print $7} else {print $5}}' | sed "/^$/d" | grep -v Active
  ${CMD_JP1PFM} "*" | grep -v KAVE06003-I
  echo
  ${CMD_JP1PFM} "*" | grep -v KAVE06003-I | sed 1,3d |  sed "s/ ¥{2,¥}/ /g" | awk '{if($7 != ""){print $7} else {print $5}}' | sed "/^$/d" | grep -v Active
  RET=$?
  if [ ${RET} -eq 0 ]; then
    ${CMD_JP1PFM} "*"
