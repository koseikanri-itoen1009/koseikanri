...
##                                                                            ##
##   [使用方法]                                                               ##
##      /uspg/jp1/zc/shl/<環境依存値>/ZCZZ_NAS_LOGMNG.ksh                     ##
##                                                                            ##
################################################################################
...
then
   echo "ZCZZ00003:[Error] ZCZZNASDELFILE.env が存在しない、または見つかりません。 HOST=${L_hosutomei}" \
        | /usr/bin/fold -w 75 | /usr/bin/tee -a ${L_rogumei} 1>&2
...
do
   ...
   then
      ...
      then
##2014/07/31 S.Takahashi Mod Start
         L_furupasufilelist=`find ${L_direkutori} -name ${L_fmei} -print`
...
#         /bin/mv ${L_direkutori}/${L_fmei} ${L_direkutori}/${L_fmei}.${L_lhizuke} 2> ${TE_ZCZZHYOUJUNERA}
            /bin/mv ${L_furupasufilemei} ${L_furupasufilemei}.${L_lhizuke} 2> ${TE_ZCZZHYOUJUNERA}
