...
#処理開始メッセージの出力
PARM="`echo $#`"
echo ${AJSJOBNAME}  `date '+%Y/%m/%d %H:%M:%S'`  "ホスト名："$HOST_NM"のジョブ実行環境定義：jbslistuserの出力を開始します" >> $LOG_FILE
...
#環境変数ファイルの読み込み（変数設定）
. $INIT_FILE
#TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbslistuser_"$DATE".txt
TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbslistuser.txt

#定義ファイル出力
/opt/jp1base/bin/jbslistuser -h $HOST_NM -ld > $TEIGI_FILENM
RC="`echo $?`"
if(( "$RC" == 0 ))
  then
