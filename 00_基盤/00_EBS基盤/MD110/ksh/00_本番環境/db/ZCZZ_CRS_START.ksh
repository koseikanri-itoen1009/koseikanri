...
##2021/09/30 Hitachi,Ltd Mod Start
    ...
    if [ `/bin/grep ${L_crschk_str} ${TE_ZCZZHYOUJUNSHUTURYOKU} | /usr/bin/wc -l` -eq ${L_crschk_num} ]
##2021/09/30 Hitachi,Ltd Mod End
      then
...
##2021/09/30 Hitachi,Ltd Mod Start
        ...
        if [ `/bin/grep -E "${L_crsstat_str}" ${TE_ZCZZHYOUJUNSHUTURYOKU} | /usr/bin/wc -l` -eq ${L_crsstat_num} ]
##2021/09/30 Hitachi,Ltd Mod End
          then
            ##CRS起動確認終了(ループ終了)
##2021/09/30 Hitachi,Ltd Mod Start
