...
##  L_kurinfairu=`/usr/bin/dirname $0`"/${L_kurinfairumei}"                                           ##コンカレントマネージャ管理表のクリーンアップスクリプト名(フルパス)
  L_kurinfairu=`dirname $0`"/${L_kurinfairumei}"                                           ##コンカレントマネージャ管理表のクリーンアップスクリプト名(フルパス)
##2014/07/31 S.Takahashi Add End
##2022/01/06 S.Yamada Mod End      ※ E_本稼動_17512対応
...
#    if [ `/usr/bin/cat ${TE_ZCZZHYOUJUNSHUTURYOKU}` -lt 3 ]
    /bin/ps -ef | grep `/usr/bin/whoami` | /bin/egrep "FNDLIBR|FNDSM|FNDIMON" | /bin/grep -v "grep" | /usr/bin/wc -l > ${TE_ZCZZHYOUJUNSHUTURYOKU}
    if [ `/bin/cat ${TE_ZCZZHYOUJUNSHUTURYOKU}` -lt 3 ]
##2022/01/06 S.Yamada Mod End      ※ E_本稼動_17512対応
      then
