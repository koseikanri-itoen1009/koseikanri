...
#環境変数ファイルの読み込み（変数設定）
. $INIT_FILE
#TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbslistacl_"$DATE".txt
TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbslistacl.txt

#定義ファイル出力
/opt/jp1base/bin/jbslistacl -h $HOST_NM > $TEIGI_FILENM
RC="`echo $?`"
if(( "$RC" == 0 ))
  then
