...
#処理開始メッセージの出力
PARM="`echo $#`"
echo ${AJSJOBNAME}  `date '+%Y/%m/%d %H:%M:%S'`  "ホスト名："$HOST_NM"のJP1アクセス権限レベル及びJP1ユーザ権限レベル定義情報の出力を開始します" >> $LOG_FILE
...
#環境変数ファイルの読み込み（変数設定）
. $INIT_FILE
#TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbsacllint_"$DATE".txt
TEIGI_FILENM="$TEIGI_DIR_HULFTLOGI""$HOST_NM"_jbsacllint.txt

#定義ファイル出力
/opt/jp1base/bin/jbsacllint -h $HOST_NM > $TEIGI_FILENM
RC="`echo $?`"
if(( "$RC" == 0 ))
  then
