...
if(( $# == 0 ))
  then
   echo ${AJSJOBNAME}  `date '+%Y/%m/%d %H:%M:%S'`  "ホスト名："$HOST_NM"のJP1ユーザ定義：jbslistusrの出力を開始します" >> $LOG_FILE
fi
...
#環境変数ファイルの読み込み（変数設定）
. $INIT_FILE
#TEIGI_FILENM="$TEIGI_DIR""$HOST_NM"_jbslistuser_"$DATE".txt
TEIGI_FILENM="$TEIGI_DIR""$HOST_NM"_jbslistuser.txt

#定義ファイル出力
/opt/jp1base/bin/jbslistuser -ld > $TEIGI_FILENM
RC="`echo $?`"
if(( "$RC" == 0 ))
  then
