...
################################################################################
# シェル機能:クラスタ用NFS 監視ShellScript(bebsfssv00)
# シェル名  :fsnfsjp1_app_check.sh
# 戻り値    : 0=確認結果正常
#             0以外=確認結果異常
# 更新履歴  :2008.04.04新規作成
#			:2009.7.14 キューレス監視追加
...
command4="/sbin/service portmap status"	            # portmap状態確認
command5="/sbin/service rpcidmapd status"	        # rpcidmapd状態確認
command6="/sbin/service nfs status"	                # nfsサービス状態確認
command7="/sbin/service nfslock status"	            # nfslock状態確認
