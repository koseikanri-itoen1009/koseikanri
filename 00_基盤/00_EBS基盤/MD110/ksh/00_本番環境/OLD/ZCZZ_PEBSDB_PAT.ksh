...
##                                                                            ##
##   [ジョブ名]                                                               ##
##      ページ・アクセス・トラッキング（PAT）のデータ収集ジョブ               ##
##                                                                            ##
##   [概要]                                                                   ##
##      コンカレント「ページ・アクセス追跡データの移行」を実施する。          ##
##                                                                            ##
##   [作成/更新履歴]                                                          ##
##        作成者  ：   SCS 北河              2009/12/01 1.0.1                 ##
##        更新履歴：   SCS 北河              2009/12/01 1.0.1                 ##
##                       初版                                                 ##
##                                                                            ##
##   [戻り値]                                                                 ##
##      0 : 正常                                                              ##
...
L_rogumei="${L_rogupasu}/ZCZZ_DB_PAT${L_hosutomei}${L_hizuke}.log"   #ログ名
...
L_konkarentomei="JTFPFDATA"                #プログラムアプリケーション名
...
if [ $? -ne 0 ]
then
   echo ${TE_ZCZZ01115} | /usr/bin/fold -w 75 | /usr/bin/tee -a ${L_rogumei} 1>&2
...
then
   echo ${TE_ZCZZ01116} "STATUS_CD="${L_konkarento_jyoutai} | /usr/bin/fold -w 75 | /usr/bin/tee -a ${L_rogumei} 1>&2
   /usr/bin/cat ${TE_ZCZZHYOUJUNSHUTURYOKU} >> ${L_rogumei}
   L_shuryo ${TE_ZCZZIJOUSHURYO}
fi
