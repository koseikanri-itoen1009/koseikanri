...
--
  ...
  -- グローバル定数
  -- ==================================================
  gc_pkg_name             CONSTANT VARCHAR2(20) := 'XXWIP730004C' ;      -- パッケージ名
  gc_report_id            CONSTANT VARCHAR2(20) := 'XXWIP730004T' ;      -- 帳票ID
...
-- E 2008/07/24 1.6 MOD BY S.Takemoto---------------------------------------------------------- E --
            ...
            ,TO_CHAR( xd.judgement_date, 'YYYY/MM/DD' )     AS judge_date_c     -- 決済日（文字）
            ,NVL( xdl.whs_code, xd.whs_code )               AS whs_code         -- 倉庫
            ,NVL( TO_CHAR( xdl.ship_date, 'MM/DD' )
                 ,TO_CHAR( xd.ship_date , 'MM/DD' ) )       AS ship_date        -- 発日
            ,NVL( TO_CHAR( xdl.arrival_date, 'MM/DD' )
                 ,TO_CHAR( xd.arrival_date , 'MM/DD' ) )    AS arrival_date     -- 着日
            ,xd.delivery_no                                 AS delivery_no      -- 配送No
            ,xdl.request_no                                 AS request_no       -- 依頼No
            ,NVL( xdl.invoice_no, xd.invoice_no )           AS invoice_no       -- 送り状No
            ,NVL( xdl.code_division, xd.code_division )     AS code_division    -- 配送先コード区分
            ,NVL( xdl.shipping_address_code
                 ,xd.shipping_address_code  )               AS ship_to_code     -- 配送先
            ,NULL                                           AS ship_to_name     -- 配送先名称
            ,NVL( xd.distance          , 0 )                AS distance_1       -- 距離１
...
-- ##### 20080725 1.8 変更要求結合テスト障害対応 END   #####
            ...
            ,NVL( xd.qty1              , 0 )                AS qty_sum          -- 数量計
            ,NVL( xdl.delivery_weight  , 0 )                AS weight           -- 重量
            ,NVL( xd.delivery_weight1  , 0 )                AS weight_sum       -- 重量計
            ,NVL( xd.charged_amount    , 0 )                AS rate_charged     -- 請求運賃
            ,NVL( xd.contract_rate     , 0 )                AS rate_contract    -- 契約運賃
            ,NVL( xd.consolid_surcharge, 0 )                AS charge_consol    -- 混載割増
            ,NVL( xd.picking_charge    , 0 )                AS charge_picking   -- PIC
            ,NVL( xd.many_rate         , 0 )                AS charge_many      -- 諸料金
            ,NVL( xd.total_amount      , 0 )                AS total_amount     -- 支払合計
            ,NVL( xd.balance           , 0 )                AS balance          -- 差額
            ,NVL( xd.congestion_charge , 0 )                AS charge_congest   -- 通行料
...
--            OR xd.goods_classe        = gr_param.prod_div )               -- 商品区分
      AND   xd.goods_classe           = NVL( gr_param.prod_div, xd.goods_classe )
-- E 2008/05/21 1.1 MOD BY M.Ikeda ------------------------------------------------------------ E --
      AND   xd.delivery_company_code  BETWEEN gr_param.carrier_code_from  -- 運送業者
                                      AND     gr_param.carrier_code_to    --
      AND   xd.whs_code               BETWEEN gr_param.whs_code_from      -- 出庫元倉庫
                                      AND     gr_param.whs_code_to        --
      AND   xd.ship_date              BETWEEN gr_param.ship_date_from     -- 出庫日
                                      AND     gr_param.ship_date_to       --
      AND   xd.arrival_date           BETWEEN gr_param.arrival_date_from  -- 着日
                                      AND     gr_param.arrival_date_to    --
      AND   xd.judgement_date         BETWEEN gr_param.judge_date_from    -- 決済日
                                      AND     gr_param.judge_date_to      --
-- S 2008/05/23 1.1 MOD BY M.Ikeda ------------------------------------------------------------ S --
--      AND   xd.report_date            BETWEEN gr_param.report_date_from   -- 報告日
...
-- E 2008/05/23 1.1 MOD BY M.Ikeda ------------------------------------------------------------ E --
      AND   xd.delivery_no            BETWEEN gr_param.delivery_no_from   -- 配送No
...
--
      AND ( EXISTS ( SELECT  1
                    ...
                    AND   (  gr_param.request_no_to      = lc_request_no_max
                          OR xdl_reqno.request_no        IS NOT NULL          )
                    AND   xdl_reqno.delivery_no          = xd.delivery_no     )       -- 配送No
...
--            OR xd.order_type              = gr_param.order_type )         -- 受注タイプ
--      AND   (  gr_param.wc_class         IS NULL
--            OR xd.weight_capacity_class   = gr_param.wc_class )           -- 重量容積区分
