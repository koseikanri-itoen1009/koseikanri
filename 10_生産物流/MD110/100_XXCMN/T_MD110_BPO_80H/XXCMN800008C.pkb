CREATE OR REPLACE PACKAGE BODY xxcmn800008c
AS
/*****************************************************************************************
 * Copyright(c)Oracle Corporation Japan, 2008. All rights reserved.
 *
 * Package Name     : xxcmn800008c(body)
 ...
 *  Name                 Description
 * -------------------- ------------------------------------------------------------
 *  get_carry_mst          運送業者マスタ取得プロシージャ (H-1)
 *  output_csv             CSVファイル出力プロシージャ (H-2)
 *  upd_last_update        最終更新日時ファイル更新プロシージャ (H-3)
 *  wf_notif               Workflow通知プロシージャ (H-4)
 *  submain                メイン処理プロシージャ
 ...
 * ------------- ----- ---------------- -------------------------------------------------
 *  2008/01/09    1.0  Oracle 椎名 昭圭  初回作成
 ...
 *  2008/09/19    1.7  Oracle 山根 一浩  T_S_460,T_S_453,T_S_575,T_S_559対応
 *
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  gv_pkg_name         CONSTANT VARCHAR2(100) := 'xxcmn800008c';      -- パッケージ名
...
--
  -- 運送業者マスタ情報を格納するレコード
  TYPE carry_mst_rec IS RECORD(
    party_id               hz_parties.party_id%TYPE,                  -- パーティID
    freight_code           wsh_carriers.freight_code%TYPE,            -- 組織番号
...
--
  -- 運送業者マスタ情報を格納するテーブル型の定義
  TYPE carry_mst_tbl IS TABLE OF carry_mst_rec INDEX BY PLS_INTEGER;
--
  -- ===============================
  -- ユーザー定義グローバル変数
  -- ===============================
  gt_carry_mst_tbl   carry_mst_tbl;                         -- 結合配列の定義
  gr_outbound_rec    xxcmn_common_pkg.outbound_rec;         -- ファイル情報のレコードの定義
  gd_sysdate         DATE;                                  -- システム現在日付
--
  /**********************************************************************************
   * Procedure Name   : get_carry_mst
   * Description      : 運送業者マスタ取得(H-1)
   ***********************************************************************************/
  PROCEDURE get_carry_mst(
  ...
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'get_carry_mst'; -- プログラム名
--
--#####################  固定ローカル変数宣言部 START   ########################
--
...
--
    SELECT hp.party_id,                 -- パーティID
           ...
           CASE
             -- 最終更新日時がパーティサイトアドオンマスタよりパーティサイトマスタの方が新しい場合
             WHEN (hp.last_update_date >= xp.last_update_date) THEN
               hp.last_update_date     -- パーティサイトマスタ最終更新日時
             ELSE
               xp.last_update_date     -- パーティサイトアドオンマスタ最終更新日時
           END
    BULK COLLECT INTO gt_carry_mst_tbl
    FROM  hz_parties              hp,   -- パーティマスタ
          xxcmn_parties           xp,   -- パーティアドオンマスタ
          wsh_carriers            wc,   -- 運送業者マスタ
          xxwip_delivery_company  xdc   -- 運送業者アドオンマスタ
    WHERE xdc.goods_classe = NVL(iv_syohin, xdc.goods_classe)
    AND   ((xdc.end_date_active     >= gd_sysdate) AND
            (xdc.start_date_active  <= gd_sysdate))
    AND   wc.freight_code           =  xdc.delivery_company_code
    ...
    AND   (EXISTS (
            ...
            AND    hp1.last_update_date >= ld_last_update
            AND    hp1.last_update_date < gd_sysdate
            AND    ROWNUM = 1)
          OR
          EXISTS (
            SELECT 1
            FROM   xxcmn_parties  xp1
            WHERE  xp1.party_id = hp.party_id
            AND    xp1.last_update_date >= ld_last_update
            AND    xp1.last_update_date < gd_sysdate
            AND    ROWNUM = 1)
          )
    ORDER BY wc.freight_code, xp.start_date_active;
--
    --==============================================================
...
--
      ...
      END IF;
      -- 運送業者マスタ情報が取得できている場合
      IF (gt_carry_mst_tbl.COUNT <> 0) THEN
        <<gt_carry_mst_tbl_loop>>
        FOR i IN gt_carry_mst_tbl.FIRST .. gt_carry_mst_tbl.LAST LOOP
...
--
          ...
          END IF;
--
        END LOOP gt_carry_mst_tbl_loop;
      END IF;
      -- CSVファイルへ出力する場合
      IF (iv_file_type = gv_csv_file) THEN
...
--
    ...
    END IF;
--
    gn_target_cnt := gt_carry_mst_tbl.COUNT;
--
    -- ===============================
    -- CSVファイル出力 (H-2)
    -- ===============================
...
--
    ...
    END IF;
--
    gn_normal_cnt := gt_carry_mst_tbl.COUNT;
--
    -- ===============================
    -- 最終更新日時ファイル更新 (H-3)
    -- ===============================
