...
--$Header: GMIPITMB.pls 115.5 99/08/23 07:38:26 porting shi $
...
--|    11/NOV/2008  Y.Suzuki      Bug of not set the who to fix              |
--|                                                                          |
--+==========================================================================+
...
-- Ensure Upper-case columns are converted
  ...
  l_item_um  := UPPER(p_item_rec.item_um);
  l_item_um2 := UPPER(p_item_rec.item_um2);
...
l_ic_item_mst_rec.fill_um            := UPPER(p_item_rec.fill_um);
...
-- Commodity Code                                                               
  IF NOT GMI_VALID_GRP.Validate_commodity_code (  p_item_rec.commodity_code     
                                                , SY$INTRASTAT                  
                                               )                                
  THEN                                                                          
    FND_MESSAGE.SET_NAME('GMI','IC_API_INVALID_COMMODITY_CODE');                
    FND_MESSAGE.SET_TOKEN('ITEM_NO',l_item_no);                                 
    FND_MSG_PUB.Add;                                                            
    RAISE FND_API.G_EXC_ERROR;                                                  
  END IF;                                                                       
