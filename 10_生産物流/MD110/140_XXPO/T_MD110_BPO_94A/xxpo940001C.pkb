CREATE OR REPLACE PACKAGE BODY xxpo940001c
AS
/*****************************************************************************************
 * Copyright(c)Oracle Corporation Japan, 2008. All rights reserved.
 *
 * Package Name     : xxpo940001c(body)
 ...
 *  Name                      Description
 * ------------------------- ----------------------------------------------------------
 *  init_proc                 関連データ取得 (A-1)
 *  get_upload_data_proc      ファイルアップロードインタフェースデータ取得 (A-2)
 *  check_proc                妥当性チェック (A-3)
 *  set_data_proc             登録データ設定
 *  insert_ve_supply_if_proc  データ登録 (A-4)
 *  submain                   メイン処理プロシージャ
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  gv_pkg_name         CONSTANT VARCHAR2(100) := 'xxpo940001c'; -- パッケージ名
--
...
--
  ...
  -- プロファイル
  gv_c_parge_term_001          CONSTANT VARCHAR2(20)   := 'XXPO_PURGE_TERM_001';
  gv_c_parge_term_name         CONSTANT VARCHAR2(36)   := 'パージ対象期間:出来高実績';
  -- クイックコード タイプ
...
--
  ...
  gv_c_manufactured_date        CONSTANT VARCHAR2(24)   := '生産日';
  gv_c_vendor_code              CONSTANT VARCHAR2(24)   := '取引先コード';
  gv_c_factory_code             CONSTANT VARCHAR2(24)   := '工場コード';
  gv_c_item_code                CONSTANT VARCHAR2(24)   := '品目コード';	
  gv_c_producted_date           CONSTANT VARCHAR2(24)   := '製造日';
  gv_c_koyu_code                CONSTANT VARCHAR2(24)   := '固有記号';
  gv_c_producted_quantity       CONSTANT VARCHAR2(24)   := '出来高数量';
  gv_c_description              CONSTANT VARCHAR2(24)   := '摘要';
...
--
  ...
  gn_c_vendor_code_l            CONSTANT NUMBER         :=   4;    -- 取引先コード
  gn_c_factory_code_l           CONSTANT NUMBER         :=   4;    -- 工場コード
  gn_c_item_code_l              CONSTANT NUMBER         :=   7;    -- 品目コード
  gn_c_koyu_code_l              CONSTANT NUMBER         :=   6;    -- 固有記号
  gn_c_producted_quantity_l     CONSTANT NUMBER         :=  12;    -- 出来高数量
  gn_c_producted_quantity_d     CONSTANT NUMBER         :=   3;    -- 出来高数量(小数点以下)
  gn_c_description_l            CONSTANT NUMBER         :=  50;    -- 摘要
--
...
--
  ...
  TYPE file_data_rec IS RECORD(
    ...
    manufactured_date       VARCHAR2(32767), -- 生産日
    vendor_code             VARCHAR2(32767), -- 取引先コード
    factory_code            VARCHAR2(32767), -- 工場コード
    item_code               VARCHAR2(32767), -- 品目コード
    producted_date          VARCHAR2(32767), -- 製造日
    koyu_code               VARCHAR2(32767), -- 固有記号
    producted_quantity      VARCHAR2(32767), -- 出来高数量
    description             VARCHAR2(32767), -- 摘要
...
--
  -- 登録用PL/SQL表型
  -- 出来高実績情報インタフェースID
  TYPE txns_if_id_type           IS TABLE OF
    xxpo_vendor_supply_txns_if.txns_if_id%TYPE             INDEX BY BINARY_INTEGER;
  -- 会社名
  TYPE corporation_name_type     IS TABLE OF
    xxpo_vendor_supply_txns_if.corporation_name%TYPE       INDEX BY BINARY_INTEGER;
  -- データ種別
  TYPE data_class_type           IS TABLE OF
    xxpo_vendor_supply_txns_if.data_class%TYPE             INDEX BY BINARY_INTEGER;
  -- 伝送用枝番
  TYPE transfer_branch_no_type   IS TABLE OF
    xxpo_vendor_supply_txns_if.transfer_branch_no%TYPE     INDEX BY BINARY_INTEGER;
  -- 生産日
  TYPE manufactured_date_type    IS TABLE OF
    xxpo_vendor_supply_txns_if.manufactured_date%TYPE      INDEX BY BINARY_INTEGER;
  -- 取引先コード
  TYPE vendor_code_type          IS TABLE OF
    xxpo_vendor_supply_txns_if.vendor_code%TYPE            INDEX BY BINARY_INTEGER;
  -- 工場コード
  TYPE factory_code_type         IS TABLE OF
    xxpo_vendor_supply_txns_if.factory_code%TYPE           INDEX BY BINARY_INTEGER;
  -- 品目コード
  TYPE item_code_type            IS TABLE OF
    xxpo_vendor_supply_txns_if.item_code%TYPE              INDEX BY BINARY_INTEGER;
  -- 製造日
  TYPE producted_date_type       IS TABLE OF
    xxpo_vendor_supply_txns_if.producted_date%TYPE         INDEX BY BINARY_INTEGER;
  -- 固有記号
  TYPE koyu_code_type            IS TABLE OF
    xxpo_vendor_supply_txns_if.koyu_code%TYPE              INDEX BY BINARY_INTEGER;
  -- 出来高数量
  TYPE producted_quantity_type   IS TABLE OF
    xxpo_vendor_supply_txns_if.producted_quantity%TYPE     INDEX BY BINARY_INTEGER;
  -- 摘要
  TYPE description_type          IS TABLE OF
    xxpo_vendor_supply_txns_if.description%TYPE            INDEX BY BINARY_INTEGER;
  -- 更新日時
  TYPE last_update_date_type     IS TABLE OF
    xxpo_vendor_supply_txns_if.last_update_date%TYPE      INDEX BY BINARY_INTEGER;
--
  gt_txns_if_id_tab                 txns_if_id_type;
                                                  -- 出来高実績情報インタフェースID
  ...
  gt_manufactured_date_tab          manufactured_date_type;           -- 生産日
  gt_vendor_code_tab                vendor_code_type;                 -- 取引先コード
  gt_factory_code_tab               factory_code_type;                -- 工場コード
  gt_item_code_tab                  item_code_type;                   -- 品目コード
  gt_producted_date_tab             producted_date_type;              -- 製造日
  gt_koyu_code_tab                  koyu_code_type;                   -- 固有記号
  gt_producted_quantity_tab         producted_quantity_type;          -- 出来高数量
  gt_description_tab                description_type;                 -- 摘要
...
--
   ...
   * Description      : ファイルアップロードインタフェースデータ取得 (A-2)
   ***********************************************************************************/
...
--
        ...
        ELSIF  (ln_col = 4) THEN
          fdata_tbl(gn_target_cnt).manufactured_date       := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 5) THEN
          fdata_tbl(gn_target_cnt).vendor_code             := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 6) THEN
          fdata_tbl(gn_target_cnt).factory_code            := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 7) THEN
          fdata_tbl(gn_target_cnt).item_code               := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 8) THEN
          fdata_tbl(gn_target_cnt).producted_date          := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 9) THEN
          fdata_tbl(gn_target_cnt).koyu_code               := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 10) THEN
          fdata_tbl(gn_target_cnt).producted_quantity      := SUBSTR(lv_line, 1, ln_length);
        ELSIF  (ln_col = 11) THEN
          fdata_tbl(gn_target_cnt).description             := SUBSTR(lv_line, 1, ln_length);
...
--
        ...
        -- 生産日
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_manufactured_date,
                                            fdata_tbl(ln_index).manufactured_date,
                                            NULL,
                                            NULL,
...
--
        ...
        -- 工場コード
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_factory_code,
                                            fdata_tbl(ln_index).factory_code,
                                            gn_c_factory_code_l,
                                            null,
                                            xxcmn_common3_pkg.gv_null_ng,
                                            xxcmn_common3_pkg.gv_attr_vc2,
...
--
        ...
        -- 製造日
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_producted_date,
                                            fdata_tbl(ln_index).producted_date,
                                            null,
                                            null,
                                            xxcmn_common3_pkg.gv_null_ng,
                                            xxcmn_common3_pkg.gv_attr_dat,
...
--
        ...
        -- 固有記号
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_koyu_code,
                                            fdata_tbl(ln_index).koyu_code,
                                            gn_c_koyu_code_l,
                                            null,
                                            xxcmn_common3_pkg.gv_null_ok,
                                            xxcmn_common3_pkg.gv_attr_vc2,
...
--
        ...
        -- 出来高数量
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_producted_quantity,
                                            fdata_tbl(ln_index).producted_quantity,
                                            gn_c_producted_quantity_l,
                                            gn_c_producted_quantity_d,
                                            xxcmn_common3_pkg.gv_null_ng,
...
--
        ...
        -- 摘要
        -- ==============================
        xxcmn_common3_pkg.upload_item_check(gv_c_description,
                                            fdata_tbl(ln_index).description,
                                            gn_c_description_l,
                                            NULL,
                                            xxcmn_common3_pkg.gv_null_ok,
                                            xxcmn_common3_pkg.gv_attr_vc2,
...
--
      -- 出来高実績情報インタフェースID採番
      SELECT  xxpo_vendor_sply_txns_if_id_s1.NEXTVAL
      INTO    ln_txns_if_id
      FROM    dual;
--
      -- レコード情報
      -- 出来高実績情報インタフェースID
      gt_txns_if_id_tab(ln_index)           := ln_txns_if_id;
      -- 会社名
      ...
      -- 生産日
      gt_manufactured_date_tab(ln_index)
                                  := TO_DATE(fdata_tbl(ln_index).manufactured_date,'YYYY/MM/DD');
      -- 取引先コード
      gt_vendor_code_tab(ln_index)          := fdata_tbl(ln_index).vendor_code;
      -- 工場コード
      gt_factory_code_tab(ln_index)         := fdata_tbl(ln_index).factory_code;
      -- 品目コード
      gt_item_code_tab(ln_index)            := fdata_tbl(ln_index).item_code;
      -- 製造日
      gt_producted_date_tab(ln_index)
                                     := TO_DATE(fdata_tbl(ln_index).producted_date,'YYYY/MM/DD');
      -- 固有記号
      gt_koyu_code_tab(ln_index)            := fdata_tbl(ln_index).koyu_code;
      -- 出来高数量
...
--
  /**********************************************************************************
   * Procedure Name   : insert_ve_supply_if_proc
   * Description      : データ登録 (A-4)
   ***********************************************************************************/
  PROCEDURE insert_ve_supply_if_proc(
    ov_errbuf     OUT NOCOPY VARCHAR2, --   エラー・メッセージ           --# 固定 #
    ov_retcode    OUT NOCOPY VARCHAR2, --   リターン・コード             --# 固定 #
    ov_errmsg     OUT NOCOPY VARCHAR2) --   ユーザー・エラー・メッセージ --# 固定 #
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'insert_ve_supply_if_proc'; -- プログラム名
--
--#####################  固定ローカル変数宣言部 START   ########################
--
    lv_errbuf  VARCHAR2(5000);  -- エラー・メッセージ
...
--
    ...
    -- *** 出来高実績インタフェーステーブル登録
    -- **************************************************
    FORALL item_cnt IN 1 .. gt_txns_if_id_tab.COUNT
      INSERT INTO xxpo_vendor_supply_txns_if
      (   txns_if_id                         -- 取引ID
        , corporation_name                   -- 会社名
        , data_class                         -- データ種別
        , transfer_branch_no                 -- 伝送用枝番
        , manufactured_date                  -- 生産日
        , vendor_code                        -- 取引先コード
        , factory_code                       -- 工場コード
        , item_code                          -- 品目コード
        , producted_date                     -- 製造日
        , koyu_code                          -- 固有記号
        , producted_quantity                 -- 出来高数量
        , description                        -- 摘要
        , created_by                         -- 作成者
        , creation_date                      -- 作成日
        ...
        , program_update_date                -- プログラム更新日
          ...
          gt_txns_if_id_tab(item_cnt)           -- 取引ID
        , gt_corporation_name_tab(item_cnt)     -- 会社名
        , gt_data_class_tab(item_cnt)           -- データ種別
        , gt_transfer_branch_no_tab(item_cnt)   -- 伝送用枝番
        , gt_manufactured_date_tab(item_cnt)    -- 生産日
        , gt_vendor_code_tab(item_cnt)          -- 取引先コード
        , gt_factory_code_tab(item_cnt)         -- 工場コード
        , gt_item_code_tab(item_cnt)            -- 品目コード
        , gt_producted_date_tab(item_cnt)       -- 製造日
        , gt_koyu_code_tab(item_cnt)            -- 固有記号
        , gt_producted_quantity_tab(item_cnt)   -- 出来高数量
        , gt_description_tab(item_cnt)          -- 摘要
        , gn_user_id                            -- 作成者
        , gd_sysdate                            -- 作成日
        , gn_user_id                            -- 最終更新者
        , gd_sysdate                            -- 最終更新日
