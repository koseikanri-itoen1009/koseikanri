CREATE OR REPLACE PACKAGE BODY xxpo440002c
AS
/*****************************************************************************************
 * Copyright(c)Oracle Corporation Japan, 2008. All rights reserved.
 *
 * Package Name     : xxpo440002(body)
 * Description      : 出庫予定表
...
--  gc_pkg_name             CONSTANT VARCHAR2(20) := 'xxpo440002C' ;      -- パッケージ名
--  gc_report_id            CONSTANT VARCHAR2(20) := 'xxpo440002T' ;      -- 帳票ID
  gc_pkg_name             CONSTANT VARCHAR2(20) := 'XXPO440002C' ;      -- パッケージ名
  gc_report_id            CONSTANT VARCHAR2(20) := 'XXPO440002T' ;      -- 帳票ID
...
--
  ...
  -- 帳票タイトル
  gc_report_name_irai     CONSTANT VARCHAR2(20) := '出庫予定表（依頼）' ;
  gc_report_name_shij     CONSTANT VARCHAR2(20) := '出庫予定表（指示）' ;
--
  -- ==================================================
  -- ユーザー定義グローバル型
  -- ==================================================
  ...
  TYPE rec_param_data  IS RECORD 
    (
     ...
     ,deliver_to_code   VARCHAR2(4)   -- 08 : 配送先
...
--
    ...
    lv_select := ' SELECT'
      ...
      || ',TO_CHAR( xoha.schedule_ship_date   ,''YYYY/MM/DD'' ) AS ship_to_date'  -- 出庫日
      ...
      || ',TO_CHAR( xoha.schedule_arrival_date,''MM/DD'' ) AS ship_in_date'  -- 入庫日
...
--
    ...
    lv_where := ' WHERE'
      ...
      || ' AND FND_DATE.CANONICAL_TO_DATE(''' || gr_param.date_from || ''')'
      ||          ' BETWEEN xim.start_date_active'
...
--
      ...
      -- ブレイク判定：出庫倉庫グループ
      -- ====================================================
      IF ( lr_ref.locat_code <> lv_locat_code ) THEN
        -- ----------------------------------------------------
        -- 下層グループ終了タグ出力
        -- ----------------------------------------------------
...
--
      ...
      -- ブレイク判定：出庫日グループ
      -- ====================================================
      IF ( lr_ref.ship_to_date <> lv_ship_to_date ) THEN
        -- ----------------------------------------------------
        -- 下層グループ終了タグ出力
        -- ----------------------------------------------------
        IF ( lv_ship_to_date <> lc_init ) THEN
--
          -- 出庫ヘッダグループ
          gl_xml_idx := gt_xml_data_table.COUNT + 1 ;
          gt_xml_data_table(gl_xml_idx).tag_name  := '/g_head' ;
...
--
      ...
      -- ブレイク判定：出庫ヘッダグループ
      -- ====================================================
      IF (   ( lr_ref.deliver_to_code <> lv_deliver_to_code )
          OR ( lr_ref.ship_in_date    <> lv_ship_in_date    )
...
--
    ...
    gr_param.locat_code       := iv_locat_code ;                      -- 02 : 出庫倉庫
    ...
    gr_param.deliver_to_code  := iv_deliver_to_code ;                 -- 08 : 配送先
...
--
    ...
    submain
      (
       ...
       ,iv_locat_code         => iv_locat_code        -- 02 : 出庫倉庫
       ...
       ,iv_deliver_to_code    => iv_deliver_to_code   -- 08 : 配送先
