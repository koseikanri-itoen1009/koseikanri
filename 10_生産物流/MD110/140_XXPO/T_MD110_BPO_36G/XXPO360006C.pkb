 ...
 *  2023/10/17    1.35  R.Oikawa         E_本稼働_19497対応
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
...
-- Ver1.35 Add Start
     ,sum_siire_tax   NUMBER                                                --取引先計_仕入金額（消費税額）
     ,sum_kousen_tax  NUMBER                                                --取引先計_口銭（消費税額）
-- Ver1.35 Add End
    ) ;
...
-- Ver1.35 Add Start
--
    ln_wk_sum_tax_siire     NUMBER DEFAULT 0;         -- 消費税(仕入金額)
    ln_wk_sum_tax_kousen    NUMBER DEFAULT 0;         -- 消費税(口銭金額)
    -- 税率別格納用レコード変数（仕入金額）
...
--
    ...
    TYPE rec_zeiritu_k_dtl_type  IS RECORD(
        zeiritu_k           NUMBER         -- 税率(口銭)
       ,amount_k            NUMBER         -- 仕入金額(口銭)
       ,tax_amount_k        NUMBER         -- 消費税額(口銭)
      ) ;
...
--
    ...
    IS
      SELECT to_number(flv_p_tax.lookup_code) zeiritu,
             0                                amount,
             0                                tax_amount
      FROM   fnd_lookup_values flv_p_tax      --クイックコード(消費税)
...
--
          ...
          FOR ln_zei_loop_index IN 1..ct_zei_tab.COUNT LOOP
            ln_wk_sum_tax_siire := ln_wk_sum_tax_siire +
              ROUND(NVL(ct_zei_tab(ln_zei_loop_index).amount, 0) * NVL(ct_zei_tab(ln_zei_loop_index).zeiritu , 0) / 100 ,0);             -- 消費税(税率別)
          END LOOP;
          --
          <<zeikin_k_loop2>>
          FOR ln_zei_k_loop_index IN 1..ct_zei_k_tab.COUNT LOOP
            ln_wk_sum_tax_kousen  := ln_wk_sum_tax_kousen +
              ROUND(NVL(ct_zei_k_tab(ln_zei_k_loop_index).amount_k, 0) * NVL(ct_zei_k_tab(ln_zei_k_loop_index).zeiritu_k , 0) / 100 ,0); -- 消費税(口銭金額)
          END LOOP;
--
          <<zeikin_loop>>
          FOR ln_zei_loop_index IN 1..ct_zei_tab.COUNT LOOP
          ...
          END LOOP;
        END IF;
        ot_data_rec(ln_count).sum_siire_tax     := ln_wk_sum_tax_siire;                   --取引先計の仕入金額（消費税額）
        ot_data_rec(ln_count).sum_kousen_tax    := ln_wk_sum_tax_kousen;                  --取引先計の口銭（消費税額）
-- Ver1.35 Add End
        -- ブレイク用変数へ代入
...
--
-- Ver1.35 Add Start
      -- 税率ごとに算出しワークテーブルに格納(仕入金額)
      <<zeikin_loop>>
      FOR ln_zei_loop_index IN 1..ct_zei_tab.COUNT LOOP
        IF ( NVL(ct_zei_tab(ln_zei_loop_index).zeiritu , 0) = NVL(it_data_rec(ln_loop_index).zeiritu , 0) ) THEN
          ct_zei_tab(ln_zei_loop_index).amount     := ct_zei_tab(ln_zei_loop_index).amount + NVL(ln_siire, 0);                     -- 仕入金額
        END IF;
      END LOOP;
--
      -- 税率ごとに算出しワークテーブルに格納(口銭金額)
      <<zeikin_k_loop>>
      FOR ln_zei_k_loop_index IN 1..ct_zei_k_tab.COUNT LOOP
        IF ( NVL(ct_zei_k_tab(ln_zei_k_loop_index).zeiritu_k , 0) = NVL(it_data_rec(ln_loop_index).zeiritu_k , 0) ) THEN
          ct_zei_k_tab(ln_zei_k_loop_index).amount_k     := ct_zei_k_tab(ln_zei_k_loop_index).amount_k + NVL(ln_kousen, 0);        -- 口銭金額
        END IF;
      END LOOP;
-- Ver1.35 Add End
...
--
      ...
      FOR ln_zei_k_loop_index IN 1..ct_zei_k_tab.COUNT LOOP
        ln_wk_sum_tax_kousen  := ln_wk_sum_tax_kousen +
          ROUND(NVL(ct_zei_k_tab(ln_zei_k_loop_index).amount_k, 0) * NVL(it_data_rec(ln_loop_index).zeiritu_k , 0) / 100 ,0);         -- 消費税(口銭金額)
      END LOOP;
--
      ot_data_rec(ln_count).sum_siire_tax     := ln_wk_sum_tax_siire;                   --取引先計の仕入金額（消費税額）
      ot_data_rec(ln_count).sum_kousen_tax    := ln_wk_sum_tax_kousen;                  --取引先計の口銭（消費税額）
...
-- Ver1.35 Add Start
    -- 斡旋者or取引先小計用
    ln_sum_mid_siire            NUMBER DEFAULT 0;         -- 仕入金額
    ln_sum_mid_kosen            NUMBER DEFAULT 0;         -- 口銭金額
    ln_sum_mid_huka             NUMBER DEFAULT 0;         -- 賦課金額
    ln_sum_mid_sasihiki         NUMBER DEFAULT 0;         -- 差引金額
    ln_sum_mid_tax_siire        NUMBER DEFAULT 0;         -- 消費税(仕入金額)
    ln_sum_mid_tax_kousen       NUMBER DEFAULT 0;         -- 消費税(口銭金額)
    ln_sum_mid_tax_sasihiki     NUMBER DEFAULT 0;         -- 消費税(差引金額)
    ln_sum_mid_jun_siire        NUMBER DEFAULT 0;         -- 純仕入金額
    ln_sum_mid_jun_kosen        NUMBER DEFAULT 0;         -- 純口銭金額
    ln_sum_mid_jun_sasihiki     NUMBER DEFAULT 0;         -- 純差引金額
    -- 税率別格納用レコード変数（仕入金額）
    TYPE rec_zeiritu_dtl_type  IS RECORD(
        zeiritu           NUMBER         -- 税率
       ,amount            NUMBER         -- 仕入金額
...
-- Ver1.35 Add Start
                    -- 斡旋者or得意先計出力
                    lb_ret := fnc_set_xml('Z', 'sum_mid_siire', ln_sum_mid_siire);                -- 仕入金額
                    lb_ret := fnc_set_xml('Z', 'sum_mid_kosen', ln_sum_mid_kosen);                -- 口銭金額
                    lb_ret := fnc_set_xml('Z', 'sum_mid_sasihiki', ln_sum_mid_sasihiki);          -- 差引金額
                    lb_ret := fnc_set_xml('Z', 'sum_mid_tax_siire', ln_sum_mid_tax_siire);        -- 消費税(仕入金額)
                    lb_ret := fnc_set_xml('Z', 'sum_mid_tax_kousen', ln_sum_mid_tax_kousen);      -- 消費税(口銭金額)
                    lb_ret := fnc_set_xml('Z', 'sum_mid_tax_sasihiki', ln_sum_mid_tax_sasihiki);  -- 消費税(差引金額)
                    lb_ret := fnc_set_xml('Z', 'sum_mid_jun_siire', ln_sum_mid_jun_siire);        -- 純仕入金額
                    lb_ret := fnc_set_xml('Z', 'sum_mid_jun_kosen', ln_sum_mid_jun_kosen);        -- 純口銭金額
                    lb_ret := fnc_set_xml('Z', 'sum_mid_jun_sasihiki', ln_sum_mid_jun_sasihiki);  -- 純差引金額
                    -- 部署小計加算
                    ln_sum_post_siire        := ln_sum_post_siire        + NVL(ln_sum_mid_siire, 0);        -- 仕入金額
                    ln_sum_post_kosen        := ln_sum_post_kosen        + NVL(ln_sum_mid_kosen, 0);        -- 口銭金額
                    ln_sum_post_sasihiki     := ln_sum_post_sasihiki     + NVL(ln_sum_mid_sasihiki, 0);     -- 差引金額
                    ln_sum_post_tax_siire    := ln_sum_post_tax_siire    + NVL(ln_sum_mid_tax_siire, 0);    -- 消費税(仕入金額)
                    ln_sum_post_tax_kousen   := ln_sum_post_tax_kousen   + NVL(ln_sum_mid_tax_kousen, 0);   -- 消費税(口銭金額)
                    ln_sum_post_tax_sasihiki := ln_sum_post_tax_sasihiki + NVL(ln_sum_mid_tax_sasihiki, 0); -- 消費税(差引金額)
                    ln_sum_post_jun_siire    := ln_sum_post_jun_siire    + NVL(ln_sum_mid_jun_siire, 0);    -- 純仕入金額
                    ln_sum_post_jun_kosen    := ln_sum_post_jun_kosen    + NVL(ln_sum_mid_jun_kosen, 0);    -- 純口銭金額
                    ln_sum_post_jun_sasihiki := ln_sum_post_jun_sasihiki + NVL(ln_sum_mid_jun_sasihiki, 0); -- 純差引金額
                    -- 総合計加算
...
-- Ver1.35 Add Start
      -- 斡旋者or得意先計加算
      ln_sum_mid_siire            := ln_sum_mid_siire + NVL(ln_siire, 0);                                                          -- 仕入金額
      ln_sum_mid_kosen            := ln_sum_mid_kosen + NVL(ln_kousen, 0);                                                         -- 口銭金額
      ln_sum_mid_huka             := ln_sum_mid_huka + NVL(ln_fuka, 0);                                                            -- 賦課金額
      ln_sum_mid_sasihiki         := ln_sum_mid_sasihiki + ( NVL(ln_siire, 0) - NVL(ln_kousen, 0) - NVL(ln_fuka, 0) );             -- 差引金額
      -- 税率ごとに算出しワークテーブルに格納(仕入金額)
      <<zeikin_loop>>
      FOR ln_zei_loop_index IN 1..ct_zei_tab.COUNT LOOP
        IF ( NVL(ct_zei_tab(ln_zei_loop_index).zeiritu , 0) = NVL(gt_main_data(ln_loop_index).zeiritu , 0) ) THEN
          ...
          ct_zei_tab(ln_zei_loop_index).tax_amount := 
            ROUND(NVL(ct_zei_tab(ln_zei_loop_index).amount, 0) * NVL(gt_main_data(ln_loop_index).zeiritu , 0) / 100 ,0);           -- 消費税(税率別)
        END IF;
      END LOOP;
      -- ワークテーブルに格納した消費税を加算
      -- 加算した消費税が入っているので毎回初期化
      ln_sum_mid_tax_siire := 0;
      <<zeikin_loop2>>
      FOR ln_zei_loop_index IN 1..ct_zei_tab.COUNT LOOP
        ln_sum_mid_tax_siire        := ln_sum_mid_tax_siire + ct_zei_tab(ln_zei_loop_index).tax_amount;                            -- 消費税(仕入金額)
      END LOOP;
--
      -- 税率ごとに算出しワークテーブルに格納(口銭金額)
      <<zeikin_k_loop>>
      FOR ln_zei_k_loop_index IN 1..ct_zei_k_tab.COUNT LOOP
        IF ( NVL(ct_zei_k_tab(ln_zei_k_loop_index).zeiritu_k , 0) = NVL(gt_main_data(ln_loop_index).zeiritu_k , 0) ) THEN
          ...
          ct_zei_k_tab(ln_zei_k_loop_index).tax_amount_k := 
            ROUND(NVL(ct_zei_k_tab(ln_zei_k_loop_index).amount_k, 0) * NVL(gt_main_data(ln_loop_index).zeiritu_k , 0) / 100 ,0);   -- 消費税(税率別)
        END IF;
      END LOOP;
      -- ワークテーブルに格納した消費税を加算
      -- 加算した消費税が入っているので毎回初期化
      ...
      <<zeikin_k_loop2>>
      FOR ln_zei_k_loop_index IN 1..ct_zei_k_tab.COUNT LOOP
        ln_sum_mid_tax_kousen        := ln_sum_mid_tax_kousen + ct_zei_k_tab(ln_zei_k_loop_index).tax_amount_k;                    -- 消費税(口銭金額)
      END LOOP;
      ln_sum_mid_tax_sasihiki     := ln_sum_mid_tax_siire - ln_sum_mid_tax_kousen;                                                 -- 消費税(差引金額)
      ln_sum_mid_jun_siire        := ln_sum_mid_siire + ln_sum_mid_tax_siire;                                                      -- 純仕入金額
      ln_sum_mid_jun_kosen        := ln_sum_mid_kosen + ln_sum_mid_tax_kousen;                                                     -- 純口銭金額
      ln_sum_mid_jun_sasihiki     := ln_sum_mid_sasihiki + ln_sum_mid_tax_sasihiki;                                                -- 純差引金額
-- Ver1.35 Add End
      -- 集計別の場合は出力しない
      IF ( gr_param_rec.out_flg <> lc_out_syukei) THEN
        -- 部署合計加算
...
-- Ver1.35 Add Start
      ...
      ln_sum_mid_huka             := ln_sum_mid_huka + NVL(ln_fuka, 0);                                                    -- 賦課金額
      ln_sum_mid_sasihiki         := ln_sum_mid_sasihiki + ( NVL(ln_siire, 0) - NVL(ln_kousen, 0) - NVL(ln_fuka, 0) );     -- 差引金額
      ln_sum_mid_tax_siire        := NVL(gt_main_data(ln_loop_index).sum_siire_tax,0);                                     -- 消費税(仕入金額)
      ln_sum_mid_tax_kousen       := NVL(gt_main_data(ln_loop_index).sum_kousen_tax,0);                                    -- 消費税(口銭金額)
      ln_sum_mid_tax_sasihiki     := ln_sum_mid_tax_siire - ln_sum_mid_tax_kousen;                                         -- 消費税(差引金額)
      ln_sum_mid_jun_siire        := ln_sum_mid_siire + ln_sum_mid_tax_siire;                                              -- 純仕入金額
      ln_sum_mid_jun_kosen        := ln_sum_mid_kosen + ln_sum_mid_tax_kousen;                                             -- 純口銭金額
      ln_sum_mid_jun_sasihiki     := ln_sum_mid_sasihiki + ln_sum_mid_tax_sasihiki;                                        -- 純差引金額
