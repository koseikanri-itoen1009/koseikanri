 ...
 *  insert_frq_item_lo_proc   データ登録 (G-4)
 *  submain                   メイン処理プロシージャ
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  gv_pkg_name         CONSTANT VARCHAR2(100) := 'xxwsh920005c'; -- パッケージ名
--
...
--
  ...
  gv_c_msg_up_name    CONSTANT VARCHAR2(15)  := 'APP-XXWSH-33307';  -- ファイルアップロード名称
--
  -- トークン
  ...
  gv_c_parge_term_name         CONSTANT VARCHAR2(100)  := 'パージ対象期間:倉庫品目アドオンマスタ';
  -- クイックコード タイプ
...
--
  ...
  -- *** 項目名 ***
  gv_c_item_location_code       CONSTANT VARCHAR2(24)   := '元倉庫';
  gv_c_item_code                CONSTANT VARCHAR2(24)   := '品目コード';
  gv_c_frq_item_location_code   CONSTANT VARCHAR2(24)   := '代表倉庫';
--
  -- *** 項目桁数 ***
  gn_c_file_id_name_l            CONSTANT NUMBER         :=   4;    -- 元倉庫
  gn_c_item_code_l               CONSTANT NUMBER         :=   7;    -- 品目コード
  gn_c_frq_item_location_code_l  CONSTANT NUMBER         :=   4;    -- 代表倉庫
--
...
--
  ...
  TYPE file_data_rec IS RECORD(
    item_location_code      VARCHAR2(32767), -- 元倉庫
    item_code               VARCHAR2(32767), -- 品目コード
    frq_item_location_code  VARCHAR2(32767), -- 代表倉庫
...
--
  gt_seq_number_tab                 seq_number_type;                  -- SEQ番号
  gt_item_location_code_tab         item_location_code_type;          -- 元倉庫
  gt_item_code_tab                  item_code_type;                   -- 品目コード
  gt_frq_item_location_code_tab     frq_item_location_code_type;      -- 代表倉庫
--
  -- ===============================
  -- ユーザー定義グローバル変数
  -- ===============================
...
--
  gn_xxwsh_parge_term       NUMBER;        -- パージ対象期間
...
--
    ...
    BEGIN
      ...
      WHERE   xlvv.lookup_type = gv_c_lookup_type       -- タイプ(XXINV_FILE_OBJECT)
...
--
  ...
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'insert_frq_item_lo_proc'; -- プログラム名
--
--#####################  固定ローカル変数宣言部 START   ########################
--
    lv_errbuf  VARCHAR2(5000);  -- エラー・メッセージ
    ...
    FORALL item_cnt IN 1 .. gt_seq_number_tab.COUNT
      ...
      (   seq_number                         -- SEQ番号
        ...
        , program_update_date                -- プログラム更新日
          ...
          gt_seq_number_tab(item_cnt)               -- SEQ番号
        , gt_item_location_code_tab(item_cnt)       -- 元倉庫
        , gt_item_code_tab(item_cnt)                -- 品目コード
        , gt_frq_item_location_code_tab(item_cnt)   -- 代表倉庫
        , gn_user_id                                -- 作成者
        , gd_sysdate                                -- 作成日
        , gn_user_id                                -- 最終更新者
        , gd_sysdate                                -- 最終更新日
