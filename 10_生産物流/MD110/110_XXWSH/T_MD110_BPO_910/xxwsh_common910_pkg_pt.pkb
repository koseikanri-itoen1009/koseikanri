CREATE OR REPLACE PACKAGE BODY xxwsh_common910_pkg_pt
AS
/*****************************************************************************************
 ...
 *  2019/08/06   1.42  SCSK  佐々木宏之 [鮮度基準日算出用の条件を細分化] E_本稼動_15398対応 追加（一般の場合）
 *  2019/08/27                          上記対応PT用モジュール
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
...
--
  ...
  gv_pkg_name      CONSTANT VARCHAR2(100) := 'xxwsh_common910_pkg_pt'; -- パッケージ名
  --
...
--
    ...
    -- 鮮度条件および鮮度条件付随情報の取得
    BEGIN
-- Ver1.39 K.Kiriu E_本稼動_09591 Mod Start
...
-- Ver1.39 K.Kiriu E_本稼動_09591 Mod End
--
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        lv_errmsg := xxcmn_common_pkg.get_msg(gv_cnst_xxwsh
