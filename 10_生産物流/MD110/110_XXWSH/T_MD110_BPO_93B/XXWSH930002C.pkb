CREATE OR REPLACE PACKAGE BODY xxwsh930002c
AS
/*****************************************************************************************
 * Copyright(c)Oracle Corporation Japan, 2008. All rights reserved.
 *
 * Package Name     : xxwsh930002c(body)
 ...
 *  2009/02/05    1.42 Oracle 佐久間尚豊 本番障害対応#1095 妥当チェックでロットステータス未設定を確認した場合、警告終了とする。
 *                                                         各プロシージャでエラー終了した場合、サブメインの例外処理で制御できるようにする。
 *  2009/02/09    1.43 Oracle 佐久間尚豊 本番障害対応#1165 拠点情報は配送先の管轄拠点より取得する。
 ...
 *  2009/03/30    1.47 SCS    飯田 甫    本番障害対応#1346 顧客マスタの顧客区分対応(拠点、又は配送先のみ抽出)
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  gv_pkg_name             CONSTANT VARCHAR2(100) := 'xxwsh930002c';     -- パッケージ名
...
--  gv_msg_93a_308                 CONSTANT VARCHAR2(15) := 'APP-XXWSH-13308';
  gv_msg_93a_159                 CONSTANT VARCHAR2(15) := 'APP-XXWSH-13159';  -- 2009/3/13 本番障害#1068 ADD 92Eのメッセージを使用していたため新規に採番
  -- ＰＧでのコンカレント呼び出しエラー
...
--
  ...
  TYPE typ_shipping_item_code
    IS TABLE OF xxwsh_order_lines_all.shipping_item_code%TYPE INDEX BY BINARY_INTEGER; -- 出荷品目(受注用)
...
--
  -- 2008/08/18 TE080_930指摘#32対応 Add Start---------------
  gt_shipping_item_code    typ_shipping_item_code;    -- 出荷品目(受注用)
...
--          ,gv_msg_93a_308             -- 重量容積小口個数更新関数エラーメッセージ
          ,gv_msg_93a_159               -- 2009/03/13 本番障害 #1068
...
--
-- TE080_930指摘#32対応 Add Start --------------------------------------------------
  /**********************************************************************************
   * F Name   :order_zero_updt
   * Description      : 指示にあって実績にない品目は実績0で更新する
   *                  : (2008/08/18 TE080_930指摘#32対応)
   ***********************************************************************************/
  ...
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'order_zero_updt'; -- プログラム名
--
--#####################  固定ローカル変数宣言部 START   ########################
--
    lv_errbuf  VARCHAR2(5000);  -- エラー・メッセージ
...
--
    ...
    lt_document_type_code      xxinv_mov_lot_details.document_type_code%TYPE;        --文書タイプ
--
    -- *** ローカル・カーソル ***
...
--
    CURSOR lot_data_get_cur
      (
        in_order_source_ref    xxwsh_shipping_headers_if.order_source_ref%TYPE -- 受注ソース参照
       ,in_item_code           xxinv_mov_lot_details.item_code%TYPE            -- 品目
       ,in_document_type       xxinv_mov_lot_details.document_type_code%TYPE   -- 文書タイプ
      )
    IS
        ...
        WHERE   xoha.request_no           = in_order_source_ref  -- 受注ソース参照
        AND     xoha.order_header_id      = xola.order_header_id
        AND     xoha.latest_external_flag = gv_yesno_y
        AND     ((xola.delete_flag        = gv_yesno_n) OR (xola.delete_flag IS NULL))
        ...
        AND     xmld.item_code            = in_item_code          -- 品目
...
--
    ...
    OPEN order_data_get_cur
      (
        gr_interface_info_rec(in_idx).order_source_ref   -- 受注ソース参照
      );
--
    --フェッチ
...
--
      lb_item_cd_found_flg := FALSE;  -- フラグを初期化
--
      <<search_item_code_loop>>
      FOR j IN 1 .. gn_zero_updt_idx LOOP  -- 同じ品目があるか検索する
--
        IF ( lr_tab_data(i).shipping_item_code = gt_shipping_item_code(j) ) THEN
          lb_item_cd_found_flg := TRUE;   -- 同じ品目があった場合はフラグをON
        END IF;
--
...
--
        ...
        IF ((gr_interface_info_rec(in_idx).eos_data_type = gv_eos_data_cd_210)  OR
            (gr_interface_info_rec(in_idx).eos_data_type = gv_eos_data_cd_215))
        THEN
          lt_document_type_code := gv_document_type_10; -- 文書タイプ=出荷依頼
        -- EOSデータ種別 = 200 有償出荷報告
        ELSIF (gr_interface_info_rec(in_idx).eos_data_type = gv_eos_data_cd_200) THEN
          lt_document_type_code := gv_document_type_30; -- 文書タイプ=支給指示
        END IF;
--
        -- カーソルオープン
        OPEN lot_data_get_cur
          (
            gr_interface_info_rec(in_idx).order_source_ref   -- 受注ソース参照
           ,lr_tab_data(i).shipping_item_code                -- 品目
           ,lt_document_type_code                            -- 文書タイプ
          );
--
        --フェッチ
...
-- 2008/08/18 TE080_930指摘#32対応 Add Start --------------------------------------------------
  ...
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'mov_zero_updt'; -- プログラム名
--
--#####################  固定ローカル変数宣言部 START   ########################
--
    lv_errbuf  VARCHAR2(5000);  -- エラー・メッセージ
...
--
      ...
      --*HHT*受注ソース参照の上4桁='9600'のデータは、指示なし実績を意味する            -- 2008/09/01 TE080_930指摘#38 Add
      --*HHT*IF (SUBSTRB(gr_interface_info_rec(i).order_source_ref,1,4) = '9600') THEN -- 2008/09/01 TE080_930指摘#38 Add -- 2008/12/26 本番障害#866 Del
      IF (SUBSTRB(gr_interface_info_rec(i).order_source_ref,1,2) = '96') THEN          -- 2008/12/26 本番障害#866 Add  --*HHT*
--
...
-- Ver1.19 M.Hokkanji END 2008/10/06 統合テスト障害#305対応
      ...
      IF ((ln_err_flg = 0) AND (lv_error_flg = '0')) THEN
--
        -- 2008/08/13 内部変更#176対応 Del Start -------------------------------------------------
        --ln_err_flg := 0;
        --
...
--
        ...
        IF ((gr_interface_info_rec(i).eos_data_type = gv_eos_data_cd_200) OR  -- 有償出荷報告
            (gr_interface_info_rec(i).eos_data_type = gv_eos_data_cd_210) OR  -- 拠点出荷確定報告
            (gr_interface_info_rec(i).eos_data_type = gv_eos_data_cd_215))    -- 庭先出荷確定報告
        THEN
--
...
--
                ...
                gt_mov_item_code(gn_zero_updt_idx) := lr_tab_data(i).item_code;  --品目
                -- 2008/08/18 TE080_930指摘#32対応 Add End --------------------------------------
--
...
--
          -- 2008/08/18 TE080_930指摘#32対応 Add Start ------------------------------------
          gn_zero_updt_idx := gn_zero_updt_idx + 1;
          gt_mov_item_code(gn_zero_updt_idx) := gr_interface_info_rec(in_idx).orderd_item_code;  --品目
          -- 2008/08/18 TE080_930指摘#32対応 Add End --------------------------------------
--
...
--
                  -- 2008/08/18 TE080_930指摘#32対応 Add Start ------------------------------------
                  gn_zero_updt_idx := gn_zero_updt_idx + 1;
                  gt_shipping_item_code(gn_zero_updt_idx) := lr_tab_data(i).shipping_item_code; --出荷品目
                  -- 2008/08/18 TE080_930指摘#32対応 Add End --------------------------------------
--
...
--
          -- 2008/08/18 TE080_930指摘#32対応 Add Start ------------------------------------
          gn_zero_updt_idx := gn_zero_updt_idx + 1;
          gt_shipping_item_code(gn_zero_updt_idx) := gr_interface_info_rec(in_idx).orderd_item_code;  --出荷品目
          -- 2008/08/18 TE080_930指摘#32対応 Add End --------------------------------------
--
...
--
    -- 2008/08/18 TE080_930指摘#32対応 Add Start -----------------
    gn_zero_updt_idx := 0;
    gt_shipping_item_code.delete;      -- 出荷品目
    -- 2008/08/18 TE080_930指摘#32対応 Add End -------------------
--
