/*============================================================================
* ファイル名 : XxinvMovementShipToLotCO
* 概要説明   : 入庫ロット明細画面コントローラ
* バージョン : 1.0
*============================================================================
* 修正履歴
* 日付       Ver. 担当者       修正内容
...
/***************************************************************************
 * 入庫ロット明細画面コントローラクラスです。
 * @author  ORACLE 伊藤 ひとみ
 * @version 1.0
 ***************************************************************************
 */
public class XxinvMovementShipToLotCO extends XxcmnOAControllerImpl
{
  ...
  public void processRequest(OAPageContext pageContext, OAWebBean webBean)
  {
    ...
    if (!pageContext.isBackNavigationFired(false)) 
    {
      ...
      {
        // 登録処理
        am.invokeMethod("entryDataShipTo");
      ...
      } else
      {
        ...
        searchParams.put("recordTypeCode", XxinvConstants.RECORD_TYPE_30);                                         // レコードタイプ  30：入庫実績
  ...
  public void processFormRequest(OAPageContext pageContext, OAWebBean webBean)
  {
    try
    {
      ...
      } else if (pageContext.getParameter("Return") != null) 
      {
        ...
        pageParams.put(XxinvConstants.URL_PARAM_PREV_URL, XxinvConstants.URL_XXINV510002J_2); // 画面URL
      ...
      } else if (pageContext.getParameter("Go") != null)
      {
        ...
        // 警告チェック処理実行
        HashMap msg = (HashMap)am.invokeMethod("checkWarningShipTo");
    
        String[] lotRevErrFlg     = (String[])msg.get("lotRevErrFlg");     // ロット逆転防止チェックエラーフラグ
        String[] shortageErrFlg   = (String[])msg.get("shortageErrFlg");   // 引当可能在庫数不足チェックエラーフラグ
        ...
        String[] shipToLocName    = (String[])msg.get("shipToLocName");    // 入庫元保管倉庫名
        ...
        for(int i = 0 ; i < lotRevErrFlg.length ; i++)
        {
          ...
          // 引当可能在庫数不足チェックでエラーの場合
          if (XxcmnConstants.STRING_Y.equals(shortageErrFlg[i]))
          {
            // 警告メッセージが複数存在する場合、改行コードを追加
            XxcmnUtility.newLineAppend(pageHeaderText);

            // 引当可能在庫数不足チェック警告メッセージ取得
            MessageToken[] tokens = { new MessageToken(XxcmnConstants.TOKEN_LOCATION, shipToLocName[i]),
