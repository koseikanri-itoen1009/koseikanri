CREATE OR REPLACE PACKAGE BODY xxinv590002c
AS
/*****************************************************************************************
 * Copyright(c)Oracle Corporation Japan, 2008. All rights reserved.
 *
 * Package Name     : xxinv590002c(body)
...
--
  ...
  -- ===============================
  -- メッセージ用定数
  gv_pkg_name       CONSTANT VARCHAR2(100) := 'xxinv590002c';                           -- パッケージ名
  gv_app_name       CONSTANT VARCHAR2(5)   := 'XXCMN';                                  -- アプリケーション短縮名
  gv_tkn_name       CONSTANT VARCHAR2(100) := '在庫倉庫ステータス/在庫倉庫クローズ表';  -- テーブル名
--
  -- メッセージ
  gv_msg_xxcmn10019 CONSTANT VARCHAR2(100) := 'APP-XXCMN-10019';   -- ロック取得エラー
...
--
  ...
  TYPE whse_code_rec IS RECORD(
   ...
   ,close_whse_ind ic_whse_sts.close_whse_ind%TYPE
  );
--
  -- ***************************************
  -- ***      項目格納テーブル型定義     ***
...
--
    -- B-1/B-2 対象データの取得/ステータス取得
    SELECT iws.whse_code
          ,iws.close_whse_ind
    BULK COLLECT INTO gt_whse_code_tbl
    FROM   gmi_clos_warehouses gcw
          ,ic_whse_sts iws
    WHERE  gcw.inventory_close_id = TO_NUMBER(iv_sequence)
    AND    gcw.whse_code          = iws.whse_code
...
--
    IF (gt_whse_code_tbl IS NOT NULL) THEN
      ...
      FOR col_cnt IN 1 .. gt_whse_code_tbl.COUNT LOOP
        -- オープンの場合
        IF (gt_whse_code_tbl(col_cnt).close_whse_ind = '1') THEN
          -- B-3 IC_PERD_BAL表存在チェック
          SELECT COUNT(*)
          INTO   ln_cnt
          FROM   ic_perd_bal ipb
          WHERE  ipb.fiscal_year = UPPER(iv_fiscal_year)
          AND    ipb.period      = iv_period
          AND    ipb.whse_code   = UPPER(gt_whse_code_tbl(col_cnt).whse_code)
          AND    ROWNUM          = 1;
--
          -- IC_PERD_BALに存在している場合
          IF (ln_cnt > 0) THEN
            -- 更新対象レコード変数に退避
            li_cnt := li_cnt + 1;
            lt_whse_code(li_cnt)    := gt_whse_code_tbl(col_cnt).whse_code;
          END IF;
        END IF;
      END LOOP upd_loop;
--
...
--
    ...
    COMMIT;
--
    -- B-6 「OPM/GMI在庫クローズ」発行
    ln_reqid := FND_REQUEST.SUBMIT_REQUEST(
                  application       => 'XXINV'              -- アプリケーション短縮名
                 ,program           => 'XXINV590002C'       -- プログラム名
                 ,argument1         => iv_sequence          -- シーケンスID
                 ,argument2         => iv_fiscal_year       -- 会計年度
                 ,argument3         => iv_period            -- 期間
                 ,argument4         => iv_period_id         -- 期間ID
                 ,argument5         => iv_start_date        -- 開始日付
                 ,argument6         => iv_end_date          -- 終了日付
                 ,argument7         => iv_op_code           -- Operators Identifier Number
                 ,argument8         => iv_orgn_code         -- 会社コード
                 ,argument9         => iv_close_ind         -- 処理区分(1:OPEN,2:暫定CLOSE,3:CLOSE)
                  );
    -- エラーの場合
    IF (ln_reqid = 0) THEN
