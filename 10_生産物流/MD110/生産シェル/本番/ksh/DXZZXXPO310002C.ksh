...
##                        リプレース_00004対応                                ##
##                    SCSK      竹浪 隼      2021/12/14 3.0.0                 ##
##                        [E_本稼動_17774対応]環境依存値の修正                ##
##                                                                            ##
##    [戻り値]                                                                ##
##        0     正常                                                          ##
##        4     警告                                                          ##
...
then
  ...
  Delivery Warehouse03
  Delivery Warehouse04
  Delivery Warehouse05
  Delivery Warehouse06
  Delivery Warehouse07
  Delivery Warehouse08
  Delivery Warehouse09
  Delivery Warehouse10
EOF
shell_end ${L_exit_eror}
fi

### Get DeliveryDate ###
output_log "Getting DeliveryDate START"
sqlplus -s apps/apps <<SQLEND >${L_std_out}
SET HEADING OFF
SET FEED OFF
SELECT TO_CHAR(SYSDATE,'YYYY/MM/DD'),TO_CHAR(SYSDATE+3,'YYYY/MM/DD')
FROM   DUAL;
EXIT
SQLEND

L_param_001=`/usr/bin/awk '( $0 != "" ){print $1}' ${L_std_out}`
L_param_002=`/usr/bin/awk '( $0 != "" ){print $2}' ${L_std_out}`
...
output_log "Getting DeliveryDate END"
