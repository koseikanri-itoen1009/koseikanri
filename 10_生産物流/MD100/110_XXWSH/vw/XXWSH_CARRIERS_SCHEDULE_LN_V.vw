...
(row_id
,default_line                -- 基準明細
,transaction_type            -- 処理種別コード
,transaction_type_name       -- 処理種別名称
,delivery_no                 -- 配送No
,request_no                  -- 依頼No
,career_id                   -- 運送業者ID
,freight_carrier_code        -- 運送業者
,shipping_method_code        -- 配送区分
,shipping_kubun_code         -- DFF小口区分
,mixed_class_code            -- DFF混載区分
,transaction_type_id         -- 取引タイプID
,schedule_ship_date          -- 出庫予定日 
,schedule_arrival_date       -- 着荷予定日
,deliver_from                -- 出荷元保管場所
,deliver_from_name           -- 摘要
,deliver_to                  -- 出荷先
,vendor_site_code            -- 取引先サイト
,deliver_to_name             -- 正式名称
,vendor_site_code_name       -- 取引先サイト略称称
,head_sales_branch           -- 管轄拠点
,head_sales_branch_name      -- 管轄拠点略称
,vendor_code                 -- 取引先
,vendor_short_name           -- 取引先略称
,based_weight                -- 基本重量
,based_capacity              -- 基本容積
,sum_weight                  -- 積載重量合計
,sum_capacity                -- 積載容積合計
,pallet_sum_quantity         -- 合計パレット枚数数
,sum_pallet_weight           -- 合計パレット重量量
...
,mixed_sign                  -- 混載記号
,mixed_no                    -- 混載元No
,result_freight_carrier_id   -- 運送業者_実績IDD
,result_freight_carrier_code -- 運送業者_実績
,result_shipping_method_code -- 配送区分_実績
,result_shipping_kubun_code  -- DFF小口区分_実績
,result_mixed_class_code     -- DFF混載区分_実績
,result_deliver_to           -- 出荷先_実績
,result_deliver_to_name      -- 出荷先_実績名称
,shipped_date                -- 出荷日
,arrival_date                -- 着荷日
,created_by                  -- 作成者
,creation_date               -- 作成日
...
,xpsv2.party_site_full_name       result_deliver_to_name      -- 出荷先_実績名称
...
AND   xpsv.party_site_id(+)  = xoha.deliver_to_id           -- 出荷先名称
AND   xpsv2.party_site_id(+) = xoha.result_deliver_to_id    -- 出荷先_実績名称
AND   xvsv.vendor_site_id(+) = xoha.vendor_site_id          -- 取引先サイト名称
AND   xpv.party_number(+)    = xoha.head_sales_branch       -- 管轄拠点名称
AND   xpv.customer_class_code(+)    = '1'       -- 顧客区分
