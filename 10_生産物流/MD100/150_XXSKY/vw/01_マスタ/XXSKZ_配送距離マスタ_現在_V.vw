/*************************************************************************
 * 
 * View  Name      : XXSKZ_配送距離マスタ_現在_V
 * Description     : XXSKZ_配送距離マスタ_現在_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_配送距離マスタ_現在_V
(
 商品区分
,商品区分名
,運送業者コード
...
-- 2010/01/28 T.Yoshimoto Del End 本稼動#1168
 WHERE  XDD.start_date_active <= TRUNC(SYSDATE)
   AND  XDD.end_date_active   >= TRUNC(SYSDATE)
-- 2010/01/28 T.Yoshimoto Del Start 本稼動#1168
   --AND  XDD.delivery_company_code = XCRV.freight_code(+)
...
-- 2010/01/28 T.Yoshimoto Del End 本稼動#1168
/
COMMENT ON TABLE APPS.XXSKZ_配送距離マスタ_現在_V IS 'SKYLINK用配送距離マスタ（現在）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.商品区分         IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.商品区分名       IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.運送業者コード   IS '運送業者コード'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.運送業者名       IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.出庫倉庫         IS '出庫倉庫'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.出庫倉庫名       IS '出庫倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.コード区分       IS 'コード区分'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.コード区分名     IS 'コード区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.配送先コード     IS '配送先コード'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.配送先名         IS '配送先名'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.適用開始日       IS '適用開始日'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.適用終了日       IS '適用終了日'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.車立距離         IS '車立距離'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.小口距離         IS '小口距離'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.混載割増距離     IS '混載割増距離'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.実際距離         IS '実際距離'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.エリアA          IS 'エリアA'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.エリアB          IS 'エリアB'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.エリアC          IS 'エリアC'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.作成者           IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.作成日           IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.最終更新者       IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.最終更新日       IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKZ_配送距離マスタ_現在_V.最終更新ログイン IS '最終更新ログイン'
/