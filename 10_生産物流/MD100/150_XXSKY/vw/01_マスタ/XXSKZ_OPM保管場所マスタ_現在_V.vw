/*************************************************************************
 * 
 * View  Name      : XXSKZ_OPM保管場所マスタ_現在_V
 * Description     : XXSKZ_OPM保管場所マスタ_現在_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_OPM保管場所マスタ_現在_V
(
 倉庫コード
,倉庫名
,プラントコード
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XILV01.description
         FROM xxskz_item_locations_v XILV01 --OPM保管場所情報VIEW(代表倉庫名)
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XCV.party_name
         FROM xxskz_carriers_v XCV    --運送業者情報VIEW
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XILV02.description
         FROM xxskz_item_locations_v XILV02 --OPM保管場所情報VIEW(ＥＯＳ宛先名)
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XLV.location_name
         FROM xxskz_locations_v XLV    --事業所情報VIEW(倉庫管理部署)
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XILV03.description
         FROM xxskz_item_locations_v XILV03 --OPM保管場所情報VIEW(主要保管倉庫)
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XVV.vendor_name
         FROM xxskz_vendors_v XVV    --仕入先情報VIEW(仕入先)
...
-- 2010/01/28 T.Yoshimoto Mod Start 本稼動#1168
       ...
       ,(SELECT XVSV.vendor_site_name
         FROM xxskz_vendor_sites_v XVSV   --仕入先サイト情報VIEW(仕入先サイト名)
...
-- 2010/01/28 T.Yoshimoto Del Start 本稼動#1168
       ...
       --,xxskz_item_locations_v    XILV01 --OPM保管場所情報VIEW(代表倉庫名)
       --,xxskz_item_locations_v    XILV02 --OPM保管場所情報VIEW(ＥＯＳ宛先名)
       --,xxskz_item_locations_v    XILV03 --OPM保管場所情報VIEW(主要保管倉庫)
       --,xxskz_carriers_v          XCV    --運送業者情報VIEW
       --,xxskz_locations_v         XLV    --事業所情報VIEW(倉庫管理部署)
       --,xxskz_vendors_v           XVV    --仕入先情報VIEW(仕入先)
       --,xxskz_vendor_sites_v      XVSV   --仕入先サイト情報VIEW(仕入先サイト名)
...
-- 2010/01/28 T.Yoshimoto Del End 本稼動#1168
/
COMMENT ON TABLE APPS.XXSKZ_OPM保管場所マスタ_現在_V IS 'SKYLINK用OPM保管場所マスタ（現在）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.倉庫コード                IS '倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.倉庫名                    IS '倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.プラントコード            IS 'プラントコード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.プラント名                IS 'プラント名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.組織有効開始日            IS '組織有効開始日'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.組織有効終了日            IS '組織有効終了日'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.相手先在庫管理対象        IS '相手先在庫管理対象'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.相手先在庫管理対象名      IS '相手先在庫管理対象名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管倉庫コード            IS '保管倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管倉庫名                IS '保管倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管倉庫略称              IS '保管倉庫略称'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管倉庫無効日            IS '保管倉庫無効日'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管場所コード            IS '保管場所コード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.保管場所名                IS '保管場所名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.内外倉庫区分              IS '内外倉庫区分'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.内外倉庫区分名            IS '内外倉庫区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.物流ブロック              IS '物流ブロック'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.物流ブロック名            IS '物流ブロック名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.代表倉庫                  IS '代表倉庫'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.代表倉庫名                IS '代表倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.代表倉庫略称              IS '代表倉庫略称'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.代表運送会社              IS '代表運送会社'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.代表運送会社名            IS '代表運送会社名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ＥＯＳ管理区分            IS 'ＥＯＳ管理区分'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ＥＯＳ管理区分名          IS 'ＥＯＳ管理区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ＥＯＳ宛先                IS 'ＥＯＳ宛先'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ＥＯＳ宛先名              IS 'ＥＯＳ宛先名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.倉庫管理部署              IS '倉庫管理部署'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.倉庫管理部署名            IS '倉庫管理部署名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.主要保管倉庫コード        IS '主要保管倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.主要保管倉庫名            IS '主要保管倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.仕入先コード              IS '仕入先コード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.仕入先名                  IS '仕入先名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.仕入先サイトコード        IS '仕入先サイトコード'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.仕入先サイト名            IS '仕入先サイト名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ドリンク基準カレンダ      IS 'ドリンク基準カレンダ'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.ドリンク基準カレンダ名    IS 'ドリンク基準カレンダ名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.リーフ基準カレンダ        IS 'リーフ基準カレンダ'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.リーフ基準カレンダ名      IS 'リーフ基準カレンダ名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.出荷引当対象フラグ        IS '出荷引当対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.出荷引当対象フラグ名      IS '出荷引当対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.Ｄ_１倉庫フラグ           IS 'Ｄ＋１倉庫フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.Ｄ_１倉庫フラグ名         IS 'Ｄ＋１倉庫フラグ名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.直送倉庫区分              IS '直送倉庫区分'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.直送倉庫区分名            IS '直送倉庫区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.作成者                    IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.作成日                    IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.最終更新者                IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.最終更新日                IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKZ_OPM保管場所マスタ_現在_V.最終更新ログイン          IS '最終更新ログイン'
/