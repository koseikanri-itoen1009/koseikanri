/*************************************************************************
 * 
 * View  Name      : XXSKZ_リーフ在庫実績_基本_V
 * Description     : XXSKZ_リーフ在庫実績_基本_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 * ------------- ----- ------------ -------------------------------------
 *  2020/09/03    1.0   Y.Koh        新規作成
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_リーフ在庫実績_基本_V
(
 対象年月
,倉庫コード
,倉庫名
...
SELECT
  ...
  FROM
             ...
             FROM
                   ( --======================================================================
                    ...
                    UNION ALL
                     ...
                     --======================================================================
                      SELECT
                             ...
                             ,0                             etc_out_qty         --その他_出庫バラ数
                        FROM
                              xxcmn_stc_inv_month_stck_arc      XSIM            --棚卸月末在庫アドオン
                             ,xxskz_item_locations_v            XILV            --保管場所取得用
                       WHERE
                    ...
                    UNION ALL
                     ...
                     --======================================================================
                      ...
                      SELECT
                             ...
                             ,0                             etc_out_qty         --その他_出庫バラ数
                        FROM
                              xxcmn_mov_req_instr_hdrs_arc  XMH                 --移動依頼/指示ヘッダ（アドオン）バックアップ
                             ,xxcmn_mov_req_instr_lines_arc XML                 --移動依頼/指示明細（アドオン）バックアップ
                             ,xxcmn_mov_lot_details_arc     XMD                 --移動ロット詳細（アドオン）バックアップ
                             ,xxskz_item_locations_v        XIL                 --OPM保管場所情報VIEW
                             ,xxskz_item_locations_v        XIL_TO              --OPM保管場所情報VIEW
                       WHERE
                         ...
                         AND  XMH.ship_to_locat_id          = XIL_TO.inventory_location_id
                         AND  XIL.whse_code                <> XIL_TO.whse_code
                      --[ １．移動出庫実績（積層中のみ） END ]--
                    UNION ALL
                      ----------------------------------------------------------------------
                      -- ２．出荷実績（積送中のみ）
                      ...
                      SELECT
                             ...
                             ,0                             etc_out_qty         --その他_出庫バラ数
                        FROM
                              xxcmn_order_headers_all_arc   XOH                 --受注ヘッダ（アドオン）バックアップ
                             ,xxcmn_order_lines_all_arc     XOL                 --受注明細（アドオン）バックアップ
                             ,xxcmn_mov_lot_details_arc     XMD                 --移動ロット詳細（アドオン）バックアップ
                             ,oe_transaction_types_all      OTA                 --受注タイプ
                             ,xxskz_item_locations2_v       XIL                 --保管場所マスタ
                       WHERE
                         -- 出庫日の年月 < 入庫日の年月   …積層中と判断
                              TO_CHAR( XOH.shipped_date, 'YYYYMM' ) 
                    ...
                    UNION ALL
                      ...
                      SELECT
                        ...
                        FROM
                             ...
                             ,xxcmn_mov_lot_details_arc     XMD                 --移動ロット詳細（アドオン）バックアップ
                             ,xxskz_item_locations_v        XIL                 --OPM保管場所情報VIEW
                             ,xxskz_item_locations_v        XIL_FROM            --OPM保管場所情報VIEW
                       WHERE
                         -- 移動依頼/指示ヘッダ(アドオン)の条件
                         ...
                         AND  XMH.shipped_locat_id          = XIL_FROM.inventory_location_id
                         AND  XIL.whse_code                <> XIL_FROM.whse_code
                      --[ ３．移動入庫実績 END ]--
                    UNION ALL
                      ----------------------------------------------------------------------
                      -- ４．倉替返品入庫実績
                    ...
                    UNION ALL
                      ...
                      SELECT
                             ...
                             ,0                         stc_r_qty               --棚卸バラ数
                             ,0                         cargo_qty               --積送中バラ数
                             ,gmd.actual_qty            month_qty               --月末在庫バラ数 …入出庫数の積み上げを使用して月末在庫数を求める
                             ,0                         rev_in_qty              --返品_入庫バラ数(仕入先返品)
                             ,0                         po_in_qty               --生産_入庫バラ数(受入)
                             ,0                         mov_in_qty              --移動_入庫バラ数
                             ,gmd.actual_qty            etc_in_qty              --その他_入庫バラ数
                             ,0                         oe_out_qty              --拠点_出庫バラ数(出荷)
                             ,0                         mov_out_qty             --移動_出庫バラ数
                             ,0                         etc_out_qty             --その他_出庫バラ数
                        FROM  xxcmn_gme_batch_header_arc      gbh               -- 生産バッチ
                             ,xxcmn_gme_material_details_arc  gmd               -- 生産原料詳細
                             ,xxcmn_ic_tran_pnd_arc           itp               -- OPM保留在庫トランザクション
                       WHERE  itp.delete_mark           = 0                     -- 有効チェック(OPM保留在庫)
                         AND  itp.completed_ind         = 1                     -- 完了(⇒実績)
                         AND  itp.reverse_id            IS NULL
                         AND  gmd.material_detail_id    = itp.line_id
                         AND  gmd.line_type             = '1'                   -- 1:完成品
                         AND  gbh.batch_id              = gmd.batch_id
                         AND  gbh.batch_status          IN ( '2', '3', '4' )    -- 2:WIP 3:完了: 4:クローズ
                      --[ ６．自社生産の入庫実績 END ]--
                     --<< 各トランザクションから月間入庫数を取得 END >>--
                    UNION ALL
                     --======================================================================
                  ...
                  ,( -- 月末在庫数を求める最大期間を取得（それ以降は月首在庫数が分からない為に月末在庫が求まらない）
                     SELECT  TO_CHAR( ADD_MONTHS( TO_DATE( MAX( XSIM.invent_ym ) || '01', 'YYYYMMDD' ), 1 ), 'YYYYMM' )  yyyymm
                       FROM  xxcmn_stc_inv_month_stck_arc    XSIM
                   )  MYM
       ...
       ,xxskz_item_mst2_v         ITEM    --品目名称取得用
 ...
 WHERE
   ...
   --商品区分:リーフの条件  (※ここで条件を絞る方がレスポンスが良い)
   AND  PRODC.prod_class_code     = '1'
...
/
COMMENT ON TABLE APPS.XXSKZ_リーフ在庫実績_基本_V IS 'SKYLINK用 リーフ在庫実績（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.対象年月                IS '対象年月'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.倉庫コード              IS '倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.倉庫名                  IS '倉庫名'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.保管場所コード          IS '保管場所コード'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.保管場所名              IS '保管場所名'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.保管場所略称            IS '保管場所略称'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.商品区分                IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.商品区分名              IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.品目区分                IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.品目区分名              IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.群コード                IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.品目                    IS '品目'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.品目名                  IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.品目略称                IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.ロットNO                IS 'ロットNO'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.製造年月日              IS '製造年月日'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.固有記号                IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.賞味期限                IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.棚卸ケース数            IS '棚卸ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.棚卸バラ数              IS '棚卸バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.積送中在庫ケース数      IS '積送中在庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.積送中在庫バラ数        IS '積送中在庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.論理_月末在庫ケース数   IS '論理_月末在庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.論理_月末在庫バラ数     IS '論理_月末在庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.返品_月間入庫ケース数   IS '返品_月間入庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.返品_月間入庫バラ数     IS '返品_月間入庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.生産_月間入庫ケース数   IS '生産_月間入庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.生産_月間入庫バラ数     IS '生産_月間入庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.移動_月間入庫ケース数   IS '移動_月間入庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.移動_月間入庫バラ数     IS '移動_月間入庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.その他_月間入庫ケース数 IS 'その他_月間入庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.その他_月間入庫バラ数   IS 'その他_月間入庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.拠点_月間出庫ケース数   IS '拠点_月間出庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.拠点_月間出庫バラ数     IS '拠点_月間出庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.移動_月間出庫ケース数   IS '移動_月間出庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.移動_月間出庫バラ数     IS '移動_月間出庫バラ数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.その他_月間出庫ケース数 IS 'その他_月間出庫ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_リーフ在庫実績_基本_V.その他_月間出庫バラ数   IS 'その他_月間出庫バラ数'
/
