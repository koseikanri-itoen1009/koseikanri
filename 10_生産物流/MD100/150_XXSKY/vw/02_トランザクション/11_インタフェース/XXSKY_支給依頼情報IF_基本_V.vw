CREATE OR REPLACE VIEW APPS.XXSKY_支給依頼情報IF_基本_V
(
会社名
,データ種別
,伝送用枝番
...
SELECT
  ...
  FROM
        ...
        ,xxsky_locations2_v                 XL2V01      -- SKYLINK用中間VIEW 事業所情報VIEW2(依頼部署名)
        ,xxsky_locations2_v                 XL2V02      -- SKYLINK用中間VIEW 事業所情報VIEW2(指示部署名)
        ,xxsky_vendors2_v                   XV2V        -- SKYLINK用中間VIEW 仕入先情報VIEW2(取引先名)
        ,xxsky_party_sites2_v               XPS2V       -- SKYLINK用中間VIEW 配送先情報VIEW2(配送先名)
        ,xxsky_item_locations_v             XILV        -- SKYLINK用中間VIEW OPM保管場所情報VIEW(出庫倉庫名)
        ,xxsky_carriers2_v                  XC2V        -- SKYLINK用中間VIEW 運送業者情報VIEW2(運送業者名)
        ,xxsky_item_mst2_v                  XIM2V01     -- SKYLINK用中間VIEW OPM品目情報VIEW2(製造品目名)
        ,xxsky_item_mst2_v                  XIM2V02     -- SKYLINK用中間VIEW OPM品目情報VIEW2(品目名)
        ,xxsky_prod_class_v                 XPCV        -- SKYLINK用中間VIEW OPM品目区分VIEW(商品区分)
        ,xxsky_item_class_v                 XICV        -- SKYLINK用中間VIEW OPM品目区分VIEW(品目区分)
        ,xxsky_crowd_code_v                 XCCV        -- SKYLINK用中間VIEW OPM品目区分VIEW(群コード)
...
/
COMMENT ON TABLE APPS.XXSKY_支給依頼情報IF_基本_V IS 'SKYLINK用支給依頼情報インターフェース（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.会社名           IS '会社名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.データ種別       IS 'データ種別'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.伝送用枝番       IS '伝送用枝番'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.発生区分         IS '発生区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.発生区分名       IS '発生区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.重量容積区分     IS '重量容積区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.重量容積区分名   IS '重量容積区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.依頼部署コード   IS '依頼部署コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.依頼部署名       IS '依頼部署名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.指示部署コード   IS '指示部署コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.指示部署名       IS '指示部署名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.取引先コード     IS '取引先コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.取引先名         IS '取引先名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.配送先コード     IS '配送先コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.配送先名         IS '配送先名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.出庫倉庫コード   IS '出庫倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.出庫倉庫名       IS '出庫倉庫名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.運送業者コード   IS '運送業者コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.運送業者名       IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.出庫日           IS '出庫日'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.入庫日           IS '入庫日'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.運賃区分         IS '運賃区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.運賃区分名       IS '運賃区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.引取区分         IS '引取区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.引取区分名       IS '引取区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.着荷時間FROM     IS '着荷時間FROM'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.着荷時間FROM名   IS '着荷時間FROM名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.着荷時間TO       IS '着荷時間TO'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.着荷時間TO名     IS '着荷時間TO名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.製造日           IS '製造日'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.製造品目コード   IS '製造品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.製造品目名       IS '製造品目名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.製造品目略称     IS '製造品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.製造番号         IS '製造番号'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.ヘッダ摘要       IS 'ヘッダ摘要'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.明細番号         IS '明細番号'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.商品区分         IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.商品区分名       IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.品目区分         IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.品目区分名       IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.群コード         IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.品目コード       IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.品目名           IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.品目略称         IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.付帯             IS '付帯'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.依頼数量         IS '依頼数量'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.明細摘要         IS '明細摘要'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.作成者           IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.作成日           IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.最終更新者       IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.最終更新日       IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_支給依頼情報IF_基本_V.最終更新ログイン IS '最終更新ログイン'
/
