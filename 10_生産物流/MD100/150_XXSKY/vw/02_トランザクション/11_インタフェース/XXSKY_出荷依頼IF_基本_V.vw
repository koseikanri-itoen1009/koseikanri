CREATE OR REPLACE VIEW APPS.XXSKY_出荷依頼IF_基本_V
(
 受注タイプ
,受注日
,出荷先
...
FROM
       ...
       ,xxsky_party_sites2_v                    XPSV        --出荷先名取得
       ,xxsky_locations2_v                      XLV_SHU     --出荷元事業所取得
       ,xxsky_cust_accounts2_v                  XCAV_KAN    --管轄拠点名取得
       ,xxsky_cust_accounts2_v                  XCAV_NYU    --入力拠点名取得
       ,fnd_lookup_values                       FLV_CHFROM  --着荷時間FROM名取得
       ,fnd_lookup_values                       FLV_CHTO    --着荷時間TO名取得用結合
       ,xxsky_carriers2_v                       XCV         --運送業者名取得
       ,fnd_lookup_values                       FLV_HAI     --配送区分名取得用結合
       ,fnd_lookup_values                       FLV_EOS     --EOSデータ種別名取得用結合
       ,fnd_lookup_values                       FLV_KURA    --倉替返品区分名取得用結合
       ,xxsky_item_locations_v                  XILV        --保管倉庫名取得
       ,( SELECT DISTINCT 
             request_class
            ,request_class_name
            ,start_date_active
       ...
       ,xxsky_locations2_v                      XLV_HOU     --報告部署所取得
       ,xxsky_item_mst2_v                       XIMV        --品目名取得(商品区分・品目区分・群コード取得にも使用)
       ,xxsky_prod_class_v                      XPCV        --商品区分取得
       ,xxsky_item_class_v                      XICV        --品目区分取得
       ,xxsky_crowd_code_v                      XCCV        --群コード取得
...
/
COMMENT ON TABLE APPS.XXSKY_出荷依頼IF_基本_V IS 'XXSKY_出荷依頼IF (基本) VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注タイプ            IS '受注タイプ'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注日                IS '受注日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷先                IS '出荷先'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷先名              IS '出荷先名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷指示              IS '出荷指示'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.顧客発注              IS '顧客発注'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注ソース参照        IS '受注ソース参照'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷予定日            IS '出荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷予定日            IS '着荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.パレット使用枚数      IS 'パレット使用枚数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.パレット回収枚数      IS 'パレット回収枚数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷元                IS '出荷元'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷元名              IS '出荷元名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.管轄拠点              IS '管轄拠点'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.管轄拠点名            IS '管轄拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.入力拠点              IS '入力拠点'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.入力拠点名            IS '入力拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷時間FROM          IS '着荷時間FROM'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷時間FROM名        IS '着荷時間FROM名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷時間TO            IS '着荷時間TO'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷時間TO名          IS '着荷時間TO名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.データタイプ          IS 'データタイプ'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.運送業者              IS '運送業者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.運送業者名            IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.配送区分              IS '配送区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.配送区分名            IS '配送区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.配送NO                IS '配送No'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷日                IS '出荷日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.着荷日                IS '着荷日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.EOSデータ種別         IS 'EOSデータ種別'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.EOSデータ種別名       IS 'EOSデータ種別名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.伝送用枝番            IS '伝送用枝番'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.入庫倉庫              IS '入庫倉庫'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.入庫倉庫名            IS '入庫倉庫名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.倉替返品区分          IS '倉替返品区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.倉替返品区分名        IS '倉替返品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.依頼区分              IS '依頼区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.依頼区分名            IS '依頼区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.報告部署              IS '報告部署'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.報告部署名            IS '報告部署名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.明細番号              IS '明細番号'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.商品区分              IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.商品区分名            IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.品目区分              IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.品目区分名            IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.群コード              IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注品目コード        IS '受注品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注品目名            IS '受注品目名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.受注品目略称          IS '受注品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.ケース数              IS 'ケース数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.数量                  IS '数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.出荷実績数量          IS '出荷実績数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.製造日                IS '製造日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.固有記号              IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.賞味期限              IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.内訳数量              IS '内訳数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.入庫実績数量          IS '入庫実績数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.保留ステータス        IS '保留ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.保留ステータス名      IS '保留ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.作成者                IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.作成日                IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.最終更新者            IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.最終更新日            IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼IF_基本_V.最終更新ログイン      IS '最終更新ログイン'
/
