CREATE OR REPLACE VIEW APPS.XXSKY_支給ヘッダ_基本_V
(
 依頼NO
,配送NO
,受注タイプ名
...
-- 2010/1/8 #627 Y.Fukami Mod End
  ...
  FROM  xxwsh_order_headers_all      XOHA    --受注ヘッダアドオン
       ...
       ,xxsky_vendors2_v             XV2V    --仕入先情報VIEW2(取引先名)
       ,xxsky_vendor_sites2_v        XVS2V   --仕入先サイト情報VIEW2(仕入先サイト名)
       ,xxsky_carriers2_v            XC2V01  --運送業者情報VIEW2(運送業者名)
       ,qp_list_headers_tl           QLHT    --価格表
       ,xxsky_item_locations2_v      XIL2V   --OPM保管場所情報VIEW2(出荷元保管場所名)
       ,xxsky_cust_accounts2_v       XCA2V02 --SKYLINK用中間VIEW 顧客情報VIEW2(入力拠点)
       ,xxsky_item_mst2_v            XIM2V   --SKYLINK用中間VIEW OPM品目情報VIEW2(製造品目名)
       ,xxsky_carriers2_v            XC2V02  --運送業者情報VIEW2(運送業者名)
       ,xxsky_locations2_v           XL2V01  --SKYLINK用中間VIEW 事業所情報VIEW2(成績管理部署名)
...
/
COMMENT ON TABLE APPS.XXSKY_支給ヘッダ_基本_V IS 'SKYLINK用支給ヘッダ（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.依頼NO IS '依頼No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送NO IS '配送No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.受注タイプ名 IS '受注タイプ名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.組織名 IS '組織名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.受注日 IS '受注日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.最新フラグ IS '最新フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.元依頼NO IS '元依頼No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.前回配送NO IS '前回配送No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.顧客 IS '顧客'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.顧客名 IS '顧客名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.取引先 IS '取引先'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.取引先名 IS '取引先名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.取引先サイト IS '取引先サイト'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.取引先サイト名 IS '取引先サイト名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷指示 IS '出荷指示'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者 IS '運送業者'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者名 IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分 IS '配送区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分名 IS '配送区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.価格表 IS '価格表'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.価格表名 IS '価格表名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.ステータス IS 'ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.ステータス名 IS 'ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷予定日 IS '出荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷予定日 IS '着荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運賃区分 IS '運賃区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運賃区分名 IS '運賃区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.支給出庫指示区分 IS '支給出庫指示区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.支給出庫指示区分名 IS '支給出庫指示区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.支給指示受領区分 IS '支給指示受領区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.支給指示受領区分名 IS '支給指示受領区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.有償金額確定区分 IS '有償金額確定区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.有償金額確定区分名 IS '有償金額確定区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.引取区分 IS '引取区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.引取区分名 IS '引取区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷元保管場所 IS '出荷元保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷元保管場所名 IS '出荷元保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.入力拠点 IS '入力拠点'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.入力拠点名 IS '入力拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.入力拠点略称 IS '入力拠点略称'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.発注NO IS '発注No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.契約外運賃区分 IS '契約外運賃区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.契約外運賃区分名 IS '契約外運賃区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷時間FROM IS '着荷時間FROM'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷時間FROM名 IS '着荷時間FROM名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷時間TO IS '着荷時間TO'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷時間TO名 IS '着荷時間TO名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.製造品目 IS '製造品目'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.製造品目名 IS '製造品目名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.製造日 IS '製造日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.製造枝番 IS '製造枝番'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.送り状NO IS '送り状No'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.合計数量 IS '合計数量'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.小口個数 IS '小口個数'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.ラベル枚数 IS 'ラベル枚数'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.重量積載効率 IS '重量積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.容積積載効率 IS '容積積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.基本重量 IS '基本重量'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.基本容積 IS '基本容積'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.積載重量合計 IS '積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.積載容積合計 IS '積載容積合計'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.混載率 IS '混載率'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.パレット合計枚数 IS 'パレット合計枚数'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.パレット実績枚数 IS 'パレット実績枚数'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.合計パレット重量 IS '合計パレット重量'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者_実績 IS '運送業者_実績'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者_予実 IS '運送業者_予実'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者_実績名 IS '運送業者_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.運送業者_予実名 IS '運送業者_予実名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分_実績 IS '配送区分_実績'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分_予実 IS '配送区分_予実'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分_実績名 IS '配送区分_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送区分_予実名 IS '配送区分_予実名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷日 IS '出荷日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.出荷日_予実 IS '出荷日_予実'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.着荷日 IS '着荷日'
/
...
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.実績計上済区分 IS '実績計上済区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.通知ステータス IS '通知ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.通知ステータス名 IS '通知ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.前回通知ステータス IS '前回通知ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.前回通知ステータス名 IS '前回通知ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.確定通知実施日時 IS '確定通知実施日時'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.新規修正フラグ IS '新規修正フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.新規修正フラグ名 IS '新規修正フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.成績管理部署 IS '成績管理部署'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.成績管理部署名 IS '成績管理部署名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.指示部署 IS '指示部署'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.指示部署名 IS '指示部署名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.混載記号 IS '混載記号'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.画面更新日時 IS '画面更新日時'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.画面更新者 IS '画面更新者'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_処理種別 IS '配送_処理種別'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_処理種別名 IS '配送_処理種別名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_混載種別 IS '配送_混載種別'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_混載種別名 IS '配送_混載種別名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_配送先コード区分 IS '配送_配送先コード区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_配送先コード区分名 IS '配送_配送先コード区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_自動配車対象区分 IS '配送_自動配車対象区分'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_自動配車対象区分名 IS '配送_自動配車対象区分名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_摘要 IS '配送_摘要'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_支払運賃計算対象フラグ IS '配送_支払運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_支払運賃計算対象フラグ名 IS '配送_支払運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_請求運賃計算対象フラグ IS '配送_請求運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_請求運賃計算対象フラグ名 IS '配送_請求運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_積載重量合計 IS '配送_積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_積載容積合計 IS '配送_積載容積合計'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_重量積載効率 IS '配送_重量積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_容積積載効率 IS '配送_容積積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_運賃形態 IS '配送_運賃形態'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.配送_運賃形態名 IS '配送_運賃形態名'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.作成者 IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.作成日 IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.最終更新者 IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.最終更新日 IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_支給ヘッダ_基本_V.最終更新ログイン IS '最終更新ログイン'
/
