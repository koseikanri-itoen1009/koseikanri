/*************************************************************************
 * 
 * View  Name      : XXSKZ_出荷明細ロット別_基本_V
 * Description     : XXSKZ_出荷明細ロット別_基本_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_出荷明細ロット別_基本_V
(
 依頼No
,明細番号
,商品区分
...
SELECT  XOL.request_no                  request_no                 --依頼No
       ...
       ,CASE WHEN ITEM1.lot_ctl = 1 THEN ILM.attribute1      --ロット管理品   →製造年月日を取得
             ELSE NULL                                       --非ロット管理品 →NULL
        END                             lot_date                   --製造年月日
       ,CASE WHEN ITEM1.lot_ctl = 1 THEN ILM.attribute2      --ロット管理品   →固有記号を取得
             ELSE NULL                                       --非ロット管理品 →NULL
        END                             lot_sign                   --固有記号
       ,CASE WHEN ITEM1.lot_ctl = 1 THEN ILM.attribute3      --ロット管理品   →賞味期限を取得
             ELSE NULL                                       --非ロット管理品 →NULL
        END                             best_bfr_date              --賞味期限
       ,XOL.instruct_qty                instruct_qty               --ロット別指示数量
       ,XOL.shipped_quantity            shipped_quantity           --ロット別出荷実績数量
  ...
  FROM  ( --名称取得系以外のデータはこの内部SQLで全て取得する
          SELECT XOLA.request_no                                   --依頼No
                ...
                ,XMLD.lot_id                                       --ロットID
                ,XMLD.lot_no                                       --ロットno
                ,XMLD.instruct_qty                                 --ロット別指示数量
                ,XMLD.shipped_quantity                             --ロット別出荷実績数量
                ,XOLA.uom_code                                     --単位
...
/
COMMENT ON TABLE APPS.XXSKZ_出荷明細ロット別_基本_V IS 'SKYLINK用 出荷明細ロット別（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.依頼No                         IS '依頼No'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.明細番号                       IS '明細番号'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.商品区分                       IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.商品区分名                     IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.品目区分                       IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.品目区分名                     IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.群コード                       IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.出荷品目                       IS '出荷品目'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.出荷品目名                     IS '出荷品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.出荷品目略称                   IS '出荷品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.ロットNO                       IS 'ロットNO'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.製造年月日                     IS '製造年月日'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.固有記号                       IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.賞味期限                       IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.ロット別指示数量               IS 'ロット別指示数量'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.ロット別出荷実績数量           IS 'ロット別出荷実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.単位                           IS '単位'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.依頼品目                       IS '依頼品目'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.依頼品目名                     IS '依頼品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.依頼品目略称                   IS '依頼品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.指定製造日                     IS '指定製造日'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.指定日付_リーフ                IS '指定日付_リーフ'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.移動No                         IS '移動No'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.発注No                         IS '発注No'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.顧客発注                       IS '顧客発注'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.警告区分                       IS '警告区分'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.警告区分名                     IS '警告区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.警告日付                       IS '警告日付'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.摘要                           IS '摘要'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.出荷依頼インタフェース済フラグ IS '出荷依頼インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.出荷実績インタフェース済フラグ IS '出荷実績インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.作成者                         IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.作成日                         IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.最終更新者                     IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.最終更新日                     IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKZ_出荷明細ロット別_基本_V.最終更新ログイン               IS '最終更新ログイン'
/
