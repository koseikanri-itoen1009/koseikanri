CREATE OR REPLACE VIEW APPS.XXSKY_出荷依頼締め_基本_V
(
 締めコンカレントID
,受注タイプ
,出荷元保管場所
...
SELECT 
  ...
  FROM  xxwsh_tightening_control    XTC                         --出荷依頼締め管理（アドオン）
       ...
       ,xxsky_item_locations_v      XILV                        --SKYLINK用中間VIEW 出荷元保管場所名取得VIEW
       ,xxsky_cust_accounts2_v      XCA2V                       --SKYLINK用中間VIEW 拠点名取得VIEW
...
/
COMMENT ON TABLE APPS.XXSKY_出荷依頼締め_基本_V                     IS 'SKYLINK用出荷依頼締め（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.締めコンカレントID IS '締めコンカレントID'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.受注タイプ         IS '受注タイプ'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.出荷元保管場所     IS '出荷元保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.出荷元保管場所名   IS '出荷元保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.商品区分           IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.商品区分名         IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.拠点               IS '拠点'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.拠点名             IS '拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.拠点カテゴリ       IS '拠点カテゴリ'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.拠点カテゴリ名     IS '拠点カテゴリ名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.生産物流LT         IS '生産物流LT'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.出荷予定日         IS '出荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.締め_解除区分      IS '締め_解除区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.締め_解除区分名    IS '締め_解除区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.締め実施日時       IS '締め実施日時'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.基準レコード区分   IS '基準レコード区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.基準レコード区分名 IS '基準レコード区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.作成者             IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.作成日             IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.最終更新者         IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.最終更新日         IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷依頼締め_基本_V.最終更新ログイン   IS '最終更新ログイン'
/                                                                       
