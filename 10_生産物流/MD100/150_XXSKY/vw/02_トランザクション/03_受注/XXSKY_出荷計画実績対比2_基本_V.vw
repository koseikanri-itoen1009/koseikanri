CREATE OR REPLACE VIEW APPS.XXSKY_出荷計画実績対比2_基本_V
(
 着日
,商品区分
,商品区分名
...
--2009.02.16 Add<-
                            ...
                            ,0                                 shipped_qty       --出荷実績数
                       FROM  xxwsh_order_headers_all           XOHA              --受注(出荷)ヘッダ
                            ,xxwsh_order_lines_all             XOLA              --受注(出荷)明細
...
--2009.02.16 Add<-
                       ...
                       FROM  mrp_forecast_designators          MFDN              --フォーキャスト名テーブル
                            ...
                            ,xxsky_item_mst2_v                 XIMV              --品目情報取得用
...
--2009.02.16 Add<-
       ...
       ,xxsky_item_mst2_v         ITEM
       ,xxsky_prod_class_v        PRODC
       ,xxsky_item_class_v        ITEMC
       ,xxsky_crowd_code_v        CROWD
       ,xxsky_cust_accounts2_v    CSACT
--2009.02.16 Add->
       ,xxsky_item_locations2_v   XIL2V
--2009.02.16 Add<-
 WHERE
   --品目情報取得条件
...
--2009.02.16 Add<-
/
COMMENT ON TABLE APPS.XXSKY_出荷計画実績対比2_基本_V IS 'SKYLINK用 XXSKY_出荷計画実績対比2（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.着日 IS '着日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.群コード IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.品目 IS '品目'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.品目名 IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.品目略称 IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.拠点 IS '拠点'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.拠点名 IS '拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.出荷元保管場所 IS '出荷元保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.出荷元保管場所名 IS '出荷元保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.引取計画数 IS '引取計画数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.出荷予定数 IS '出荷予定数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.出荷実績数 IS '出荷実績数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.差異数 IS '差異数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷計画実績対比2_基本_V.引取率 IS '引取率'
/
