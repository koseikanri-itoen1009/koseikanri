/*************************************************************************
 * 
 * View  Name      : XXSKZ_入出庫配送明細_基本_V
 * Description     : XXSKZ_入出庫配送明細_基本_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_入出庫配送明細_基本_V
(
 依頼_移動NO
,タイプ
,移動タイプ
...
-- 2010/1/7	#627 Y.Fukami Mod Start
  ...
  FROM (
        ...
        UNION ALL
         ...
         SELECT
                ...
                ,NVL( XMVH.actual_arrival_date, XMVH.schedule_arrival_date )    --NVL( 着荷日, 着荷予定日 )
                                                            arrival_date                  --着荷日 (⇒品目名称取得で使用)
           FROM  xxcmn_mov_req_instr_hdrs_arc  XMVH             --移動依頼/指示ヘッダ（アドオン）バックアップ
                ,xxcmn_mov_req_instr_lines_arc   XMVL           --移動依頼/指示明細（アドオン）バックアップ
                ,xxcmn_mov_lot_details_arc       XMLD           --移動ロット詳細（アドオン）バックアップ
       ...
       ,xxskz_item_mst2_v               ITEM1                    --SKYLINK用中間VIEW OPM品目情報VIEW(出荷移動_品目)
       ,xxskz_item_mst2_v               ITEM2                    --SKYLINK用中間VIEW OPM品目情報VIEW(依頼品目)
       ,xxskz_prod_class_v              PRODC                    --SKYLINK用中間VIEW 商品区分情報VIEW
       ,xxskz_item_class_v              ITEMC                    --SKYLINK用中間VIEW 品目区分情報VIEW
       ,xxskz_crowd_code_v              CROWD                    --SKYLINK用中間VIEW 群コード情報VIEW
...
/
COMMENT ON TABLE APPS.XXSKZ_入出庫配送明細_基本_V IS 'SKYLINK用入出庫配送明細（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.依頼_移動NO IS '依頼_移動No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.タイプ IS 'タイプ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.移動タイプ IS '移動タイプ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.移動タイプ名 IS '移動タイプ名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.明細番号 IS '明細番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.レコードタイプ IS 'レコードタイプ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.レコードタイプ名 IS 'レコードタイプ名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.組織名 IS '組織名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.群コード IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出荷移動_品目 IS '出荷移動_品目'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出荷移動_品目名 IS '出荷移動_品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出荷移動_品目略称 IS '出荷移動_品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.ロットNO IS 'ロットNo'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.製造年月日 IS '製造年月日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.固有記号 IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.賞味期限 IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.削除フラグ IS '削除フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.依頼数量 IS '依頼数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.指示数量 IS '指示数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.単位 IS '単位'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.単価 IS '単価'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.指定製造日 IS '指定製造日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.依頼品目 IS '依頼品目'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.依頼品目名 IS '依頼品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.依頼品目略称 IS '依頼品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.付帯コード IS '付帯コード'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.指定日付_リーフ IS '指定日付_リーフ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.移動NO IS '移動No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.発注NO IS '発注No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.顧客発注 IS '顧客発注'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.パレット数 IS 'パレット数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.段数 IS '段数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.ケース数 IS 'ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.重量 IS '重量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.容積 IS '容積'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.パレット枚数 IS 'パレット枚数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.パレット重量 IS 'パレット重量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.引当数 IS '引当数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.参照移動番号 IS '参照移動番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.参照発注番号 IS '参照発注番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.初回指示数量 IS '初回指示数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出庫実績数量 IS '出庫実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.入庫実績数量 IS '入庫実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.警告区分 IS '警告区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.警告区分名 IS '警告区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.警告日付 IS '警告日付'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.摘要 IS '摘要'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出荷依頼インタフェース済フラグ IS '出荷依頼インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.出荷実績インタフェース済フラグ IS '出荷実績インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.実績日 IS '実績日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.実績数量 IS '実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.訂正前実績数量 IS '訂正前実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.自動手動引当区分 IS '自動手動引当区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.自動手動引当区分名 IS '自動手動引当区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.作成者 IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.作成日 IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.最終更新者 IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.最終更新日 IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送明細_基本_V.最終更新ログイン IS '最終更新ログイン'
/
