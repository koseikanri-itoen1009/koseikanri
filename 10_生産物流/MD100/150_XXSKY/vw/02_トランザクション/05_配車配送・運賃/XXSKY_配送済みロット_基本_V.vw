CREATE OR REPLACE VIEW APPS.XXSKY_配送済みロット_基本_V
(
 配送先コード
,配送先名
,商品区分
...
-- *----------* 2009/06/23 本番#1438対応 end   *----------*
       ...
       ,xxsky_party_sites2_v       XPS2V                    --SKYLINK用中間VIEW 配送先情報VIEW2
 WHERE
   --品目情報取得条件
...
/
COMMENT ON TABLE APPS.XXSKY_配送済みロット_基本_V IS 'SKYLINK用配送済みロットマスタ（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.配送先コード IS '配送先コード'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.配送先名 IS '配送先名'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.群コード IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.品目コード IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.品目名 IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.品目略称 IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.出庫日 IS '出庫日'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.最大製造年月日 IS '最大製造年月日'
/
COMMENT ON COLUMN APPS.XXSKY_配送済みロット_基本_V.最大賞味期限 IS '最大賞味期限'
/
