/*************************************************************************
 * 
 * View  Name      : XXSKZ_入出庫配送品目別_基本_V
 * Description     : XXSKZ_入出庫配送品目別_基本_V
 * MD.070          : 
 * Version         : 1.0
 * 
 * Change Record
 ...
 ************************************************************************/
CREATE OR REPLACE VIEW APPS.XXSKZ_入出庫配送品目別_基本_V
(
 依頼_移動NO
,タイプ
,移動タイプ
...
-- 2010/1/7 #627 Y.Fukami Mod End
  ...
  FROM (
         ...
         SELECT
                ...
                ,XOLA.created_by                            created_by                    --作成者
                ,XOLA.creation_date                         creation_date                 --作成日
                ,XOLA.last_updated_by                       last_updated_by               --最終更新者
                ,XOLA.last_update_date                      last_update_date              --最終更新日
                ,XOLA.last_update_login                     last_update_login             --最終更新ログイン
        ...
        UNION ALL
         ...
         SELECT
                ...
                ,NULL                                       shipping_request_if_flg       --出荷依頼インタフェース済フラグ
                ,NULL                                       shipping_result_if_flg        --出荷実績インタフェース済フラグ
                ,XMVL.created_by                            created_by                    --作成者
                ,XMVL.creation_date                         creation_date                 --作成日
                ,XMVL.last_updated_by                       last_updated_by               --最終更新者
                ,XMVL.last_update_date                      last_update_date              --最終更新日
                ,XMVL.last_update_login                     last_update_login             --最終更新ログイン
...
/
COMMENT ON TABLE APPS.XXSKZ_入出庫配送品目別_基本_V IS 'SKYLINK用入出庫配送品目別（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.依頼_移動NO IS '依頼_移動No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.タイプ IS 'タイプ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.移動タイプ IS '移動タイプ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.移動タイプ名 IS '移動タイプ名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.明細番号 IS '明細番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.組織名 IS '組織名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.群コード IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出荷移動_品目 IS '出荷移動_品目'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出荷移動_品目名 IS '出荷移動_品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出荷移動_品目略称 IS '出荷移動_品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.削除フラグ IS '削除フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.依頼数量 IS '依頼数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.指示数量 IS '指示数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.単位 IS '単位'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.単価 IS '単価'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.指定製造日 IS '指定製造日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.依頼品目 IS '依頼品目'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.依頼品目名 IS '依頼品目名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.依頼品目略称 IS '依頼品目略称'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.付帯コード IS '付帯コード'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.指定日付_リーフ IS '指定日付_リーフ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.移動NO IS '移動No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.発注NO IS '発注No'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.顧客発注 IS '顧客発注'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.パレット数 IS 'パレット数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.段数 IS '段数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.ケース数 IS 'ケース数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.重量 IS '重量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.容積 IS '容積'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.パレット枚数 IS 'パレット枚数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.パレット重量 IS 'パレット重量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.引当数 IS '引当数'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.参照移動番号 IS '参照移動番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.参照発注番号 IS '参照発注番号'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.初回指示数量 IS '初回指示数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出庫実績数量 IS '出庫実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.入庫実績数量 IS '入庫実績数量'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.警告区分 IS '警告区分'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.警告区分名 IS '警告区分名'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.警告日付 IS '警告日付'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.摘要 IS '摘要'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出荷依頼インタフェース済フラグ IS '出荷依頼インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.出荷実績インタフェース済フラグ IS '出荷実績インタフェース済フラグ'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.作成者 IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.作成日 IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.最終更新者 IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.最終更新日 IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKZ_入出庫配送品目別_基本_V.最終更新ログイン IS '最終更新ログイン'
/
