CREATE OR REPLACE VIEW APPS.XXSKY_運賃品目別_基本_V2
(
 配送NO
,依頼_移動NO
,区分
...
--                        ,XOLA.shipping_item_code                                   --出荷品目コード
                          ...
                          ,(    -- 配送NO単位の積載重量合計算出
                            SELECT delivery_no                                       --配送NO
                              ...
                              FROM (SELECT delivery_no
                                    ...
                                    SELECT delivery_no
                                          ,sum_weight
                                      FROM xxinv_mov_req_instr_headers
                                   )
               ...
               ,xxsky_cust_accounts2_v          XCAV     --SKYLINK用中間VIEW 顧客情報VIEW2(管轄拠点)
               ,xxsky_carriers2_v               XCRV     --SKYLINK用中間VIEW 運送業者情報VIEW2(運送業者名)
               ,xxsky_party_sites2_v            XPSV     --SKYLINK用中間VIEW 配送先情報VIEW2(配送先名)
               ,xxsky_item_locations2_v         XILV     --SKYLINK用中間VIEW OPM保管場所情報VIEW2(出庫元名)
               ,xxsky_item_mst2_v               ITEM     --SKYLINK用中間VIEW OPM品目情報VIEW2(品目情報)
                          ...
                          ,(    -- 配送NO単位の最大依頼NO、最大明細番号取得
                            SELECT delivery_no
                              ...
                              FROM (SELECT OH.delivery_no
                                      ...
                                      FROM xxwsh_order_headers_all OH
                                          ,xxwsh_order_lines_all   OL
                                    ...
                                    SELECT MH.delivery_no
                                      ...
                                      FROM xxinv_mov_req_instr_headers MH
                                          ,xxinv_mov_req_instr_lines   ML
                                      ...
                                      FROM (
                                            SELECT XMRH11.delivery_no
                                              ...
                                              FROM xxinv_mov_req_instr_headers XMRH11,
                                                   xxinv_mov_req_instr_lines   XMRL11
...
--                        ,XOLA2.shipping_item_code                                   --出荷品目コード
                     ...
                     FROM  xxwsh_order_headers_all         XOHA2                     --受注ヘッダアドオン
                          ,oe_transaction_types_all        OTTA2                     --受注タイプマスタ
                          ,oe_transaction_types_tl         OTTT2                     --受注タイプ名マスタ
                          ,xxwsh_order_lines_all           XOLA2                     --受注明細アドオン
                          ,(    -- 依頼NO単位の積載重量合計算出
...
--                        ,XMRL.shipped_quantity             quantity                --品目_数量
                          ...
                          ,XDLV.qty1                                                 --配送_個数１
                          ,XDLV.total_amount                                         --配送_合計金額
                     FROM  xxinv_mov_req_instr_headers     XMRH                      --移動ヘッダアドオン
                          ,xxinv_mov_req_instr_lines       XMRL                      --移動明細アドオン
                          ,(    -- 配送NO単位の積載重量合計算出
                            SELECT delivery_no                                       --配送NO
                                  ,SUM(sum_weight)   sum_loading_weight              --積載重量合計
                              FROM (SELECT delivery_no
               ...
               ,xxsky_locations2_v              XL2V     --SKYLINK用中間VIEW 事業所情報VIEW2(管轄拠点名)
               ,xxsky_carriers2_v               XCRV     --SKYLINK用中間VIEW 運送業者情報VIEW2(運送業者名)
               ,xxsky_item_locations2_v         XILV1    --SKYLINK用中間VIEW OPM保管場所情報VIEW2(入庫先名)
               ,xxsky_item_locations2_v         XILV2    --SKYLINK用中間VIEW OPM保管場所情報VIEW2(出庫元名)
               ,xxsky_item_mst2_v               ITEM     --SKYLINK用中間VIEW OPM品目情報VIEW2(品目情報)
               ,fnd_lookup_values               FLV01    --クイックコード(ステータス名)
               ,fnd_lookup_values               FLV02    --クイックコード(配送区分名)
         WHERE
...
-- 2009.01.21↑
       ...
       ,xxsky_prod_class_v              PRODC    --SKYLINK用中間VIEW 商品区分VIEW
       ,xxsky_item_class_v              ITEMC    --SKYLINK用中間VIEW 品目区分VIEW
       ,xxsky_crowd_code_v              CROWD    --SKYLINK用中間VIEW 群コードVIEW
 WHERE
   -- 出荷品目のカテゴリ情報取得条件
...
/
COMMENT ON TABLE APPS.XXSKY_運賃品目別_基本_V2 IS 'SKYLINK用運賃品目別（基本） VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.配送NO IS '配送No'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.依頼_移動NO IS '依頼_移動No'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.区分 IS '区分'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.受注タイプ IS '受注タイプ'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.ステータス IS 'ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.ステータス名 IS 'ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.管轄拠点 IS '管轄拠点'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.管轄拠点名 IS '管轄拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.運送業者 IS '運送業者'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.運送業者名 IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.運送業者略称 IS '運送業者略称'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.入庫先_配送先 IS '入庫先_配送先'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.入庫先_配送先名 IS '入庫先_配送先名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.入庫先_配送先略称 IS '入庫先_配送先略称'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.出庫元 IS '出庫元'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.出庫元名 IS '出庫元名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.出庫元略称 IS '出庫元略称'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.配送区分 IS '配送区分'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.配送区分名 IS '配送区分名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.出庫日 IS '出庫日'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.入庫日 IS '入庫日'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.商品区分 IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.商品区分名 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目区分 IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目区分名 IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.群コード IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目コード IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目名称 IS '品目名称'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目略称 IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目バラ数 IS '品目バラ数'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目ケース数 IS '品目ケース数'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.合計ケース数 IS '合計ケース数'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.積載重量合計 IS '積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.按分計算用_積載重量合計 IS '按分計算用_積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目重量合計 IS '品目重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.按分計算用_品目重量合計 IS '按分計算用_品目重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.合計金額 IS '合計金額'
/
COMMENT ON COLUMN APPS.XXSKY_運賃品目別_基本_V2.品目_合計金額 IS '品目_合計金額'
/
