CREATE OR REPLACE VIEW APPS.XXSKY_入出庫配送ヘッダ_基本_V
(
 依頼_移動NO
,配送NO
,タイプ
...
-- *----------* 2009/06/23 本番#1438対応 end   *----------*
           ...
           FROM
                ...
                ,xxsky_party_sites2_v        PSIT1          --SKYLINK用中間VIEW 配送先情報VIEW(出荷_入庫先)
                ,xxsky_party_sites2_v        PSIT2          --SKYLINK用中間VIEW 配送先情報VIEW(出荷先_実績)
...
-- *----------* 2009/06/23 本番#1438対応 end   *----------*
        ...
        UNION ALL
         ...
         SELECT
           ...
           FROM
                ...
                ,oe_transaction_types_all    OTTA           --受注タイプマスタ
                ,oe_transaction_types_tl     OTTT           --受注タイプマスタ(日本語)
                ,xxsky_vendors2_v            VNDR           --SKYLINK用中間VIEW 配送先情報VIEW(取引先)
                ,xxsky_vendor_sites2_v       VSIT           --SKYLINK用中間VIEW 配送先情報VIEW(取引先サイト)
                ,fnd_lookup_values           FLV01          --クイックコード(ステータス名)
          WHERE
            --出荷情報の取得
        ...
        UNION ALL
         ...
         SELECT
           ...
           FROM
                 xxinv_mov_req_instr_headers XMVH           --移動依頼/指示ヘッダアドオン
                ,xxsky_item_locations2_v     ILCT           --SKYLINK用中間VIEW 保管場所情報VIEW(出荷_入庫先保管場所)
       ...
       ,xxsky_cust_accounts2_v               CUST1          --SKYLINK用中間VIEW 顧客情報VIEW(顧客)
       ,xxsky_cust_accounts2_v               CUST2          --SKYLINK用中間VIEW 顧客情報VIEW(管轄拠点)
       ,xxsky_cust_accounts2_v               CUST3          --SKYLINK用中間VIEW 顧客情報VIEW(入力拠点)
       ,xxsky_carriers2_v                    CARR1          --SKYLINK用中間VIEW 運送業者情報VIEW(運送業者)
       ,xxsky_carriers2_v                    CARR2          --SKYLINK用中間VIEW 運送業者情報VIEW(運送業者_実績)
       ,xxsky_carriers2_v                    CARR3          --SKYLINK用中間VIEW 運送業者情報VIEW(運送業者_予実)
       ,xxsky_item_locations2_v              ILCT1           --SKYLINK用中間VIEW 保管場所情報VIEW(出荷_出庫元保管場所)
       ,xxsky_item_mst2_v                    ITEM           --SKYLINK用中間VIEW 品目情報VIEW(製造品目)
       ,xxsky_locations2_v                   LOCT1          --SKYLINK用中間VIEW 事業所情報VIEW(成績管理部署)
       ,xxsky_locations2_v                   LOCT2          --SKYLINK用中間VIEW 事業所情報VIEW(指示部署)
       ,xxsky_locations2_v                   LOCT3          --SKYLINK用中間VIEW 事業所情報VIEW(振替先)
...
/
COMMENT ON TABLE APPS.XXSKY_入出庫配送ヘッダ_基本_V IS 'SKYLINK用入出庫配送ヘッダ（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.依頼_移動NO                   IS '依頼_移動NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送NO                        IS '配送NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.タイプ                        IS 'タイプ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.移動タイプ                    IS '移動タイプ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.移動タイプ名                  IS '移動タイプ名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.組織名                        IS '組織名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.受注_入力日                   IS '受注_入力日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.最新フラグ                    IS '最新フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.元依頼NO                      IS '元依頼NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.前回配送NO                    IS '前回配送NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.顧客                          IS '顧客'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.顧客名                        IS '顧客名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_入庫先                   IS '出荷_入庫先'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_入庫先名                 IS '出荷_入庫先名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷指示                      IS '出荷指示'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.取引先                        IS '取引先'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.取引先名                      IS '取引先名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.取引先サイト                  IS '取引先サイト'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.取引先サイト名                IS '取引先サイト名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者                      IS '運送業者'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者名                    IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分                      IS '配送区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分名                    IS '配送区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.顧客発注                      IS '顧客発注'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.価格表                        IS '価格表'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.価格表名                      IS '価格表名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.ステータス                    IS 'ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.ステータス名                  IS 'ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_出庫予定日               IS '出荷_出庫予定日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷_入庫予定日               IS '着荷_入庫予定日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.混載元NO                      IS '混載元NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.パレット回収枚数              IS 'パレット回収枚数'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.パレット枚数_出               IS 'パレット枚数_出'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.パレット枚数_入               IS 'パレット枚数_入'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.移動_摘要                     IS '移動_摘要'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.物流担当確認依頼区分          IS '物流担当確認依頼区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.物流担当確認依頼区分名        IS '物流担当確認依頼区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運賃区分                      IS '運賃区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運賃区分名                    IS '運賃区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.支給出庫指示区分              IS '支給出庫指示区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.支給出庫指示区分名            IS '支給出庫指示区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.支給指示受領区分              IS '支給指示受領区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.支給指示受領区分名            IS '支給指示受領区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.有償金額確定区分              IS '有償金額確定区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.有償金額確定区分名            IS '有償金額確定区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.引取区分                      IS '引取区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.引取区分名                    IS '引取区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_出庫元保管場所           IS '出荷_出庫元保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_出庫元保管場所名         IS '出荷_出庫元保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.管轄拠点                      IS '管轄拠点'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.管轄拠点名                    IS '管轄拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.管轄拠点略称                  IS '管轄拠点略称'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.入力拠点                      IS '入力拠点'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.入力拠点名                    IS '入力拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.入力拠点略称                  IS '入力拠点略称'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.発注NO                        IS '発注NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.商品区分                      IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.商品区分名                    IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.品目区分                      IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.品目区分名                    IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製品識別区分                  IS '製品識別区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製品識別区分名                IS '製品識別区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.指示なし実績区分              IS '指示なし実績区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.契約外運賃区分                IS '契約外運賃区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.契約外運賃区分名              IS '契約外運賃区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷時間FROM                  IS '着荷時間FROM'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷時間FROM名                IS '着荷時間FROM名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷時間TO                    IS '着荷時間TO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷時間TO名                  IS '着荷時間TO名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製造品目                      IS '製造品目'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製造品目名                    IS '製造品目名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製造品目略称                  IS '製造品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製造日                        IS '製造日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.製造枝番                      IS '製造枝番'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.送り状NO                      IS '送り状NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.合計数量                      IS '合計数量'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.小口個数                      IS '小口個数'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.ラベル枚数                    IS 'ラベル枚数'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.重量積載効率                  IS '重量積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.容積積載効率                  IS '容積積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.基本重量                      IS '基本重量'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.基本容積                      IS '基本容積'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.積載重量合計                  IS '積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.積載容積合計                  IS '積載容積合計'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.混載率                        IS '混載率'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.パレット合計枚数              IS 'パレット合計枚数'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.パレット実績枚数              IS 'パレット実績枚数'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.合計パレット重量              IS '合計パレット重量'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者_実績                 IS '運送業者_実績'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者_予実                 IS '運送業者_予実'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者_実績名               IS '運送業者_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.運送業者_予実名               IS '運送業者_予実名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分_実績                 IS '配送区分_実績'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分_予実                 IS '配送区分_予実'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分_実績名               IS '配送区分_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送区分_予実名               IS '配送区分_予実名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷先_実績                   IS '出荷先_実績'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷先_予実                   IS '出荷先_予実'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷先_実績名                 IS '出荷先_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷先_予実名                 IS '出荷先_予実名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_出庫日                   IS '出荷_出庫日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷_出庫日_予実              IS '出荷_出庫日_予実'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷_入庫日                   IS '着荷_入庫日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.着荷_入庫日_予実              IS '着荷_入庫日_予実'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.重量容積区分                  IS '重量容積区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.重量容積区分名                IS '重量容積区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.実績計上済区分                IS '実績計上済区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.実績訂正フラグ                IS '実績訂正フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.通知ステータス                IS '通知ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.通知ステータス名              IS '通知ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.前回通知ステータス            IS '前回通知ステータス'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.前回通知ステータス名          IS '前回通知ステータス名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.確定通知実施日時              IS '確定通知実施日時'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.新規修正フラグ                IS '新規修正フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.新規修正フラグ名              IS '新規修正フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.成績管理部署                  IS '成績管理部署'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.成績管理部署名                IS '成績管理部署名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.指示部署                      IS '指示部署'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.指示部署名                    IS '指示部署名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.振替先                        IS '振替先'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.振替先名                      IS '振替先名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.混載記号                      IS '混載記号'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.手配NO                        IS '手配NO'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.画面更新日時                  IS '画面更新日時'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.画面更新者                    IS '画面更新者'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.出荷依頼締め日時              IS '出荷依頼締め日時'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.締めコンカレントID            IS '締めコンカレントID'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.締め後修正区分                IS '締め後修正区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.締め後修正区分名              IS '締め後修正区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_処理種別                 IS '配送_処理種別'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_処理種別名               IS '配送_処理種別名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_混載種別                 IS '配送_混載種別'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_混載種別名               IS '配送_混載種別名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_配送先コード区分         IS '配送_配送先コード区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_配送先コード区分名       IS '配送_配送先コード区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_自動配車対象区分         IS '配送_自動配車対象区分'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_自動配車対象区分名       IS '配送_自動配車対象区分名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_摘要                     IS '配送_摘要'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_支払運賃計算対象フラグ   IS '配送_支払運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_支払運賃計算対象フラグ名 IS '配送_支払運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_請求運賃計算対象フラグ   IS '配送_請求運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_請求運賃計算対象フラグ名 IS '配送_請求運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_積載重量合計             IS '配送_積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_積載容積合計             IS '配送_積載容積合計'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_重量積載効率             IS '配送_重量積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_容積積載効率             IS '配送_容積積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_運賃形態                 IS '配送_運賃形態'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.配送_運賃形態名               IS '配送_運賃形態名'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.作成者                        IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.作成日                        IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.最終更新者                    IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.最終更新日                    IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_入出庫配送ヘッダ_基本_V.最終更新ログイン              IS '最終更新ログイン'
/
