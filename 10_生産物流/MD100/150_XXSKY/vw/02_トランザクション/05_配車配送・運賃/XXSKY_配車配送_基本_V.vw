CREATE OR REPLACE VIEW APPS.XXSKY_配車配送_基本_V
(
 処理種別_配車
,処理種別_配車名
,混載種別
...
-- 2010/1/7 #627 Y.Fukami Mod End
  ...
  FROM  xxwsh_carriers_schedule XCS                         --配車配送計画アドオンインタフェース
       ,xxsky_carriers2_v       XC2V01                      --SKYLINK用中間VIEW 運送業者取得VIEW
       ,xxsky_carriers2_v       XC2V02                      --SKYLINK用中間VIEW 運送業者取得VIEW
       ,xxsky_item_locations2_v XIL2V                       --SKYLINK用中間VIEW 配送元取得VIEW
...
-- *----------* 2009/06/23 本番#1438対応 end   *----------*
                 ...
                 ,start_date_active      dstart  -- 適用開始日
                 ,end_date_active        dend    -- 適用終了日
           FROM   xxsky_vendor_sites2_v          -- 仕入先サイトVIEW
        )                       DVTO                        --拠点_取引先名取得用
...
/
COMMENT ON TABLE APPS.XXSKY_配車配送_基本_V                             IS 'SKYLINK用配車配送（基本）VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.処理種別_配車              IS '処理種別_配車'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.処理種別_配車名            IS '処理種別_配車名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.混載種別                   IS '混載種別'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.混載種別名                 IS '混載種別名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送NO                     IS '配送No'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.基準明細NO                 IS '基準明細No'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運送業者                   IS '運送業者'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運送業者名                 IS '運送業者名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送元                     IS '配送元'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送元名                   IS '配送元名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送先コード区分           IS '配送先コード区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送先コード区分名         IS '配送先コード区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送先                     IS '配送先'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送先名                   IS '配送先名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送区分                   IS '配送区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送区分名                 IS '配送区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.出庫形態                   IS '出庫形態'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.自動配車対象区分           IS '自動配車対象区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.自動配車対象区分名         IS '自動配車対象区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.出庫予定日                 IS '出庫予定日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.着荷予定日                 IS '着荷予定日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.摘要                       IS '摘要'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.支払運賃計算対象フラグ     IS '支払運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.支払運賃計算対象フラグ名   IS '支払運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.請求運賃計算対象フラグ     IS '請求運賃計算対象フラグ'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.請求運賃計算対象フラグ名   IS '請求運賃計算対象フラグ名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.積載重量合計               IS '積載重量合計'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.積載容積合計               IS '積載容積合計'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.重量積載効率               IS '重量積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.容積積載効率               IS '容積積載効率'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.基本重量                   IS '基本重量'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.基本容積                   IS '基本容積'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運送業者_実績              IS '運送業者_実績'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運送業者_実績名            IS '運送業者_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送区分_実績              IS '配送区分_実績'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.配送区分_実績名            IS '配送区分_実績名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.出荷日                     IS '出荷日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.着荷日                     IS '着荷日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.重量容積区分               IS '重量容積区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.重量容積区分名             IS '重量容積区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運賃形態                   IS '運賃形態'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.運賃形態名                 IS '運賃形態名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.送り状NO                   IS '送り状No'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.小口個数                   IS '小口個数'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.ラベル枚数                 IS 'ラベル枚数'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.商品区分                   IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.商品区分名                 IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.伝票なし配車区分           IS '伝票なし配車区分'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.伝票なし配車区分名         IS '伝票なし配車区分名'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.作成者                     IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.作成日                     IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.最終更新者                 IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.最終更新日                 IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_配車配送_基本_V.最終更新ログイン           IS '最終更新ログイン'
/
