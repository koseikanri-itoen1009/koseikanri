CREATE OR REPLACE VIEW APPS.XXSKY_倉庫料入庫_基本_V
(
 入庫先保管場所
,入庫先保管場所名
,入庫先保管場所略称
  ...
  FROM ( --入庫実績データを取得
          ...
          SELECT
            ...
            FROM
                 ...
                 ,xxsky_item_locations2_v         ILOC                -- SKYLINK用中間VIEW OPM保管場所情報VIEW(出庫元保管場所名取得用)
           WHERE
             -- 移動 入庫情報取得
         ...
         UNION ALL
          ...
          SELECT
            ...
            FROM
                 ...
                 ,xxsky_vendors2_v                VNDR                -- SKYLINK用中間VIEW 仕入先情報VIEW(取引先名)
        ...
        --以下名称取得用
       ,xxsky_item_locations_v     ILOC                -- SKYLINK用中間VIEW OPM保管場所情報VIEW(入庫先保管場所名取得用)
       ,xxsky_item_mst2_v          ITEM                -- SKYLINK用中間VIEW OPM品目情報VIEW
       ,xxsky_prod_class_v         PRODC               -- SKYLINK用商品区分取得VIEW
       ,xxsky_item_class_v         ITEMC               -- SKYLINK用品目区分取得VIEW
       ,xxsky_crowd_code_v         CROWD               -- SKYLINK用郡コード取得VIEW
...
/
COMMENT ON TABLE APPS.XXSKY_倉庫料入庫_基本_V IS 'SKYLINK用倉庫料入庫基本VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.入庫先保管場所     IS '入庫先保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.入庫先保管場所名   IS '入庫先保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.入庫先保管場所略称 IS '入庫先保管場所略称'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.出庫形態           IS '出庫形態'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.出庫実績日         IS '出庫実績日'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.入庫実績日         IS '入庫実績日'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.出庫元保管場所     IS '出庫元保管場所'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.出庫元保管場所名   IS '出庫元保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.出庫元保管場所略称 IS '出庫元保管場所略称'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.商品区分           IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.商品区分名         IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.品目区分           IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.品目区分名         IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.群コード           IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.品目コード         IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.品目名             IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.品目略称           IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.ロットNO           IS 'ロットNO'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.製造年月日         IS '製造年月日'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.固有記号           IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.賞味期限           IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.実績数量           IS '実績数量'
/
COMMENT ON COLUMN APPS.XXSKY_倉庫料入庫_基本_V.実績ケース数量     IS '実績ケース数量'
/
