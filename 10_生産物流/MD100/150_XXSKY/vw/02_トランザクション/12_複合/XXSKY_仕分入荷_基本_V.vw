CREATE OR REPLACE VIEW APPS.XXSKY_仕分入荷_基本_V
(
 生産予定日
,納品場所
,納品場所名
...
SELECT
  ...
  FROM  ( --生産予定データを抽出
          ...
          SELECT
            ...
            FROM
                 ...
                 ,gmd_routings_b                            GRB                           --工順マスタ(生産データのみを抽出する為に結合)
                  --生産データ(完成品)
                 ,gme_material_details                      GMDF                          --原料詳細(完成品のみ)
                 ,ic_tran_pnd                               ITP                           --保留在庫トランザクション(完成品のロットID取得用)
                  --生産データ(投入品)
                 ,gme_material_details                      GMDM                          --原料詳細(投入品のみ)
                 ,xxwip_material_detail                     XMD                           --原料詳細アドオン
                 ,xxsky_item_class_v                        ITEMC                         --投入品の品目区分取得用(資材を除外する為)
                  --移動データ(投入品)
                 ...
                 ,xxsky_item_locations2_v                   ILCT                          --移動元倉庫名取得用
           WHERE
             --生産バッチヘッダの条件
                  GBH.batch_type                            = 0
       ...
       ,xxsky_item_locations_v          ILCT                --納品場所名取得用
       ,xxsky_item_mst2_v               FITM                --出来高品目名取得用
       ,ic_lots_mst                     FLOT                --出来高品目ロット情報取得用
       ,xxsky_item_mst2_v               MITM                --投入品目名取得用
       ,ic_lots_mst                     MLOT                --投入品目ロット情報取得用
       ,xxsky_prod_class_v              PRODC               --投入品目商品区分取得用
       ,xxsky_item_class_v              ITEMC               --投入品目品目区分取得用
       ,xxsky_crowd_code_v              CROWD               --投入品目群コード取得用
       ,gmd_operations_tl               GOT                 --工程マスタ(投入口区分名取得用)
       ,gme_batch_steps                 GBS                 --バッチステップ(投入口区分名取得用)
...
/
COMMENT ON TABLE APPS.XXSKY_仕分入荷_基本_V IS 'SKYLINK用仕分入荷基本VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.生産予定日     IS '生産予定日'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.納品場所       IS '納品場所'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.納品場所名     IS '納品場所名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.手配NO         IS '手配NO'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高品目     IS '出来高品目'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高品目名   IS '出来高品目名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高品目略称 IS '出来高品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高ロットNo IS '出来高ロットNo'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高製造日   IS '出来高製造日'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高賞味期限 IS '出来高賞味期限'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高固有記号 IS '出来高固有記号'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.出来高依頼数   IS '出来高依頼数'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.摘要           IS '摘要'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.商品区分       IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.商品区分名     IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.品目区分       IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.品目区分名     IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.郡コード       IS '郡コード'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入品目       IS '投入品目'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入品目名     IS '投入品目名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入品目略称   IS '投入品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入ロットNo   IS '投入ロットNo'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入製造日     IS '投入製造日'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入賞味期限   IS '投入賞味期限'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入固有記号   IS '投入固有記号'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.予定区分       IS '予定区分'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.予定区分名     IS '予定区分名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.予定番号       IS '予定番号'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.指示_実績数    IS '指示_実績数'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.入庫元         IS '入庫元'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.入庫元名       IS '入庫元名'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.納入日         IS '納入日'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入口区分     IS '投入口区分'
/
COMMENT ON COLUMN APPS.XXSKY_仕分入荷_基本_V.投入口区分名   IS '投入口区分名'
/
