CREATE OR REPLACE VIEW APPS.XXSKY_出荷数制限_基本_V
(
 需要予測日
,終了日
,予測セットコード
...
FROM
       (SELECT
            TO_CHAR(MFDT.forecast_date, 'YYYY/MM/DD') AS forecast_date   --需要予測日
           ,TO_CHAR(MFDT.rate_end_date, 'YYYY/MM/DD') AS rate_end_date   --終了日
       ...
       WHERE
            --フォーキャスト日付・フォーキャストDESIGNATOR 内部結合
              ...
              AND MFDS.attribute1 IN ('03', '04')        --attribute1=03:出荷制限Ａ・attribute1=04:出荷制限Ｂ
       )                        MFD                      --フォーキャスト日付・フォーキャストDESIGNATOR
       ,xxsky_item_mst2_v       XIMV                     --品目名取得用
       ...
       ,xxsky_item_locations2_v XILV                     --出庫倉庫名取得用
       ,xxsky_cust_accounts2_v  XCAV                     --拠点名取得用
       ,xxsky_locations2_v      XLV                      --事業所取得用
...
/
COMMENT ON TABLE APPS.XXSKY_出荷数制限_基本_V IS 'XXSKY_出荷数制限 (基本) VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.需要予測日       IS '需要予測日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.終了日           IS '終了日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.予測セットコード IS '予測セットコード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.商品区分         IS '商品区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.商品区分名       IS '商品区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.品目区分         IS '品目区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.品目区分名       IS '品目区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.群コード         IS '群コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.品目コード       IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.品目名           IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.品目略称         IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.内外区分         IS '内外区分'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.内外区分名       IS '内外区分名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.ケース入数       IS 'ケース入数'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.出庫元倉庫コード IS '出庫元倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.出庫元倉庫名     IS '出庫元倉庫名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.拠点コード       IS '拠点コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.拠点名           IS '拠点名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.取込部署コード   IS '取込部署コード'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.取込部署名       IS '取込部署名'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.予測当初数量     IS '予測当初数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.予測現在数量     IS '予測現在数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.元ケース数量     IS '元ケース数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.元バラ数量       IS '元バラ数量'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.作成者           IS '作成者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.作成日           IS '作成日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.最終更新者       IS '最終更新者'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.最終更新日       IS '最終更新日'
/
COMMENT ON COLUMN APPS.XXSKY_出荷数制限_基本_V.最終更新ログイン IS '最終更新ログイン'
/
