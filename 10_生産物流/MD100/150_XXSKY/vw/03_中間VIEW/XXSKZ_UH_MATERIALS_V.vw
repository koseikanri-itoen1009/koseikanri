/*************************************************************************
 * 
 * View  Name      : XXSKZ_UH_MATERIALS_V
 * Description     : XXSKZ_UH_MATERIALS_V
 * MD.070          : 
 * Version         : 1.1
 * 
 * Change Record
...
--*******************************************************************
CREATE OR REPLACE VIEW APPS.XXSKZ_UH_MATERIALS_V
(
 whse_code
,item_id
,lot_id
...
--Add 2013/3/19 V1.1 Start 解体データがバックアップされるまではここで解体データを参照（元テーブル）
      -- ----------------------------------------------------
      --  PROD :経理受払区分情報ＶＩＷ生産関連（Reverse_idなし）解体
      -- ----------------------------------------------------
      SELECT
             itp.whse_code                        whse_code
            ,itp.item_id                          item_id
...
-- 2010/02/16 T.Yoshimoto Del End 本稼動#1168
      ...
      FROM   xxcmn_ic_tran_pnd_arc            itp
            ,xxcmn_rcv_shipment_lines_arc     rsl
            ,xxcmn_oe_order_headers_all_arc   ooha
            ,oe_transaction_types_all         otta
            ,xxcmn_order_headers_all_arc      xoha
            ,xxcmn_order_lines_all_arc        xola
            ,ic_item_mst_b                    iimb
...
/
COMMENT ON TABLE APPS.XXSKZ_UH_MATERIALS_V IS 'XXSKZ_受払情報_製品以外_中間VIEW'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.whse_code  IS '倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.item_id    IS '品目ID'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.lot_id     IS 'ロットID'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.trans_qty  IS '受払数'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.column_no  IS '項目番号'
/
COMMENT ON COLUMN APPS.XXSKZ_UH_MATERIALS_V.trans_date IS '取引日'
/
