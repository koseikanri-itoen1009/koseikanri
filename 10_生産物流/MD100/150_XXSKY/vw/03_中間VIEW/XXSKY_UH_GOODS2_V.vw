...
--     ・XXSKY_受払情報_製品_数量2_V
--*******************************************************************
CREATE OR REPLACE VIEW APPS.XXSKY_UH_GOODS2_V
(
 whse_code
,item_id
,lot_id
...
-- 2009/12/17 T.Yoshimoto Add Start
         AND  xsims.invent_ym          >= TO_CHAR(ADD_MONTHS(trunc(sysdate,'MM'), -2), 'YYYYMM')    -- test_yoshimoto Add 1217
         AND  xsims.invent_ym          <  TO_CHAR(ADD_MONTHS(trunc(sysdate,'MM'),  1), 'YYYYMM')    -- test_yoshimoto Add 1217
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    itp.trans_date         >= ADD_MONTHS(trunc(sysdate,'MM'), -2)   -- test_yoshimoto Add 1217
      AND    itp.trans_date         <  ADD_MONTHS(trunc(sysdate,'MM'),  1)    -- test_yoshimoto Add 1217
-- 2009/12/17 T.Yoshimoto Add End
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    xoha.arrival_date      >= ADD_MONTHS(trunc(sysdate,'MM'), -2)   -- test_yoshimoto Add 1217
      AND    xoha.arrival_date      <  ADD_MONTHS(trunc(sysdate,'MM'),  1)   -- test_yoshimoto Add 1217
-- 2009/12/17 T.Yoshimoto Add End
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    xicv1.item_class_code   = '5'                                      -- test_yoshimoto Add
-- 2009/12/17 T.Yoshimoto Add End
...
-- 2009/11/12 Del End
            ...
            ,xxcmn_rcv_pay_mst                xrpm
            ,ic_item_mst_b                    iimb
            ,xxsky_prod_class_v               prdc1  --振替前品目 商品区分
            ,xxsky_item_class_v               itmc1  --振替前品目 品目区分
            ,xxsky_prod_class_v               prdc2  --振替後品目 商品区分
            ,xxsky_item_class_v               itmc2  --振替後品目 商品区分
      WHERE  itp.doc_type            = 'PORC'
      AND    itp.completed_ind       = 1
      AND    xoha.latest_external_flag = 'Y'
-- 2009/12/17 T.Yoshimoto Add Start
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    itmc1.item_class_code   = '5'                                      -- test_yoshimoto Add
-- 2009/12/17 T.Yoshimoto Add End
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    xicv.item_class_code    = '5'                                      -- test_yoshimoto Add
-- 2009/12/17 T.Yoshimoto Add End
      AND    xola.request_item_code <> xola.shipping_item_code
      AND    rsl.shipment_header_id  = itp.doc_id
      AND    rsl.line_num            = itp.doc_line
...
-- 2009/11/12 Del End
            ...
            ,xxwsh_order_lines_all            xola
            ,xxcmn_rcv_pay_mst                xrpm
            ,xxsky_prod_class_v               prdc
            ,xxsky_item_class_v               itmc
      WHERE  itp.doc_type            = 'OMSO'
      AND    itp.completed_ind       = 1
      AND    xoha.latest_external_flag = 'Y'
-- 2009/12/17 T.Yoshimoto Add Start
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    itmc.item_class_code    = '5'                                      -- test_yoshimoto Add
-- 2009/12/17 T.Yoshimoto Add End
      AND    wdd.delivery_detail_id  = itp.line_detail_id
      AND    wdd.source_header_id    = xoha.header_id
      AND    wdd.source_line_id      = xola.line_id
...
-- 2009/11/12 Add End
      ...
      AND    itp.completed_ind       = 1
-- 2009/12/17 T.Yoshimoto Add Start
      AND    xmrih.actual_arrival_date  >= ADD_MONTHS(trunc(sysdate,'MM'), -2)  -- test_yoshimoto Add 1217
      AND    xmrih.actual_arrival_date  <  ADD_MONTHS(trunc(sysdate,'MM'),  1)    -- test_yoshimoto Add 1217
-- 2009/12/17 T.Yoshimoto Add End
...
-- 2009/12/17 T.Yoshimoto Add Start
      AND    itc.trans_date         >= ADD_MONTHS(trunc(sysdate,'MM'), -2)  -- test_yoshimoto Add 1217
      AND    itc.trans_date         <  ADD_MONTHS(trunc(sysdate,'MM'),  1)    -- test_yoshimoto Add 1217
-- 2009/12/17 T.Yoshimoto Add End
      AND    xrpm.doc_type           = itc.doc_type
      AND    xrpm.reason_code        = itc.reason_code
      AND    xrpm.break_col_03       IS NOT NULL
...
-- 2009/11/12 Add End
    ...
    --****************************************************************************
/
COMMENT ON TABLE APPS.XXSKY_UH_GOODS2_V IS 'XXSKY_受払情報_製品_中間VIEW2'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.whse_code  IS '倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.item_id    IS '品目ID'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.lot_id     IS 'ロットID'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.trans_qty  IS '受払数'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.column_no  IS '項目番号'
/
COMMENT ON COLUMN APPS.XXSKY_UH_GOODS2_V.trans_date IS '取引日'
/
