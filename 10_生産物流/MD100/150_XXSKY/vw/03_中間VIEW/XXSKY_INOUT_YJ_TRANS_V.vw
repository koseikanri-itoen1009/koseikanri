--*******************************************************************
-- 入出庫予実 中間VIEW
--   当VIEWではロット割り当てされていない予定データも出力する
--    ⇒ リーフ部門向け
--
--   【使用対象VIEW】
--     ・XXSKY_入出庫予実_基本_V
...
--     ・XXSKY_入出庫予実_発日_V
--*******************************************************************
CREATE OR REPLACE VIEW APPS.XXSKY_INOUT_YJ_TRANS_V
(
 reason_code
,whse_code
,location_code
...
--■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
  ...
  SELECT
         ...
         ,NULL                                          AS lot_id                 -- ロットID
         ,NULL                                          AS lot_no                 -- ロットNo
         ,NULL                                          AS manufacture_date       -- 製造年月日
         ,NULL                                          AS uniqe_sign             -- 固有記号
         ,NULL                                          AS expiration_date        -- 賞味期限 -- <---- ここまで共通
         ,xpha_in_po.po_header_number                   AS voucher_no             -- 伝票番号
         ,xpla_in_po.requisition_line_number            AS line_no                -- 行番号
         ,NULL                                          AS delivery_no            -- 配送番号
         ,NULL                                          AS loct_code              -- 部署コード
         ,NULL                                          AS loct_name              -- 部署名
         ,'1'                                           AS in_out_kbn             -- 入出庫区分（1:入庫）
         ,xpha_in_po.promised_date                      AS leaving_date           -- 入出庫日_発日
         ,xpha_in_po.promised_date                      AS arrival_date           -- 入出庫日_着日
         ,xpha_in_po.promised_date                      AS standard_date          -- 基準日（着日）
         ...
         ,xpla_in_po.requested_quantity                 AS stock_quantity         -- 入庫数
         ,0                                             AS leaving_quantity       -- 出庫数
         ,xpla_in_po.requested_quantity                 AS quantity               -- 入出庫数
    FROM
         ...
         ,xxpo_requisition_headers                      xpha_in_po                -- 発注依頼ヘッダアドオン
         ,xxpo_requisition_lines                        xpla_in_po                -- 発注依頼明細アドオン
   ...
   WHERE
     ...
     --発注依頼ヘッダアドオンの条件
     AND  xpha_in_po.status                             IN ( '5'                  -- 依頼作成中
                                                            ,'10' )               -- 依頼作成済
     --発注依頼明細アドオンとの結合
     AND  xpha_in_po.requisition_header_id              = xpla_in_po.requisition_header_id
     --品目マスタとの結合
     AND  xpla_in_po.item_id                            = ximv_in_po.item_id
     AND  xpha_in_po.promised_date                     >= ximv_in_po.start_date_active
     AND  xpha_in_po.promised_date                     <= ximv_in_po.end_date_active
     --保管場所情報取得
     AND  xpha_in_po.location_code                      = xilv_in_po.segment1
     --取引先情報取得
     AND  xpha_in_po.vendor_id                          = xvv_in_po.vendor_id
     AND  xpha_in_po.promised_date                     >= xvv_in_po.start_date_active
     AND  xpha_in_po.promised_date                     <= xvv_in_po.end_date_active
  -- [ １．発注依頼  END ]
UNION ALL
  -------------------------------------------------------------
  -- ２．発注受入予定
  --  ※ロット割当の有無に関わらずデータ取得する為、ロットマスタと外部結合
  --     ⇒ 対象データを副問い合わせする
  -------------------------------------------------------------
  SELECT
          xrpm_in_po.new_div_invent                     AS reason_code            -- 事由コード
...
UNION ALL
  -------------------------------------------------------------
  -- ３．移動入庫予定(指示 積送あり)
  --  ※ロット割当の有無に関わらずデータ取得する為、移動ロット詳細と外部結合
  -------------------------------------------------------------
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_in_xf                -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_xf                -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_in_xf2               -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_xf                 -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,CASE WHEN xmld_in_tr.mov_lot_dtl_id IS NULL THEN xmril_in_tr.instruct_qty     -- ロット割当無しなら品目単位の数量
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_in_tr                -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_tr                -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_in_tr2               -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_tr                 -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
     ...
     --移動ロット詳細との結合（ロット割当てされていないデータも出力対照とする為に外部結合を行う）
...
UNION ALL
  ...
  SELECT
         ...
         ,CASE WHEN xmld_in_xf20.mov_lot_dtl_id IS NULL THEN xmril_in_xf20.instruct_qty     -- ロット割当無しなら品目単位の数量
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_in_xf20              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_xf20              -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_in_xf202             -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_xf20               -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_pr                -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_pr                -- OPM品目情報VIEW
...
UNION ALL
  ...
  SELECT
   ...
   WHERE
     ...
     -- ロット未割当のデータを抽出（保留在庫トランザクションにデータが存在しないものが該当）
     AND  NOT EXISTS
          (
            SELECT  'X'
              FROM  ic_tran_pnd                         itp_in_pr                 -- 保留在庫トランザクション
             WHERE  itp_in_pr.delete_mark               = 0                       -- 有効チェック(OPM保留在庫)
               AND  itp_in_pr.reverse_id                IS NULL
               AND  itp_in_pr.doc_type                  = xrpm_in_pr.doc_type
               AND  itp_in_pr.line_id                   = gmd_in_pr.material_detail_id
               AND  itp_in_pr.location                  = grb_in_pr.attribute9
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_pr70              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_pr70              -- OPM品目情報VIEW
         ,ic_lots_mst                                   ilm_in_pr70               -- OPMロットマスタ
         ,xxcmn_rcv_pay_mst                             xrpm_in_pr70              -- 受払区分アドオンマスタ -- <---- ここまで共通
         ,gme_batch_header                              gbh_in_pr70               -- 生産バッチ
         ,gme_material_details                          gmd_in_pr70a              -- 生産原料詳細(振替先)
         ,gme_material_details                          gmd_in_pr70b              -- 生産原料詳細(振替元)
         ,gmd_routings_b                                grb_in_pr70               -- 工順マスタ
         ,gmd_routings_tl                               grt_in_pr70               -- 工順マスタ日本語
         ,ic_tran_pnd                                   itp_in_pr70               -- OPM保留在庫トランザクション
         ,xxsky_item_class_v                            xicv_in_pr70b             -- OPM品目カテゴリ割当情報VIEW5(振替元)
   WHERE
     -- 受払区分アドオンマスタの条件
...
--■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
  ...
  SELECT
         ...
         ,CASE WHEN xmld_out_xf.mov_lot_dtl_id IS NULL THEN xmril_out_xf.instruct_qty     -- ロット割当無しなら品目単位の数量
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_xf               -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_xf               -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_out_xf2              -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_xf                -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,CASE WHEN xmld_out_tr.mov_lot_dtl_id IS NULL THEN xmril_out_tr.instruct_qty     -- ロット割当無しなら品目単位の数量
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_tr               -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_tr               -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_out_tr2              -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_tr                -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,CASE WHEN xmld_out_xf20.mov_lot_dtl_id IS NULL THEN xmril_out_xf20.instruct_qty     -- ロット割当無しなら品目単位の数量
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_xf20             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_xf20             -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_out_xf202            -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_xf20              -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,0                                             AS stock_quantity         -- 入庫数
         ,DECODE( xmld_out_om.mov_lot_dtl_id, NULL, xola_out_om.quantity,           -- ロット割当無しなら品目単位の数量
                                                    xmld_out_om.actual_quantity )   -- ロット割当有りならロット単位の数量
                                                        AS leaving_quantity       -- 出庫数
         ,DECODE( xmld_out_om.mov_lot_dtl_id, NULL, xola_out_om.quantity,           -- ロット割当無しなら品目単位の数量
                                                    xmld_out_om.actual_quantity )   -- ロット割当有りならロット単位の数量
                                                        AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_om               -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_om_s             -- OPM品目情報VIEW(出荷品目)
         ...
         ,xxsky_item_mst2_v                             ximv_out_om_r             -- OPM品目情報VIEW(依頼品目)
         ,xxsky_cust_accounts2_v                        xcst_out_om               -- 受払先(拠点)取得用
         ,xxsky_cust_accounts2_v                        xcst_out_om_h             -- 受払先(管轄拠点)取得用
         ,xxsky_party_sites2_v                          xpas_out_om               -- 配送先名取得用
         ,xxsky_locations2_v                            xlc_out_om                -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,NULL                                          AS deliver_to_name        -- 配送先名
         ,0                                             AS stock_quantity         -- 入庫数
         ,DECODE( xmld_out_om2.mov_lot_dtl_id, NULL, xola_out_om2.quantity,          -- ロット割当無しなら品目単位の数量
                                                     xmld_out_om2.actual_quantity )  -- ロット割当有りならロット単位の数量
          * DECODE( otta_out_om2.order_category_code, 'RETURN' , -1,                 -- 入庫扱いの為、マイナス値にする
                                                                  1  )               -- 出庫扱いの為、プラス値にする
                                                        AS leaving_quantity       -- 出庫数
         ,DECODE( xmld_out_om2.mov_lot_dtl_id, NULL, xola_out_om2.quantity,          -- ロット割当無しなら品目単位の数量
                                                     xmld_out_om2.actual_quantity )  -- ロット割当有りならロット単位の数量
          * DECODE( otta_out_om2.order_category_code, 'RETURN' , -1,                 -- 入庫扱いの為、マイナス値にする
                                                        ...
                                                        AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_om2              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_om2_s            -- OPM品目情報VIEW(出荷品目)
         ...
         ,xxsky_item_mst2_v                             ximv_out_om2_r            -- OPM品目情報VIEW(依頼品目)
         ,xxsky_vendor_sites_v                          xvsv_out_om2              -- 仕入先サイト情報VIEW(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_om2               -- 部署名取得用
  WHERE
     --受払区分アドオンマスタの条件
     ...
     --受注ヘッダの条件
     AND  xoha_out_om2.req_status                       IN ('05',                 -- 05:入力中
                                                            '06',                 -- 06:入力完了
                                                            '07' )                -- 07:受領済
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_pr               -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_pr               -- OPM品目情報VIEW
...
UNION ALL
  ...
  SELECT
         ...
         ,NULL                                          AS deliver_to_name        -- 配送先名
         ,0                                             AS stock_quantity         -- 入庫数
         ,NVL( TO_NUMBER( gmd_out_pr.attribute25 ), gmd_out_pr.original_qty )  --新･計画数は｢ATTR25｣、旧は｢ORIGINAL｣
                                                        AS leaving_quantity       -- 出庫数
         ,NVL( TO_NUMBER( gmd_out_pr.attribute25 ), gmd_out_pr.original_qty )  --新･計画数は｢ATTR25｣、旧は｢ORIGINAL｣
                                                        AS quantity               -- 入出庫数
    FROM
   ...
   WHERE
     ...
     AND  NOT EXISTS
          (
            SELECT  'X'
              FROM  ic_tran_pnd                         itp_out_pr                -- 保留在庫トランザクション
             WHERE  itp_out_pr.delete_mark              = 0                       -- 有効チェック(OPM保留在庫)
               AND  itp_out_pr.reverse_id               IS NULL
               AND  itp_out_pr.doc_type                 = xrpm_out_pr.doc_type
               AND  itp_out_pr.line_id                  = gmd_out_pr.material_detail_id
               AND  itp_out_pr.location                 = grb_out_pr.attribute9
     ...
     AND  gbh_out_pr.plan_start_date                   >= ximv_out_pr.start_date_active --適用開始日
     AND  gbh_out_pr.plan_start_date                   <= ximv_out_pr.end_date_active   --適用終了日
     --保管場所情報取得
     AND  grb_out_pr.attribute9                         = xilv_out_pr.segment1
     --工順マスタ日本語取得
     AND  grt_out_pr.language                           = 'JA'
     AND  grb_out_pr.routing_id                         = grt_out_pr.routing_id
  -- [ ７．生産原料投入予定(ロット未割当)  END ] --
UNION ALL
  -------------------------------------------------------------
  -- ８．生産出庫予定 品目振替 品種振替
  -- 【注】以下のSQLは変更次第で処理速度が遅くなります
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_pr70             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_pr70             -- OPM品目情報VIEW
         ,ic_lots_mst                                   ilm_out_pr70              -- OPMロットマスタ
         ,xxcmn_rcv_pay_mst                             xrpm_out_pr70             -- 受払区分アドオンマスタ -- <---- ここまで共通
         ,gme_batch_header                              gbh_out_pr70              -- 生産バッチ
         ,gme_material_details                          gmd_out_pr70a             -- 生産原料詳細(振替元)
         ,gme_material_details                          gmd_out_pr70b             -- 生産原料詳細(振替先)
         ,gmd_routings_b                                grb_out_pr70              -- 工順マスタ
         ,gmd_routings_tl                               grt_out_pr70              -- 工順マスタ日本語
         ,ic_tran_pnd                                   itp_out_pr70              -- OPM保留在庫トランザクション
         ,xxsky_item_class_v                            xicv_out_pr70b            -- OPM品目カテゴリ割当情報VIEW5(振替先)
   WHERE
     --受払区分アドオンマスタの条件
...
--■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_po_e              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_po_e              -- OPM品目情報VIEW
         ...
         ,xxsky_vendors2_v                              xvv_in_po_e               -- 仕入先情報VIEW(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_po_e               -- 部署名取得用
   WHERE
     -- 受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_in_xf_e              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_xf_e              -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_in_xf_e2             -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_xf_e               -- 部署名取得用
   WHERE
     -- 受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_in_tr_e              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_tr_e              -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_in_tr_e2             -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_in_tr_e               -- 部署名取得用
   WHERE
     -- 受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_pr_e              -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_pr_e              -- OPM品目情報VIEW
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_pr_e70            -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_pr_e70            -- OPM品目情報VIEW
         ...
         ,xxsky_item_class_v                            xicv_in_pr_e70b           -- OPM品目カテゴリ割当情報VIEW5(振替元)
   WHERE
     -- 受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
         ...
         ,CASE WHEN otta_in_po_e_rma.order_category_code = 'ORDER' THEN xmld_in_po_e_rma.actual_quantity * -1
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_in_po_e_rma          -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_po_e_rma          -- OPM品目情報VIEW
         ...
         ,xxsky_cust_accounts2_v                        xcst_in_po_e_rma          -- 受払先
         ,xxsky_party_sites2_v                          xpas_in_po_e_rma          -- SKYLINK用中間VIEW 配送先情報VIEW2
         ,xxsky_locations2_v                            xlc_in_po_e_rma           -- 部署名取得用
   WHERE
     -- 受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_ad_e_x97          -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_ad_e_x97          -- OPM品目情報VIEW
...
UNION ALL
  ...
  SELECT
    ...
    FROM
         ...
         ,xxsky_vendors2_v                              xvv_in_ad_e_x97           -- 仕入先情報(受払先名取得用)
   WHERE
     -- OPM品目情報VIEW取得
          sitc_in_ad_e_x97.item_id                      = ximv_in_ad_e_x97.item_id
     AND  sitc_in_ad_e_x97.tran_date                   >= ximv_in_ad_e_x97.start_date_active
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_ad_e_x9           -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_ad_e_x9           -- OPM品目情報VIEW
         ...
         ,xxsky_locations2_v                            xlc_in_ad_e_x9            -- 部署名取得用
   WHERE
     -- OPM品目情報取得
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_ad_e_x2           -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_ad_e_x2           -- OPM品目情報VIEW
         ...
         ,(  -- 対象データを集計
             SELECT
               ...
               FROM
                    ...
                    ,ic_tran_cmp                          itc_in_ad_e_x2          -- OPM完了在庫トランザクション
         ...
         ,xxsky_vendors_v                               xvv_in_ad_e_x2            -- 仕入先情報VIEW
         ,xxsky_locations2_v                            xlc_in_ad_e_x2            -- 部署名取得用
   WHERE
     --品目マスタとの結合
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_in_ad_e_xx           -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_in_ad_e_xx           -- OPM品目情報VIEW
...
--■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_out_xf_e             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_xf_e             -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_out_xf_e2            -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_xf_e              -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_out_tr_e             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_tr_e             -- OPM品目情報VIEW
         ...
         ,xxsky_item_locations2_v                       xilv_out_tr_e2            -- OPM保管場所情報VIEW2(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_tr_e              -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_pr_e             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_pr_e             -- OPM品目情報VIEW
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_pr_e70           -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_pr_e70           -- OPM品目情報VIEW
         ...
         ,xxsky_item_class_v                            xicv_out_pr_e70b          -- OPM品目カテゴリ割当情報VIEW5(振替先)
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_out_om_e             -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_om_e_s           -- OPM品目情報VIEW(出荷品目)
         ...
         ,xxsky_item_mst2_v                             ximv_out_om_e_r           -- OPM品目情報VIEW(依頼品目)
         ,xxsky_cust_accounts2_v                        xcst_out_om_e             -- 受払先(拠点)取得用
         ,xxsky_cust_accounts2_v                        xcst_out_om_e_h           -- 受払先(管轄拠点)取得用
         ,xxsky_party_sites2_v                          xpas_out_om_e             -- 配送先名取得用
         ,xxsky_locations2_v                            xlc_out_om_e              -- 部署名取得用
   WHERE
...
-- *----------* 2009/04/23 M.Nomura update end   *----------*
         ...
         ,CASE WHEN otta_out_om2_e.order_category_code = 'RETURN' THEN xmld_out_om2_e.actual_quantity * -1
          ...
          END                                           AS quantity               -- 入出庫数
    FROM
          xxsky_item_locations2_v                       xilv_out_om2_e            -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_om2_e_s          -- OPM品目情報VIEW(出荷品目)
         ...
         ,xxsky_item_mst2_v                             ximv_out_om2_e_r          -- OPM品目情報VIEW(依頼品目)
         ,xxsky_vendor_sites_v                          xvsv_out_om2_e            -- 仕入先サイト情報VIEW(受払先名取得用)
         ,xxsky_locations2_v                            xlc_out_om2_e             -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件(受注タイプ結合)
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations2_v                       xilv_out_om3_e            -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_om3_e            -- OPM品目情報VIEW
         ...
         ,xxsky_cust_accounts2_v                        xcst_out_om3_e            -- 受払先
         ,xxsky_party_sites2_v                          xpas_out_om3_e            -- 配送先名取得用
         ,xxsky_locations2_v                            xlc_out_om3_e             -- 部署名取得用
   WHERE
     --受払区分アドオンマスタの条件
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_ad_e_x97         -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_ad_e_x97         -- OPM品目情報VIEW
    ...
    FROM
         ...
         ,(  -- 対象データを集計
             SELECT
               ...
               FROM
                     xxcmn_rcv_pay_mst                  xrpm_out_ad_e_x97         -- 受払区分アドオンマスタ
                    ,ic_tran_cmp                        itc_out_ad_e_x97          -- OPM完了在庫トランザクション -- <---- ここまで共通
                    ,ic_jrnl_mst                        ijm_out_ad_e_x97          -- OPMジャーナルマスタ
                    ,ic_adjs_jnl                        iaj_out_ad_e_x97          -- OPM在庫調整ジャーナル
         ...
         ,xxsky_vendors_v                               xvv_out_ad_e_x97          -- 仕入先情報
         ,xxsky_locations2_v                            xlc_out_ad_e_x97          -- 部署名取得用
   WHERE
     --品目マスタとの結合
          sitc_out_ad_e_x97.item_id                     = ximv_out_ad_e_x97.item_id
     AND  sitc_out_ad_e_x97.tran_date                  >= ximv_out_ad_e_x97.start_date_active --適用開始日
...
UNION ALL
  ...
  SELECT
    ...
    FROM
          xxsky_item_locations_v                        xilv_out_ad_e_xx          -- OPM保管場所情報VIEW
         ,xxsky_item_mst2_v                             ximv_out_ad_e_xx          -- OPM品目情報VIEW
...
-- << 出庫実績 END >>
/
COMMENT ON TABLE APPS.XXSKY_INOUT_YJ_TRANS_V IS 'XXSKY_入出庫予実_中間VIEW'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.reason_code       IS '事由コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.whse_code         IS '倉庫コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.location_code     IS '保管場所コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.location          IS '保管場所名'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.location_s_name   IS '保管場所略称'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.item_id           IS '品目ID'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.item_no           IS '品目コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.item_name         IS '品目名'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.item_short_name   IS '品目略称'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.case_content      IS 'ケース入数'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.lot_ctl           IS 'ロット管理区分'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.lot_no            IS 'ロットNo'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.lot_id            IS 'ロットID'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.manufacture_date  IS '製造年月日'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.uniqe_sign        IS '固有記号'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.expiration_date   IS '賞味期限'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.voucher_no        IS '伝票番号'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.line_no           IS '行番号'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.delivery_no       IS '配送番号'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.loct_code         IS '部署コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.loct_name         IS '部署名'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.in_out_kbn        IS '入出庫区分'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.leaving_date      IS '入出庫日_発日'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.arrival_date      IS '入出庫日_着日'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.standard_date     IS '基準日'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.ukebaraisaki_code IS '受払先コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.ukebaraisaki_name IS '受払先名'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.status            IS '予定実績区分'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.deliver_to_no     IS '配送先コード'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.deliver_to_name   IS '配送先名'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.stock_quantity    IS '入庫数'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.leaving_quantity  IS '出庫数'
/
COMMENT ON COLUMN APPS.XXSKY_INOUT_YJ_TRANS_V.quantity          IS '入出庫数'
/
