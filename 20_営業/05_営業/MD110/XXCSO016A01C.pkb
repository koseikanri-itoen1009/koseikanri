 ...
 *
 * Package Name     : XXCSO016A01C(body)
...
--
  ...
  cv_tkn_number_03    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00123';  -- CSVファイル残存エラーメッセージ
  ...
  cv_tkn_number_06    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00017';  -- CSVファイル出力エラー(営業員別計画)
  ...
  cv_tkn_number_09    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00148';  -- パラメータ（年度）
  cv_tkn_number_10    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00149';  -- 年度取得エラー
  cv_tkn_number_11    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00152';  -- インターフェースファイル名
  cv_tkn_number_12    CONSTANT VARCHAR2(100) := 'APP-XXCSO1-00224';  -- CSVファイル出力0件エラーメッセージ
  -- トークンコード
...
--
  ...
  -- 営業員コード格納用
  gt_employee_number xxcso_sls_prsn_mnthly_plns.employee_number%TYPE DEFAULT NULL;
  -- 年月格納用
  gt_year_month      xxcso_sls_prsn_mnthly_plns.year_month%TYPE DEFAULT NULL;
  -- ===============================
  -- ユーザー定義グローバル型
  -- ===============================
  -- CSV出力データ格納用レコード型定義
  TYPE g_get_data_rtype IS RECORD(
    ...
    fiscal_year      xxcso_sls_prsn_mnthly_plns.fiscal_year%TYPE,                       -- 年度
    year_month       xxcso_sls_prsn_mnthly_plns.year_month%TYPE,                        -- 年月
    ...
    bsc_sls_prsn_total_amt  xxcso_sls_prsn_mnthly_plns.bsc_sls_prsn_total_amt%TYPE,     -- 売上計画
    tgt_sales_prsn_total_amt  xxcso_sls_prsn_mnthly_plns.tgt_sales_prsn_total_amt%TYPE, -- 売上目標
    cntrbt_sls       NUMBER(10),                                                        -- 貢献売上計画
    vis_prsn_total_amt  xxcso_sls_prsn_mnthly_plns.vis_prsn_total_amt%TYPE,             -- 訪問軒数計画
...
--
  ...
  PROCEDURE get_profile_info(
    ...
    ,ov_cntrbt_sls     OUT NOCOPY VARCHAR2  -- 貢献売上の値(固定値15000)
    ,ov_errbuf         OUT NOCOPY VARCHAR2  -- エラー・メッセージ            --# 固定 #
    ,ov_retcode        OUT NOCOPY VARCHAR2  -- リターン・コード              --# 固定 #
    ,ov_errmsg         OUT NOCOPY VARCHAR2  -- ユーザー・エラー・メッセージ  --# 固定 #
  )
...
--
    ...
    -- XXCSO:情報系連携用CSVファイル名(営業員別計画)
    cv_prfnm_csv_sls_pln    CONSTANT VARCHAR2(30)   := 'XXCSO1_INFO_OUT_CSV_SLS_PLN';
    -- XXCSO:情報系連携用貢献売上
    cv_prfnm_cntrbt_sls     CONSTANT VARCHAR2(30)   := 'XXCSO1_INFO_OUT_CNTRBT_SLS';
--
    -- *** ローカル変数 ***
    -- プロファイル値取得戻り値格納用
...
--
    ...
    fnd_file.put_line(
      ...
      ,buff   => cv_debug_msg7  || CHR(10) ||
                 ...
                 cv_debug_msg11 || lv_cntrbt_sls || CHR(10) ||
                 ''
    );
--
...
--
    ...
    -- 貢献売上の値取得失敗時
    ELSIF (lv_cntrbt_sls IS NULL) THEN
      lv_tkn_value := cv_prfnm_cntrbt_sls;
    END IF;
    -- エラーメッセージ取得
...
--
    ...
    ov_cntrbt_sls     :=  lv_cntrbt_sls;       -- 貢献売上の値(固定値15000)
--
  EXCEPTION
--
--#################################  固定例外処理部 START   ####################################
...
--
   ...
   * Description      : 営業員別計画データCSVファイルオープン (A-4)
   ***********************************************************************************/
...
--
  ...
  PROCEDURE create_csv_rec(
     ir_xspmp_data       IN  g_get_data_rtype    -- 営業員別計画抽出データ
    ,ov_errbuf           OUT NOCOPY VARCHAR2     -- エラー・メッセージ            --# 固定 #
    ,ov_retcode          OUT NOCOPY VARCHAR2     -- リターン・コード              --# 固定 #
    ,ov_errmsg           OUT NOCOPY VARCHAR2     -- ユーザー・エラー・メッセージ  --# 固定 #
  )
...
--
    -- *** ローカル・レコード ***
    l_xspmp_data_rec g_get_data_rtype; -- INパラメータ.営業員別計画抽出データ格納
    lv_company_cd    VARCHAR2(2000);   -- INパラメータ.会社コード格納
    lv_cntrbt_sls    VARCHAR2(2000);   -- INパラメータ.貢献売上の値格納
...
--
    -- INパラメータをレコード変数に格納
    l_xspmp_data_rec := ir_xspmp_data; -- 営業員別計画抽出データ
--
    -- ======================
    -- CSV出力処理 
    -- ======================
    BEGIN
      -- 取得した営業員ＣＤ、年月が、前回取得値と同じである場合
      IF (gt_employee_number = l_xspmp_data_rec.employee_number)
        AND (gt_year_month   = l_xspmp_data_rec.year_month) THEN
        -- 貢献売上計画に'0'を設定する。
        l_xspmp_data_rec.cntrbt_sls := '0';
      END IF;
--
      -- 今回取得値を変数に格納
      gt_employee_number := l_xspmp_data_rec.employee_number;     -- 営業員ＣＤ
      gt_year_month      := l_xspmp_data_rec.year_month;          -- 年月
      -- データ作成
      lv_data := cv_sep_wquot || l_xspmp_data_rec.company_cd || cv_sep_wquot        -- 会社コード
        || cv_sep_com || l_xspmp_data_rec.fiscal_year                               -- 年度
        || cv_sep_com || l_xspmp_data_rec.year_month                                -- 年月
        || cv_sep_com ||
        cv_sep_wquot  || l_xspmp_data_rec.base_code          || cv_sep_wquot        -- 拠点ＣＤ
        || cv_sep_com ||
        cv_sep_wquot  || l_xspmp_data_rec.employee_number    || cv_sep_wquot        -- 営業員ＣＤ
        || cv_sep_com || TO_CHAR(NVL((l_xspmp_data_rec.bsc_sls_prsn_total_amt * 1000), 0))   -- 売上計画
        || cv_sep_com || TO_CHAR(NVL((l_xspmp_data_rec.tgt_sales_prsn_total_amt * 1000), 0)) -- 売上目標
        || cv_sep_com || TO_CHAR(l_xspmp_data_rec.cntrbt_sls)                       -- 貢献売上計画
        || cv_sep_com || TO_CHAR(NVL(l_xspmp_data_rec.vis_prsn_total_amt, 0))       -- 訪問軒数計画
        || cv_sep_com || TO_CHAR(l_xspmp_data_rec.cprtn_date, 'yyyymmddhh24miss');  -- 連携日時
--
      -- データ出力
      UTL_FILE.PUT_LINE(
        file   => gf_file_hand
    ...
    EXCEPTION
      WHEN UTL_FILE.INVALID_FILEHANDLE OR     -- ファイル・ハンドル無効エラー
        ...
        lv_errmsg := xxccp_common_pkg.get_msg(
                      ...
                      ,iv_token_value1 => l_xspmp_data_rec.fiscal_year   --トークン値1
                      ,iv_token_name2  => cv_tkn_ym                      --トークンコード2
                      ,iv_token_value2 => l_xspmp_data_rec.year_month    --トークン値2
                      ,iv_token_name3  => cv_tkn_slspsn_cd               --トークンコード3
                      ,iv_token_value3 => l_xspmp_data_rec.employee_number   --トークン値3
...
--
    -- *** ローカル・カーソル ***
    CURSOR xspmp_data_cur
    IS
      SELECT  xspmp.base_code         base_code                           -- 拠点ＣＤ
             ,xspmp.employee_number   employee_number                     -- 営業員ＣＤ
             ,xspmp.fiscal_year       fiscal_year                         -- 年度
             ,xspmp.year_month        year_month                          -- 年月
             ,xspmp.bsc_sls_prsn_total_amt      bsc_sls_prsn_total_amt    -- 売上計画
             ,xspmp.tgt_sales_prsn_total_amt    tgt_sales_prsn_total_amt  -- 売上目標
             ,xspmp.vis_prsn_total_amt          vis_prsn_total_amt        -- 訪問軒数計画
      FROM   xxcso_sls_prsn_mnthly_plns   xspmp               -- 営業員別月別計画テーブル
      WHERE  xspmp.fiscal_year = TO_CHAR(ln_year)
      ORDER BY  xspmp.year_month      ASC                     -- 年月
               ,xspmp.employee_number ASC                     -- 営業員ＣＤ
               ,xspmp.creation_date   DESC;                   -- 作成日
--
    -- *** ローカル・レコード ***
    l_xspmp_data_rec   xspmp_data_cur%ROWTYPE;
...
--
    <<get_data_loop>>
    LOOP
      FETCH xspmp_data_cur INTO l_xspmp_data_rec;
      -- 処理対象件数格納
      gn_target_cnt := xspmp_data_cur%ROWCOUNT;
--
      EXIT WHEN xspmp_data_cur%NOTFOUND
      OR  xspmp_data_cur%ROWCOUNT = 0;
      -- レコード変数初期化
      l_get_data_rec := NULL;
      -- 取得データを格納
      l_get_data_rec.company_cd               := lv_company_cd;                       -- 会社コード
      l_get_data_rec.fiscal_year              := l_xspmp_data_rec.fiscal_year;        -- 年度
      l_get_data_rec.year_month               := l_xspmp_data_rec.year_month;         -- 年月
      l_get_data_rec.base_code                := l_xspmp_data_rec.base_code;          -- 拠点ＣＤ
      l_get_data_rec.employee_number          := l_xspmp_data_rec.employee_number;    -- 営業員ＣＤ
      l_get_data_rec.bsc_sls_prsn_total_amt   := l_xspmp_data_rec.bsc_sls_prsn_total_amt;   -- 売上計画
      l_get_data_rec.tgt_sales_prsn_total_amt := l_xspmp_data_rec.tgt_sales_prsn_total_amt; -- 売上目標
      l_get_data_rec.cntrbt_sls               := lv_cntrbt_sls;                       -- 貢献売上計画
      l_get_data_rec.vis_prsn_total_amt       := l_xspmp_data_rec.vis_prsn_total_amt; -- 訪問軒数計画
...
--
      ...
      -- ========================================
      create_csv_rec(
        ir_xspmp_data  =>  l_get_data_rec        -- 営業員別計画抽出データ
