CREATE OR REPLACE VIEW XXCSM_ITEM_GROUP_2_NM_V
(
  item_group_cd
 ,item_group_nm
)
AS
  ...
  AND     INSTR(flv.lookup_code,'*',1,1) = 3   --政策群2桁のみ対象
  UNION
  SELECT  xicv.segment1    item_group_cd
         ,xicv.description item_group_nm
  FROM    xxcsm_item_category_v xicv
  WHERE   xicv.segment1 LIKE '%*%'
  AND     INSTR(xicv.segment1,'*',1,1) = 3   --政策群2桁のみ対象
;
--
COMMENT ON COLUMN xxcsm_item_group_2_nm_v.item_group_cd         IS '商品群コード';
COMMENT ON COLUMN xxcsm_item_group_2_nm_v.item_group_nm         IS '商品群名';
--
COMMENT ON TABLE  xxcsm_item_group_2_nm_v                       IS '商品群2桁名称ビュー';
