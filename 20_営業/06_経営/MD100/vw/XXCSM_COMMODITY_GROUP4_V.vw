...
AS
  SELECT /*+ INDEX(iv_cmpcst.ccmd CM_CMPT_DTL_U1) */
        ...
        ,NVL((SELECT SUM(ccmd.cmpnt_cost)  cmpnt_cost
              ...
              ,NVL(iimb.attribute8,0))                               now_cmpnt_cost                 --標準原価(現時点)
...
COMMENT ON COLUMN xxcsm_commodity_group4_v.group4_nm             IS '商品群名称４';
COMMENT ON COLUMN xxcsm_commodity_group4_v.group1_cd             IS '商品群コード１';
COMMENT ON COLUMN xxcsm_commodity_group4_v.group1_nm             IS '商品群名称１';
COMMENT ON COLUMN xxcsm_commodity_group4_v.now_item_cost         IS '標準原価(現時点)';
COMMENT ON COLUMN xxcsm_commodity_group4_v.now_business_cost     IS '営業原価(現時点)';
COMMENT ON COLUMN xxcsm_commodity_group4_v.now_unit_price        IS '定価(現時点)';
