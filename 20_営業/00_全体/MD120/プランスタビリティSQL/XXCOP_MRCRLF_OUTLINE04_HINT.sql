...
WHERE NOT EXISTS(SELECT /*+ INDEX ( items MTL_SYSTEM_ITEMS_B_U1 ) */
NULL FROM mtl_system_items items
      WHERE items.mrp_planning_code IN (4, 
           8)
      AND   items.organization_id = int.organization_id
      AND   items.inventory_item_id = int.inventory_item_id)
AND EXISTS (SELECT NULL
    FROM mrp_plan_organizations_v mpo,
         mrp_schedule_designators desig
    WHERE desig.schedule_type = 2
    AND desig.schedule_designator = int.schedule_designator
    AND desig.schedule_designator = mpo.compile_designator (+)
    AND desig.organization_id     = mpo.organization_id (+)
    AND NVL(mpo.planned_organization, desig.organization_id)
                                  = int.organization_id)
