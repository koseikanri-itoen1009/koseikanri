 ...
 *  Name                   Description
 * ---------------------- ----------------------------------------------------------
 *  put_log_level          ログ出力関数                                         (A-27)
 *  entry_xwypo            横持計画出力ワークテーブル登録                       (A-26)
 *  entry_xli_lot          横持計画手持在庫テーブル登録(ロット計画数)           (A-25)
 *  update_xwyl_schedule   横持計画品目別代表倉庫ワークテーブル更新             (A-24)
 *  proc_lot_quantity      計画ロットの決定                                     (A-23)
 *  entry_supply_failed    横持計画手持在庫テーブル登録(計画不可)               (A-22)
 *  entry_xli_balance      横持計画手持在庫テーブル登録(バランス計画数)         (A-21)
 *  proc_balance_quantity  バランス計画数の計算                                 (A-20)
 *  proc_ship_loct         移動元倉庫の特定                                     (A-19)
 *  proc_safety_quantity   安全在庫の計算                                       (A-18)
 *  entry_xli_shipment     横持計画手持在庫テーブル登録(出荷ペース)             (A-17)
 *  entry_xli_po           横持計画手持在庫テーブル登録(購入計画)               (A-16)
 *  entry_xli_fs           横持計画手持在庫テーブル登録(工場出荷計画)           (A-15)
 *  entry_xwyp             横持計画物流ワークテーブル登録                       (A-14)
 *  chk_freshness_cond     鮮度条件チェック                                     (A-13)
 *  chk_effective_route    特別横持計画有効期間チェック                         (A-12)
 *  delete_table           テーブルデータ削除                                   (A-11)
 *  init                   初期処理                                             (A-1)
 *  get_msr_route          横持計画制御マスタ取得                               (A-2)
 *  entry_xwyl             品目別代表倉庫取得                                   (A-3)
 *  proc_shipping_pace     出荷ペースの計算                                     (A-4)
 *  proc_total_pace        総出荷ペースの計算                                   (A-5)
 *  create_xli             手持在庫テーブル作成                                 (A-6)
 *  get_msd_schedule       基準計画トランザクション作成                         (A-7)
 *  get_shipment_schedule  出荷トランザクション作成                             (A-8)
 *  create_yoko_plan       横持計画作成                                         (A-9)
 *  output_xwypo           横持計画CSV出力                                      (A-10)
 *  submain                メイン処理プロシージャ
...
--
  ...
  not_need_expt             EXCEPTION;     -- 
  outside_scope_expt        EXCEPTION;     -- 
  lot_skip_expt             EXCEPTION;     -- 
--
...
--20091020_Ver2.1_I_E_479_001_SCS.Goto_ADD_START
  cv_planning_omit          CONSTANT VARCHAR2(10)  := 'O';                      -- 除外
--20091020_Ver2.1_I_E_479_001_SCS.Goto_ADD_END
  --倉庫識別
...
--
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_START
  --補充ステータス
  cv_supply_enough          CONSTANT VARCHAR2(1)   := '1';                      -- 補充完了
  cv_supply_shortage        CONSTANT VARCHAR2(1)   := '0';                      -- 補充数不足
  --出力対象フラグ
  cv_output_on              CONSTANT VARCHAR2(1)   := '0';                      -- 対象外
  cv_output_off             CONSTANT VARCHAR2(1)   := '1';                      -- 対象
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_END
--
  -- ===============================
  -- ユーザー定義グローバル型
  -- ===============================
...
--
   ...
   * Description      : 横持計画出力ワークテーブル登録(A-26)
   ***********************************************************************************/
...
--
   ...
   * Description      : ロット計画数を横持計画手持在庫テーブルに登録(A-25)
   ***********************************************************************************/
...
--
   ...
   * Description      : 横持計画品目別代表倉庫ワークテーブル更新(A-24)
   ***********************************************************************************/
...
--20091026_Ver2.4_I_E_479_004_SCS.Goto_ADD_START
    ln_lot_freshness_quantity NUMBER;         --鮮度条件別ロット在庫数(出荷ペースで按分した結果)
--20091026_Ver2.4_I_E_479_004_SCS.Goto_ADD_END
...
--20091111_Ver2.8_I_E_479_013_SCS.Goto_ADD_START
    ln_surpluses_quantity     NUMBER;         --移動元倉庫の余剰在庫数
--20091111_Ver2.8_I_E_479_013_SCS.Goto_ADD_END
...
--
    ...
    CURSOR xliv_cur(
      ...
      FROM (
        SELECT xwyl.frq_loct_id                         loct_id
...
--20091109_Ver2.7_I_E_479_012_SCS.Goto_DEL_START
--      WHERE xliv.manufacture_date >= NVL(it_sy_manufacture_date, xliv.manufacture_date)
--20091109_Ver2.7_I_E_479_012_SCS.Goto_DEL_END
      GROUP BY ROLLUP(
         xliv.lot_id
        ,xliv.lot_no
        ,xliv.manufacture_date
...
--20091020_Ver2.1_I_E_479_001_SCS.Goto_MOD_START
--              ln_lot_quantity     := l_xliv_rec.loct_onhand;
              ln_lot_quantity     := l_xliv_rec.loct_onhand - ln_filled_quantity;
--20091020_Ver2.1_I_E_479_001_SCS.Goto_MOD_END
              --鮮度条件に合致する出荷ペースを集計
...
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_START
--                  l_ship_lot_tab(ln_ship_idx).lot_quantity := CEIL((ln_lot_quantity
...
--                                                              );
                  ...
                  l_ship_lot_tab(ln_ship_idx).lot_quantity := ln_lot_freshness_quantity
                                                            - l_ship_lot_tab(ln_ship_idx).adjust_quantity;
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_END
                  --横持後在庫数が上限
...
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_START
--                  ln_lot_quantity := ln_lot_quantity - l_ship_lot_tab(ln_ship_idx).lot_quantity;
              ...
              FOR ln_rcpt_idx IN io_xwypo_tab.FIRST .. io_xwypo_tab.LAST LOOP
                ...
                IF (io_xwypo_tab(ln_rcpt_idx).rcpt_loct_id  = l_xliv_rec.loct_id) THEN
                  IF (l_rcpt_lot_tab(ln_rcpt_idx).stock_proc_flag = cv_planning_yes) THEN
                    ...
                    --端数は優先順位の高い鮮度条件に引き当てる
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_START
--                    l_rcpt_lot_tab(ln_rcpt_idx).lot_quantity := CEIL((ln_lot_quantity
...
--20091020_Ver2.1_I_E_479_001_SCS.Goto_ADD_END
                    ...
                    --鮮度条件別在庫に引当したロット在庫数を減算
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_START
--                    ln_lot_quantity := ln_lot_quantity - l_rcpt_lot_tab(ln_rcpt_idx).lot_quantity;
                    ln_lot_quantity := ln_lot_quantity - ln_lot_freshness_quantity;
--20091026_Ver2.4_I_E_479_004_SCS.Goto_MOD_END
                    ln_shipping_pace := ln_shipping_pace - io_xwypo_tab(ln_rcpt_idx).shipping_pace;
                    --ロット在庫数の符号と鮮度条件別ロット在庫数の符号が違う場合、按分から除外する
...
--
          ...
          FOR ln_ship_idx IN io_gbqt_tab.FIRST .. io_gbqt_tab.LAST LOOP
            ...
            IF (l_ship_lot_tab(ln_ship_idx).adjust_proc_flag = cv_planning_yes) THEN
              ...
              ln_total_shipping_pace := ln_total_shipping_pace - io_gbqt_tab(ln_ship_idx).shipping_pace;
--20091111_Ver2.8_I_E_479_013_SCS.Goto_MOD_START
--              --過不足在庫数がプラスの場合、補充可能数に加算
--              ln_lot_supplies_quantity := ln_lot_supplies_quantity
...
--20091111_Ver2.8_I_E_479_013_SCS.Goto_MOD_END
            END IF;
...
--20091111_Ver2.8_I_E_479_013_SCS.Goto_MOD_END
          ...
          FOR ln_rcpt_idx IN io_xwypo_tab.FIRST .. io_xwypo_tab.LAST LOOP
            ...
            IF (l_rcpt_lot_tab(ln_rcpt_idx).adjust_proc_flag = cv_planning_yes) THEN
              ...
              ln_total_lot_quantity := ln_total_lot_quantity - ln_div_quantity;
              ln_total_shipping_pace := ln_total_shipping_pace - io_xwypo_tab(ln_rcpt_idx).shipping_pace;
--20091028_Ver2.6_I_E_479_006_SCS.Goto_ADD_START
              IF (ln_lot_supplies_quantity > 0) THEN
--20091028_Ver2.6_I_E_479_006_SCS.Goto_ADD_END
--20091020_Ver2.2_I_E_479_002_SCS.Goto_ADD_START
--20091022_Ver2.3_I_E_479_003_SCS.Goto_MOD_START
--                IF (l_xliv_rec.manufacture_date <= NVL(it_sy_manufacture_date, l_xliv_rec.manufacture_date)) THEN
                IF (l_xliv_rec.manufacture_date >= NVL(it_sy_manufacture_date, l_xliv_rec.manufacture_date)) THEN
--20091022_Ver2.3_I_E_479_003_SCS.Goto_MOD_END
--20091020_Ver2.2_I_E_479_002_SCS.Goto_ADD_END
...
--20091020_Ver2.2_I_E_479_002_SCS.Goto_ADD_START
                END IF;
--20091020_Ver2.2_I_E_479_002_SCS.Goto_DEL_END
--20091028_Ver2.6_I_E_479_006_SCS.Goto_ADD_START
              END IF;
--20091028_Ver2.6_I_E_479_006_SCS.Goto_ADD_END
            END IF;
...
--20091020_Ver2.2_I_E_479_002_SCS.Goto_MOD_START
--                        LEAST(CEIL((ln_supplies_quantity + ln_require_quantity)
--                                  * io_xwypo_tab(ln_rcpt_idx).shipping_pace
...
--20091020_Ver2.2_I_E_479_002_SCS.Goto_MOD_END
                      --調整したロット在庫数を減算
...
--
   ...
   * Description      : 横持計画手持在庫テーブル登録(計画不可)(A-22)
   ***********************************************************************************/
...
--20091109_Ver2.7_I_E_479_011_SCS.Goto_MOD_START
--        IF    (io_xwypo_tab(ln_xwypo_idx).plan_bal_quantity = 0)
        IF    (io_xwypo_tab(ln_xwypo_idx).plan_lot_quantity = 0)
--20091109_Ver2.7_I_E_479_011_SCS.Goto_MOD_END
...
--
   ...
   * Description      : バランス計画数を横持計画手持在庫テーブルに登録(A-21)
   ***********************************************************************************/
...
--20091026_Ver2.5_I_E_479_005_SCS.Goto_DEL_START
--        AND xwyp.planning_flag       = cv_planning_yes
--20091026_Ver2.5_I_E_479_005_SCS.Goto_DEL_END
...
--
        ...
        BEGIN
          ...
          FROM xxcop_wk_yoko_planning xwyp
          WHERE xwyp.transaction_id       = gn_transaction_id
--20091027_Ver2.5_I_E_479_005_SCS.Goto_DEL_START
--            AND xwyp.planning_flag        = cv_planning_yes
--20091027_Ver2.5_I_E_479_005_SCS.Goto_DEL_END
...
--                     ,iv_token_value1 => i_ship_tab(o_git_tab(ln_priority_idx)).loct_code
--                     ,iv_token_name2  => cv_msg_10054_token_2
--                     ,iv_token_value2 => TO_CHAR(l_glpt_tab(o_git_tab(ln_priority_idx)).manufacture_date, cv_date_format)
--                     ,iv_token_name3  => cv_msg_10054_token_3
--                     ,iv_token_value3 => TO_CHAR(l_glpt_tab(o_git_tab(ln_priority_idx)).stock_days, cv_num42_format)
--                     ,iv_token_name4  => cv_msg_10054_token_4
--                     ,iv_token_value4 => TO_CHAR(l_glpt_tab(o_git_tab(ln_priority_idx)).delivery_lead_time)
--                   );
--      fnd_file.put_line(
--         which  => FND_FILE.LOG
--        ,buff   => lv_errmsg
...
--
   ...
   * Description      : 出荷ペースを横持計画手持在庫テーブルに登録(A-17)
   ***********************************************************************************/
...
--
   ...
   * Description      : 購入計画を横持計画手持在庫テーブルに登録(A-16)
   ***********************************************************************************/
...
--
   ...
   * Description      : 工場出荷計画を横持計画手持在庫テーブルに登録(A-15)
   ***********************************************************************************/
...
--
          ...
          WITH msd_vw AS (
            SELECT msd.schedule_date                          receipt_date
          ...
          FROM (
            SELECT l_xwyp_rec.rcpt_loct_id                    loct_id
                  ...
                  ,l_xwyp_rec.rcpt_organization_id            organization_id
                  ,l_xwyp_rec.rcpt_organization_code          organization_code
                  ,msdv.manufacture_date                      manufacture_date
                  ,msdv.receipt_date                          schedule_date
                  ,msdv.schedule_quantity                     schedule_quantity
            FROM msd_vw msdv
            UNION ALL
            SELECT l_xwyp_rec.ship_loct_id                    loct_id
                  ...
                  ,msdv.manufacture_date                      manufacture_date
                  ,msdv.shipping_date                         schedule_date
                  ,msdv.schedule_quantity * -1                schedule_quantity
            FROM msd_vw msdv
          ) msdvv
...
--
   ...
   * Description      : 横持計画物流ワークテーブル登録(A-14)
   ***********************************************************************************/
...
--
   ...
   * Description      : 特別横持計画有効期間チェック(A12)
   ***********************************************************************************/
...
--
    ...
    BEGIN
      ...
      FOR UPDATE NOWAIT;
      --データ削除
      DELETE FROM xxcop_wk_yoko_planning;
--
    EXCEPTION
...
--
    ...
    BEGIN
      ...
      FOR UPDATE NOWAIT;
      --データ削除
      DELETE FROM xxcop_wk_yoko_locations;
--
    EXCEPTION
      WHEN resource_busy_expt THEN
        NULL;
...
--
    ...
    CURSOR item_cur(
      ...
      FROM   ic_item_mst_b             iimb      --OPM品目マスタ
            ...
            ,gmi_item_categories       gic_p     --OPM品目カテゴリ割当(本社商品区分)
            ,mtl_categories_b          mcb_p     --品目カテゴリマスタ(本社商品区分)
            ,gmi_item_categories       gic_a     --OPM品目カテゴリ割当(商品製品区分)
      ...
      WHERE iimb.inactive_ind                = cn_iimb_status_active
        ...
        AND NOT EXISTS(
          ...
          WHERE  xsr.plan_item_flag          = cn_xsr_plan_item                 --計画商品FLAG(1:計画商品)
            AND  xsr.item_code               = iimb.item_no
            AND  id_planning_date      BETWEEN NVL(xsr.start_date_active, id_planning_date)
                                           AND NVL(xsr.end_date_active  , id_planning_date)
        )
    ;
    --経路情報の取得
    CURSOR msr_cur(
...
--            );
            ...
            IF (lv_item_category <> cv_item_class_product) THEN
              RAISE outside_scope_expt;
            END IF;
            --ケース入数チェック
...
--                                  || 'msr_cur:'
--                                  || TO_CHAR(ld_planning_date, cv_date_format)  || ','
--                                  || l_xwyp_rec.assignment_set_name             || ','
--                                  || l_xwyp_rec.assignment_type                 || ','
--                                  || l_xwyp_rec.sourcing_rule_name              || ','
--                                  || l_xwyp_rec.ship_organization_id            || ','
--                                  || l_xwyp_rec.rcpt_organization_id            || ','
...
--
   ...
   * Description      : 基準計画トランザクション作成(A-7)
   ***********************************************************************************/
...
--
   ...
   * Description      : 横持計画手持在庫テーブル登録(出荷ペース)(A-8)
   ***********************************************************************************/
...
--
    -- *** ローカル・カーソル ***
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_START
    CURSOR rcpt_storage_cur IS
      SELECT xwypov.receipt_date                                receipt_date
            ,xwypov.rcpt_loct_code                              rcpt_loct_code
            ,xwypov.item_no                                     item_no
            ,xwypov.freshness_condition                         freshness_condition
            ,MAX(xwypov.supply_status)                          supply_status
      FROM (
        SELECT xwypo.shipping_date                              shipping_date
              ...
              ,CASE
                 WHEN SUM(xwypo.plan_lot_quantity) >= ((xwypo.max_stock_days * xwypo.shipping_pace)
                                                      - xwypo.before_stock)
        ...
        GROUP BY xwypo.shipping_date
                ,xwypo.receipt_date
                ,xwypo.ship_loct_code
                ,xwypo.rcpt_loct_code
                ,xwypo.item_no
                ,xwypo.freshness_condition
                ,xwypo.safety_stock_days
                ,xwypo.max_stock_days
                ,xwypo.shipping_pace
                ,xwypo.before_stock
      ) xwypov
      GROUP BY xwypov.receipt_date
              ,xwypov.rcpt_loct_code
              ,xwypov.item_no
...
--20091120_Ver2.10_I_E_479_017_SCS.Goto_MOD_START
--            ,GREATEST((xwypo.safety_stock_days * xwypo.shipping_pace) - xwypo.before_stock
--                    , 0
--             )                                                plan_min_quantity
--            ,GREATEST((xwypo.max_stock_days    * xwypo.shipping_pace) - xwypo.before_stock
--                    , 0
--             )                                                plan_max_quantity
--            ,xwypo.plan_bal_quantity                          plan_bal_quantity
            ,xwypo.plan_min_quantity                          plan_min_quantity
            ,xwypo.plan_max_quantity                          plan_max_quantity
--20091120_Ver2.10_I_E_479_017_SCS.Goto_MOD_END
            ,SUM(xwypo.plan_lot_quantity)                     plan_lot_quantity
...
--20091117_Ver2.9_I_E_479_015_SCS.Goto_MOD_START
--            ,MIN(xwypo.before_lot_stock)                      before_lot_stock
--            ,MAX(xwypo.after_lot_stock)                       after_lot_stock
            ,MIN(SUM(xwypo.before_lot_stock)) OVER (PARTITION BY xwypo.receipt_date
                                                                ,xwypo.rcpt_loct_code
                                                                ,xwypo.item_no
                                                                ,xwypo.freshness_condition
                                                                ,xwypo.manufacture_date
                                               )              before_lot_stock
--20091117_Ver2.9_I_E_479_015_SCS.Goto_MOD_END
            ,(xwypo.safety_stock_days * xwypo.shipping_pace)  safety_stock_quantity
            ,(xwypo.max_stock_days    * xwypo.shipping_pace)  max_stock_quantity
            ,xwypo.shipping_pace                              shipping_pace
            ,CASE
               WHEN xwypo.assignment_set_type = cv_custom_plan
                 THEN cv_csv_mark
                 ELSE NULL
             END                                              custom_plan_mark
--20091120_Ver2.10_I_E_479_017_SCS.Goto_MOD_START
----20091111_Ver2.8_I_E_479_013_SCS.Goto_MOD_START
----            ,CASE
----               WHEN (xwypo.max_stock_days * xwypo.shipping_pace) > xwypo.before_stock + xwypo.plan_bal_quantity
----                 THEN cv_csv_mark
----                 ELSE NULL
----             END                                              short_supply_mark
--            ,SUM(SUM(xwypo.plan_lot_quantity)) OVER (PARTITION BY xwypo.receipt_date
--                                                                 ,xwypo.rcpt_loct_code
--                                                                 ,xwypo.item_no
--                                                                 ,xwypo.freshness_condition
--                                               )              total_plan_lot_quantity
----20091111_Ver2.8_I_E_479_013_SCS.Goto_MOD_END
            ,xwypo.short_supply_flag                          short_supply_flag
--20091120_Ver2.10_I_E_479_017_SCS.Goto_MOD_END
            ,CASE
               WHEN MAX(xwypo.latest_manufacture_date) > xwypo.manufacture_date
                 THEN cv_csv_mark
                 ELSE NULL
             END                                              lot_revers_mark
            ,ROW_NUMBER() OVER (PARTITION BY xwypo.shipping_date
                                            ,xwypo.receipt_date
                                            ,xwypo.ship_loct_code
                                            ,xwypo.rcpt_loct_code
                                            ,xwypo.item_no
      ...
      GROUP BY xwypo.shipping_date
              ,xwypo.receipt_date
              ,xwypo.ship_loct_code
              ,xwypo.ship_loct_name
              ,xwypo.rcpt_loct_code
              ,xwypo.rcpt_loct_name
              ,xwypo.item_no
              ,xwypo.item_name
              ,xwypo.manufacture_date
              ,xwypo.freshness_condition
...
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_START
--              ,xwypo.before_stock
--              ,xwypo.plan_bal_quantity
              ,xwypo.plan_min_quantity
              ,xwypo.plan_max_quantity
              ,xwypo.short_supply_flag
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_END
              ,xwypo.delivery_unit
              ,xwypo.palette_max_cs_qty
              ,xwypo.palette_max_step_qty
...
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_START
    --補充不可フラグ、出力対象フラグの更新
    <<rcpt_storage_loop>>
    FOR rcpt_storage_rec IN rcpt_storage_cur LOOP
      UPDATE xxcop_wk_yoko_plan_output xwypo
      SET    xwypo.short_supply_flag    = CASE
                                            WHEN rcpt_storage_rec.supply_status = cv_supply_enough THEN
                                              NULL
                                            ELSE
                                              cv_csv_mark
                                          END
            ,xwypo.plan_min_quantity    = GREATEST((xwypo.safety_stock_days * xwypo.shipping_pace) - xwypo.before_stock
                                                  , 0
                                          )
            ,xwypo.plan_max_quantity    = GREATEST((xwypo.max_stock_days    * xwypo.shipping_pace) - xwypo.before_stock
                                                 , 0
                                          )
        WHERE xwypo.transaction_id      = gn_transaction_id
          AND xwypo.receipt_date        = rcpt_storage_rec.receipt_date
          AND xwypo.rcpt_loct_code      = rcpt_storage_rec.rcpt_loct_code
          AND xwypo.item_no             = rcpt_storage_rec.item_no
          AND xwypo.freshness_condition = rcpt_storage_rec.freshness_condition
      ;
    END LOOP rcpt_storage_loop;
--20091120_Ver2.10_I_E_479_017_SCS.Goto_ADD_END
    --CSVファイルヘッダ出力
    ...
    FOR l_xwypo_rec IN xwypo_cur LOOP
      ...
      --項目の編集
      --出荷日
      lv_csvbuff := cv_csv_char_bracket
                 || TO_CHAR(l_xwypo_rec.shipping_date, cv_csv_date_format)
                 || cv_csv_delimiter
      ;
      --着日
      ...
      ;
      --計画数(最小)
      IF (l_xwypo_rec.lot_row_number = 1) THEN
        lv_csvbuff := lv_csvbuff
      ...
      IF (l_xwypo_rec.lot_row_number = 1) THEN
        lv_csvbuff := lv_csvbuff
                   || cv_csv_char_bracket || l_xwypo_rec.delivery_unit
                   || cv_csv_delimiter
        ;
      ELSE
        lv_csvbuff := lv_csvbuff
...
--      ;
      lv_csvbuff := lv_csvbuff
                 || TO_CHAR(l_xwypo_rec.before_lot_stock + l_xwypo_rec.plan_lot_quantity)
                 || cv_csv_delimiter
      ;
--20091117_Ver2.9_I_E_479_015_SCS.Goto_MOD_END
      --安全在庫数
...
----      lv_csvbuff := lv_csvbuff
----                 || cv_csv_char_bracket
----                 || l_xwypo_rec.short_supply_mark
----                 || cv_csv_delimiter
----      ;
--      IF (l_xwypo_rec.plan_max_quantity <= l_xwypo_rec.total_plan_lot_quantity) THEN
--        lv_csvbuff := lv_csvbuff
--                   || cv_csv_char_bracket
--                   || cv_csv_delimiter
--        ;
