 ...
 *
 * View Name       : XXCOP_SALES_EXP_MV2
 * Description     : 計画_販売実績マテリアライズドビュー2（前年度 対象３ヶ月前実績数量）
 * Version         : 1.0
 *
 * Change Record
 * ------------- ----- ---------------- ---------------------------------
 ...
 *
 ************************************************************************/
CREATE MATERIALIZED VIEW APPS.XXCOP_SALES_EXP_MV2
  TABLESPACE "XXDATA2"
  BUILD IMMEDIATE
  USING INDEX
  REFRESH COMPLETE
  ...
  AS
    ...
    AND    xseh.delivery_date BETWEEN ADD_MONTHS(TRUNC(xxccp_common_pkg2.get_process_date,'MM'),-14)
                              AND     ADD_MONTHS(TRUNC(xxccp_common_pkg2.get_process_date,'MM'),-13) - (1/24/60/60)
  ...
  ;
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV2.TARGET_MONTH       IS '対象年月'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV2.DELIVERY_BASE_CODE IS '納品拠点コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV2.ITEM_CODE          IS '品目コード（販売実績）'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV2.GROUP_ITEM_CODE    IS '集約コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV2.SUM_STANDARD_QTY   IS '基準数量合計'
/