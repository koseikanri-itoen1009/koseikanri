 ...
 *
 * View Name       : XXCOP_SALES_EXP_MV5
 * Description     : 計画_販売実績マテリアライズドビュー5（前年度 対象月実績数量）
 * Version         : 1.0
 *
 * Change Record
 * ------------- ----- ---------------- ---------------------------------
 ...
 *
 ************************************************************************/
CREATE MATERIALIZED VIEW APPS.XXCOP_SALES_EXP_MV5
  TABLESPACE "XXDATA2"
  BUILD IMMEDIATE
  USING INDEX
  REFRESH COMPLETE
  ...
  AS
    ...
    AND    xseh.delivery_date BETWEEN ADD_MONTHS(TRUNC(xxccp_common_pkg2.get_process_date,'MM'),-11)
                              AND     ADD_MONTHS(TRUNC(xxccp_common_pkg2.get_process_date,'MM'),-10) - (1/24/60/60)
  ...
  ;
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV5.TARGET_MONTH       IS '対象年月'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV5.DELIVERY_BASE_CODE IS '納品拠点コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV5.ITEM_CODE          IS '品目コード（販売実績）'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV5.GROUP_ITEM_CODE    IS '集約コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV5.SUM_STANDARD_QTY   IS '基準数量合計'
/