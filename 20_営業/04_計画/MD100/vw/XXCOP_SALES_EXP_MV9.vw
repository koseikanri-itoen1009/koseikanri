 ...
 *
 * View Name       : XXCOP_SALES_EXP_MV9
 * Description     : 計画_販売実績マテリアライズドビュー9（当年度 対象前月実績数量）
 * Version         : 1.0
 *
 * Change Record
 * ------------- ----- ---------------- ---------------------------------
 ...
 *
 ************************************************************************/
CREATE MATERIALIZED VIEW APPS.XXCOP_SALES_EXP_MV9
  TABLESPACE "XXDATA2"
  BUILD IMMEDIATE
  USING INDEX
  REFRESH COMPLETE
  ...
  ;
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV9.TARGET_MONTH       IS '対象年月'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV9.DELIVERY_BASE_CODE IS '納品拠点コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV9.ITEM_CODE          IS '品目コード（販売実績）'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV9.GROUP_ITEM_CODE    IS '集約コード'
/
COMMENT ON COLUMN APPS.XXCOP_SALES_EXP_MV9.SUM_STANDARD_QTY   IS '基準数量合計'
/