 ...
 ************************************************************************/
  ...
  SELECT cust_hier.cash_account_id                          AS cash_account_id        --入金先顧客ID
        ...
        ,cust_hier.bill_last_update_date                    AS bill_last_update_date  --請求先顧客最終更新日
  FROM   (
  --①入金先顧客＆請求先顧客－出荷先顧客
    SELECT
           /*+
...
-- Ver1.1 Mod End
    ...
    AND    bill_hasa_1.status = 'A'                                          --請求先顧客所在地.ステータス = 'A'
...
-- Ver1.1 Mod End
    ...
    AND    cash_hasa_2.status = 'A'                                          --入金先顧客所在地.ステータス = 'A'
    AND    bill_hasa_2.status = 'A'                                          --請求先顧客所在地.ステータス = 'A'
...
-- Ver1.1 Add End
    ...
    AND    cash_hasa_3.status = 'A'                                          --入金先顧客所在地.ステータス = 'A'
...
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.cash_account_name       IS  '入金先顧客名称';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_account_id         IS  '請求先顧客ID';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_account_number     IS  '請求先顧客コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_account_name       IS  '請求先顧客名称';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.ship_account_id         IS  '出荷先顧客ID';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.ship_account_number     IS  '出荷先顧客コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.ship_account_name       IS  '出荷先顧客名称';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.cash_receiv_base_code   IS  '入金拠点コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_party_id           IS  'パーティID';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_bill_base_code     IS  '請求拠点コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_postal_code        IS  '郵便番号';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_state              IS  '都道府県';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_city               IS  '市・区';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_address1           IS  '住所1';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_address2           IS  '住所2';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_tel_num            IS  '電話番号';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_cons_inv_flag      IS  '一括請求書発行フラグ';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_torihikisaki_code  IS  '取引先コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_store_code         IS  '店舗コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_cust_store_name    IS  '顧客店舗名称';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_tax_div            IS  '消費税区分';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_cred_rec_code1     IS  '売掛コード1(請求書)';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_cred_rec_code2     IS  '売掛コード2(事業所)';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_cred_rec_code3     IS  '売掛コード3(その他)';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_invoice_type       IS  '請求書出力形式';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_payment_term_id    IS  '支払条件';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_payment_term2      IS  '第2支払条件';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_payment_term3      IS  '第3支払条件';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_tax_round_rule     IS  '税金－端数処理';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.ship_sale_base_code     IS  '売上拠点コード';
COMMENT ON  COLUMN  apps.xxcos_cust_hierarchy_mv.bill_last_update_date   IS  '請求先顧客使用目的最終更新日';
--
COMMENT ON  MATERIALIZED VIEW   apps.xxcos_cust_hierarchy_mv             IS  'XXCOS顧客階層マテリアライズドビュー';
