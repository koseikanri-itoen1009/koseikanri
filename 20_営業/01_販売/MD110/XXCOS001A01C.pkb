...
--****************************** 2010/01/18 1.21 M.Uehara ADD END   *******************************--
-- Ver.1.28 ADD Start
  cv_hht_received    CONSTANT VARCHAR2(1)   := 'Y';                    -- HHT受信フラグ＝Y:HHT受信データ
-- Ver.1.28 ADD End
--
  -- クイックコードタイプ
...
-- Ver.1.29 MOD End
-- Ver.1.30 ADD Start
     ,visit_class1      xxcos_dlv_headers_work.visit_class1%TYPE        -- 訪問区分1
     ,visit_class2      xxcos_dlv_headers_work.visit_class2%TYPE        -- 訪問区分2
     ,visit_class3      xxcos_dlv_headers_work.visit_class3%TYPE        -- 訪問区分3
...
-- Ver.1.29 ADD End
-- Ver.1.30 ADD Start
  gt_head_visit_class1      g_tab_head_visit_class1;        -- 訪問区分1
  gt_head_visit_class2      g_tab_head_visit_class2;        -- 訪問区分2
  gt_head_visit_class3      g_tab_head_visit_class3;        -- 訪問区分3
...
-- Ver.1.29 ADD End
-- Ver.1.30 ADD Start
            ,headers.visit_class1                    visit_class1              -- 訪問区分1
            ,headers.visit_class2                    visit_class2              -- 訪問区分2
            ,headers.visit_class3                    visit_class3              -- 訪問区分3
...
-- 2011/03/16 Ver.1.26 S.Ochiai ADD End
-- Ver.1.28 ADD Start
    lt_ttl_sales_amt        xxcos_dlv_headers.total_sales_amt%TYPE;          -- 総販売金額
    lt_cs_ttl_sales_amt     xxcos_dlv_headers.cash_total_sales_amt%TYPE;     -- 現金売りトータル販売金額
...
-- Ver.1.29 ADD End
-- Ver.1.30 ADD Start
    lt_visit_class1         xxcos_dlv_headers_work.visit_class1%TYPE;        -- 訪問区分1
    lt_visit_class2         xxcos_dlv_headers_work.visit_class2%TYPE;        -- 訪問区分2
    lt_visit_class3         xxcos_dlv_headers_work.visit_class3%TYPE;        -- 訪問区分3
...
-- 2011/03/16 Ver.1.26 S.Ochiai ADD End
-- Ver.1.28 ADD Start
      lt_ttl_sales_amt    := gt_headers_work_data(ck_no).ttl_sales_amt;       -- 総販売金額
      lt_cs_ttl_sales_amt := gt_headers_work_data(ck_no).cs_ttl_sales_amt;    -- 現金売りトータル販売金額
...
-- Ver.1.29 ADD End
-- Ver.1.30 ADD Start
      lt_visit_class1     := gt_headers_work_data(ck_no).visit_class1;        -- 訪問区分1
      lt_visit_class2     := gt_headers_work_data(ck_no).visit_class2;        -- 訪問区分2
      lt_visit_class3     := gt_headers_work_data(ck_no).visit_class3;        -- 訪問区分3
...
-- 2011/03/16 Ver.1.26 S.Ochiai ADD End
-- Ver.1.28 ADD Start
        gt_head_ttl_sales_amt(ln_header_ok_no)    := lt_ttl_sales_amt;       -- 総販売金額
        gt_head_cs_ttl_sales_amt(ln_header_ok_no) := lt_cs_ttl_sales_amt;    -- 現金売りトータル販売金額
...
-- Ver.1.29 ADD End
-- Ver.1.30 ADD Start
        gt_head_visit_class1(ln_header_ok_no)     := lt_visit_class1;         -- 訪問区分1
        gt_head_visit_class2(ln_header_ok_no)     := lt_visit_class2;         -- 訪問区分2
        gt_head_visit_class3(ln_header_ok_no)     := lt_visit_class3;         -- 訪問区分3
        gt_head_visit_class4(ln_header_ok_no)     := lt_visit_class4;         -- 訪問区分4
        gt_head_visit_class5(ln_header_ok_no)     := lt_visit_class5;         -- 訪問区分5
-- Ver.1.30 ADD End
...
-- Ver.1.30 ADD Start
       ,gt_head_visit_class1(i)   -- DFF1（訪問区分1）
       ,gt_head_visit_class2(i)   -- DFF2（訪問区分2）
       ,gt_head_visit_class3(i)   -- DFF3（訪問区分3）
       ,gt_head_visit_class4(i)   -- DFF4（訪問区分4）
       ,gt_head_visit_class5(i)   -- DFF5（訪問区分5）
       ,NULL                      -- DFF6（訪問区分6）
       ,NULL                      -- DFF7（訪問区分7）
       ,NULL                      -- DFF8（訪問区分8）
       ,NULL                      -- DFF9（訪問区分9）
...
-- 2011/03/16 Ver.1.26 S.Ochiai ADD End
-- Ver.1.28 ADD Start
            gt_head_ttl_sales_amt(i),                    -- 総販売金額
            gt_head_cs_ttl_sales_amt(i),                 -- 現金売りトータル販売金額
            gt_head_pp_ttl_sales_amt(i),                 -- PPカードトータル販売金額
