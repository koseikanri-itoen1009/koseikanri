CREATE OR REPLACE PACKAGE BODY APPS.XXCOS012A07R
AS
 /*****************************************************************************************
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name               CONSTANT VARCHAR2(100) := 'XXCOS012A07R';          -- パッケージ名
--
  cv_conc_name              CONSTANT VARCHAR2(100) := 'XXCOS012A07R';          -- コンカレント名
  cv_file_id                CONSTANT VARCHAR2(100) := 'XXCOS012A07R';          -- 帳票ＩＤ
  cv_extension_pdf          CONSTANT VARCHAR2(100) := '.pdf';                  -- 拡張子（ＰＤＦ）
  cv_frm_file               CONSTANT VARCHAR2(100) := 'XXCOS012A07S.xml';      -- フォーム様式ファイル名
  cv_vrq_file               CONSTANT VARCHAR2(100) := 'XXCOS012A07S.vrq';      -- クエリー様式ファイル名
...
--
  ...
  IS
    TABLE OF
      xxcos_rep_lot_pick_deli_sale%ROWTYPE
...
--  Add Ver1.1 S.Yamashita End
  ...
  gt_shipping_sts_cd3                 fnd_lookup_values.attribute3%TYPE;
                                                                      -- 出荷情報ステータスコード3
--  Add Ver1.1 S.Yamashita Start
  gv_login_customer_name              VARCHAR(40);                    -- 顧客名
--  Add Ver1.1 S.Yamashita End
  --帳票ワーク内部テーブル
...
--
    ...
    BEGIN
      ...
      BEGIN
        SELECT
          xxcos_rep_l_pick_deli_sale_s01.NEXTVAL          record_id
        INTO
          ln_record_id
        FROM
          dual
...
--
    ...
    IS
      SELECT
        xrlpds.record_id                record_id
      FROM
        xxcos_rep_lot_pick_deli_sale    xrlpds              --ロット別ピックリスト_出荷先_製品_販売先別帳票ワークテーブル
      WHERE
        xrlpds.request_id               = cn_request_id     --要求ID
      FOR UPDATE NOWAIT
      ;
--
    -- *** ローカル・レコード ***
...
--
    ...
    BEGIN
      DELETE FROM
        xxcos_rep_lot_pick_deli_sale    xrlpds
      WHERE
        xrlpds.request_id               = cn_request_id
      ;
    EXCEPTION
      WHEN OTHERS THEN
        --要求ID文字列取得
