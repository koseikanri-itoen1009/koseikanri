CREATE OR REPLACE PACKAGE BODY APPS.XXCOS012A03R
AS
/*****************************************************************************************
 ...
 * ------------- ----- ---------------- -------------------------------------------------
 *  2008/12/24    1.0   K.Kakishita      新規作成
 ...
 *                                                バラ数に数量を設定する。
 *  2009/06/16    1.6   K.Kiriu          [T1_1358]定番特売区分クイックコード値変更対応
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name      CONSTANT VARCHAR2(100) := 'XXCOS012A03R';  -- パッケージ名
--
  cv_conc_name              CONSTANT VARCHAR2(100) := 'XXCOS012A03R';          -- コンカレント名
  cv_file_id                CONSTANT VARCHAR2(100) := 'XXCOS012A03R';          -- 帳票ＩＤ
  cv_extension_pdf          CONSTANT VARCHAR2(100) := '.pdf';                  -- 拡張子（ＰＤＦ）
  cv_frm_file               CONSTANT VARCHAR2(100) := 'XXCOS012A03S.xml';      -- フォーム様式ファイル名
  cv_vrq_file               CONSTANT VARCHAR2(100) := 'XXCOS012A03S.vrq';      -- クエリー様式ファイル名
...
--
    ...
    IS
      ...
      FROM
         xxcos_rep_pick_deli_sale       xrpds               --ピックリスト_出荷先_製品_販売先別帳票ワークテーブル
      WHERE
        xrpds.request_id                = cn_request_id     --要求ID
      FOR UPDATE NOWAIT
      ;
--
    -- *** ローカル・レコード ***
...
--
    ...
    lv_retcode VARCHAR2(1);     -- リターン・コード
    lv_errmsg  VARCHAR2(5000);  -- ユーザー・エラー・メッセージ
-- ************************* 2009/07/27 N.Maeda 1.7 ADD START ****************** --
...
-- ************************* 2009/07/27 N.Maeda 1.7 ADD  END  ****************** --
--
--###########################  固定部 END   ####################################
  BEGIN
--
...
--
-- ***************** 2009/07/27 N.Maeda 1.7 MOD START ****************************** --
...
--      RAISE global_process_expt;
--    END IF;
-- ***************** 2009/07/27 N.Maeda 1.7 MOD  END  ****************************** --
--
    -- ===============================
    -- A-3  帳票ワークテーブル削除
    -- ===============================
...
-- ***************** 2009/07/27 N.Maeda 1.7 ADD START ****************************** --
    ...
    IF ( lv_retcode_svf = cv_status_error ) THEN
      ...
      RAISE global_process_expt;
    END IF;
-- ***************** 2009/07/27 N.Maeda 1.7 AD	D  END  ****************************** --
--
    --明細０件時の警告終了制御
    IF ( g_rpt_data_tab.COUNT = 0 ) THEN
