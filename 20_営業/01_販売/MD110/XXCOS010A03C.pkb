 ...
 *  2010/03/23    1.19  M.Sano           [E_本稼動_01728] 営業担当の日付チェック変更
 *  2010/04/20    1.20  Y.Goto           [E_本稼動_01719] 営業担当チェックを共通関数に変更
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name            CONSTANT VARCHAR2(100) := 'XXCOS010A03C';              -- パッケージ名
  --
...
-- 2010/04/20 Ver.1.20 Y.Goto add End
  ...
  cv_msg_edi_wk_tbl      CONSTANT VARCHAR2(20)  := 'APP-XXCOS1-00117';          -- EDI納品返品情報ワークテーブル
...
-- 2010/05/10 Ver.1.22 K.Oomata Add End
  ...
  cv_data_type_31        CONSTANT VARCHAR2(10)  := '31';                        -- データ種コード：納品確定
  cv_data_type_32        CONSTANT VARCHAR2(10)  := '32';                        -- データ種コード：出庫確定
  cv_creation_class      CONSTANT VARCHAR2(10)  := '20';                        -- 作成元区分：納品確定
...
--
  -- EDI納品返品情報ワークテーブル レコードタイプ定義
  TYPE g_edi_work_rtype IS RECORD
    (
      delivery_return_work_id                xxcos_edi_delivery_work.delivery_return_work_id%TYPE,       -- 納品返品情報ワークID
      medium_class                           xxcos_edi_delivery_work.medium_class%TYPE,                  -- 媒体区分
      data_type_code                         xxcos_edi_delivery_work.data_type_code%TYPE,                -- データ種コード
      file_no                                xxcos_edi_delivery_work.file_no%TYPE,                       -- ファイルＮｏ
      info_class                             xxcos_edi_delivery_work.info_class%TYPE,                    -- 情報区分
      process_date                           xxcos_edi_delivery_work.process_date%TYPE,                  -- 処理日
      process_time                           xxcos_edi_delivery_work.process_time%TYPE,                  -- 処理時刻
      base_code                              xxcos_edi_delivery_work.base_code%TYPE,                     -- 拠点（部門）コード
      base_name                              xxcos_edi_delivery_work.base_name%TYPE,                     -- 拠点名（正式名）
      base_name_alt                          xxcos_edi_delivery_work.base_name_alt%TYPE,                 -- 拠点名（カナ）
      edi_chain_code                         xxcos_edi_delivery_work.edi_chain_code%TYPE,                -- ＥＤＩチェーン店コード
      edi_chain_name                         xxcos_edi_delivery_work.edi_chain_name%TYPE,                -- ＥＤＩチェーン店名（漢字）
      edi_chain_name_alt                     xxcos_edi_delivery_work.edi_chain_name_alt%TYPE,            -- ＥＤＩチェーン店名（カナ）
      chain_code                             xxcos_edi_delivery_work.chain_code%TYPE,                    -- チェーン店コード
      chain_name                             xxcos_edi_delivery_work.chain_name%TYPE,                    -- チェーン店名（漢字）
      chain_name_alt                         xxcos_edi_delivery_work.chain_name_alt%TYPE,                -- チェーン店名（カナ）
      report_code                            xxcos_edi_delivery_work.report_code%TYPE,                   -- 帳票コード
      report_show_name                       xxcos_edi_delivery_work.report_show_name%TYPE,              -- 帳票表示名
      customer_code                          xxcos_edi_delivery_work.customer_code%TYPE,                 -- 顧客コード
      customer_name                          xxcos_edi_delivery_work.customer_name%TYPE,                 -- 顧客名（漢字）
      customer_name_alt                      xxcos_edi_delivery_work.customer_name_alt%TYPE,             -- 顧客名（カナ）
      company_code                           xxcos_edi_delivery_work.company_code%TYPE,                  -- 社コード
      company_name                           xxcos_edi_delivery_work.company_name%TYPE,                  -- 社名（漢字）
      company_name_alt                       xxcos_edi_delivery_work.company_name_alt%TYPE,              -- 社名（カナ）
      shop_code                              xxcos_edi_delivery_work.shop_code%TYPE,                     -- 店コード
      shop_name                              xxcos_edi_delivery_work.shop_name%TYPE,                     -- 店名（漢字）
      shop_name_alt                          xxcos_edi_delivery_work.shop_name_alt%TYPE,                 -- 店名（カナ）
      delivery_center_code                   xxcos_edi_delivery_work.delivery_center_code%TYPE,          -- 納入センターコード
      delivery_center_name                   xxcos_edi_delivery_work.delivery_center_name%TYPE,          -- 納入センター名（漢字）
      delivery_center_name_alt               xxcos_edi_delivery_work.delivery_center_name_alt%TYPE,      -- 納入センター名（カナ）
      order_date                             xxcos_edi_delivery_work.order_date%TYPE,                    -- 発注日
      center_delivery_date                   xxcos_edi_delivery_work.center_delivery_date%TYPE,          -- センター納品日
      result_delivery_date                   xxcos_edi_delivery_work.result_delivery_date%TYPE,          -- 実納品日
      shop_delivery_date                     xxcos_edi_delivery_work.shop_delivery_date%TYPE,            -- 店舗納品日
      data_creation_date_edi_data            xxcos_edi_delivery_work.data_creation_date_edi_data%TYPE,   -- データ作成日（ＥＤＩデータ中）
      data_creation_time_edi_data            xxcos_edi_delivery_work.data_creation_time_edi_data%TYPE,   -- データ作成時刻（ＥＤＩデータ中）
      invoice_class                          xxcos_edi_delivery_work.invoice_class%TYPE,                 -- 伝票区分
      small_classification_code              xxcos_edi_delivery_work.small_classification_code%TYPE,     -- 小分類コード
      small_classification_name              xxcos_edi_delivery_work.small_classification_name%TYPE,     -- 小分類名
      middle_classification_code             xxcos_edi_delivery_work.middle_classification_code%TYPE,    -- 中分類コード
      middle_classification_name             xxcos_edi_delivery_work.middle_classification_name%TYPE,    -- 中分類名
      big_classification_code                xxcos_edi_delivery_work.big_classification_code%TYPE,       -- 大分類コード
      big_classification_name                xxcos_edi_delivery_work.big_classification_name%TYPE,       -- 大分類名
      other_party_department_code            xxcos_edi_delivery_work.other_party_department_code%TYPE,   -- 相手先部門コード
      other_party_order_number               xxcos_edi_delivery_work.other_party_order_number%TYPE,      -- 相手先発注番号
      check_digit_class                      xxcos_edi_delivery_work.check_digit_class%TYPE,             -- チェックデジット有無区分
      invoice_number                         xxcos_edi_delivery_work.invoice_number%TYPE,                -- 伝票番号
      check_digit                            xxcos_edi_delivery_work.check_digit%TYPE,                   -- チェックデジット
      close_date                             xxcos_edi_delivery_work.close_date%TYPE,                    -- 月限
      order_no_ebs                           xxcos_edi_delivery_work.order_no_ebs%TYPE,                  -- 受注Ｎｏ（ＥＢＳ）
      ar_sale_class                          xxcos_edi_delivery_work.ar_sale_class%TYPE,                 -- 特売区分
      delivery_classe                        xxcos_edi_delivery_work.delivery_classe%TYPE,               -- 配送区分
      opportunity_no                         xxcos_edi_delivery_work.opportunity_no%TYPE,                -- 便Ｎｏ
      contact_to                             xxcos_edi_delivery_work.contact_to%TYPE,                    -- 連絡先
      route_sales                            xxcos_edi_delivery_work.route_sales%TYPE,                   -- ルートセールス
      corporate_code                         xxcos_edi_delivery_work.corporate_code%TYPE,                -- 法人コード
      maker_name                             xxcos_edi_delivery_work.maker_name%TYPE,                    -- メーカー名
      area_code                              xxcos_edi_delivery_work.area_code%TYPE,                     -- 地区コード
      area_name                              xxcos_edi_delivery_work.area_name%TYPE,                     -- 地区名（漢字）
      area_name_alt                          xxcos_edi_delivery_work.area_name_alt%TYPE,                 -- 地区名（カナ）
      vendor_code                            xxcos_edi_delivery_work.vendor_code%TYPE,                   -- 取引先コード
      vendor_name                            xxcos_edi_delivery_work.vendor_name%TYPE,                   -- 取引先名（漢字）
      vendor_name1_alt                       xxcos_edi_delivery_work.vendor_name1_alt%TYPE,              -- 取引先名１（カナ）
      vendor_name2_alt                       xxcos_edi_delivery_work.vendor_name2_alt%TYPE,              -- 取引先名２（カナ）
      vendor_tel                             xxcos_edi_delivery_work.vendor_tel%TYPE,                    -- 取引先ＴＥＬ
      vendor_charge                          xxcos_edi_delivery_work.vendor_charge%TYPE,                 -- 取引先担当者
      vendor_address                         xxcos_edi_delivery_work.vendor_address%TYPE,                -- 取引先住所（漢字）
      deliver_to_code_itouen                 xxcos_edi_delivery_work.deliver_to_code_itouen%TYPE,        -- 届け先コード（伊藤園）
      deliver_to_code_chain                  xxcos_edi_delivery_work.deliver_to_code_chain%TYPE,         -- 届け先コード（チェーン店）
      deliver_to                             xxcos_edi_delivery_work.deliver_to%TYPE,                    -- 届け先（漢字）
      deliver_to1_alt                        xxcos_edi_delivery_work.deliver_to1_alt%TYPE,               -- 届け先１（カナ）
      deliver_to2_alt                        xxcos_edi_delivery_work.deliver_to2_alt%TYPE,               -- 届け先２（カナ）
      deliver_to_address                     xxcos_edi_delivery_work.deliver_to_address%TYPE,            -- 届け先住所（漢字）
      deliver_to_address_alt                 xxcos_edi_delivery_work.deliver_to_address_alt%TYPE,        -- 届け先住所（カナ）
      deliver_to_tel                         xxcos_edi_delivery_work.deliver_to_tel%TYPE,                -- 届け先ＴＥＬ
      balance_accounts_code                  xxcos_edi_delivery_work.balance_accounts_code%TYPE,         -- 帳合先コード
      balance_accounts_company_code          xxcos_edi_delivery_work.balance_accounts_company_code%TYPE, -- 帳合先社コード
      balance_accounts_shop_code             xxcos_edi_delivery_work.balance_accounts_shop_code%TYPE,    -- 帳合先店コード
      balance_accounts_name                  xxcos_edi_delivery_work.balance_accounts_name%TYPE,         -- 帳合先名（漢字）
      balance_accounts_name_alt              xxcos_edi_delivery_work.balance_accounts_name_alt%TYPE,     -- 帳合先名（カナ）
      balance_accounts_address               xxcos_edi_delivery_work.balance_accounts_address%TYPE,      -- 帳合先住所（漢字）
      balance_accounts_address_alt           xxcos_edi_delivery_work.balance_accounts_address_alt%TYPE,  -- 帳合先住所（カナ）
      balance_accounts_tel                   xxcos_edi_delivery_work.balance_accounts_tel%TYPE,          -- 帳合先ＴＥＬ
      order_possible_date                    xxcos_edi_delivery_work.order_possible_date%TYPE,           -- 受注可能日
      permission_possible_date               xxcos_edi_delivery_work.permission_possible_date%TYPE,      -- 許容可能日
      forward_month                          xxcos_edi_delivery_work.forward_month%TYPE,                 -- 先限年月日
      payment_settlement_date                xxcos_edi_delivery_work.payment_settlement_date%TYPE,       -- 支払決済日
      handbill_start_date_active             xxcos_edi_delivery_work.handbill_start_date_active%TYPE,    -- チラシ開始日
      billing_due_date                       xxcos_edi_delivery_work.billing_due_date%TYPE,              -- 請求締日
      shipping_time                          xxcos_edi_delivery_work.shipping_time%TYPE,                 -- 出荷時刻
      delivery_schedule_time                 xxcos_edi_delivery_work.delivery_schedule_time%TYPE,        -- 納品予定時間
      order_time                             xxcos_edi_delivery_work.order_time%TYPE,                    -- 発注時間
      general_date_item1                     xxcos_edi_delivery_work.general_date_item1%TYPE,            -- 汎用日付項目１
      general_date_item2                     xxcos_edi_delivery_work.general_date_item2%TYPE,            -- 汎用日付項目２
      general_date_item3                     xxcos_edi_delivery_work.general_date_item3%TYPE,            -- 汎用日付項目３
      general_date_item4                     xxcos_edi_delivery_work.general_date_item4%TYPE,            -- 汎用日付項目４
      general_date_item5                     xxcos_edi_delivery_work.general_date_item5%TYPE,            -- 汎用日付項目５
      arrival_shipping_class                 xxcos_edi_delivery_work.arrival_shipping_class%TYPE,        -- 入出荷区分
      vendor_class                           xxcos_edi_delivery_work.vendor_class%TYPE,                  -- 取引先区分
      invoice_detailed_class                 xxcos_edi_delivery_work.invoice_detailed_class%TYPE,        -- 伝票内訳区分
      unit_price_use_class                   xxcos_edi_delivery_work.unit_price_use_class%TYPE,          -- 単価使用区分
      sub_distribution_center_code           xxcos_edi_delivery_work.sub_distribution_center_code%TYPE,  -- サブ物流センターコード
      sub_distribution_center_name           xxcos_edi_delivery_work.sub_distribution_center_name%TYPE,  -- サブ物流センターコード名
      center_delivery_method                 xxcos_edi_delivery_work.center_delivery_method%TYPE,        -- センター納品方法
      center_use_class                       xxcos_edi_delivery_work.center_use_class%TYPE,              -- センター利用区分
      center_whse_class                      xxcos_edi_delivery_work.center_whse_class%TYPE,             -- センター倉庫区分
      center_area_class                      xxcos_edi_delivery_work.center_area_class%TYPE,             -- センター地域区分
      center_arrival_class                   xxcos_edi_delivery_work.center_arrival_class%TYPE,          -- センター入荷区分
      depot_class                            xxcos_edi_delivery_work.depot_class%TYPE,                   -- デポ区分
      tcdc_class                             xxcos_edi_delivery_work.tcdc_class%TYPE,                    -- ＴＣＤＣ区分
      upc_flag                               xxcos_edi_delivery_work.upc_flag%TYPE,                      -- ＵＰＣフラグ
      simultaneously_class                   xxcos_edi_delivery_work.simultaneously_class%TYPE,          -- 一斉区分
      business_id                            xxcos_edi_delivery_work.business_id%TYPE,                   -- 業務ＩＤ
      whse_directly_class                    xxcos_edi_delivery_work.whse_directly_class%TYPE,           -- 倉直区分
      premium_rebate_class                   xxcos_edi_delivery_work.premium_rebate_class%TYPE,          -- 景品割戻区分
      item_type                              xxcos_edi_delivery_work.item_type%TYPE,                     -- 項目種別
      cloth_house_food_class                 xxcos_edi_delivery_work.cloth_house_food_class%TYPE,        -- 衣家食区分
      mix_class                              xxcos_edi_delivery_work.mix_class%TYPE,                     -- 混在区分
      stk_class                              xxcos_edi_delivery_work.stk_class%TYPE,                     -- 在庫区分
      last_modify_site_class                 xxcos_edi_delivery_work.last_modify_site_class%TYPE,        -- 最終修正場所区分
      report_class                           xxcos_edi_delivery_work.report_class%TYPE,                  -- 帳票区分
      addition_plan_class                    xxcos_edi_delivery_work.addition_plan_class%TYPE,           -- 追加・計画区分
      registration_class                     xxcos_edi_delivery_work.registration_class%TYPE,            -- 登録区分
      specific_class                         xxcos_edi_delivery_work.specific_class%TYPE,                -- 特定区分
      dealings_class                         xxcos_edi_delivery_work.dealings_class%TYPE,                -- 取引区分
      order_class                            xxcos_edi_delivery_work.order_class%TYPE,                   -- 発注区分
      sum_line_class                         xxcos_edi_delivery_work.sum_line_class%TYPE,                -- 集計明細区分
      shipping_guidance_class                xxcos_edi_delivery_work.shipping_guidance_class%TYPE,       -- 出荷案内以外区分
      shipping_class                         xxcos_edi_delivery_work.shipping_class%TYPE,                -- 出荷区分
      product_code_use_class                 xxcos_edi_delivery_work.product_code_use_class%TYPE,        -- 商品コード使用区分
      cargo_item_class                       xxcos_edi_delivery_work.cargo_item_class%TYPE,              -- 積送品区分
      ta_class                               xxcos_edi_delivery_work.ta_class%TYPE,                      -- Ｔ／Ａ区分
      plan_code                              xxcos_edi_delivery_work.plan_code%TYPE,                     -- 企画コード
      category_code                          xxcos_edi_delivery_work.category_code%TYPE,                 -- カテゴリーコード
      category_class                         xxcos_edi_delivery_work.category_class%TYPE,                -- カテゴリー区分
      carrier_means                          xxcos_edi_delivery_work.carrier_means%TYPE,                 -- 運送手段
      counter_code                           xxcos_edi_delivery_work.counter_code%TYPE,                  -- 売場コード
      move_sign                              xxcos_edi_delivery_work.move_sign%TYPE,                     -- 移動サイン
      eos_handwriting_class                  xxcos_edi_delivery_work.eos_handwriting_class%TYPE,         -- ＥＯＳ・手書区分
      delivery_to_section_code               xxcos_edi_delivery_work.delivery_to_section_code%TYPE,      -- 納品先課コード
      invoice_detailed                       xxcos_edi_delivery_work.invoice_detailed%TYPE,              -- 伝票内訳
      attach_qty                             xxcos_edi_delivery_work.attach_qty%TYPE,                    -- 添付数
      other_party_floor                      xxcos_edi_delivery_work.other_party_floor%TYPE,             -- フロア
      text_no                                xxcos_edi_delivery_work.text_no%TYPE,                       -- ＴＥＸＴＮｏ
      in_store_code                          xxcos_edi_delivery_work.in_store_code%TYPE,                 -- インストアコード
      tag_data                               xxcos_edi_delivery_work.tag_data%TYPE,                      -- タグ
      competition_code                       xxcos_edi_delivery_work.competition_code%TYPE,              -- 競合
      billing_chair                          xxcos_edi_delivery_work.billing_chair%TYPE,                 -- 請求口座
      chain_store_code                       xxcos_edi_delivery_work.chain_store_code%TYPE,              -- チェーンストアーコード
      chain_store_short_name                 xxcos_edi_delivery_work.chain_store_short_name%TYPE,        -- チェーンストアーコード略式名称
      direct_delivery_rcpt_fee               xxcos_edi_delivery_work.direct_delivery_rcpt_fee%TYPE,      -- 直配送／引取料
      bill_info                              xxcos_edi_delivery_work.bill_info%TYPE,                     -- 手形情報
      description                            xxcos_edi_delivery_work.description%TYPE,                   -- 摘要
      interior_code                          xxcos_edi_delivery_work.interior_code%TYPE,                 -- 内部コード
      order_info_delivery_category           xxcos_edi_delivery_work.order_info_delivery_category%TYPE,  -- 発注情報　納品カテゴリー
      purchase_type                          xxcos_edi_delivery_work.purchase_type%TYPE,                 -- 仕入形態
      delivery_to_name_alt                   xxcos_edi_delivery_work.delivery_to_name_alt%TYPE,          -- 納品場所名（カナ）
      shop_opened_site                       xxcos_edi_delivery_work.shop_opened_site%TYPE,              -- 店出場所
      counter_name                           xxcos_edi_delivery_work.counter_name%TYPE,                  -- 売場名
      extension_number                       xxcos_edi_delivery_work.extension_number%TYPE,              -- 内線番号
      charge_name                            xxcos_edi_delivery_work.charge_name%TYPE,                   -- 担当者名
      price_tag                              xxcos_edi_delivery_work.price_tag%TYPE,                     -- 値札
      tax_type                               xxcos_edi_delivery_work.tax_type%TYPE,                      -- 税種
      consumption_tax_class                  xxcos_edi_delivery_work.consumption_tax_class%TYPE,         -- 消費税区分
      brand_class                            xxcos_edi_delivery_work.brand_class%TYPE,                   -- ＢＲ
      id_code                                xxcos_edi_delivery_work.id_code%TYPE,                       -- ＩＤコード
      department_code                        xxcos_edi_delivery_work.department_code%TYPE,               -- 百貨店コード
      department_name                        xxcos_edi_delivery_work.department_name%TYPE,               -- 百貨店名
      item_type_number                       xxcos_edi_delivery_work.item_type_number%TYPE,              -- 品別番号
      description_department                 xxcos_edi_delivery_work.description_department%TYPE,        -- 摘要（百貨店）
      price_tag_method                       xxcos_edi_delivery_work.price_tag_method%TYPE,              -- 値札方法
      reason_column                          xxcos_edi_delivery_work.reason_column%TYPE,                 -- 自由欄
      a_column_header                        xxcos_edi_delivery_work.a_column_header%TYPE,               -- Ａ欄ヘッダ
      d_column_header                        xxcos_edi_delivery_work.d_column_header%TYPE,               -- Ｄ欄ヘッダ
      brand_code                             xxcos_edi_delivery_work.brand_code%TYPE,                    -- ブランドコード
      line_code                              xxcos_edi_delivery_work.line_code%TYPE,                     -- ラインコード
      class_code                             xxcos_edi_delivery_work.class_code%TYPE,                    -- クラスコード
      a1_column                              xxcos_edi_delivery_work.a1_column%TYPE,                     -- Ａ－１欄
      b1_column                              xxcos_edi_delivery_work.b1_column%TYPE,                     -- Ｂ－１欄
      c1_column                              xxcos_edi_delivery_work.c1_column%TYPE,                     -- Ｃ－１欄
      d1_column                              xxcos_edi_delivery_work.d1_column%TYPE,                     -- Ｄ－１欄
      e1_column                              xxcos_edi_delivery_work.e1_column%TYPE,                     -- Ｅ－１欄
      a2_column                              xxcos_edi_delivery_work.a2_column%TYPE,                     -- Ａ－２欄
      b2_column                              xxcos_edi_delivery_work.b2_column%TYPE,                     -- Ｂ－２欄
      c2_column                              xxcos_edi_delivery_work.c2_column%TYPE,                     -- Ｃ－２欄
      d2_column                              xxcos_edi_delivery_work.d2_column%TYPE,                     -- Ｄ－２欄
      e2_column                              xxcos_edi_delivery_work.e2_column%TYPE,                     -- Ｅ－２欄
      a3_column                              xxcos_edi_delivery_work.a3_column%TYPE,                     -- Ａ－３欄
      b3_column                              xxcos_edi_delivery_work.b3_column%TYPE,                     -- Ｂ－３欄
      c3_column                              xxcos_edi_delivery_work.c3_column%TYPE,                     -- Ｃ－３欄
      d3_column                              xxcos_edi_delivery_work.d3_column%TYPE,                     -- Ｄ－３欄
      e3_column                              xxcos_edi_delivery_work.e3_column%TYPE,                     -- Ｅ－３欄
      f1_column                              xxcos_edi_delivery_work.f1_column%TYPE,                     -- Ｆ－１欄
      g1_column                              xxcos_edi_delivery_work.g1_column%TYPE,                     -- Ｇ－１欄
      h1_column                              xxcos_edi_delivery_work.h1_column%TYPE,                     -- Ｈ－１欄
      i1_column                              xxcos_edi_delivery_work.i1_column%TYPE,                     -- Ｉ－１欄
      j1_column                              xxcos_edi_delivery_work.j1_column%TYPE,                     -- Ｊ－１欄
      k1_column                              xxcos_edi_delivery_work.k1_column%TYPE,                     -- Ｋ－１欄
      l1_column                              xxcos_edi_delivery_work.l1_column%TYPE,                     -- Ｌ－１欄
      f2_column                              xxcos_edi_delivery_work.f2_column%TYPE,                     -- Ｆ－２欄
      g2_column                              xxcos_edi_delivery_work.g2_column%TYPE,                     -- Ｇ－２欄
      h2_column                              xxcos_edi_delivery_work.h2_column%TYPE,                     -- Ｈ－２欄
      i2_column                              xxcos_edi_delivery_work.i2_column%TYPE,                     -- Ｉ－２欄
      j2_column                              xxcos_edi_delivery_work.j2_column%TYPE,                     -- Ｊ－２欄
      k2_column                              xxcos_edi_delivery_work.k2_column%TYPE,                     -- Ｋ－２欄
      l2_column                              xxcos_edi_delivery_work.l2_column%TYPE,                     -- Ｌ－２欄
      f3_column                              xxcos_edi_delivery_work.f3_column%TYPE,                     -- Ｆ－３欄
      g3_column                              xxcos_edi_delivery_work.g3_column%TYPE,                     -- Ｇ－３欄
      h3_column                              xxcos_edi_delivery_work.h3_column%TYPE,                     -- Ｈ－３欄
      i3_column                              xxcos_edi_delivery_work.i3_column%TYPE,                     -- Ｉ－３欄
      j3_column                              xxcos_edi_delivery_work.j3_column%TYPE,                     -- Ｊ－３欄
      k3_column                              xxcos_edi_delivery_work.k3_column%TYPE,                     -- Ｋ－３欄
      l3_column                              xxcos_edi_delivery_work.l3_column%TYPE,                     -- Ｌ－３欄
      chain_peculiar_area_header             xxcos_edi_delivery_work.chain_peculiar_area_header%TYPE,    -- チェーン店固有エリア（ヘッダー）
      order_connection_number                xxcos_edi_delivery_work.order_connection_number%TYPE,       -- 受注関連番号
      line_no                                xxcos_edi_delivery_work.line_no%TYPE,                       -- 行Ｎｏ
      stockout_class                         xxcos_edi_delivery_work.stockout_class%TYPE,                -- 欠品区分
      stockout_reason                        xxcos_edi_delivery_work.stockout_reason%TYPE,               -- 欠品理由
      product_code_itouen                    xxcos_edi_delivery_work.product_code_itouen%TYPE,           -- 商品コード（伊藤園）
      product_code1                          xxcos_edi_delivery_work.product_code1%TYPE,                 -- 商品コード１
      product_code2                          xxcos_edi_delivery_work.product_code2%TYPE,                 -- 商品コード２
      jan_code                               xxcos_edi_delivery_work.jan_code%TYPE,                      -- ＪＡＮコード
      itf_code                               xxcos_edi_delivery_work.itf_code%TYPE,                      -- ＩＴＦコード
      extension_itf_code                     xxcos_edi_delivery_work.extension_itf_code%TYPE,            -- 内箱ＩＴＦコード
      case_product_code                      xxcos_edi_delivery_work.case_product_code%TYPE,             -- ケース商品コード
      ball_product_code                      xxcos_edi_delivery_work.ball_product_code%TYPE,             -- ボール商品コード
      product_code_item_type                 xxcos_edi_delivery_work.product_code_item_type%TYPE,        -- 商品コード品種
      prod_class                             xxcos_edi_delivery_work.prod_class%TYPE,                    -- 商品区分
      product_name                           xxcos_edi_delivery_work.product_name%TYPE,                  -- 商品名（漢字）
      product_name1_alt                      xxcos_edi_delivery_work.product_name1_alt%TYPE,             -- 商品名１（カナ）
      product_name2_alt                      xxcos_edi_delivery_work.product_name2_alt%TYPE,             -- 商品名２（カナ）
      item_standard1                         xxcos_edi_delivery_work.item_standard1%TYPE,                -- 規格１
      item_standard2                         xxcos_edi_delivery_work.item_standard2%TYPE,                -- 規格２
      qty_in_case                            xxcos_edi_delivery_work.qty_in_case%TYPE,                   -- 入数
      num_of_cases                           xxcos_edi_delivery_work.num_of_cases%TYPE,                  -- ケース入数
      num_of_ball                            xxcos_edi_delivery_work.num_of_ball%TYPE,                   -- ボール入数
      item_color                             xxcos_edi_delivery_work.item_color%TYPE,                    -- 色
      item_size                              xxcos_edi_delivery_work.item_size%TYPE,                     -- サイズ
      expiration_date                        xxcos_edi_delivery_work.expiration_date%TYPE,               -- 賞味期限日
      product_date                           xxcos_edi_delivery_work.product_date%TYPE,                  -- 製造日
      order_uom_qty                          xxcos_edi_delivery_work.order_uom_qty%TYPE,                 -- 発注単位数
      shipping_uom_qty                       xxcos_edi_delivery_work.shipping_uom_qty%TYPE,              -- 出荷単位数
      packing_uom_qty                        xxcos_edi_delivery_work.packing_uom_qty%TYPE,               -- 梱包単位数
      deal_code                              xxcos_edi_delivery_work.deal_code%TYPE,                     -- 引合
      deal_class                             xxcos_edi_delivery_work.deal_class%TYPE,                    -- 引合区分
      collation_code                         xxcos_edi_delivery_work.collation_code%TYPE,                -- 照合
      uom_code                               xxcos_edi_delivery_work.uom_code%TYPE,                      -- 単位
      unit_price_class                       xxcos_edi_delivery_work.unit_price_class%TYPE,              -- 単価区分
      parent_packing_number                  xxcos_edi_delivery_work.parent_packing_number%TYPE,         -- 親梱包番号
      packing_number                         xxcos_edi_delivery_work.packing_number%TYPE,                -- 梱包番号
      product_group_code                     xxcos_edi_delivery_work.product_group_code%TYPE,            -- 商品群コード
      case_dismantle_flag                    xxcos_edi_delivery_work.case_dismantle_flag%TYPE,           -- ケース解体不可フラグ
      case_class                             xxcos_edi_delivery_work.case_class%TYPE,                    -- ケース区分
      indv_order_qty                         xxcos_edi_delivery_work.indv_order_qty%TYPE,                -- 発注数量（バラ）
      case_order_qty                         xxcos_edi_delivery_work.case_order_qty%TYPE,                -- 発注数量（ケース）
      ball_order_qty                         xxcos_edi_delivery_work.ball_order_qty%TYPE,                -- 発注数量（ボール）
      sum_order_qty                          xxcos_edi_delivery_work.sum_order_qty%TYPE,                 -- 発注数量（合計、バラ）
      indv_shipping_qty                      xxcos_edi_delivery_work.indv_shipping_qty%TYPE,             -- 出荷数量（バラ）
      case_shipping_qty                      xxcos_edi_delivery_work.case_shipping_qty%TYPE,             -- 出荷数量（ケース）
      ball_shipping_qty                      xxcos_edi_delivery_work.ball_shipping_qty%TYPE,             -- 出荷数量（ボール）
      pallet_shipping_qty                    xxcos_edi_delivery_work.pallet_shipping_qty%TYPE,           -- 出荷数量（パレット）
      sum_shipping_qty                       xxcos_edi_delivery_work.sum_shipping_qty%TYPE,              -- 出荷数量（合計、バラ）
      indv_stockout_qty                      xxcos_edi_delivery_work.indv_stockout_qty%TYPE,             -- 欠品数量（バラ）
      case_stockout_qty                      xxcos_edi_delivery_work.case_stockout_qty%TYPE,             -- 欠品数量（ケース）
      ball_stockout_qty                      xxcos_edi_delivery_work.ball_stockout_qty%TYPE,             -- 欠品数量（ボール）
      sum_stockout_qty                       xxcos_edi_delivery_work.sum_stockout_qty%TYPE,              -- 欠品数量（合計、バラ）
      case_qty                               xxcos_edi_delivery_work.case_qty%TYPE,                      -- ケース個口数
      fold_container_indv_qty                xxcos_edi_delivery_work.fold_container_indv_qty%TYPE,       -- オリコン（バラ）個口数
      order_unit_price                       xxcos_edi_delivery_work.order_unit_price%TYPE,              -- 原単価（発注）
      shipping_unit_price                    xxcos_edi_delivery_work.shipping_unit_price%TYPE,           -- 原単価（出荷）
      order_cost_amt                         xxcos_edi_delivery_work.order_cost_amt%TYPE,                -- 原価金額（発注）
      shipping_cost_amt                      xxcos_edi_delivery_work.shipping_cost_amt%TYPE,             -- 原価金額（出荷）
      stockout_cost_amt                      xxcos_edi_delivery_work.stockout_cost_amt%TYPE,             -- 原価金額（欠品）
      selling_price                          xxcos_edi_delivery_work.selling_price%TYPE,                 -- 売単価
      order_price_amt                        xxcos_edi_delivery_work.order_price_amt%TYPE,               -- 売価金額（発注）
      shipping_price_amt                     xxcos_edi_delivery_work.shipping_price_amt%TYPE,            -- 売価金額（出荷）
      stockout_price_amt                     xxcos_edi_delivery_work.stockout_price_amt%TYPE,            -- 売価金額（欠品）
      a_column_department                    xxcos_edi_delivery_work.a_column_department%TYPE,           -- Ａ欄（百貨店）
      d_column_department                    xxcos_edi_delivery_work.d_column_department%TYPE,           -- Ｄ欄（百貨店）
      standard_info_depth                    xxcos_edi_delivery_work.standard_info_depth%TYPE,           -- 規格情報・奥行き
      standard_info_height                   xxcos_edi_delivery_work.standard_info_height%TYPE,          -- 規格情報・高さ
      standard_info_width                    xxcos_edi_delivery_work.standard_info_width%TYPE,           -- 規格情報・幅
      standard_info_weight                   xxcos_edi_delivery_work.standard_info_weight%TYPE,          -- 規格情報・重量
      general_succeeded_item1                xxcos_edi_delivery_work.general_succeeded_item1%TYPE,       -- 汎用引継ぎ項目１
      general_succeeded_item2                xxcos_edi_delivery_work.general_succeeded_item2%TYPE,       -- 汎用引継ぎ項目２
      general_succeeded_item3                xxcos_edi_delivery_work.general_succeeded_item3%TYPE,       -- 汎用引継ぎ項目３
      general_succeeded_item4                xxcos_edi_delivery_work.general_succeeded_item4%TYPE,       -- 汎用引継ぎ項目４
      general_succeeded_item5                xxcos_edi_delivery_work.general_succeeded_item5%TYPE,       -- 汎用引継ぎ項目５
      general_succeeded_item6                xxcos_edi_delivery_work.general_succeeded_item6%TYPE,       -- 汎用引継ぎ項目６
      general_succeeded_item7                xxcos_edi_delivery_work.general_succeeded_item7%TYPE,       -- 汎用引継ぎ項目７
      general_succeeded_item8                xxcos_edi_delivery_work.general_succeeded_item8%TYPE,       -- 汎用引継ぎ項目８
      general_succeeded_item9                xxcos_edi_delivery_work.general_succeeded_item9%TYPE,       -- 汎用引継ぎ項目９
      general_succeeded_item10               xxcos_edi_delivery_work.general_succeeded_item10%TYPE,      -- 汎用引継ぎ項目１０
      general_add_item1                      xxcos_edi_delivery_work.general_add_item1%TYPE,             -- 汎用付加項目１
      general_add_item2                      xxcos_edi_delivery_work.general_add_item2%TYPE,             -- 汎用付加項目２
      general_add_item3                      xxcos_edi_delivery_work.general_add_item3%TYPE,             -- 汎用付加項目３
      general_add_item4                      xxcos_edi_delivery_work.general_add_item4%TYPE,             -- 汎用付加項目４
      general_add_item5                      xxcos_edi_delivery_work.general_add_item5%TYPE,             -- 汎用付加項目５
      general_add_item6                      xxcos_edi_delivery_work.general_add_item6%TYPE,             -- 汎用付加項目６
      general_add_item7                      xxcos_edi_delivery_work.general_add_item7%TYPE,             -- 汎用付加項目７
      general_add_item8                      xxcos_edi_delivery_work.general_add_item8%TYPE,             -- 汎用付加項目８
      general_add_item9                      xxcos_edi_delivery_work.general_add_item9%TYPE,             -- 汎用付加項目９
      general_add_item10                     xxcos_edi_delivery_work.general_add_item10%TYPE,            -- 汎用付加項目１０
      chain_peculiar_area_line               xxcos_edi_delivery_work.chain_peculiar_area_line%TYPE,      -- チェーン店固有エリア（明細）
      invoice_indv_order_qty                 xxcos_edi_delivery_work.invoice_indv_order_qty%TYPE,        -- （伝票計）発注数量（バラ）
      invoice_case_order_qty                 xxcos_edi_delivery_work.invoice_case_order_qty%TYPE,        -- （伝票計）発注数量（ケース）
      invoice_ball_order_qty                 xxcos_edi_delivery_work.invoice_ball_order_qty%TYPE,        -- （伝票計）発注数量（ボール）
      invoice_sum_order_qty                  xxcos_edi_delivery_work.invoice_sum_order_qty%TYPE,         -- （伝票計）発注数量（合計、バラ）
      invoice_indv_shipping_qty              xxcos_edi_delivery_work.invoice_indv_shipping_qty%TYPE,     -- （伝票計）出荷数量（バラ）
      invoice_case_shipping_qty              xxcos_edi_delivery_work.invoice_case_shipping_qty%TYPE,     -- （伝票計）出荷数量（ケース）
      invoice_ball_shipping_qty              xxcos_edi_delivery_work.invoice_ball_shipping_qty%TYPE,     -- （伝票計）出荷数量（ボール）
      invoice_pallet_shipping_qty            xxcos_edi_delivery_work.invoice_pallet_shipping_qty%TYPE,   -- （伝票計）出荷数量（パレット）
      invoice_sum_shipping_qty               xxcos_edi_delivery_work.invoice_sum_shipping_qty%TYPE,      -- （伝票計）出荷数量（合計、バラ）
      invoice_indv_stockout_qty              xxcos_edi_delivery_work.invoice_indv_stockout_qty%TYPE,     -- （伝票計）欠品数量（バラ）
      invoice_case_stockout_qty              xxcos_edi_delivery_work.invoice_case_stockout_qty%TYPE,     -- （伝票計）欠品数量（ケース）
      invoice_ball_stockout_qty              xxcos_edi_delivery_work.invoice_ball_stockout_qty%TYPE,     -- （伝票計）欠品数量（ボール）
      invoice_sum_stockout_qty               xxcos_edi_delivery_work.invoice_sum_stockout_qty%TYPE,      -- （伝票計）欠品数量（合計、バラ）
      invoice_case_qty                       xxcos_edi_delivery_work.invoice_case_qty%TYPE,              -- （伝票計）ケース個口数
      invoice_fold_container_qty             xxcos_edi_delivery_work.invoice_fold_container_qty%TYPE,    -- （伝票計）オリコン（バラ）個口数
      invoice_order_cost_amt                 xxcos_edi_delivery_work.invoice_order_cost_amt%TYPE,        -- （伝票計）原価金額（発注）
      invoice_shipping_cost_amt              xxcos_edi_delivery_work.invoice_shipping_cost_amt%TYPE,     -- （伝票計）原価金額（出荷）
      invoice_stockout_cost_amt              xxcos_edi_delivery_work.invoice_stockout_cost_amt%TYPE,     -- （伝票計）原価金額（欠品）
      invoice_order_price_amt                xxcos_edi_delivery_work.invoice_order_price_amt%TYPE,       -- （伝票計）売価金額（発注）
      invoice_shipping_price_amt             xxcos_edi_delivery_work.invoice_shipping_price_amt%TYPE,    -- （伝票計）売価金額（出荷）
      invoice_stockout_price_amt             xxcos_edi_delivery_work.invoice_stockout_price_amt%TYPE,    -- （伝票計）売価金額（欠品）
      total_indv_order_qty                   xxcos_edi_delivery_work.total_indv_order_qty%TYPE,          -- （総合計）発注数量（バラ）
      total_case_order_qty                   xxcos_edi_delivery_work.total_case_order_qty%TYPE,          -- （総合計）発注数量（ケース）
      total_ball_order_qty                   xxcos_edi_delivery_work.total_ball_order_qty%TYPE,          -- （総合計）発注数量（ボール）
      total_sum_order_qty                    xxcos_edi_delivery_work.total_sum_order_qty%TYPE,           -- （総合計）発注数量（合計、バラ）
      total_indv_shipping_qty                xxcos_edi_delivery_work.total_indv_shipping_qty%TYPE,       -- （総合計）出荷数量（バラ）
      total_case_shipping_qty                xxcos_edi_delivery_work.total_case_shipping_qty%TYPE,       -- （総合計）出荷数量（ケース）
      total_ball_shipping_qty                xxcos_edi_delivery_work.total_ball_shipping_qty%TYPE,       -- （総合計）出荷数量（ボール）
      total_pallet_shipping_qty              xxcos_edi_delivery_work.total_pallet_shipping_qty%TYPE,     -- （総合計）出荷数量（パレット）
      total_sum_shipping_qty                 xxcos_edi_delivery_work.total_sum_shipping_qty%TYPE,        -- （総合計）出荷数量（合計、バラ）
      total_indv_stockout_qty                xxcos_edi_delivery_work.total_indv_stockout_qty%TYPE,       -- （総合計）欠品数量（バラ）
      total_case_stockout_qty                xxcos_edi_delivery_work.total_case_stockout_qty%TYPE,       -- （総合計）欠品数量（ケース）
      total_ball_stockout_qty                xxcos_edi_delivery_work.total_ball_stockout_qty%TYPE,       -- （総合計）欠品数量（ボール）
      total_sum_stockout_qty                 xxcos_edi_delivery_work.total_sum_stockout_qty%TYPE,        -- （総合計）欠品数量（合計、バラ）
      total_case_qty                         xxcos_edi_delivery_work.total_case_qty%TYPE,                -- （総合計）ケース個口数
      total_fold_container_qty               xxcos_edi_delivery_work.total_fold_container_qty%TYPE,      -- （総合計）オリコン（バラ）個口数
      total_order_cost_amt                   xxcos_edi_delivery_work.total_order_cost_amt%TYPE,          -- （総合計）原価金額（発注）
      total_shipping_cost_amt                xxcos_edi_delivery_work.total_shipping_cost_amt%TYPE,       -- （総合計）原価金額（出荷）
      total_stockout_cost_amt                xxcos_edi_delivery_work.total_stockout_cost_amt%TYPE,       -- （総合計）原価金額（欠品）
      total_order_price_amt                  xxcos_edi_delivery_work.total_order_price_amt%TYPE,         -- （総合計）売価金額（発注）
      total_shipping_price_amt               xxcos_edi_delivery_work.total_shipping_price_amt%TYPE,      -- （総合計）売価金額（出荷）
      total_stockout_price_amt               xxcos_edi_delivery_work.total_stockout_price_amt%TYPE,      -- （総合計）売価金額（欠品）
      total_line_qty                         xxcos_edi_delivery_work.total_line_qty%TYPE,                -- トータル行数
...
--
  ...
  TYPE g_edi_delivery_work_ttype             IS TABLE OF edi_delivery_work_cur%ROWTYPE
    INDEX BY PLS_INTEGER;
  TYPE g_edi_work_ttype                      IS TABLE OF g_edi_work_rtype;
-- EDIステータス更新定義
  TYPE g_delivery_work_id_ttype              IS TABLE OF xxcos_edi_delivery_work.delivery_return_work_id%TYPE
    INDEX BY PLS_INTEGER;     -- EDI納品返品情報ワークID
  TYPE g_edi_err_status_ttype                IS TABLE OF xxcos_edi_delivery_work.err_status%TYPE
...
--
  -- EDI納品返品情報ワークテーブル用変数（カーソルレコード型）
  gt_edi_delivery_work                       g_edi_delivery_work_ttype;
--
  -- EDI納品返品情報ワークテーブル用変数
  gt_delivery_return_work_id                 g_delivery_work_id_ttype;
...
--
    ...
    IS
      ...
      AND     lup_values.lookup_code    = cv_creation_class           -- 作成元区分：納品確定
      AND     TRUNC( SYSDATE )
      BETWEEN lup_values.start_date_active
      AND     NVL( lup_values.end_date_active, TRUNC( SYSDATE ) );
--
...
--
    -- *** ローカル変数 ***
    lv_status            xxcos_edi_delivery_work.err_status%TYPE;     -- ステータス
--
    -- *** ローカル・カーソル ***
--
    -- *** ローカル・レコード ***
...
-- 2010/01/29 Ver1.17 K.Hosoi Mod End
    ...
    -- 抽出件数セット
    on_target_cnt := edi_delivery_work_cur%ROWCOUNT;
    -- カーソルクローズ
    CLOSE edi_delivery_work_cur;
--
    -- 対象データが存在しない場合
...
--
  EXCEPTION
    ...
    WHEN lock_expt THEN
      ...
      IF ( edi_delivery_work_cur%ISOPEN ) THEN
        CLOSE edi_delivery_work_cur;
      END IF;
--
--#################################  固定例外処理部 START   ####################################
...
--
    ...
    PROCEDURE customer_conv_check(
      it_edi_work        IN  g_edi_work_rtype,              -- IN：EDI納品返品情報ワークレコード
...
--    END IF;
    ...
    IF (  lv_retcode           = cv_status_normal
      AND gv_error_check_flag  = cv_yes
    ...
    THEN
      ...
      ov_retcode := cv_status_warn;
      RETURN;
-- 2010/05/06 Ver.1.21 K.Oomata Add Start
    --チェック対象でなければ、営業担当員取得の可否に限らず共通関数のリターンコードを正常にする。
    ELSIF ( gv_error_check_flag = cv_no )
    THEN
      lv_retcode := cv_status_normal;
-- 2010/05/06 Ver.1.21 K.Oomata Add End
    END IF;
-- 2010/04/20 Ver.1.20 Y.Goto mod End
-- 2010/01/29 Ver1.17 K.Hosoi Add End
-- **************************** 2009/06/26 N.Maeda MOD Ver1.6  END  ************************************************ --
--
    -- 同一伝票内の全明細のチェック
...
--
      ...
      item_code_check(
        ...
        gt_edi_work(ln_idx),                 -- IN：EDI納品返品情報ワークデータ
...
-- 2011/02/04 Ver.1.24 T.Ishiwata del Start
--      -- 全データに警告ステータスを設定
--      set_check_status_all( cv_edi_status_warning );
-- 2011/02/04 Ver.1.24 T.Ishiwata del End
      ov_retcode := cv_status_warn;
    END IF;
-- 2010/04/20 Ver.1.20 Y.Goto add End
--
...
--
   ...
   * Description      : EDI納品返品情報ワーク変数に設定する(A-5)
   ***********************************************************************************/
  PROCEDURE proc_set_edi_work(
    it_edi_work   IN  edi_delivery_work_cur%ROWTYPE,   -- EDI納品返品情報ワークデータ
    ov_errbuf     OUT NOCOPY VARCHAR2,     --   エラー・メッセージ           --# 固定 #
    ov_retcode    OUT NOCOPY VARCHAR2,     --   リターン・コード             --# 固定 #
    ov_errmsg     OUT NOCOPY VARCHAR2)     --   ユーザー・エラー・メッセージ --# 固定 #
  IS
...
--
    gt_edi_work(ln_idx).delivery_return_work_id        := it_edi_work.delivery_return_work_id;           -- 納品返品情報ワークID
...
-- 2010/03/23 Ver.1.19 M.Sano add Start
    gt_edi_work(ln_idx).request_date                   := it_edi_work.request_date;                      -- 要求日
-- 2010/03/23 Ver.1.19 M.Sano add End
-- 2010/04/20 Ver.1.20 Y.Goto add Start
...
--
  ...
  PROCEDURE proc_set_edi_status(
    in_delivery_return_work_id IN NUMBER,  -- EDI受注ワークID
...
--###########################  固定部 END   ############################
--
    ln_idx := gt_delivery_return_work_id.COUNT + 1;
--
    -- EDI納品返品情報ワークID、ステータスを保持する
    gt_delivery_return_work_id(ln_idx)  := in_delivery_return_work_id;
...
--
    ...
    WHERE   head.data_type_code         = it_edi_work.data_type_code                 -- データ種コード：納品確定
...
--
   ...
   * Description      : EDI納品返品情報ワークテーブルステータス更新(A-20)
   ***********************************************************************************/
...
--###########################  固定部 END   ############################
--
    -- EDI納品返品情報ワークテーブルのステータスを一括更新する
    FORALL ln_idx IN 1..gt_delivery_return_work_id.COUNT
      UPDATE  xxcos_edi_delivery_work                                           -- EDI納品返品情報ワークテーブル
      SET     err_status                = gt_edi_err_status(ln_idx),            -- ステータス
              ...
              program_id                = cn_program_id,                        -- コンカレント・プログラムID
              program_update_date       = cd_program_update_date                -- プログラム更新日
      WHERE   delivery_return_work_id   = gt_delivery_return_work_id(ln_idx);   -- EDI納品返品情報ワークID
--
  EXCEPTION
--
--#################################  固定例外処理部 START   ####################################
...
--
   ...
   * Description      : EDI納品返品情報ワークテーブルデータ削除(A-21)
   ***********************************************************************************/
...
--###########################  固定部 END   ############################
--
    -- EDI納品返品情報ワークテーブルデータ削除（正常データのみ）
    DELETE
    FROM    xxcos_edi_delivery_work                         -- EDI納品返品情報ワークテーブル
    WHERE   if_file_name     = iv_filename                  -- インタフェースファイル名
    AND     data_type_code   IN ( cv_data_type_31,          -- データ種コード：納品確定
                                  cv_data_type_32 )         -- データ種コード：出庫確定
    AND     err_status       = cv_edi_status_normal;        -- ステータス：正常
--
  EXCEPTION
--
--#################################  固定例外処理部 START   ####################################
...
-- 2010/05/10 Ver.1.22 K.Oomata Add End
    ...
    IS
      SELECT  xee.edi_err_id                          -- EDIエラーID
      FROM    xxcos_edi_errors  xee                   -- EDIエラーテーブル
-- 2010/05/10 Ver.1.22 K.Oomata Mod Start
...
-- 2010/05/10 Ver.1.22 K.Oomata Mod End
      FOR UPDATE NOWAIT;
--
-- 2010/01/29 Ver1.17 K.Hosoi Add End
    -- *** ローカル・レコード ***
...
--
        ...
        FOR ln_cnt IN 1..gt_edi_work.COUNT LOOP
          -- ステータスを保持
          proc_set_edi_status(
            gt_edi_work(ln_cnt).delivery_return_work_id,
...
--
    ...
    gn_skip_cnt   := 0;
    gt_edi_work   := g_edi_work_ttype();
    gt_edi_delivery_work.DELETE;
    gt_delivery_return_work_id.DELETE;
