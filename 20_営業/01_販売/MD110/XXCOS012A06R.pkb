CREATE OR REPLACE PACKAGE BODY APPS.XXCOS012A06R
AS
 /*****************************************************************************************
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name               CONSTANT VARCHAR2(100) := 'XXCOS012A06R';          -- パッケージ名
--
  cv_conc_name              CONSTANT VARCHAR2(100) := 'XXCOS012A06R';          -- コンカレント名
  cv_file_id                CONSTANT VARCHAR2(100) := 'XXCOS012A06R';          -- 帳票ＩＤ
  cv_extension_pdf          CONSTANT VARCHAR2(100) := '.pdf';                  -- 拡張子（ＰＤＦ）
  cv_frm_file               CONSTANT VARCHAR2(100) := 'XXCOS012A06S.xml';      -- フォーム様式ファイル名
  cv_vrq_file               CONSTANT VARCHAR2(100) := 'XXCOS012A06S.vrq';      -- クエリー様式ファイル名
...
--
  ...
  IS
    TABLE OF
      xxcos_rep_lot_pick_deli_pro%ROWTYPE
...
--
    ...
    lt_key_delivery_order_edi      xxcoi_lot_reserve_info.delivery_order_edi%TYPE;
    lt_key_shop_code               xxcoi_lot_reserve_info.shop_code%TYPE;
    lt_key_shop_name               xxcoi_lot_reserve_info.shop_name%TYPE;
    lt_key_content                 xxcoi_lot_reserve_info.case_in_qty%TYPE;
    lt_key_slip_num                xxcoi_lot_reserve_info.slip_num%TYPE;
    lt_key_regular_sale_class_line xxcoi_lot_reserve_info.regular_sale_class_line%TYPE;
    ...
    lt_key_mark                    xxcoi_lot_reserve_info.mark%TYPE;
--
    -- *** ローカル・カーソル ***
    CURSOR data_cur
    IS
      SELECT
       ...
       ,xlri.delivery_order_edi         delivery_order_edi            --配送順
...
--  Add Ver1.1 S.Yamashita End
      ...
      ORDER BY
       ...
       ,xlri.delivery_order_edi                                       --配送順
       ,xlri.shop_code                                                --店舗コード
       ,xlri.item_div                                                 --商品区分
...
--
    ...
    BEGIN
      ...
      lt_key_delivery_order_edi      := l_data_rec.delivery_order_edi;
      lt_key_shop_code               := l_data_rec.shop_code ;
      lt_key_shop_name               := l_data_rec.shop_name;
      lt_key_content                 := l_data_rec.case_in_qty;
      lt_key_slip_num                := l_data_rec.slip_num ;
...
--
    ...
    BEGIN
      ...
      BEGIN
        SELECT
          xxcos_rep_l_pick_deli_pro_s01.NEXTVAL          record_id
        INTO
          ln_record_id
        FROM
          dual
...
--  Add Ver1.1 S.Yamashita End
      ...
      g_rpt_data_tab(ln_idx).delivery_order_edi           := lt_key_delivery_order_edi;
      g_rpt_data_tab(ln_idx).shop_code                    := lt_key_shop_code;
      g_rpt_data_tab(ln_idx).shop_name                    := lt_key_shop_name;
      ...
      g_rpt_data_tab(ln_idx).entry_number                 := lt_key_slip_num;
      ...
      g_rpt_data_tab(ln_idx).mark                         := lt_key_mark;
...
--
    ...
    lt_key_delivery_order_edi      := NULL;                 --配送順
    lt_key_shop_code               := NULL;                 --店舗コード
    lt_key_shop_name               := NULL;                 --店舗名
    lt_key_content                 := NULL;                 --入数
    lt_key_slip_num                := NULL;                 --伝票NO
...
--
    ...
    LOOP
      ...
      IF ( (  lt_key_base_code               IS NULL )      --拠点コード
        ...
        AND ( lt_key_delivery_order_edi      IS NULL )      --配送順
        AND ( lt_key_shop_code               IS NULL )      --店舗コード
        AND ( lt_key_shop_name               IS NULL )      --店舗名
        AND ( lt_key_content                 IS NULL )      --入数
        AND ( lt_key_slip_num                IS NULL )      --伝票NO
      ...
      ELSE
        IF ( (  comp_char(lt_key_base_code               , l_data_rec.base_code))               --拠点コード
          ...
          AND ( comp_char(lt_key_delivery_order_edi      , l_data_rec.delivery_order_edi))      --配送順
          AND ( comp_char(lt_key_shop_code               , l_data_rec.shop_code))               --店舗コード
          AND ( comp_char(lt_key_shop_name               , l_data_rec.shop_name))               --店舗名
          AND ( comp_num (lt_key_content                 , l_data_rec.case_in_qty))             --入数
          AND ( comp_char(lt_key_slip_num                , l_data_rec.slip_num))                --伝票NO
          ...
          AND ( comp_char(lt_key_mark                    , l_data_rec.mark)))                   --ロット逆転
        THEN
          --数量集計
          add_quantity;
        ELSE
...
--
    ...
    IS
      SELECT
        xrlpdp.record_id                record_id
      FROM
        xxcos_rep_lot_pick_deli_pro     xrlpdp              --ロット別ピックリスト_出荷先_販売先_製品別帳票ワークテーブル
      WHERE
        xrlpdp.request_id               = cn_request_id     --要求ID
      FOR UPDATE NOWAIT
      ;
--
    -- *** ローカル・レコード ***
...
--
    ...
    BEGIN
      DELETE FROM
        xxcos_rep_lot_pick_deli_pro     xrlpdp
      WHERE
        xrlpdp.request_id               = cn_request_id
      ;
    EXCEPTION
      WHEN OTHERS THEN
        --要求ID文字列取得
