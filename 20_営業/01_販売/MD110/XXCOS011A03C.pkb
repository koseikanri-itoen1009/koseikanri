...
/*  Ver1.31 Add End     */
/*  Ver1.32 Add Start   */
  cv_deli_ord_keep_day_e  CONSTANT fnd_profile_options.profile_option_name%TYPE := 'XXCOS1_DELI_ORD_KEEP_DAY_E'; -- XXCOS:納品用受注保持日数終了
/*  Ver1.32 Add End     */
...
/*  Ver1.31 Add End     */
/*  Ver1.32 Add Start   */
    gn_deli_ord_keep_day_e  NUMBER;                                    -- 納品用受注保持日数終了
/*  Ver1.32 Add End     */
...
--    AND ooha.ordered_date             < cd_process_date + 1                                     -- 業務日付
    AND ooha.ordered_date            < cd_process_date + gn_deli_ord_keep_day_e + 1               -- 業務日付 + プロファイル対象期間終了
/*  Ver1.32 Mod End   */
/*  Ver1.31 Add End   */
    ORDER BY
--****************************** 2009/06/12 1.10 T.Kitajima MOD START ******************************--
...
/*  Ver1.32 Add Start   */
    ...
    IF ( gn_deli_ord_keep_day_e IS NULL ) THEN
      ...
      lv_err_msg := xxccp_common_pkg.get_msg(
                      ...
                      ,iv_name         => cv_msg_prf_err  -- プロファイル取得エラー
                      ,iv_token_name1  => cv_tkn_profile  -- トークンコード１
                      ,iv_token_value1 => cv_deli_ord_keep_day_e   -- プロファイル名
                    );
      -- メッセージに出力
      FND_FILE.PUT_LINE(
         which  => FND_FILE.OUTPUT
