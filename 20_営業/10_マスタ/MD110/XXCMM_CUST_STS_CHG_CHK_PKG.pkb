...
-- Ver1.6 add start
    -- *** ローカル定数 ***
    cv_inv       CONSTANT VARCHAR2(3)    := 'INV';              -- 請求書
-- Ver1.6 add end
    -- *** ローカル変数 ***
    lv_errmsg    VARCHAR2(5000);                                -- ユーザー・エラー・メッセージ
   ...
   BEGIN
     ...
     FROM  ( SELECT /*+ LEADING(hca_ship hcas_ship hcsu_ship) */
             ...
             AND    EXISTS ( SELECT /*+ INDEX(aps AR_PAYMENT_SCHEDULES_N6) */
                             ...
                             AND    aps.status      = cv_status_op  --オープン
                             AND    aps.class       = cv_inv        --請求書
                             AND    ROWNUM          = 1
                    )
             UNION ALL
             ...
             AND    EXISTS ( SELECT /*+ INDEX(aps2 AR_PAYMENT_SCHEDULES_N6) */
                             ...
                             AND    aps2.status      = cv_status_op  --オープン
                             AND    aps2.class       = cv_inv        --請求書
                             AND    ROWNUM           = 1
                    )
           ) cust_bal
     WHERE  ROWNUM = 1
...
-- 2020/03/24 Ver1.7 add start
  ...
  IS
    ...
    -- 固定ローカル定数
    -- ===============================
    cv_prg_name   CONSTANT VARCHAR2(100) := 'bm_expect_payment_chk'; -- プログラム名
--
    -- ===============================
    -- ユーザー宣言部
    -- ===============================
...
--
    ...
    SELECT COUNT(1)  -- 件数
     INTO  ln_count
    FROM   xxcok_backmargin_balance    xbb   -- BM残高情報
