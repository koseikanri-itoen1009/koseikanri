CREATE OR REPLACE PACKAGE BODY XXCFR003A15C
AS
/*****************************************************************************************
 * Copyright(c)Sumisho Computer Systems Corporation, 2008. All rights reserved.
 *
 * Package Name     : XXCFR003A15C(body)
 * Description      : 標準請求書税込
 ...
 *  get_company_info       P 会社別関連情報の取得処理                (A-12)
 ...
 *  2013/12/11    1.10 SCSK 中野 徹也   [E_本稼動_11330] 税別内訳出力対応
 ...
 *  2023/11/20    1.15 SCSK 大山 洋介   [E_本稼動_19496] グループ会社統合対応
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name        CONSTANT VARCHAR2(100) := 'XXCFR003A15C'; -- パッケージ名
  cv_msg_kbn_cmn     CONSTANT VARCHAR2(5)   := 'XXCMN';
  cv_msg_kbn_ccp     CONSTANT VARCHAR2(5)   := 'XXCCP';
  cv_msg_kbn_cfr     CONSTANT VARCHAR2(5)   := 'XXCFR';
--
  -- メッセージ番号
  cv_msg_003a15_001  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90000'; -- 対象件数メッセージ
  cv_msg_003a15_002  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90001'; -- 成功件数メッセージ
  cv_msg_003a15_003  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90002'; -- エラー件数メッセージ
  cv_msg_003a15_004  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90003'; -- スキップ件数メッセージ
  cv_msg_003a15_005  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90004'; -- 正常終了メッセージ
  cv_msg_003a15_006  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90005'; -- 警告終了メッセージ
  cv_msg_003a15_007  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90006'; -- エラー終了全ロールバックメッセージ
  cv_msg_003a15_008  CONSTANT VARCHAR2(20) := 'APP-XXCCP1-90007'; -- エラー終了一部処理メッセージ
  cv_msg_003a15_009  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00056'; -- システムエラーメッセージ
--
  cv_msg_003a15_010  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00004'; -- プロファイル取得エラーメッセージ
  cv_msg_003a15_011  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00003'; -- ロックエラーメッセージ
  cv_msg_003a15_012  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00007'; -- データ削除エラーメッセージ
  cv_msg_003a15_013  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00016'; -- テーブル挿入エラー
  cv_msg_003a15_014  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00023'; -- 帳票０件メッセージ
  cv_msg_003a15_015  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00011'; -- APIエラーメッセージ
  cv_msg_003a15_016  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00024'; -- 帳票０件ログメッセージ
  cv_msg_003a15_017  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00015'; -- 値取得エラーメッセージ
  cv_msg_003a15_018  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00038'; -- 振込口座未登録メッセージ
  cv_msg_003a15_019  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00051'; -- 振込口座未登録情報
  cv_msg_003a15_020  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00052'; -- 振込口座未登録件数メッセージ
  cv_msg_003a15_021  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00010'; -- 共通関数エラーメッセージ
-- Modify 2009.09.10 Ver1.3 Start
  cv_msg_003a15_022  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00079'; -- 請求用顧客コード未定義エラー
  cv_msg_003a15_023  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00080'; -- 顧客関連未設定エラー
  cv_msg_003a15_024  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00081'; -- パラメータ設定エラー
-- Modify 2009.09.10 Ver1.3 End
-- Add 2013.12.11 Ver1.10 Start
  cv_msg_003a15_025  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00017'; -- テーブル更新エラー
-- Add 2013.12.11 Ver1.10 End
-- Ver1.15 ADD START
  cv_msg_003a15_026  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00165'; -- 会社コード指定不可エラーメッセージ
  cv_msg_003a15_027  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00166'; -- パラメータ制約エラーメッセージ
  cv_msg_003a15_028  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00163'; -- トークン値(登録番号)
  cv_msg_003a15_029  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00164'; -- トークン値(請求書SVF情報)
-- Ver1.15 ADD END
--
-- トークン
...
--
  -- 使用DB名
  cv_table           CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INVOICE_INC_TAX';  -- ワークテーブル名
--
  -- 請求書タイプ
...
-- Add 2013.12.11 Ver1.10 End
-- Add 2014.03.27 Ver1.11 Start
                                   ,iv_conc_param7  => iv_bill_invoice_type                        -- コンカレントパラメータ７
-- Add 2014.03.27 Ver1.11 End
-- Ver1.15 ADD START
                                   ,iv_conc_param8  => iv_company_cd                               -- コンカレントパラメータ８
-- Ver1.15 ADD END
...
--
    ...
    IF (gv_comp_spin_off_flag = 'N' AND iv_company_cd IS NOT NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                    ...
                    ,cv_msg_003a15_026   -- 会社コード指定不可エラーメッセージ
                    ,cv_tkn_date         -- トークン
    ...
    IF (gv_comp_spin_off_flag = 'Y') THEN
      -- 分社化対応フラグがYの場合
      IF (iv_custome_cd IS NULL AND iv_invoice_cd IS NULL AND iv_payment_cd IS NULL) THEN
        -- パラメータ「顧客番号」がすべてNULL
      ...
      IF (lv_err_flag = 'Y') THEN
        ...
        lv_errmsg := xxccp_common_pkg.get_msg(
                       cv_msg_kbn_cfr      -- 'XXCFR'
                      ,cv_msg_003a15_027   -- パラメータ制約エラーメッセージ
                     );
        RAISE global_api_expt;
      END IF;
    END IF;
    ...
    ELSE
      -- 分社化対応フラグがYの場合
      --
      IF (iv_custome_cd IS NOT NULL) THEN
        -- パラメータ「顧客番号(顧客)」がNOT NULLの場合
        lv_cond_cust_cd := iv_custome_cd;
      ELSIF (iv_invoice_cd IS NOT NULL) THEN
        -- パラメータ「顧客番号(請求用)」がNOT NULLの場合
        lv_cond_cust_cd := iv_invoice_cd;
      ELSIF (iv_payment_cd IS NOT NULL) THEN
        -- パラメータ「顧客番号(売掛管理先)」がNOT NULLの場合
        lv_cond_cust_cd := iv_payment_cd;
      END IF;
      --
    ...
    IF (gv_t_number IS NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                     cv_msg_kbn_cfr      -- 'XXCFR'
                    ,cv_msg_003a15_017   -- 値取得エラーメッセージ
                    ,cv_tkn_get_data     -- トークン
                    ,cv_msg_003a15_028   -- トークン値(登録番号)
                   );
      RAISE global_api_expt;
    END IF;
    --
    ...
    ELSIF (lv_frm_file IS NULL OR lv_vrq_file IS NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                    ...
                    ,cv_msg_003a15_017   -- 値取得エラーメッセージ
                    ,cv_tkn_get_data     -- トークン
                    ,cv_msg_003a15_029   -- トークン値(請求書SVF情報)
                   );
      RAISE global_api_expt;
    ELSE
      -- 各様式ファイル名をグローバル変数に格納
...
--
    -- *** ローカル変数 ***
    lt_bill_cust_code  xxcfr_rep_st_invoice_inc_tax.bill_cust_code%TYPE;
...
--             SUM( xrsi.slip_sum )     tax_rate_by_sum        --税率別お買上げ金額
             SUM( CASE WHEN xrsi.invoice_tax_div IS NULL THEN
                xrsi.slip_sum - xrsi.slip_tax_sum            --伝票金額 - 伝票税額
...
-- Modify 2023.05.08 Ver1.14 End
      FROM   xxcfr_rep_st_invoice_inc_tax  xrsi
...
-- Add 2023.05.08 Ver1.14 Start
    ...
    IS
      SELECT xrsi.bill_cust_code      bill_cust_code      ,  --顧客コード
             SUM( CASE WHEN xrsi.invoice_tax_div IS NULL THEN
                    xrsi.slip_sum - xrsi.slip_tax_sum        --伝票金額
                  WHEN xrsi.invoice_tax_div = cn_invoice_tax_div THEN
                    xrsi.inv_amount_sum                      --税抜合計１（税込請求金額サマリに消費税率を除した値）
                  ELSE
                    xrsi.inv_amount_sum2                     --税抜合計２（明細毎の税抜額）
             END )                    slip_sum,              --ヘッダー用当月お買上げ額
             SUM( CASE WHEN xrsi.invoice_tax_div IS NULL THEN
                    xrsi.slip_tax_sum                        --伝票税額
                  WHEN xrsi.invoice_tax_div = cn_invoice_tax_div THEN
                    xrsi.tax_amount_sum                      --税額合計１（税込請求金額 - 税込請求金額サマリに消費税率を除した値）
...
--
    -- *** ローカル・タイプ ***
    TYPE l_bill_cust_code_ttype IS TABLE OF xxcfr_rep_st_invoice_inc_tax.bill_cust_code%TYPE  INDEX BY PLS_INTEGER;
-- 2019/07/25 Ver1.12 MOD Start
--    TYPE l_tax_rate_ttype       IS TABLE OF xxcfr_rep_st_invoice_inc_tax.tax_rate1%TYPE       INDEX BY PLS_INTEGER;
    TYPE l_category_ttype       IS TABLE OF xxcfr_rep_st_invoice_inc_tax.category1%TYPE       INDEX BY PLS_INTEGER;
-- 2019/07/25 Ver1.12 MOD End
    TYPE l_inc_tax_charge_ttype IS TABLE OF xxcfr_rep_st_invoice_inc_tax.inc_tax_charge1%TYPE INDEX BY PLS_INTEGER;
-- Add 2023.05.08 Ver1.14 Start
    TYPE l_tax_sum_ttype        IS TABLE OF xxcfr_rep_st_invoice_inc_tax.tax_sum1%TYPE        INDEX BY PLS_INTEGER;
-- Add 2023.05.08 Ver1.14 End
--
...
--
    ...
    BEGIN
      ...
      FORALL i IN l_bill_cust_code_tab.FIRST..l_bill_cust_code_tab.LAST
        UPDATE  xxcfr_rep_st_invoice_inc_tax  xrsi
-- 2019/07/25 Ver1.12 MOD Start
--        SET     xrsi.tax_rate1        = l_tax_rate1_tab(i)          --消費税率１
        SET     xrsi.category1        = l_category1_tab(i)          --内訳分類１
-- 2019/07/25 Ver1.12 MOD End
               ,xrsi.inc_tax_charge1  = l_inc_tax_charge1_tab(i)    --当月お買上げ額１
-- Add 2023.05.08 Ver1.14 Start
...
-- 2019/07/25 Ver1.12 MOD End
               ,xrsi.inc_tax_charge2  = l_inc_tax_charge2_tab(i)    --当月お買上げ額２
-- Add 2023.05.08 Ver1.14 Start
               ,xrsi.tax_sum2         = l_tax_sum2_tab(i)           --消費税額２
-- Add 2023.05.08 Ver1.14 End
-- 2019/07/25 Ver1.12 ADD Start
               ,xrsi.category3        = l_category3_tab(i)          --内訳分類３
               ,xrsi.inc_tax_charge3  = l_inc_tax_charge3_tab(i)    --当月お買上げ額３
-- 2019/07/25 Ver1.12 ADD End
-- Add 2023.05.08 Ver1.14 Start
...
--        OPEN get_all_account_cur FOR cv_get_14account_cur USING iv_payment_cd;
        OPEN get_all_account_cur FOR cv_get_14account_cur USING iv_payment_cd, gv_target_date;
-- Ver1.15 MOD END
      -- 請求書用顧客指定時
      ELSIF (iv_invoice_cd IS NOT NULL) THEN
-- Ver1.15 MOD START
--        OPEN get_all_account_cur FOR cv_get_20account_cur USING iv_invoice_cd;
        OPEN get_all_account_cur FOR cv_get_20account_cur USING iv_invoice_cd, gv_target_date;
-- Ver1.15 MOD END
      -- 顧客指定時
      ELSIF (iv_custome_cd IS NOT NULL) THEN
-- Ver1.15 MOD START
--        OPEN get_all_account_cur FOR cv_get_10account_cur USING iv_custome_cd;
        OPEN get_all_account_cur FOR cv_get_10account_cur USING iv_custome_cd, gv_target_date;
-- Ver1.15 MOD END
      -- パラメータ指定なし時
      ELSE
-- Ver1.15 MOD START
...
--
          INSERT INTO xxcfr_rep_st_invoice_inc_tax(
            report_id               , -- 帳票ＩＤ
            issue_date              , -- 発行日
            zip_code                , -- 郵便番号
            send_address1           , -- 住所１
...
-- Add 2011.01.17 Ver1.8 End
                 ...
                 SUM(xil.tax_amount) + SUM(xil.ship_amount)                         slip_sum         , -- 伝票金額(税込額)
-- Add 2023.05.08 Ver1.14 Start
                 SUM(xil.tax_amount)                                                slip_tax_sum     , -- 税金額
-- Add 2023.05.08 Ver1.14 End
-- Add 2013.12.11 Ver1.10 Start
-- 2019/07/25 Ver1.12 DEL Start
...
-- Modify 2013.12.11 Ver1.10 Start
--                   xil.slip_num;
                   xil.slip_num,
-- Modify 2014.03.27 Ver1.11 Start
--                   xil.tax_rate
...
--                      AND (xb.tax_div = cv_syohizei_kbn_is OR
--                           xb.tax_div = cv_syohizei_kbn_iu)       -- 内税(伝票)　OR　内税(単価)
...
--
    ...
    -- ===============================
    -- *** ローカル定数 ***
    cv_svf_form_name  CONSTANT  VARCHAR2(20) := 'XXCFR003A15S.xml';  -- フォーム様式ファイル名
-- Ver1.13 add start
    cv_svf_form_name2 CONSTANT  VARCHAR2(20) := 'XXCFR003A15S_2.xml';-- フォーム様式ファイル名
-- Ver1.13 add end
    cv_svf_query_name CONSTANT  VARCHAR2(20) := 'XXCFR003A15S.vrq';  -- クエリー様式ファイル名
...
--
    -- 抽出
    CURSOR del_rep_st_inv_inc_cur
    IS
      SELECT xrsi.rowid        ln_rowid
      FROM xxcfr_rep_st_invoice_inc_tax xrsi -- 標準請求書税込帳票ワークテーブル
      WHERE xrsi.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_inc_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_inc_data    g_del_rep_st_inv_inc_ttype;
--
  BEGIN
--
--##################  固定ステータス初期化部 START   ###################
...
--
    -- カーソルオープン
    OPEN del_rep_st_inv_inc_cur;
--
    -- データの一括取得
    FETCH del_rep_st_inv_inc_cur BULK COLLECT INTO lt_del_rep_st_inv_inc_data;
--
    -- 処理件数のセット
    ln_target_cnt := lt_del_rep_st_inv_inc_data.COUNT;
--
    -- カーソルクローズ
    CLOSE del_rep_st_inv_inc_cur;
--
    ...
    IF (ln_target_cnt > 0) THEN
      BEGIN
        ...
        FORALL ln_loop_cnt IN 1..ln_target_cnt
          DELETE FROM xxcfr_rep_st_invoice_inc_tax
          WHERE ROWID = lt_del_rep_st_inv_inc_data(ln_loop_cnt);
--
        -- コミット発行
        COMMIT;
--
