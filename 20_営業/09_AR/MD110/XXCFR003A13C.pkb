CREATE OR REPLACE PACKAGE BODY XXCFR003A13C AS
/*************************************************************************
 ...
 *  put             P         ファイル出力処理
 *  end_proc        P         終了処理
 *  submain         P         汎用商品（店コラム毎集計）請求データ作成処理実行部
 ...
 *  2023-12-21    1.5   SCSK Y.Ryu    E_本稼動_19496対応
 ************************************************************************/
...
--
  ...
  cv_pkg_name        CONSTANT VARCHAR2(100) := 'XXCFR003A13C';  -- パッケージ名
...
--
  ...
  -- メッセージ番号
  cv_msg_cfr_00010  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00010';  -- 共通関数実行エラーメッセージ
  cv_msg_cfr_00015  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00015';  -- 所属部門取得エラーメッセージ
  cv_msg_cfr_00016  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00016';  -- データ挿入エラーエラーメッセージ
  cv_msg_cfr_00024  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00024';  -- 対象データ0件警告
  cv_msg_cfr_00056  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00056';  -- システムエラーメッセージ
-- Modify 2009.10.05 Ver1.1 Start
--  
  cv_msg_cfr_00006  CONSTANT VARCHAR2(30) := 'APP-XXCFR1-00006';  -- 業務処理日付エラーメッセージ	
-- Modify 2009.10.05 Ver1.1 End
  cv_msg_ccp_90000  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90000';  -- 対象件数メッセージ
  cv_msg_ccp_90001  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90001';  -- 成功件数メッセージ
  cv_msg_ccp_90002  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90002';  -- エラー件数メッセージ
  cv_msg_ccp_90004  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90004';  -- 正常終了メッセージ
  cv_msg_ccp_90005  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90005';  -- 警告終了メッセージ
  cv_msg_ccp_90006  CONSTANT VARCHAR2(30) := 'APP-XXCCP1-90006';  -- エラー終了メッセージ
...
-- 2023/05/17 Ver1.4 ADD End
  ...
  -- プロファイルオプション
  cv_prof_name_set_of_bks_id  CONSTANT VARCHAR2(20) := 'GL_SET_OF_BKS_ID';  -- プロファイル会計帳簿取得
  cv_prof_name_org_id         CONSTANT VARCHAR2(10) := 'ORG_ID';            -- プロファイル営業単位取得
-- 2023/12/21 Ver1.5 MOD Start
---- 2023/05/17 Ver1.4 ADD Start
...
--
    ...
    -- コンカレントパラメータログ出力
    xxcfr_common_pkg.put_log_param(iv_which       => cv_log,         -- パラメータ出力関数 ログ出力時のiv_which値
                                   iv_conc_param1 => iv_target_date, -- 締日
-- Modify 2009.10.05 Ver1.1 Start
--                                   iv_conc_param2 => iv_ar_code1,    -- 売掛コード１(請求書)
                                   iv_conc_param2 => iv_cust_code,   -- 顧客コード
                                   iv_conc_param3 => iv_cust_class,  -- 顧客区分
-- Modify 2009.10.05 Ver1.1 End
...
-- 2023/12/21 Ver1.5 MOD End
    ...
    -- 所属部門コード取得
    gv_user_dept_code := xxcfr_common_pkg.get_user_dept(in_user_id  => FND_GLOBAL.USER_ID,  -- ユーザID
                                                        id_get_date => SYSDATE              -- 取得日付
                                                       );
  ...
  EXCEPTION
    ...
    WHEN get_user_dept_expt THEN
      BEGIN
        ...
        WHERE ffcu.descriptive_flexfield_name = cv_person_dff_name  -- 従業員マスタDFF名
          AND ffcu.application_column_name    = cv_peson_dff_att28; -- 従業員マスタDFF28(所属部署)カラム名
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          NULL;
      END;
      lv_errmsg := xxccp_common_pkg.get_msg(iv_application  => cv_xxcfr_app_name, -- 'XXCFR'
                                            iv_name         => cv_msg_cfr_00015,  -- 所属部門取得エラーメッセージ
                                            iv_token_name1  => cv_tkn_get_data,   -- トークン'DATA'
                                            iv_token_value1 => lv_token_value);   -- ユーザセグメント名
...
--    
--    -- ソートキー項目NULL時の値
--    cv_sort_null_value CONSTANT VARCHAR2(1) := '0';  -- NULL時の値
--    
-- Modify 2009.10.05 Ver1.1 End
    -- テーブル名
...
-- Modify 2009.10.05 Ver1.1 Start
    --
    lt_ship_cust_tab     get_ship_cust_ttype;                          -- 出荷先顧客情報
    --カーソルパラメータ変数
...
--
    ...
    --
    -- コレクション変数に代入
    FETCH get_ship_cust_cur BULK COLLECT INTO lt_ship_cust_tab;
    --
    CLOSE get_ship_cust_cur;
    --
    <<ship_cust_loop>>
    FOR i IN 1..lt_ship_cust_tab.COUNT LOOP
-- Modify 2009.10.05 Ver1.1 End
    -- CSV出力ワークテーブルへ挿入
...
--                  xih.credit_receiv_name3                          credit_receiv_name3,      -- 売掛コード３（その他）名称
                  lt_ship_cust_tab(i).vender_code                  vender_code,              -- 取引先コード
                  ...
                  lt_ship_cust_tab(i).credit_cust_code             credit_cust_code,         -- 与信先顧客コード
                  lt_ship_cust_tab(i).credit_cust_name             credit_cust_name,         -- 与信先顧客名
                  lt_ship_cust_tab(i).receipt_cust_code            receipt_cust_code,        -- 入金先顧客コード
                  lt_ship_cust_tab(i).receipt_cust_name            receipt_cust_name,        -- 入金先顧客名
                  lt_ship_cust_tab(i).payment_cust_code            payment_cust_code,        -- 親請求先顧客コード
                  lt_ship_cust_tab(i).payment_cust_name            payment_cust_name,        -- 親請求先顧客名
                  lt_ship_cust_tab(i).bill_cust_code               bill_cust_code,           -- 請求先顧客コード
                  lt_ship_cust_tab(i).bill_cust_name               bill_cust_name,           -- 請求先顧客名
                  lt_ship_cust_tab(i).credit_receiv_code2          credit_receiv_code2,      -- 売掛コード２（事業所）
                  NULL                                             credit_receiv_name2,      -- 売掛コード２（事業所）
                  lt_ship_cust_tab(i).credit_receiv_code3          credit_receiv_code3,      -- 売掛コード３（その他）
...
-- Modify 2009.10.05 Ver1.1 End
                  ...
                  xil.ship_cust_code                               ship_cust_code,           -- 出荷先顧客コード
                  xil.ship_cust_name                               ship_cust_name,           -- 出荷先顧客名
-- Modify 2009.10.05 Ver1.1 Start
...
--             AND EXISTS (SELECT 'X'
--                         FROM xxcfr_bill_customers_v xbcv                                    -- 請求先顧客ビュー
--                         WHERE xih.bill_cust_code = xbcv.bill_customer_code                  -- 請求先顧客コード
--                           AND ((cv_enable_all = gv_enable_all AND                           -- 全社出力権限あり
--                                 xbcv.bill_base_code = xbcv.bill_base_code)                  -- 請求拠点コード
--                                OR
--                                (cv_disable_all = gv_enable_all AND                          -- 全社出力権限なし
--                                 xbcv.bill_base_code = gv_user_dept_code))                   -- 請求拠点コード
--                           AND xbcv.receiv_code1 = NVL(iv_ar_code1, xbcv.receiv_code1)       -- 売掛コード１（請求書）
--                           AND xbcv.inv_prt_type = cv_inv_prt_type                           -- 請求書出力形式
--                           AND xbcv.cons_inv_flag = cv_cons_inv_flag                         -- 一括請求書発行フラグ
--                           AND xbcv.org_id = gn_org_id                                       -- 組織ID
--                        )
--             AND xih.cutoff_date = iv_target_date                                            -- 締日
           WHERE xil.ship_cust_code = lt_ship_cust_tab(i).ship_cust_code
...
-- Add 2019-07-26 Ver1.3 End   ----------------------------------------------
           ...
           GROUP BY xih.itoen_name,                                                          -- 取引先名
                    ...
                    TO_CHAR(xih.object_date_from,'YYYY/MM/DD'),                              -- 対象期間(自)
                    TO_CHAR(xih.object_date_to,'YYYY/MM/DD'),                                -- 対象期間(至)
-- Modify 2009.10.05 Ver1.1 Start
...
-- Modify 2009.10.05 Ver1.1 End
                    xil.ship_shop_code,                                                      -- 出荷先顧客店NO
                    xil.ship_shop_name,                                                      -- 出荷先顧客店名
                    xil.vd_num,                                                              -- 自動販売機番号
                    xil.column_num,                                                          -- コラム
                    xil.item_code,                                                           -- 商品コード
                    xil.jan_code,                                                            -- JANコード
                    xil.item_name,                                                           -- 商品名
                    xil.vessel_type_name,                                                    -- 容器区分名
                    xil.unit_price,                                                          -- 卸単価
                    xil.unit_price,                                                          -- 売価
...
--                    xih.bill_cust_code,                                                      -- 請求先顧客コード
--                    NVL(xil.ship_shop_code,cv_sort_null_value),                              -- 出荷先顧客店NO
--                    xil.ship_cust_code,                                                      -- 出荷先顧客コード
--                    xil.column_num,                                                          -- コラム
--                    xil.policy_group,                                                        -- 政策群コード
--                    xil.item_code                                                            -- 商品コード
-- Modify 2009.10.05 Ver1.1 End
...
-- Modify 2009.10.05 Ver1.1 End
  ...
  EXCEPTION
    WHEN OTHERS THEN
      lv_errmsg := xxccp_common_pkg.get_msg(iv_application => cv_xxcfr_app_name,  -- 'XXCFR'
                                            iv_name => cv_msg_cfr_00016,          -- データ挿入エラーメッセージ
                                            iv_token_name1 => cv_tkn_tab_name,    -- トークン'TABLE'
                                            iv_token_value1 => xxcfr_common_pkg.get_table_comment(cv_table_name)
                                           );
      ov_errbuf := cv_pkg_name||cv_msg_cont||cv_prg_name||cv_msg_part||SQLERRM;
      ov_retcode := cv_status_error;
      ov_errmsg := lv_errmsg;
...
--##############################  固定部 END   ##################################
    ...
    --===============================================================
    -- 参照タイプ
    cv_lookup_type_func_name CONSTANT VARCHAR2(30) := 'XXCFR1_ERR_MSG_TOKEN';  -- エラーメッセージ出力用参照タイプ名
    -- 参照コード
    cv_lookup_code_func_name CONSTANT VARCHAR2(30) := 'CFR000A00006';          -- エラーメッセージ出力用参照タイプコード
    --===============================================================
    -- ローカル変数
    --===============================================================
...
--
    -- OUTファイル出力処理実行
    xxcfr_common_pkg.csv_out(in_request_id => FND_GLOBAL.CONC_REQUEST_ID,  -- 要求ID
                             iv_lookup_type => cv_lookup_type_out,         -- 参照タイプ
                             in_rec_cnt => gn_rec_count,                   -- レコード件数
  ...
  EXCEPTION
    ...
    WHEN global_api_others_expt THEN
      BEGIN
        SELECT flvv.description description                                     -- 汎用請求出力処理共通関数名
        INTO lv_func_name
        FROM fnd_lookup_values_vl flvv                                          -- 汎用請求出力テーブル
        WHERE flvv.lookup_type = cv_lookup_type_func_name                       -- エラーメッセージ出力用参照タイプ名
          AND flvv.lookup_code = cv_lookup_code_func_name                       -- エラーメッセージ出力用参照タイプコード
          AND flvv.enabled_flag = cv_yes                                        -- 全社出力権限あり
          AND SYSDATE BETWEEN flvv.start_date_active AND flvv.end_date_active;  -- 有効期間
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          NULL;
      END;
      lv_errmsg := xxccp_common_pkg.get_msg(iv_application => cv_xxcfr_app_name,  -- 'XXCFR'
                                            iv_name => cv_msg_cfr_00010,          -- 共通関数実行エラーメッセージ
                                            iv_token_name1 => cv_func_name,       -- トークン'FUNC_NAME'
                                            iv_token_value1 => lv_func_name);     -- 汎用請求出力処理共通関数名
      ov_errbuf := cv_pkg_name||cv_msg_cont||cv_prg_name||cv_msg_part||SQLERRM;
      ov_retcode := cv_status_error;
      ov_errmsg := lv_errmsg;
    WHEN OTHERS THEN
...
--
    ...
    IF (iv_retcode = cv_status_warn) THEN
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxcfr_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_cfr_00024          -- 対象データ0件警告
                                                )
                       );
      FND_FILE.PUT_LINE(FND_FILE.LOG,'');
    END IF;
    ...
    IF ((iv_retcode = cv_status_normal) OR (iv_retcode = cv_status_warn)) THEN
      -- 対象件数出力
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90000,         -- 対象件数メッセージ
                                                 iv_token_name1 => cv_tkn_count,      -- トークン'COUNT'
                                                 iv_token_value1 => gn_rec_count      -- 請求書情報取得件数
                                                )
                       );
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90001,         -- 成功件数メッセージ
                                                 iv_token_name1 => cv_tkn_count,      -- トークン'COUNT'
                                                 iv_token_value1 => gn_rec_count      -- 請求書情報成功件数
                                                )
                       );
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90002,         -- エラー件数メッセージ
                                                 iv_token_name1 => cv_tkn_count,      -- トークン'COUNT'
                                                 iv_token_value1 => 0                 -- 請求書情報エラー件数
                                                )
                       );
    -- エラー終了の場合
    ELSIF (iv_retcode = cv_status_error) THEN
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90000,         -- 対象件数件数メッセージ
                                                 iv_token_name1 => cv_tkn_count,      -- トークン'COUNT'
                                                 iv_token_value1 => 0                 -- 請求書情報取得件数
                                                )
                       );
    ...
    IF (iv_retcode = cv_status_error) THEN
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90006          -- エラー終了メッセージ
                                                )
                       );
    -- 対象データ0件の場合(警告終了)
    ELSIF (iv_retcode = cv_status_warn) THEN
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90005          -- 警告終了メッセージ
                                                )
                       );
    -- 正常終了の場合
    ELSE
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG,
                        xxccp_common_pkg.get_msg(iv_application => cv_xxccp_app_name, -- 'XXCFR'
                                                 iv_name => cv_msg_ccp_90004          -- 正常終了メッセージ
                                                )
                       );
    END IF;
   ...
   * Description      : 汎用商品（店コラム毎集計）請求データ作成処理実行部
   ***********************************************************************************/
  PROCEDURE submain(
    iv_target_date   IN  VARCHAR2,    -- 締日
-- Modify 2009.10.05 Ver1.1 Start
...
--
  ...
  BEGIN
    ...
    xxccp_common_pkg.put_log_header(iv_which => cv_put_log_which,  -- ログヘッダ出力関数iv_whichパラメータ
                                    ov_retcode => lv_retcode,      -- リターン・コード
    ...
    IF (lv_retcode = cv_status_error) THEN
      ...
      FND_FILE.PUT_LINE(
        ...
        ,buff   => xxccp_common_pkg.get_msg(
                      iv_application  => cv_xxcfr_app_name  -- 'XXCFR'
                     ,iv_name         => cv_msg_cfr_00056   -- エラーメッセージ
                   )
      );
      FND_FILE.PUT_LINE(
         which  => FND_FILE.LOG
