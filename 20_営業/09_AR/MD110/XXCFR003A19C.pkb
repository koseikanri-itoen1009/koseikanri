 ...
 *  2013/11/25    1.60 SCSK 桐生 和幸   障害票「E_本稼動_11330」対応
 ...
 *  2023/07/04    1.95 SCSK 奥山 徹     障害票「E_本稼動_19082」インボイス対応
 *  2023/11/20    1.96 SCSK 大山 洋介   障害票「E_本稼動_19496」グループ会社統合対応
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
...
-- Add 2013.11.25 Ver1.60 Start
  cv_msg_003a19_026  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00017'; -- テーブル更新エラー
-- Add 2013.11.25 Ver1.60 End
-- Add 2015.07.31 Ver1.80 Start
  cv_msg_003a19_027  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00153'; -- 請求書タイプ定義なしエラーメッセージ
  cv_msg_003a19_028  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00154'; -- コンカレント起動エラー
  cv_msg_003a19_029  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00155'; -- コンカレント待機時間経過エラー
  cv_msg_003a19_030  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00156'; -- コンカレント待機正常メッセージ
  cv_msg_003a19_031  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00157'; -- コンカレント待機警告メッセージ
  cv_msg_003a19_032  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00158'; -- コンカレント待機エラーメッセージ
-- Add 2015.07.31 Ver1.80 End
-- Ver1.96 ADD START
  cv_msg_003a19_033  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00165'; -- 会社コード指定不可エラーメッセージ
  cv_msg_003a19_034  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00166'; -- パラメータ制約エラーメッセージ
  cv_msg_003a19_035  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00163'; -- トークン値(登録番号)
  cv_msg_003a19_036  CONSTANT VARCHAR2(20) := 'APP-XXCFR1-00164'; -- トークン値(請求書SVF情報)
-- Ver1.96 ADD END
--
-- トークン
...
-- Add 2015.07.31 Ver1.80 Start
  cv_table_a_h       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_A_H';    -- 標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダ名
  cv_table_a_l       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_A_L';    -- 標準請求書税抜帳票内訳印刷単位Aワークテーブル明細名
  cv_table_b_h       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_B_H';    -- 標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダ名
  cv_table_b_l       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_B_L';    -- 標準請求書税抜帳票内訳印刷単位Bワークテーブル明細名
-- Add 2015.07.31 Ver1.80 End
-- Add 2016.04.04 Ver1.90 Start
  cv_table_c_h       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_C_H';    -- 標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダ名
  cv_table_c_l       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_C_L';    -- 標準請求書税抜帳票内訳印刷単位Cワークテーブル明細名
...
-- Ver.1.92 [障害E_本稼動_15307] ADD START
  ...
  cv_table_d_l       CONSTANT VARCHAR2(100) := 'XXCFR_REP_ST_INV_EX_TAX_D_L';    -- 標準請求書税込帳票内訳印刷単位Dワークテーブル明細名
...
--  cv_report_id_05    CONSTANT VARCHAR2(14)  := 'XXCFR003A2005C';  -- 請求書（14顧客合計_店舗別一覧）税抜
--  cv_report_id_06    CONSTANT VARCHAR2(14)  := 'XXCFR003A2006C';  -- 店舗別明細（店舗別改ページ）税抜
--  cv_report_id_07    CONSTANT VARCHAR2(14)  := 'XXCFR003A2007C';  -- 請求総括表（14顧客合計）税抜
--  cv_report_id_08    CONSTANT VARCHAR2(14)  := 'XXCFR003A2008C';  -- 請求書（店舗別改ページ）税抜
---- Add 2016.04.04 Ver1.90 Start
--  cv_report_id_11    CONSTANT VARCHAR2(14)  := 'XXCFR003A2011C';  -- 請求書（14顧客合計_店舗別一覧）税抜
--  cv_report_id_12    CONSTANT VARCHAR2(14)  := 'XXCFR003A2012C';  -- 伝票別明細税抜
---- Add 2016.04.04 Ver1.90 End
  cv_report_id_05    CONSTANT VARCHAR2(14)  := 'XXCFR003A2005C';  -- 請求書A税抜ヘッダ
  cv_report_id_06    CONSTANT VARCHAR2(14)  := 'XXCFR003A2006C';  -- 請求書A税抜明細
  cv_report_id_07    CONSTANT VARCHAR2(14)  := 'XXCFR003A2007C';  -- 請求書B税抜ヘッダ
  cv_report_id_08    CONSTANT VARCHAR2(14)  := 'XXCFR003A2008C';  -- 請求書B税抜明細
  cv_report_id_11    CONSTANT VARCHAR2(14)  := 'XXCFR003A2011C';  -- 請求書C税抜ヘッダ
  cv_report_id_12    CONSTANT VARCHAR2(14)  := 'XXCFR003A2012C';  -- 請求書C税抜明細
-- Ver.1.92 [障害E_本稼動_15307] MOD END
-- Ver.1.92 [障害E_本稼動_15307] ADD START
  cv_report_id_13    CONSTANT VARCHAR2(14)  := 'XXCFR003A2013C';  -- 請求書Dヘッダ
  cv_report_id_15    CONSTANT VARCHAR2(14)  := 'XXCFR003A2015C';  -- 請求書D税抜明細
-- Ver.1.92 [障害E_本稼動_15307] ADD END
--
  -- 店舗別明細の請求書タイプ
-- Ver.1.92 [障害E_本稼動_15307] MOD START
--  cv_bill_type_05    CONSTANT VARCHAR2(2)   := '05';              -- 請求書（14顧客合計_店舗別一覧）税抜
--  cv_bill_type_06    CONSTANT VARCHAR2(2)   := '06';              -- 店舗別明細（店舗別改ページ）税抜
--  cv_bill_type_07    CONSTANT VARCHAR2(2)   := '07';              -- 請求総括表（14顧客合計）税抜
--  cv_bill_type_08    CONSTANT VARCHAR2(2)   := '08';              -- 請求書（店舗別改ページ）税抜
---- Add 2016.04.04 Ver1.90 Start
--  cv_bill_type_11    CONSTANT VARCHAR2(2)   := '11';              -- 請求書（14顧客合計_店舗別一覧）税抜
--  cv_bill_type_12    CONSTANT VARCHAR2(2)   := '12';              -- 伝票別明細税抜
---- Add 2016.04.04 Ver1.90 End
  cv_bill_type_05    CONSTANT VARCHAR2(2)   := '05';              -- 請求書A税抜ヘッダ
  cv_bill_type_06    CONSTANT VARCHAR2(2)   := '06';              -- 請求書A税抜明細
  cv_bill_type_07    CONSTANT VARCHAR2(2)   := '07';              -- 請求書B税抜ヘッダ
  cv_bill_type_08    CONSTANT VARCHAR2(2)   := '08';              -- 請求書B税抜明細
  cv_bill_type_11    CONSTANT VARCHAR2(2)   := '11';              -- 請求書C税抜ヘッダ
  cv_bill_type_12    CONSTANT VARCHAR2(2)   := '12';              -- 請求書C税抜明細
-- Ver.1.92 [障害E_本稼動_15307] MOD END
-- Ver.1.92 [障害E_本稼動_15307] ADD START
...
--
    ...
    IF (gv_comp_spin_off_flag = 'N' AND iv_company_cd IS NOT NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                    ...
                    ,cv_msg_003a19_033   -- 会社コード指定不可エラーメッセージ
                    ,cv_tkn_date         -- トークン
    ...
    IF (gv_comp_spin_off_flag = 'Y') THEN
      ...
      IF (lv_err_flag = 'Y') THEN
        ...
        lv_errmsg := xxccp_common_pkg.get_msg(
                       cv_msg_kbn_cfr      -- 'XXCFR'
                      ,cv_msg_003a19_034   -- パラメータ制約エラーメッセージ
                     );
        RAISE global_api_expt;
      END IF;
    END IF;
    ...
    IF (gv_t_number IS NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                     cv_msg_kbn_cfr      -- 'XXCFR'
                    ,cv_msg_003a19_017   -- 値取得エラーメッセージ
                    ,cv_tkn_get_data     -- トークン
                    ,cv_msg_003a19_035   -- トークン値(登録番号)
                   );
      RAISE global_api_expt;
    END IF;
    --
    ...
    ELSIF (lv_frm_file IS NULL OR lv_vrq_file IS NULL) THEN
      ...
      lv_errmsg := xxccp_common_pkg.get_msg(
                    ...
                    ,cv_msg_003a19_017   -- 値取得エラーメッセージ
                    ,cv_tkn_get_data     -- トークン
                    ,cv_msg_003a19_036   -- トークン値(請求書SVF情報)
                   );
      RAISE global_api_expt;
    ELSE
      -- 各様式ファイル名をグローバル変数に格納
...
--
    ...
    lt_location_code   xxcfr_rep_st_invoice_ex_tax_d.location_code%TYPE;
-- Add 2015.07.31 Ver1.80 Start
    lt_bill_cust_code2 xxcfr_rep_st_inv_ex_tax_a_l.bill_cust_code%TYPE;
    lt_location_code2  xxcfr_rep_st_inv_ex_tax_a_l.location_code%TYPE;
    lt_bill_cust_code3 xxcfr_rep_st_inv_ex_tax_a_l.bill_cust_code%TYPE;
    lt_location_code3  xxcfr_rep_st_inv_ex_tax_a_l.location_code%TYPE;
    lt_ship_cust_code3 xxcfr_rep_st_inv_ex_tax_a_l.ship_cust_code%TYPE;
    lt_bill_cust_code5 xxcfr_rep_st_inv_ex_tax_b_l.bill_cust_code%TYPE;
    lt_location_code5  xxcfr_rep_st_inv_ex_tax_b_l.location_code%TYPE;
    lt_bill_cust_code6 xxcfr_rep_st_inv_ex_tax_b_l.bill_cust_code%TYPE;
    lt_location_code6  xxcfr_rep_st_inv_ex_tax_b_l.location_code%TYPE;
    lt_ship_cust_code6 xxcfr_rep_st_inv_ex_tax_b_l.ship_cust_code%TYPE;
-- Add 2015.07.31 Ver1.80 End
-- Add 2016.04.04 Ver1.90 Start
    lt_bill_cust_code8 xxcfr_rep_st_inv_ex_tax_c_l.bill_cust_code%TYPE;
    lt_location_code8  xxcfr_rep_st_inv_ex_tax_c_l.location_code%TYPE;
-- Add 2016.04.04 Ver1.90 End
-- Ver.1.92 [障害E_本稼動_15307] ADD START
    lt_bill_cust_code9 xxcfr_rep_st_inv_ex_tax_d_l.bill_cust_code%TYPE;
    lt_location_code9  xxcfr_rep_st_inv_ex_tax_d_l.location_code%TYPE;
    lt_ship_cust_code9 xxcfr_rep_st_inv_ex_tax_d_l.ship_cust_code%TYPE;
-- Ver.1.92 [障害E_本稼動_15307] ADD END
...
--             SUM( xrsi.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
...
-- 2019/08/09 Ver1.93 MOD End
             SUM( xrsial.slip_sum )     tax_rate_by_sum     ,  -- 税別お買上げ額
-- Modify 2023.07.04 Ver1.95 Start
--             SUM( xrsialal.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
             ...
             WHEN xrsial.invoice_tax_div = cn_invoice_tax_div THEN
                xrsial.tax_amount_sum                          -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
             ELSE
...
-- Add 2023.07.04 Ver1.95 Start
    ...
    IS
      SELECT xrsial.bill_cust_code      bill_cust_code      ,  -- 顧客コード
             xrsial.location_code       location_code       ,  -- 担当拠点コード
             SUM( xrsial.slip_sum )     slip_sum            ,  -- ヘッダー用当月お買上げ額
             SUM( CASE WHEN xrsial.invoice_tax_div IS NULL THEN
                    xrsial.slip_tax_sum                        -- 伝票税額
                  WHEN xrsial.invoice_tax_div = cn_invoice_tax_div THEN
                    xrsial.tax_amount_sum                      -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
...
-- 2019/08/09 Ver1.93 MOD End
             SUM( xrsial.slip_sum )     tax_rate_by_sum     ,  -- 税別お買上げ額
             SUM( xrsial.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
      FROM   xxcfr_rep_st_inv_ex_tax_a_l  xrsial
      WHERE  xrsial.request_id  = cn_request_id
-- 2019/08/09 Ver1.93 MOD Start
--      AND    xrsial.tax_rate   <> cn_no_tax                    -- 非課税（税率0%)以外
...
--
    ...
    IS
      SELECT xrsial.bill_cust_code      bill_cust_code      ,  -- 顧客コード
             xrsial.location_code       location_code       ,  -- 担当拠点コード
             xrsial.ship_cust_code      ship_cust_code      ,  -- 納品先顧客コード
             SUM( xrsial.slip_sum )     store_sum           ,  -- 税別お買上げ額
             SUM( xrsial.slip_tax_sum ) store_tax_sum          -- 税別消費税額
      FROM   xxcfr_rep_st_inv_ex_tax_a_l  xrsial
      WHERE  xrsial.request_id  = cn_request_id
      GROUP BY
             xrsial.bill_cust_code, -- 顧客コード
...
-- 2019/08/09 Ver1.93 MOD End
             SUM( xrsibl.slip_sum )     tax_rate_by_sum     ,  -- 税別お買上げ額
-- Modify 2023.07.04 Ver1.95 Start
--             SUM( xrsibl.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
             ...
             WHEN xrsibl.invoice_tax_div = cn_invoice_tax_div THEN
                xrsibl.tax_amount_sum                          -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
             ELSE
...
-- Add 2023.07.04 Ver1.95 Start
    ...
    IS
      SELECT xrsibl.bill_cust_code      bill_cust_code      ,  -- 顧客コード
             xrsibl.location_code       location_code       ,  -- 担当拠点コード
             SUM( xrsibl.slip_sum )     slip_sum            ,  -- ヘッダー用当月お買上げ額
             SUM( CASE WHEN xrsibl.invoice_tax_div IS NULL THEN
                    xrsibl.slip_tax_sum                        -- 伝票税額
                  WHEN xrsibl.invoice_tax_div = cn_invoice_tax_div THEN
                    xrsibl.tax_amount_sum                      -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
...
--
    ...
    IS
      SELECT xrsibl.bill_cust_code      bill_cust_code      ,  -- 顧客コード
             xrsibl.location_code       location_code       ,  -- 担当拠点コード
             xrsibl.ship_cust_code      ship_cust_code      ,  -- 納品先顧客コード
             SUM( xrsibl.slip_sum )     store_sum           ,  -- 当月ご請求額（税込）
             SUM( xrsibl.slip_tax_sum ) store_tax_sum          -- 消費税等
      FROM   xxcfr_rep_st_inv_ex_tax_b_l  xrsibl
      WHERE  xrsibl.request_id  = cn_request_id
      GROUP BY
             xrsibl.bill_cust_code, -- 顧客コード
...
-- 2019/08/09 Ver1.93 MOD End
             SUM( xrsicl.slip_sum )     tax_rate_by_sum     ,  -- 税別お買上げ額
-- Modify 2023.07.04 Ver1.95 Start
--             SUM( xrsicl.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
             ...
             WHEN xrsicl.invoice_tax_div = cn_invoice_tax_div THEN
                xrsicl.tax_amount_sum                          -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
             ELSE
...
-- Add 2023.07.04 Ver1.95 Start
    ...
    IS
      SELECT xrsicl.bill_cust_code      bill_cust_code      ,  -- 顧客コード
             xrsicl.location_code       location_code       ,  -- 担当拠点コード
             SUM( xrsicl.slip_sum )     slip_sum            ,  -- ヘッダー用当月お買上げ額
             SUM( CASE WHEN xrsicl.invoice_tax_div IS NULL THEN
                    xrsicl.slip_tax_sum                        -- 伝票税額
                  WHEN xrsicl.invoice_tax_div = cn_invoice_tax_div THEN
                    xrsicl.tax_amount_sum                      -- 税額合計１（税抜請求金額サマリに消費税率を乗じた値）
...
--             SUM( xrsidl.slip_sum )     tax_rate_by_sum     ,  -- 税別お買上げ額
--             SUM( xrsidl.slip_tax_sum ) tax_rate_by_tax_sum    -- 税別消費税額
...
--             SUM( xrsidl.slip_sum )     store_sum           ,  -- 当月ご請求額（税込）
--             SUM( xrsidl.slip_tax_sum ) store_tax_sum          -- 消費税等
-- 2023/07/04 Ver1.95 MOD End
      FROM   xxcfr_rep_st_inv_ex_tax_d_l  xrsidl
      WHERE  xrsidl.request_id  = cn_request_id
      GROUP BY
...
--
    TYPE l_bill_cust_code_4_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_a_l.bill_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_location_code_4_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_a_l.location_code%TYPE  INDEX BY PLS_INTEGER;
    TYPE l_ship_cust_code_4_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_a_l.ship_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_store_sum_4_ttype      IS TABLE OF xxcfr_rep_st_inv_ex_tax_a_l.store_sum%TYPE      INDEX BY PLS_INTEGER;
    TYPE l_store_tax_sum_4_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_a_l.store_tax_sum%TYPE  INDEX BY PLS_INTEGER;
--
...
--
    TYPE l_bill_cust_code_7_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_b_l.bill_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_location_code_7_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_b_l.location_code%TYPE  INDEX BY PLS_INTEGER;
    TYPE l_ship_cust_code_7_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_b_l.ship_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_store_sum_7_ttype      IS TABLE OF xxcfr_rep_st_inv_ex_tax_b_l.store_sum%TYPE      INDEX BY PLS_INTEGER;
    TYPE l_store_tax_sum_7_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_b_l.store_tax_sum%TYPE  INDEX BY PLS_INTEGER;
-- Add 2015.07.31 Ver1.80 End
-- Add 2016.04.04 Ver1.90 Start
...
--
    TYPE l_bill_cust_code_10_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_d_l.bill_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_location_code_10_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_d_l.location_code%TYPE  INDEX BY PLS_INTEGER;
    TYPE l_ship_cust_code_10_ttype IS TABLE OF xxcfr_rep_st_inv_ex_tax_d_l.ship_cust_code%TYPE INDEX BY PLS_INTEGER;
    TYPE l_store_sum_10_ttype      IS TABLE OF xxcfr_rep_st_inv_ex_tax_d_l.store_sum%TYPE      INDEX BY PLS_INTEGER;
    TYPE l_store_tax_sum_10_ttype  IS TABLE OF xxcfr_rep_st_inv_ex_tax_d_l.store_tax_sum%TYPE  INDEX BY PLS_INTEGER;
-- Ver.1.92 [障害E_本稼動_15307] ADD END
-- 2023/07/04 Ver1.95 ADD Start
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_2_tab   l_ex_tax_charge_2_ttype;   -- 当月お買上げ額１
    l_tax_sum1_2_tab         l_tax_sum_2_ttype;         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--    l_tax_rate2_2_tab        l_tax_rate_2_ttype;        -- 消費税率２
    l_category2_2_tab        l_category_2_ttype;        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_2_tab   l_ex_tax_charge_2_ttype;   -- 当月お買上げ額２
    l_tax_sum2_2_tab         l_tax_sum_2_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_2_tab        l_category_2_ttype;        -- 内訳分類３
    l_ex_tax_charge3_2_tab   l_ex_tax_charge_2_ttype;   -- 当月お買上げ額３
    l_tax_sum3_2_tab         l_tax_sum_2_ttype;         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
--
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_3_tab   l_ex_tax_charge_3_ttype;   -- 当月お買上げ額１
    l_tax_sum1_3_tab         l_tax_sum_3_ttype;         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--    l_tax_rate2_3_tab        l_tax_rate_3_ttype;        -- 消費税率２
    l_category2_3_tab        l_category_3_ttype;        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_3_tab   l_ex_tax_charge_3_ttype;   -- 当月お買上げ額２
    l_tax_sum2_3_tab         l_tax_sum_3_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_3_tab        l_category_3_ttype;        -- 内訳分類３
    l_ex_tax_charge3_3_tab   l_ex_tax_charge_3_ttype;   -- 当月お買上げ額３
    l_tax_sum3_3_tab         l_tax_sum_3_ttype;         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
--
    ...
    l_store_tax_sum_4_tab    l_store_tax_sum_4_ttype;   -- 消費税等
--
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_5_tab   l_ex_tax_charge_5_ttype;   -- 当月お買上げ額１
    l_tax_sum1_5_tab         l_tax_sum_5_ttype;         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--    l_tax_rate2_5_tab        l_tax_rate_5_ttype;        -- 消費税率２
    l_category2_5_tab        l_category_5_ttype;        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_5_tab   l_ex_tax_charge_5_ttype;   -- 当月お買上げ額２
    l_tax_sum2_5_tab         l_tax_sum_5_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_5_tab        l_category_5_ttype;        -- 内訳分類３
    l_ex_tax_charge3_5_tab   l_ex_tax_charge_5_ttype;   -- 当月お買上げ額３
    l_tax_sum3_5_tab         l_tax_sum_5_ttype;         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
--
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_6_tab   l_ex_tax_charge_6_ttype;   -- 当月お買上げ額１
    l_tax_sum1_6_tab         l_tax_sum_6_ttype;         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--    l_tax_rate2_6_tab        l_tax_rate_6_ttype;        -- 消費税率２
    l_category2_6_tab        l_category_6_ttype;        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_6_tab   l_ex_tax_charge_6_ttype;   -- 当月お買上げ額２
    l_tax_sum2_6_tab         l_tax_sum_6_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_6_tab        l_category_6_ttype;        -- 内訳分類３
    l_ex_tax_charge3_6_tab   l_ex_tax_charge_6_ttype;   -- 当月お買上げ額３
    l_tax_sum3_6_tab         l_tax_sum_6_ttype;         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
--
    ...
    l_store_tax_sum_7_tab    l_store_tax_sum_7_ttype;   -- 消費税等
-- Add 2015.07.31 Ver1.80 End
-- Add 2016.04.04 Ver1.90 Start
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_8_tab   l_ex_tax_charge_8_ttype;   -- 当月お買上げ額１
    l_tax_sum1_8_tab         l_tax_sum_8_ttype;         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--    l_tax_rate2_8_tab        l_tax_rate_8_ttype;        -- 消費税率２
    l_category2_8_tab        l_category_8_ttype;        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_8_tab   l_ex_tax_charge_8_ttype;   -- 当月お買上げ額２
    l_tax_sum2_8_tab         l_tax_sum_8_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_8_tab        l_category_8_ttype;        -- 内訳分類３
    l_ex_tax_charge3_8_tab   l_ex_tax_charge_8_ttype;   -- 当月お買上げ額３
    l_tax_sum3_8_tab         l_tax_sum_8_ttype;         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
-- Add 2016.04.04 Ver1.90 End
-- Ver.1.92 [障害E_本稼動_15307] ADD START
--
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge1_9_tab   l_ex_tax_charge_9_ttype;   -- 当月お買上げ額１
...
-- 2019/08/09 Ver1.93 MOD End
    l_ex_tax_charge2_9_tab   l_ex_tax_charge_9_ttype;   -- 当月お買上げ額２
    l_tax_sum2_9_tab         l_tax_sum_9_ttype;         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
    l_category3_9_tab        l_category_9_ttype;        -- 内訳分類３
    l_ex_tax_charge3_9_tab   l_ex_tax_charge_9_ttype;   -- 当月お買上げ額３
...
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate1_tab(ln_int)       := update_work_rec.tax_rate;            --消費税率1
          l_category1_tab(ln_int)       := update_work_rec.category;            --内訳分類1
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_tab(ln_int)  := update_work_rec.tax_rate_by_sum;     --当月お買上げ額１
          l_tax_sum1_tab(ln_int)        := update_work_rec.tax_rate_by_tax_sum; --消費税額１
-- 2019/08/09 Ver1.93 MOD Start
...
-- 2019/08/09 Ver1.93 ADD Start
          ...
          l_ex_tax_charge3_tab(ln_int)  := NULL;                                --当月お買上げ額２
          l_tax_sum3_tab(ln_int)        := NULL;                                --消費税額２
-- 2019/08/09 Ver1.93 ADD End
          lt_bill_cust_code             := update_work_rec.bill_cust_code;      --ブレークコード設定(顧客コード)
          lt_location_code              := update_work_rec.location_code;       --ブレークコード設定(担当拠点コード)
        ELSE
...
-- 2019/08/09 Ver1.93 MOD Start
--          SET     xrsi.tax_rate1        = l_tax_rate1_tab(i)        --消費税率1
          SET     xrsi.category1        = l_category1_tab(i)        --内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsi.ex_tax_charge1   = l_ex_tax_charge1_tab(i)   --当月お買上げ額１
                 ,xrsi.tax_sum1         = l_tax_sum1_tab(i)         --消費税額１
-- 2019/08/09 Ver1.93 MOD Start
...
-- Add 2023.07.04 Ver1.95 Start
      ...
      --ヘッダー用の当月お買上げ額、及び消費税額の一括更新
        BEGIN
          UPDATE  xxcfr_rep_st_invoice_ex_tax_d  xrsi                            -- 標準請求書税抜帳票内訳ワークテーブル
...
--          l_tax_rate1_2_tab(ln_int)       := update_work_2_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_2_tab(ln_int)       := update_work_2_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_2_tab(ln_int)  := update_work_2_rec.tax_rate_by_sum;     -- 当月お買上げ額１
          l_tax_sum1_2_tab(ln_int)        := update_work_2_rec.tax_rate_by_tax_sum; -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate2_2_tab(ln_int)       := NULL;                                  -- 消費税率２
          l_category2_2_tab(ln_int)       := NULL;                                  -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_2_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_2_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_2_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_2_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum3_2_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD End
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_2_tab(ln_int) := update_work_2_rec.tax_rate_by_sum;     -- 当月お買上げ額２
            l_tax_sum2_2_tab(ln_int)       := update_work_2_rec.tax_rate_by_tax_sum; -- 消費税額２
          END IF;
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            --３レコード目
            l_category3_2_tab(ln_int)      := update_work_2_rec.category;            -- 内訳分類３
            l_ex_tax_charge3_2_tab(ln_int) := update_work_2_rec.tax_rate_by_sum;     -- 当月お買上げ額３
            l_tax_sum3_2_tab(ln_int)       := update_work_2_rec.tax_rate_by_tax_sum; -- 消費税額３
          END IF;
-- 2019/08/09 Ver1.93 ADD End
        END IF;
--
...
--          SET     xrsiah.tax_rate1        = l_tax_rate1_2_tab(i)        -- 消費税率1
          SET     xrsiah.category1        = l_category1_2_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsiah.ex_tax_charge1   = l_ex_tax_charge1_2_tab(i)   -- 当月お買上げ額１
                 ,xrsiah.tax_sum1         = l_tax_sum1_2_tab(i)         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--                 ,xrsiah.tax_rate2        = l_tax_rate2_2_tab(i)        -- 消費税率２
                 ,xrsiah.category2        = l_category2_2_tab(i)        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsiah.ex_tax_charge2   = l_ex_tax_charge2_2_tab(i)   -- 当月お買上げ額２
                 ,xrsiah.tax_sum2         = l_tax_sum2_2_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsiah.category3        = l_category3_2_tab(i)        -- 内訳分類３
                 ,xrsiah.ex_tax_charge3   = l_ex_tax_charge3_2_tab(i)   -- 当月お買上げ額３
                 ,xrsiah.tax_sum3         = l_tax_sum3_2_tab(i)         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
      ...
      END;
--
      -- 3.税別の消費税額、及び、当月お買上げ額を計算し標準請求書税抜帳票内訳印刷単位Aワークテーブル明細を更新
      -- 変数の初期化
      ln_cust_cnt := 0;
      ln_int      := 0;
--
...
--          l_tax_rate1_3_tab(ln_int)       := update_work_3_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_3_tab(ln_int)       := update_work_3_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_3_tab(ln_int)  := update_work_3_rec.tax_rate_by_sum;     -- 当月お買上げ額１
          l_tax_sum1_3_tab(ln_int)        := update_work_3_rec.tax_rate_by_tax_sum; -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate2_3_tab(ln_int)       := NULL;                                  -- 消費税率２
          l_category2_3_tab(ln_int)       := NULL;                                  -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_3_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_3_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_3_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_3_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum3_3_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD End
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_3_tab(ln_int) := update_work_3_rec.tax_rate_by_sum;     -- 当月お買上げ額２
            l_tax_sum2_3_tab(ln_int)       := update_work_3_rec.tax_rate_by_tax_sum; -- 消費税額２
          END IF;
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            --３レコード目
            l_category3_3_tab(ln_int)      := update_work_3_rec.category;            -- 内訳分類３
            l_ex_tax_charge3_3_tab(ln_int) := update_work_3_rec.tax_rate_by_sum;     -- 当月お買上げ額３
            l_tax_sum3_3_tab(ln_int)       := update_work_3_rec.tax_rate_by_tax_sum; -- 消費税額３
          END IF;
-- 2019/08/09 Ver1.93 ADD End
        END IF;
--
...
--          SET     xrsial.tax_rate1        = l_tax_rate1_3_tab(i)        -- 消費税率1
          SET     xrsial.category1        = l_category1_3_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsial.ex_tax_charge1   = l_ex_tax_charge1_3_tab(i)   -- 当月お買上げ額１
                 ,xrsial.tax_sum1         = l_tax_sum1_3_tab(i)         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--                 ,xrsial.tax_rate2        = l_tax_rate2_3_tab(i)        -- 消費税率２
                 ,xrsial.category2        = l_category2_3_tab(i)        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsial.ex_tax_charge2   = l_ex_tax_charge2_3_tab(i)   -- 当月お買上げ額２
                 ,xrsial.tax_sum2         = l_tax_sum2_3_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsial.category3        = l_category3_3_tab(i)        -- 内訳分類３
                 ,xrsial.ex_tax_charge3   = l_ex_tax_charge3_3_tab(i)   -- 当月お買上げ額３
                 ,xrsial.tax_sum3         = l_tax_sum3_3_tab(i)         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
...
-- Add 2023.07.04 Ver1.95 Start
      ...
      --ヘッダー用の当月お買上げ額、及び消費税額の一括更新
        BEGIN
          UPDATE  xxcfr_rep_st_inv_ex_tax_a_h  xrsiah                            -- 標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダ
...
--
      ...
      FOR update_work_4_rec IN update_work_4_cur LOOP
        ...
        l_store_tax_sum_4_tab(ln_int)   := update_work_4_rec.store_tax_sum;       -- 消費税等
      END LOOP edit_loop4;
--
      --一括更新
      BEGIN
        ...
        FORALL i IN l_bill_cust_code_4_tab.FIRST..l_bill_cust_code_4_tab.LAST
          ...
          SET     xrsial.store_sum        = l_store_sum_4_tab(i)        -- 当月ご請求額（税込）
                 ,xrsial.store_tax_sum    = l_store_tax_sum_4_tab(i)    -- 消費税等
...
--          l_tax_rate1_5_tab(ln_int)       := update_work_5_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_5_tab(ln_int)       := update_work_5_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_5_tab(ln_int)  := update_work_5_rec.tax_rate_by_sum;     -- 当月お買上げ額１
          l_tax_sum1_5_tab(ln_int)        := update_work_5_rec.tax_rate_by_tax_sum; -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate2_5_tab(ln_int)       := NULL;                                  -- 消費税率２
          l_category2_5_tab(ln_int)       := NULL;                                  -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_5_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_5_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_5_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_5_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum3_5_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD End
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_5_tab(ln_int) := update_work_5_rec.tax_rate_by_sum;     -- 当月お買上げ額２
            l_tax_sum2_5_tab(ln_int)       := update_work_5_rec.tax_rate_by_tax_sum; -- 消費税額２
          END IF;
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            --３レコード目
            l_category3_5_tab(ln_int)      := update_work_5_rec.category;            -- 内訳分類３
            l_ex_tax_charge3_5_tab(ln_int) := update_work_5_rec.tax_rate_by_sum;     -- 当月お買上げ額３
            l_tax_sum3_5_tab(ln_int)       := update_work_5_rec.tax_rate_by_tax_sum; -- 消費税額３
          END IF;
-- 2019/08/09 Ver1.93 ADD End
        END IF;
--
...
--          SET     xrsibh.tax_rate1        = l_tax_rate1_5_tab(i)        -- 消費税率1
          SET     xrsibh.category1        = l_category1_5_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsibh.ex_tax_charge1   = l_ex_tax_charge1_5_tab(i)   -- 当月お買上げ額１
                 ,xrsibh.tax_sum1         = l_tax_sum1_5_tab(i)         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--                 ,xrsibh.tax_rate2        = l_tax_rate2_5_tab(i)        -- 消費税率２
                 ,xrsibh.category2        = l_category2_5_tab(i)        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsibh.ex_tax_charge2   = l_ex_tax_charge2_5_tab(i)   -- 当月お買上げ額２
                 ,xrsibh.tax_sum2         = l_tax_sum2_5_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsibh.category3        = l_category3_5_tab(i)        -- 内訳分類３
                 ,xrsibh.ex_tax_charge3   = l_ex_tax_charge3_5_tab(i)   -- 当月お買上げ額３
                 ,xrsibh.tax_sum3         = l_tax_sum3_5_tab(i)         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
      ...
      END;
--
      -- 6.税別の消費税額、及び、当月お買上げ額を計算し標準請求書税抜帳票内訳印刷単位Bワークテーブル明細を更新
      -- 変数の初期化
      ln_cust_cnt := 0;
      ln_int      := 0;
--
...
--          l_tax_rate1_6_tab(ln_int)       := update_work_6_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_6_tab(ln_int)       := update_work_6_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_6_tab(ln_int)  := update_work_6_rec.tax_rate_by_sum;     -- 当月お買上げ額１
          l_tax_sum1_6_tab(ln_int)        := update_work_6_rec.tax_rate_by_tax_sum; -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate2_6_tab(ln_int)       := NULL;                                  -- 消費税率２
          l_category2_6_tab(ln_int)       := NULL;                                  -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_6_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_6_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_6_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_6_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum3_6_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD End
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_6_tab(ln_int) := update_work_6_rec.tax_rate_by_sum;     -- 当月お買上げ額２
            l_tax_sum2_6_tab(ln_int)       := update_work_6_rec.tax_rate_by_tax_sum; -- 消費税額２
          END IF;
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            --３レコード目
            l_category3_6_tab(ln_int)      := update_work_6_rec.category;            -- 内訳分類３
            l_ex_tax_charge3_6_tab(ln_int) := update_work_6_rec.tax_rate_by_sum;     -- 当月お買上げ額３
            l_tax_sum3_6_tab(ln_int)       := update_work_6_rec.tax_rate_by_tax_sum; -- 消費税額３
          END IF;
-- 2019/08/09 Ver1.93 ADD End
        END IF;
--
...
--          SET     xrsibl.tax_rate1        = l_tax_rate1_6_tab(i)        -- 消費税率1
          SET     xrsibl.category1        = l_category1_6_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsibl.ex_tax_charge1   = l_ex_tax_charge1_6_tab(i)   -- 当月お買上げ額１
                 ,xrsibl.tax_sum1         = l_tax_sum1_6_tab(i)         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--                 ,xrsibl.tax_rate2        = l_tax_rate2_6_tab(i)        -- 消費税率２
                 ,xrsibl.category2        = l_category2_6_tab(i)        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsibl.ex_tax_charge2   = l_ex_tax_charge2_6_tab(i)   -- 当月お買上げ額２
                 ,xrsibl.tax_sum2         = l_tax_sum2_6_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsibl.category3        = l_category3_6_tab(i)        -- 内訳分類３
                 ,xrsibl.ex_tax_charge3   = l_ex_tax_charge3_6_tab(i)   -- 当月お買上げ額３
                 ,xrsibl.tax_sum3         = l_tax_sum3_6_tab(i)         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
...
-- Add 2023.07.04 Ver1.95 Start
      ...
      --ヘッダー用の当月お買上げ額、及び消費税額の一括更新
        BEGIN
          UPDATE  xxcfr_rep_st_inv_ex_tax_b_h  xrsibh                            -- 標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダ
...
--
      ...
      FOR update_work_7_rec IN update_work_7_cur LOOP
        ...
        l_store_tax_sum_7_tab(ln_int)   := update_work_7_rec.store_tax_sum;       -- 消費税等
      END LOOP edit_loop7;
--
      --一括更新
      BEGIN
        ...
        FORALL i IN l_bill_cust_code_7_tab.FIRST..l_bill_cust_code_7_tab.LAST
          ...
          SET     xrsibl.store_sum        = l_store_sum_7_tab(i)        -- 当月ご請求額（税込）
                 ,xrsibl.store_tax_sum    = l_store_tax_sum_7_tab(i)    -- 消費税等
...
--          l_tax_rate1_8_tab(ln_int)       := update_work_8_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_8_tab(ln_int)       := update_work_8_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_8_tab(ln_int)  := update_work_8_rec.tax_rate_by_sum;     -- 当月お買上げ額１
          l_tax_sum1_8_tab(ln_int)        := update_work_8_rec.tax_rate_by_tax_sum; -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--          l_tax_rate2_8_tab(ln_int)       := NULL;                                  -- 消費税率２
          l_category2_8_tab(ln_int)       := NULL;                                  -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_8_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_8_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_8_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_8_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum3_8_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD End
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_8_tab(ln_int) := update_work_8_rec.tax_rate_by_sum;     -- 当月お買上げ額２
            l_tax_sum2_8_tab(ln_int)       := update_work_8_rec.tax_rate_by_tax_sum; -- 消費税額２
          END IF;
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            --３レコード目
            l_category3_8_tab(ln_int)      := update_work_8_rec.category;            -- 内訳分類３
            l_ex_tax_charge3_8_tab(ln_int) := update_work_8_rec.tax_rate_by_sum;     -- 当月お買上げ額３
            l_tax_sum3_8_tab(ln_int)       := update_work_8_rec.tax_rate_by_tax_sum; -- 消費税額３
          END IF;
-- 2019/08/09 Ver1.93 ADD End
        END IF;
--
...
--          SET     xrsich.tax_rate1        = l_tax_rate1_8_tab(i)        -- 消費税率1
          SET     xrsich.category1        = l_category1_8_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsich.ex_tax_charge1   = l_ex_tax_charge1_8_tab(i)   -- 当月お買上げ額１
                 ,xrsich.tax_sum1         = l_tax_sum1_8_tab(i)         -- 消費税額１
-- 2019/08/09 Ver1.93 MOD Start
--                 ,xrsich.tax_rate2        = l_tax_rate2_8_tab(i)        -- 消費税率２
                 ,xrsich.category2        = l_category2_8_tab(i)        -- 内訳分類２
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsich.ex_tax_charge2   = l_ex_tax_charge2_8_tab(i)   -- 当月お買上げ額２
                 ,xrsich.tax_sum2         = l_tax_sum2_8_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsich.category3        = l_category3_8_tab(i)        -- 内訳分類３
                 ,xrsich.ex_tax_charge3   = l_ex_tax_charge3_8_tab(i)   -- 当月お買上げ額３
                 ,xrsich.tax_sum3         = l_tax_sum3_8_tab(i)         -- 消費税額３
-- 2019/08/09 Ver1.93 ADD End
...
-- Add 2023.07.04 Ver1.95 Start
    IF ( gv_target_c_flag = cv_taget_flag_2 ) THEN
      ...
      --ヘッダー用の当月お買上げ額、及び消費税額の一括更新
        BEGIN
          UPDATE  xxcfr_rep_st_inv_ex_tax_c_h  xrsich                            -- 標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダ
...
--
      ...
      END;
-- 2023/07/04 Ver1.95 ADD End
      -- 9.税別の消費税額、及び、当月お買上げ額を計算し標準請求書税抜帳票内訳印刷単位Dワークテーブル明細を更新
      -- 変数の初期化
      ln_cust_cnt := 0;
      ln_int      := 0;
--
...
--          l_tax_rate1_9_tab(ln_int)       := update_work_9_rec.tax_rate;            -- 消費税率(編集用)
          l_category1_9_tab(ln_int)       := update_work_9_rec.category;            -- 内訳分類(編集用)
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge1_9_tab(ln_int)  := update_work_9_rec.tax_rate_by_sum;     -- 当月お買上げ額１
...
-- 2019/08/09 Ver1.93 MOD End
          l_ex_tax_charge2_9_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
          l_tax_sum2_9_tab(ln_int)        := NULL;                                  -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
          l_category3_9_tab(ln_int)       := NULL;                                  -- 内訳分類３
          l_ex_tax_charge3_9_tab(ln_int)  := NULL;                                  -- 当月お買上げ額２
...
-- 2019/08/09 Ver1.93 MOD End
            l_ex_tax_charge2_9_tab(ln_int) := update_work_9_rec.tax_rate_by_sum;     -- 当月お買上げ額２
...
-- 2019/08/09 Ver1.93 ADD Start
          IF ( ln_cust_cnt = 3 ) THEN
            ...
            l_ex_tax_charge3_9_tab(ln_int) := update_work_9_rec.tax_rate_by_sum;     -- 当月お買上げ額３
...
--          SET     xrsidl.tax_rate1        = l_tax_rate1_9_tab(i)        -- 消費税率1
          SET     xrsidl.category1        = l_category1_9_tab(i)        -- 内訳分類1
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsidl.ex_tax_charge1   = l_ex_tax_charge1_9_tab(i)   -- 当月お買上げ額１
...
-- 2019/08/09 Ver1.93 MOD End
                 ,xrsidl.ex_tax_charge2   = l_ex_tax_charge2_9_tab(i)   -- 当月お買上げ額２
                 ,xrsidl.tax_sum2         = l_tax_sum2_9_tab(i)         -- 消費税額２
-- 2019/08/09 Ver1.93 ADD Start
                 ,xrsidl.category3        = l_category3_9_tab(i)        -- 内訳分類３
                 ,xrsidl.ex_tax_charge3   = l_ex_tax_charge3_9_tab(i)   -- 当月お買上げ額３
...
-- 2023/07/04 Ver1.95 ADD End
      -- 10.当月お買上げ額と消費税額を計算し標準請求書税抜帳票内訳印刷単位Dワークテーブル明細を更新
      -- 変数の初期化
      ln_int      := 0;
--
      <<edit_loop10>>
...
-- Modify 2013.11.25 Ver1.60 Start
--                     xil.slip_num;
                     xil.slip_num,
-- Modify 2014.03.27 Ver1.70 Start
--                     xil.tax_rate
...
--
              INSERT INTO xxcfr_rep_st_inv_ex_tax_a_l(
                report_id               , -- 帳票ＩＤ
                issue_date              , -- 発行日付
                zip_code                , -- 郵便番号
                send_address1           , -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
                ...
                program_id              , -- コンカレント・プログラムID
                program_update_date     ) -- プログラム更新日
              SELECT cv_report_id_06                                                                  report_id             , -- 帳票ＩＤ
                     TO_CHAR(xih.inv_creation_date,lv_format_date_jpymd4)                             issue_date            , -- 発行日
                     DECODE(get_14account_rec.bill_postal_code,
                            NULL,NULL,
                            lv_format_zip_mark||SUBSTR(get_14account_rec.bill_postal_code,1,3)||'-'||
                     ...
                     TO_CHAR(DECODE(xil.acceptance_date,
                                    NULL,xil.delivery_date,
                                    xil.acceptance_date),
                             cv_format_date_ymds2)                                                    slip_date             , -- 伝票日付
...
-- 2019/08/09 Ver1.93 ADD Start
                ...
                AND flv.enabled_flag(+) = cv_enabled_yes
-- 2019/08/09 Ver1.93 ADD End
              GROUP BY cv_report_id_06,                                                   -- 帳票ID
                       xih.inv_creation_date,                                             -- 発効日
                       DECODE(get_14account_rec.bill_postal_code,
                                   NULL,NULL,
                                   lv_format_zip_mark||SUBSTR(get_14account_rec.bill_postal_code,1,3)||'-'||
                                   SUBSTR(get_14account_rec.bill_postal_code,4,4)),       -- 郵便番号
                       get_14account_rec.bill_state||get_14account_rec.bill_city,         -- 住所１
                       get_14account_rec.bill_address1,                                   -- 住所２
                       get_14account_rec.bill_address2,                                   -- 住所３
                       get_14account_rec.cash_account_number,                             -- 入金先顧客コード
                       get_14account_rec.cash_account_name,                               -- 入金先顧客名
                       xffvv.description,                                                 -- 担当拠点名
                       xih.object_month,                                                  -- 対象年月
                       get_14account_rec.cash_account_number||' '||xih.term_name,         -- 売掛管理コード連結文字列
                       ...
                       ELSE
                         ...
                         account.account_holder_name_alt)                    -- 口座名義人カナ名
                       END,                                                               -- 振込口座情報
                       xil.ship_cust_code,                                                -- 納品先顧客コード(ソート用)
                       hzp.party_name,                                                    -- 納品先顧客名
                       TO_CHAR(DECODE(xil.acceptance_date,
                                      NULL,xil.delivery_date,
                                      xil.acceptance_date),
                               cv_format_date_ymds2),                                     -- 伝票日付
                       xil.slip_num,                                                      -- 伝票番号
-- 2019/08/09 Ver1.93 MOD Start
--                       xil.tax_rate,                                                      -- 消費税率
                       flv.attribute1,                                                    -- 摘要
...
--
            BEGIN
              INSERT INTO xxcfr_rep_st_inv_ex_tax_b_l(
                report_id               , -- 帳票ＩＤ
                issue_date              , -- 発行日付
                zip_code                , -- 郵便番号
                send_address1           , -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
                ...
                program_id              , -- コンカレント・プログラムID
                program_update_date     ) -- プログラム更新日
              SELECT cv_report_id_08                                                                  report_id             , -- 帳票ＩＤ
                     TO_CHAR(xih.inv_creation_date,lv_format_date_jpymd4)                             issue_date            , -- 発行日
                     DECODE(get_14account_rec.bill_postal_code,
                            NULL,NULL,
                            lv_format_zip_mark||SUBSTR(get_14account_rec.bill_postal_code,1,3)||'-'||
...
--
              INSERT INTO xxcfr_rep_st_inv_ex_tax_c_l(
                report_id               , -- 帳票ＩＤ
                issue_date              , -- 発行日付
                zip_code                , -- 郵便番号
                send_address1           , -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
                ...
                program_id              , -- コンカレント・プログラムID
                program_update_date     ) -- プログラム更新日
              SELECT cv_report_id_12                                                                  report_id             , -- 帳票ＩＤ
                     TO_CHAR(xih.inv_creation_date,lv_format_date_jpymd4)                             issue_date            , -- 発行日
                     DECODE(get_14account_rec.bill_postal_code,
                            NULL,NULL,
                            lv_format_zip_mark||SUBSTR(get_14account_rec.bill_postal_code,1,3)||'-'||
...
--
            BEGIN
              INSERT INTO xxcfr_rep_st_inv_ex_tax_d_l(
                report_id               , -- 帳票ＩＤ
                issue_date              , -- 発行日付
                zip_code                , -- 郵便番号
                send_address1           , -- 住所１
...
-- 2023/07/04 Ver1.95 ADD End
                ...
                program_id              , -- コンカレント・プログラムID
                program_update_date     ) -- プログラム更新日
              SELECT cv_report_id_15                                                                  report_id             , -- 帳票ＩＤ
                     TO_CHAR(xih.inv_creation_date,lv_format_date_jpymd4)                             issue_date            , -- 発行日
                     DECODE(get_14account_rec.bill_postal_code,
                            NULL,NULL,
                            lv_format_zip_mark||SUBSTR(get_14account_rec.bill_postal_code,1,3)||'-'||
...
--
      ...
      IF ( gn_target_cnt_a_l <> 0 ) THEN
        ...
        -- 標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダの登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_a_h(
           report_id                -- 帳票ID
          ,issue_date               -- 発行日付
          ,zip_code                 -- 郵便番号
          ,send_address1            -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
              ...
              ,cn_program_id               -- コンカレント・プログラムID
              ,cd_program_update_date      -- プログラム更新日
        FROM   xxcfr_rep_st_inv_ex_tax_a_l  xrsal
        WHERE  xrsal.request_id = cn_request_id
        GROUP BY cv_report_id_05             -- 帳票ID
                ,xrsal.issue_date            -- 発行日付
                ,xrsal.zip_code              -- 郵便番号
                ,xrsal.send_address1         -- 住所１
                ,xrsal.send_address2         -- 住所２
...
--
      ...
      ELSIF ( gv_target_a_flag = cv_taget_flag_1 ) THEN
        -- 帳票０件メッセージを標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダに登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_a_h (
          data_empty_message           , -- 0件メッセージ
          outsourcing_flag             , -- 業者委託フラグ
          created_by                   , -- 作成者
...
--
        -- 標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダの登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_b_h(
           report_id                -- 帳票ID
          ,issue_date               -- 発行日付
          ,zip_code                 -- 郵便番号
          ,send_address1            -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
        ...
        SELECT cv_report_id_07             -- 帳票ID
              ...
              ,SUM(xrsbl.slip_sum)         -- 店舗金額
              ,SUM(xrsbl.slip_tax_sum)     -- 店舗税額
-- Add 2023.07.04 Ver1.95 Start
...
-- Add 2023.07.04 Ver1.95 End
              ...
              ,cn_program_id               -- コンカレント・プログラムID
              ,cd_program_update_date      -- プログラム更新日
        FROM   xxcfr_rep_st_inv_ex_tax_b_l  xrsbl
        WHERE  xrsbl.request_id = cn_request_id
        GROUP BY cv_report_id_07             -- 帳票ID
                ,xrsbl.issue_date            -- 発行日付
                ,xrsbl.zip_code              -- 郵便番号
                ,xrsbl.send_address1         -- 住所１
                ,xrsbl.send_address2         -- 住所２
...
--
      ...
      ELSIF ( gv_target_b_flag = cv_taget_flag_1 ) THEN
        -- 帳票０件メッセージを標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダに登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_b_h (
          data_empty_message           , -- 0件メッセージ
          outsourcing_flag             , -- 業者委託フラグ
          created_by                   , -- 作成者
...
-- Add 2016.04.04 Ver1.90 Start
      ...
      IF ( gn_target_cnt_c_l <> 0 ) THEN
        ...
        -- 標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダの登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_c_h(
           report_id                -- 帳票ID
          ,issue_date               -- 発行日付
          ,zip_code                 -- 郵便番号
          ,send_address1            -- 住所１
...
-- Add 2023.07.04 Ver1.95 End
              ...
              ,cn_program_id               -- コンカレント・プログラムID
              ,cd_program_update_date      -- プログラム更新日
        FROM   xxcfr_rep_st_inv_ex_tax_c_l  xrscl
        WHERE  xrscl.request_id = cn_request_id
        GROUP BY cv_report_id_11             -- 帳票ID
                ,xrscl.issue_date            -- 発行日付
                ,xrscl.zip_code              -- 郵便番号
                ,xrscl.send_address1         -- 住所１
                ,xrscl.send_address2         -- 住所２
...
--
      ...
      ELSIF ( gv_target_c_flag = cv_taget_flag_1 ) THEN
        -- 帳票０件メッセージを標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダに登録
        INSERT INTO xxcfr_rep_st_inv_ex_tax_c_h (
          data_empty_message           , -- 0件メッセージ
          outsourcing_flag             , -- 業者委託フラグ
          created_by                   , -- 作成者
...
-- 2023/07/04 Ver1.95 ADD End
              ...
              ,cn_program_id                                     -- コンカレント・プログラムID
              ,cd_program_update_date                            -- プログラム更新日
        FROM   xxcfr_rep_st_inv_ex_tax_d_l  xrsdl
...
-- Add 2015.07.31 Ver1.80 Start
      ...
      FROM xxcfr_rep_st_inv_ex_tax_a_h  xrsia
      ...
      FROM xxcfr_rep_st_inv_ex_tax_b_h  xrsib
...
-- Add 2016.04.04 Ver1.90 Start
      ...
      FROM xxcfr_rep_st_inv_ex_tax_c_h  xrsic
...
--
-- Add 2015.07.31 Ver1.80 Start
    -- 標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダ抽出
    CURSOR del_rep_st_inv_ex_a_h_cur
    IS
      SELECT xrsiah.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_a_h xrsiah  -- 標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダ
      WHERE xrsiah.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_a_h_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_a_h_data    g_del_rep_st_inv_ex_a_h_ttype;
--
    -- 標準請求書税抜帳票内訳印刷単位Aワークテーブル明細抽出
    CURSOR del_rep_st_inv_ex_a_l_cur
    IS
      SELECT xrsial.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_a_l xrsial  -- 標準請求書税抜帳票内訳印刷単位Aワークテーブル明細
      WHERE xrsial.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_a_l_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_a_l_data    g_del_rep_st_inv_ex_a_l_ttype;
--
    -- 標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダ抽出
    CURSOR del_rep_st_inv_ex_b_h_cur
    IS
      SELECT xrsibh.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_b_h xrsibh  -- 標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダ
      WHERE xrsibh.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_b_h_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_b_h_data    g_del_rep_st_inv_ex_b_h_ttype;
--
    -- 標準請求書税抜帳票内訳印刷単位Bワークテーブル明細抽出
    CURSOR del_rep_st_inv_ex_b_l_cur
    IS
      SELECT xrsibl.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_b_l xrsibl  -- 標準請求書税抜帳票内訳印刷単位Bワークテーブル明細
      WHERE xrsibl.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_b_l_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_b_l_data    g_del_rep_st_inv_ex_b_l_ttype;
--
-- Add 2015.07.31 Ver1.80 End
-- Add 2016.04.04 Ver1.90 Start
    -- 標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダ抽出
    CURSOR del_rep_st_inv_ex_c_h_cur
    IS
      SELECT xrsich.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_c_h xrsich  -- 標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダ
      WHERE xrsich.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_c_h_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_c_h_data    g_del_rep_st_inv_ex_c_h_ttype;
--
    -- 標準請求書税抜帳票内訳印刷単位Cワークテーブル明細抽出
    CURSOR del_rep_st_inv_ex_c_l_cur
    IS
      SELECT xrsicl.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_c_l xrsicl  -- 標準請求書税抜帳票内訳印刷単位Cワークテーブル明細
      WHERE xrsicl.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_c_l_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_c_l_data    g_del_rep_st_inv_ex_c_l_ttype;
--
-- Add 2016.04.04 Ver1.90 End
-- Ver.1.92 [障害E_本稼動_15307] ADD START
...
--
    ...
    CURSOR del_rep_st_inv_ex_d_l_cur
    IS
      SELECT xrsidl.rowid        ln_rowid
      FROM xxcfr_rep_st_inv_ex_tax_d_l xrsidl  -- 標準請求書税抜帳票内訳印刷単位Dワークテーブル明細
      WHERE xrsidl.request_id = cn_request_id  -- 要求ID
      FOR UPDATE NOWAIT;
--
    TYPE g_del_rep_st_inv_ex_d_l_ttype IS TABLE OF ROWID INDEX BY PLS_INTEGER;
    lt_del_rep_st_inv_ex_d_l_data    g_del_rep_st_inv_ex_d_l_ttype;
--
-- Ver.1.92 [障害E_本稼動_15307] ADD END
  BEGIN
--
...
--
      ...
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_invoice_ex_tax_d rep
...
--
    ...
    END;
--
    -- ②標準請求書税抜帳票内訳印刷単位Aワークテーブルヘッダ削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_a_h_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_a_h_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_a_h_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_a_h_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_a_h_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_a_h rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_a_h_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
    ...
    END;
--
    -- ③標準請求書税抜帳票内訳印刷単位Aワークテーブル明細削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_a_l_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_a_l_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_a_l_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_a_l_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_a_l_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_a_l rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_a_l_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
    ...
    END;
--
    -- ④標準請求書税抜帳票内訳印刷単位Bワークテーブルヘッダ削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_b_h_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_b_h_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_b_h_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_b_h_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_b_h_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_b_h rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_b_h_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
    ...
    END;
--
    -- ⑤標準請求書税抜帳票内訳印刷単位Bワークテーブル明細削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_b_l_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_b_l_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_b_l_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_b_l_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_b_l_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_b_l rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_b_l_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
-- Add 2016.04.04 Ver1.90 Start
    -- ⑥標準請求書税抜帳票内訳印刷単位Cワークテーブルヘッダ削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_c_h_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_c_h_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_c_h_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_c_h_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_c_h_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_c_h rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_c_h_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
    ...
    END;
--
    -- ⑦標準請求書税抜帳票内訳印刷単位Cワークテーブル明細削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_c_l_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_c_l_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_c_l_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_c_l_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_c_l_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_c_l rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_c_l_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
...
--
    ...
    END;
--
    -- ⑨標準請求書税込帳票内訳印刷単位Dワークテーブル明細削除処理
    BEGIN
      -- カーソルオープン
      OPEN del_rep_st_inv_ex_d_l_cur;
--
      -- データの一括取得
      FETCH del_rep_st_inv_ex_d_l_cur BULK COLLECT INTO lt_del_rep_st_inv_ex_d_l_data;
--
      -- 処理件数のセット
      ln_target_cnt := lt_del_rep_st_inv_ex_d_l_data.COUNT;
--
      -- カーソルクローズ
      CLOSE del_rep_st_inv_ex_d_l_cur;
--
      -- 対象データが存在する場合レコードを削除する
      IF (ln_target_cnt > 0) THEN
        BEGIN
          <<data_loop>>
          FORALL ln_loop_cnt IN 1..ln_target_cnt
            DELETE FROM xxcfr_rep_st_inv_ex_tax_d_l rep
            WHERE rep.rowid = lt_del_rep_st_inv_ex_d_l_data(ln_loop_cnt);
--
          -- コミット発行
          COMMIT;
--
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        lv_errmsg   := xxccp_common_pkg.get_msg(
                         ...
                         , iv_name         => cv_msg_003a19_027      -- メッセージコード
...
-- Ver1.96 ADD END
    ...
    IF ( g_org_request(in_req_cnt).request_id = 0 ) THEN
      lv_errmsg := xxccp_common_pkg.get_msg(
                       iv_application  => cv_msg_kbn_cfr                                -- アプリケーション短縮名
                     , iv_name         => cv_msg_003a19_028                             -- メッセージコード
