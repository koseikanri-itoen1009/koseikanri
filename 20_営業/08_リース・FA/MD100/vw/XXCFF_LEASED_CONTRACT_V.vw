...
AS
SELECT XLV.LEASE_CLASS_NAME     AS LEASE_CLASS_NAME
      ...
      ,XPP.NOT_LEASE_TIMES      AS NOT_LEASE_TIMES
      ,XPP.NOT_CHARGE           AS NOT_CHARGE
...
FROM   XXCFF_CONTRACT_HEADERS  XCH
      ...
      ,(SELECT CONTRACT_HEADER_ID       AS CONTRACT_HEADER_ID
              ,CONTRACT_LINE_ID         AS CONTRACT_LINE_ID
              ,COUNT(PAYMENT_FREQUENCY) AS NOT_LEASE_TIMES
              ,SUM(LEASE_CHARGE)        AS NOT_CHARGE
        FROM   XXCFF_PAY_PLANNING
        where  ACCOUNTING_IF_FLAG = '1'
        GROUP BY CONTRACT_HEADER_ID,CONTRACT_LINE_ID) XPP
      ,AP_TAX_CODES APT
...
AND    XCL.CONTRACT_HEADER_ID = XPP.CONTRACT_HEADER_ID(+)
