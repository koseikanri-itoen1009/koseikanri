CREATE OR REPLACE PACKAGE BODY XXCCP009A09C
AS
/*****************************************************************************************
 * Copyright(c)SCSK Corporation, 2011. All rights reserved.
 *
 * Package Name     : XXCCP009A09C(body)
...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name      CONSTANT VARCHAR2(100) := 'XXCCP009A09C'; -- パッケージ名
--
  -- ===============================
  -- ユーザー定義グローバル型
  -- ===============================
      ...
      IS
        SELECT /*+ USE_NL(acra arm arc fu papf base_hp base_hc gcc araa)
          ...
          ,(SELECT cash_hc.account_number
           ...
           )                               AS pfrom_cust_account_number   -- 入金顧客番号
          ,(SELECT cash_hp.party_name
            ...
            AND    cash_hc.party_id = cash_hp.party_id
            AND    cash_hc.status   = 'A'
           )                               AS pfrom_cust_party_name       -- 入金顧客名
          ,DECODE( acra.status
          ...
          ,NVL(araa.amount_applied,0) * -1 AS amount_applied              -- 消込額
        ...
        AND   araa.application_type = 'CASH'                      -- 入金のレコードのみ
     ...
     LOOP
       ...
       FND_FILE.PUT_LINE(
         ...
         ,buff   => '"'|| TO_CHAR(unid_sus_receipt_rec.creation_date, 'YYYY/MM/DD HH24:MI:SS')     || '","'
                       ...
                       || unid_sus_receipt_rec.pfrom_cust_account_number                           || '","'
                       || unid_sus_receipt_rec.pfrom_cust_party_name                               || '","'
