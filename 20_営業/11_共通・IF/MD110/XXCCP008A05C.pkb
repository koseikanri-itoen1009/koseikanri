CREATE OR REPLACE PACKAGE BODY XXCCP008A05C
AS
/*****************************************************************************************
 * Copyright(c)SCSK Corporation, 2011. All rights reserved.
 *
 * Package Name     : XXCCP008A05C(body)
...
--
  cv_pkg_name               CONSTANT VARCHAR2(100) := 'XXCCP008A05C';   -- プログラム名
--
  /**********************************************************************************
...
--
    ...
    IS
      SELECT   xdpw.execute_mode     AS execute_mode       -- 実行モード
              ...
              ,xoh.object_header_id  AS object_header_id   -- 物件ヘッダID
              ...
              ,xoh.object_code       AS object_code        -- ログ出力用：物件コード(リース物件)
              ,xohi.object_code      AS object_code_hist   -- ログ出力用：物件コード(リース物件履歴)
              ,xohi.object_status    AS object_status_hist -- ログ出力用：物件ステータス(リース物件履歴)
      FROM     xxccp_data_patch_work   xdpw -- パッチワークテーブル
              ,xxcff_object_headers    xoh  -- リース物件テーブル
              ,xxcff_object_histories  xohi -- リース物件履歴テーブル
              ,xxcso_ib_info_h         xiih -- 物件履歴テーブル(営業)
              ,xxcso_install_base_v    xibv -- 物件マスタ(営業)
      WHERE    xdpw.file_id         = ln_file_id                 -- ファイルID
      AND      xdpw.condition_1     = xoh.object_code(+)         -- 条件1(物件コード)
      AND      xoh.object_header_id = xohi.object_header_id(+)   -- 物件ヘッダID
      AND      xoh.object_code      = xiih.install_code(+)       -- 物件コード
      AND      xoh.object_code      = xibv.install_code(+)       -- 物件コード
      ORDER BY
               xdpw.data_sequence -- シーケンス番号
              ,xohi.history_num   -- 履歴番号
    ;
--
...
--
        -- 物件存在チェック
        IF ( data_rec.object_header_id IS NULL ) THEN
          fnd_file.put_line(
            which  => FND_FILE.LOG
...
--
        -- 物件が存在する場合
        IF ( data_rec.object_header_id IS NOT NULL ) THEN
          -- リース種別チェック
          IF ( data_rec.lease_class NOT LIKE '1%' ) THEN
            fnd_file.put_line(
              which  => FND_FILE.LOG
             ,buff   => '[' || ln_cnt || '行目] 自販機関連の物件ではありません'
            );
            lv_err_flg := 'Y';
          END IF;
--
...
--
        ...
        IF ( ( lv_err_flg <> 'Y' ) AND ( lv_exec_flag = '1' ) )  THEN
          ...
          WHERE xoh.object_code = data_rec.condition_1  -- 条件1(物件コード)
          ;
--
          -- リース物件履歴テーブル削除
          DELETE
          FROM  xxcff_object_histories xohi
          WHERE xohi.object_header_id = data_rec.object_header_id -- 物件ヘッダID
          ;
--
          -- 物件履歴テーブル(営業)更新
