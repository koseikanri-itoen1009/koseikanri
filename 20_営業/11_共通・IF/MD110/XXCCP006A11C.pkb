CREATE OR REPLACE PACKAGE BODY XXCCP006A11C
AS
/*****************************************************************************************
 * Copyright(c)SCSK Corporation, 2011. All rights reserved.
 *
 * Package Name     : XXCCP006A11C(body)
 * Description      : 新規ポイント更新処理
 * Version          : 1.0
 *
 * Program List
 * ------------------------- ------------------------------------------------------------
 ...
 * ------------- ----- ---------------- -------------------------------------------------
 *  2012/10/16    1.0   K.Onotuska       [E_本稼動_10052]新規作成
 *
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
--
  ...
  --WHOカラム
  cn_last_updated_by        CONSTANT NUMBER      := -1;                                 --LAST_UPDATED_BY
...
--
  cv_pkg_name               CONSTANT VARCHAR2(100) := 'XXCCP006A11C';                 -- プログラム名
--
  /**********************************************************************************
...
--
    ...
    IS
      SELECT   xdpw.execute_mode                execute_mode  --実行モード
              ,xdpw.condition_1                 condition_1   --条件１（顧客コード）
              ,xdpw.num_column_1                num_column_1  --変更値１（新規ポイント）
              ,xca.customer_id                  customer_id   --テーブル更新キー(ID）
              ,xca.new_point                    new_point     --更新前（新規ポイント）
              ,xca.last_updated_by              last_updated_by        --更新前（最終更新者）
              ,TO_CHAR(xca.last_update_date,'YYYY/MM/DD HH24:MI:SS')
                                                last_update_date       --更新前（最終更新日）
              ,xca.last_update_login            last_update_login      --更新前（最終更新ログイン）
              ,xca.request_id                   request_id             --更新前（要求ID）
              ,xca.program_application_id       program_application_id --更新前（ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑ･ｱﾌﾟﾘｹｰｼｮﾝID）
              ,xca.program_id                   program_id             --更新前（ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑID）
              ,TO_CHAR(xca.program_update_date,'YYYY/MM/DD HH24:MI:SS')
                                                program_update_date    --更新前（ﾌﾟﾛｸﾞﾗﾑ更新日）
      FROM     xxccp_data_patch_work xdpw
              ,xxcmm_cust_accounts   xca  --顧客追加情報(アドオン)
...
--
    ...
    FOR ln_line_cnt IN 2 .. lt_path_data_tab.COUNT LOOP
      ...
      INSERT INTO xxccp_data_patch_work (
        ...
        ,lt_path_data_tab(ln_line_cnt)(3)                          -- 変更値１
      )
      ;
      --対象件数カウント
...
--
      --データ存在チェック
      IF ( data_rec.customer_id IS NULL )THEN
        -- 警告ログ出力
        fnd_file.put_line(
           which  => FND_FILE.OUTPUT
...
--
      ...
      IF ( ( lv_err_flg <> 'Y' ) AND ( lv_exec_flag = '1' ) )  THEN
        ...
        SET    xca.new_point               = data_rec.num_column_1      --新規ポイント
