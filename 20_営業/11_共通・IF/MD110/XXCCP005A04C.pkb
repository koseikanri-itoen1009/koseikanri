CREATE OR REPLACE PACKAGE BODY XXCCP005A04C
AS
/*****************************************************************************************
 * Copyright(c)SCSK Corporation, 2011. All rights reserved.
 *
 * Package Name     : XXCCP005A04C(body)
 * Description      : マスタ連携済フラグ更新処理
 * Version          : 1.0
 *
 * Program List
 * ------------------------- ------------------------------------------------------------
 ...
 * ------------- ----- ---------------- -------------------------------------------------
 *  2012/10/18    1.0   M.Nagai         [E_本稼動_09946]新規作成
 *
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
...
--
  cv_pkg_name               CONSTANT VARCHAR2(100) := 'XXCCP005A04C';                   -- プログラム名
--
  /**********************************************************************************
...
--
    ...
    lv_cooperate_flag    VARCHAR2(1)  := 0;    --マスタ連携フラグ
    lv_batch_proc_status VARCHAR2(1)  := NULL; --バッチ処理ステータス
--
    --マスタ連携フラグ更新カーソル
    CURSOR data_cur
    IS
      SELECT   xdpw.execute_mode           execute_mode                      --実行モード
              ,xdpw.condition_1            condition_1                       --条件１（契約書番号）
              ,xcm.contract_management_id  contract_management_id            --更新キー
              ,xcm.cooperate_flag          cooperate_flag                    --更新前（マスタ連携フラグ）
              ,xcm.batch_proc_status       batch_proc_status                 --更新前（バッチ処理ステータス）
              ,xcm.last_updated_by         last_updated_by                   --更新前（最終更新者）
              ,TO_CHAR(xcm.last_update_date,'YYYY/MM/DD HH24:MI:SS')
                                           last_update_date                  --更新前（最終更新日）
              ,xcm.last_update_login       last_update_login                 --更新前（最終更新ログイン）
              ,xcm.request_id              request_id                        --更新前（要求ID）
              ,xcm.program_application_id  program_application_id            --更新前（ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑ･ｱﾌﾟﾘｹｰｼｮﾝID）
              ,xcm.program_id              program_id                        --更新前（ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑID）
              ,TO_CHAR(xcm.program_update_date,'YYYY/MM/DD HH24:MI:SS')
      ...
      AND      xdpw.condition_1   = xcm.contract_number(+)
      ORDER BY
               xdpw.data_sequence
      ;
--
...
--
    ...
    FOR ln_line_cnt IN 2 .. lt_path_data_tab.COUNT LOOP
      ...
      INSERT INTO xxccp_data_patch_work (
        ...
        ,lt_path_data_tab(ln_line_cnt)(2)                         -- 条件値1（契約書番号)
      )
      ;
      --対象件数カウント
...
--
      ...
      END IF;
--
      --指定された契約番号のデータが存在しない場合
      IF ( data_rec.contract_management_id IS NULL ) THEN
        lv_err_flg := 'Y';
        -- 警告件数カウント
...
--
      IF ( lv_err_flg <> 'Y' ) THEN
        ...
        fnd_file.put_line(
           which  => FND_FILE.OUTPUT
          ,buff   =>  '"'|| data_rec.condition_1                                     || '","'  --契約書番号
                         || data_rec.cooperate_flag                                  || '","'  --マスタ連携フラグ
                         || lv_cooperate_flag                                        || '","'  --固定（マスタ連携フラグ）
                         || data_rec.batch_proc_status                               || '","'  --バッチ処理ステータス
                         || lv_batch_proc_status                                     || '","'  --固定（バッチ処理ステータス）
                         || data_rec.last_updated_by                                 || '","'  --最終更新者
                         || cn_last_updated_by                                       || '","'
                         || data_rec.last_update_date                                || '","'  --最終更新日
                         || TO_CHAR(cd_last_update_date,'YYYY/MM/DD HH24:MI:SS')     || '","'
                         || data_rec.last_update_login                               || '","'  --最終更新ログイン
                         || cn_last_update_login                                     || '","'
                         || data_rec.request_id                                      || '","'  --要求ID
                         || cn_request_id                                            || '","'
                         || data_rec.program_application_id                          || '","'  --ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑ･ｱﾌﾟﾘｹｰｼｮﾝID
                         || cn_program_application_id                                || '","'
                         || data_rec.program_id                                      || '","'  --ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑID
                         || cn_program_id                                            || '","'
                         || data_rec.program_update_date                             || '","'  --ﾌﾟﾛｸﾞﾗﾑ更新日
...
--
      ...
      IF ( ( lv_err_flg <> 'Y' ) AND ( lv_exec_flag = '1' ) )  THEN
        ...
        SET    xcm.cooperate_flag          = lv_cooperate_flag               --固定(マスタ連携フラグ)
              ,xcm.batch_proc_status       = lv_batch_proc_status            --固定(バッチ処理ステータス)
              ,xcm.last_updated_by         = cn_last_updated_by              --固定(最終更新者)
              ,xcm.last_update_date        = cd_last_update_date             --固定(最終更新日)
              ,xcm.last_update_login       = cn_last_update_login            --固定(最終更新ログイン)
              ,xcm.request_id              = cn_request_id                   --固定(要求ID)
              ,xcm.program_application_id  = cn_program_application_id       --固定(ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑ･ｱﾌﾟﾘｹｰｼｮﾝID)
              ,xcm.program_id              = cn_program_id                   --固定(ｺﾝｶﾚﾝﾄ･ﾌﾟﾛｸﾞﾗﾑID)
              ,xcm.program_update_date     = cd_program_update_date          --固定(ﾌﾟﾛｸﾞﾗﾑ更新日)
        WHERE  xcm.contract_management_id  = data_rec.contract_management_id
        ;
--
        -- 成功件数カウント
