...
##  2021/12/23    3.0   Shun.Takenami    障害番号[E_本稼動_17774]               ##
##                                       ・環境依存値の修正                     ##
##                                       ・IaaS化に伴いCONCSUBの呼び出しを      ##
##                                         submit_requestに変更                 ##
##                                       ・SQL-Loader "???????????"文字列を     ##
##                                         "xxxxxxxxxxx"に変更                  ##
##                                                                              ##
...
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami START
#L_logpath="/var/log/jp1/${L_envname}"                             #ログファイルパス
L_logpath="/var/EBS/jp1/${L_envname}/log"                         #ログファイルパス
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami END
#2014/11/25 ADD Ver.1.14 BY Daisuke.Nemoto END
...
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami START
#L_tmppath="/tmp"       #一時ファイルパス[環境依存値]
L_tmppath="/xebst2/`whoami`/`whoami`comn/temp"       #一時ファイルパス[環境依存値]
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami END
...
# 2021/12/23 ADD Ver.3.0 BY Shun.Takenami START
L_path_spo_log="${L_tmppath}/${L_tmpbase}.lst.tmp"    #一時ファイル（SQL実行結果）
# 2021/12/23 ADD Ver.3.0 BY Shun.Takenami END
...
# 2021/12/23 UPDATE Ver.3.0 BY Shun.Takenami START
# コンカレントパラメータ取得
  _idx=1
  L_paracount=`expr ${L_paracount} - 5`

  while [ ${_idx} -le ${L_paracount} ]
  do
    if [ ${1} != \"\" ]
    then
      L_conc_param[${_idx}]=${1}
...
# 2021/12/23 UPDATE Ver.3.0 BY Shun.Takenami END
...
# 2021/12/23 UPDATE Ver.3.0 BY Shun.Takenami START
  ...
  #----------------------------------------------------------------------------
  # ユーザIDの取得sql作成
  cat << SQL_BLOCK > ${L_path_sql_log} 2>&1
  SELECT 'L_user_id="' || USER_ID || '"'
  FROM FND_USER
  WHERE USER_NAME = '${L_para_user}'
  /
SQL_BLOCK

  # ユーザIDの取得sql実行
  sqlplus -s /NOLOG << SQLEOF >> ${L_std_out} 2>&1
  set HEADING OFF
  WHENEVER SQLERROR EXIT ${C_ret_code_eror}
  connect apps/apps
  spool ${L_path_spo_log}
  @${L_path_sql_log}
  spool off
SQLEOF
  ...
  if [ ${?} -ne 0 ]
  then
    output_log "The specified user name does not exist.(USER_NAME : ${L_para_user} )"
    return ${C_ret_code_eror}
  fi
  ...
  if [ ${?} -ne 0 ]
  then
    output_log "Don't remove SQL tempfile.(Temp File : ${L_path_sql_log} )"
    return ${C_ret_code_eror}
  fi
  ...
  if [ ${?} -ne 0 ]
  then
    output_log "Don't read spoolfile.(Spool File : ${L_path_spo_log} )"
    return ${C_ret_code_eror}
  fi
  ...
  if [ ${?} -ne 0 ]
  then
    output_log "Don't remove spoolfile.(pool File : ${L_path_spo_log} )"
    return ${C_ret_code_eror}
  fi

  if [ "${L_user_id}" = "" ]
  then
    output_log "The specified user ID is invalid.(User ID : ${L_user_id} )"
    return ${C_ret_code_eror}
  fi

  #----------------------------------------------------------------------------
  # 職責ID、アプリケーションID取得sql作成
  cat << SQL_BLOCK > ${L_path_sql_log} 2>&1
...
SQLEOF
  ...
  if [ ${L_resp_id} = "" ]
  then
    output_log "The specified Responsibility ID is invalid.(User ID : ${L_resp_id} )"
    return ${C_ret_code_eror}
  fi
  ...
  if [ "${L_resp_appl_id}" = "" ]
  then
    output_log "The specified Responsibility Application ID is invalid.(User ID : ${L_resp_appl_id} )"
    return ${C_ret_code_eror}
  fi
...
SQL_BLOCK
  ...
  while [ ${_idx} -le ${L_paracount} ]
  do
    echo "    ,argument${_idx} =>'${L_conc_param[${_idx}]}'" >>  ${L_path_sql_log}
    _idx=`expr  ${_idx} + 1`
  done
...
SQL_BLOCK
  ...
  # コンカレント実行sql実行
  sqlplus -s /NOLOG << SQLEOF >> ${L_std_out} 2>${L_err_out}
  set HEADING OFF
  WHENEVER SQLERROR EXIT ${C_ret_code_eror}
  connect apps/apps
  spool ${L_path_spo_log}
...
EOF
  ...
  # リクエストID設定
  L_reqid=`grep L_reqid ${L_path_spo_log} | awk -F= '{print $2}'`
...
# 2021/12/23 UPDATE Ver.3.0 BY Shun.Takenami END
# 2021/12/23 DELETE Ver.3.0 BY Shun.Takenami START
#
...
#  fi
##2009/04/15 UPDATE Ver.1.7 BY Masayuki.Sano END
# 2021/12/23 DELETE Ver.3.0 BY Shun.Takenami END
...
SQL_LOADER_EXECUTE()
{
  ...
  #"???????????"を"xxxxxxxxxxx" へ変更（2021/12/23 E_本稼動_17774対応）

  G_path_nas_tmp=`echo "${G_path_nas}" | sed 's/\\//\\\\\\//g'`
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami START
#  sed -e 's/\?\?\?\?\?\?\?\?\?\?\?/'"${G_path_nas_tmp}"'/g' "${G_path_sldr}" > "${L_path_ou_sldr}"
  sed -e 's/xxxxxxxxxxx/'"${G_path_nas_tmp}"'/g' "${G_path_sldr}" > "${L_path_ou_sldr}"
# 2021/12/23 MOD Ver.3.0 BY Shun.Takenami END
