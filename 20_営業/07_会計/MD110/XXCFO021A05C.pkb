...
--
  ...
  -- ユーザー定義グローバル定数
  -- ===============================
  cv_pkg_name      CONSTANT VARCHAR2(100) := 'XXCFO021A05C'; -- パッケージ名
  -- メッセージ
...
--
  -- メッセージ(トークン)
  cv_msg_cfo_11123            CONSTANT VARCHAR2(500) := 'APP-XXCFO1-11123';   -- 日本語文字列(「受払取引（出荷）未連携テーブル」)
  ...
  cv_msg_cfo_11131            CONSTANT VARCHAR2(500) := 'APP-XXCFO1-11131';   -- 日本語文字列(「受払取引（出荷）情報」)
  cv_msg_cfo_11132            CONSTANT VARCHAR2(500) := 'APP-XXCFO1-11132';   -- 日本語文字列(「生産システム」）
  cv_msg_cfo_11133            CONSTANT VARCHAR2(500) := 'APP-XXCFO1-11133';   -- 日本語文字列(「受払（出荷）」）
--
  -- トークン
...
--
  ...
  cv_lookup_item_chk_ship    CONSTANT VARCHAR2(30) := 'XXCFO1_ELECTRIC_ITEM_CHK_SHIP';       --電子帳簿項目チェック（受払取引（出荷））
--
...
--
  --プロファイル
  cv_data_filepath            CONSTANT VARCHAR2(100) := 'XXCFO1_ELECTRIC_BOOK_DATA_FILEPATH'; -- 電子帳簿受払取引（出荷）データファイル格納パス
  ...
  cv_add_filename             CONSTANT VARCHAR2(100) := 'XXCFO1_ELECTRIC_BOOK_SHIP_DATA_FILENAME'; -- 電子帳簿受払取引（出荷）データファイル名
--
  --実行モード
...
--
  ...
  gv_tbl_nm_wait_coop         VARCHAR2(50);    -- 日本語文字列(「受払取引（出荷）未連携テーブル」)
  ...
  gv_msg_shipment_info        VARCHAR2(50);    -- 日本語文字列(「受払取引（出荷）情報」)
  gv_je_source_mfg            VARCHAR2(50);    -- 日本語文字列(「生産システム」)
  gv_je_category_shipment     VARCHAR2(50);    -- 日本語文字列(「受払（出荷）」)
--
  -- テーブル型
...
--
    gv_tbl_nm_wait_coop := 
      SUBSTRB(xxccp_common_pkg.get_msg(iv_application => cv_xxcfo_appl_name   -- アプリケーション短縮名
                                      ,iv_name        => cv_msg_cfo_11123     -- メッセージコード
                                      )
             ,1
             ,5000
             );
...
--
    gv_msg_shipment_info := 
      SUBSTRB(xxccp_common_pkg.get_msg(iv_application => cv_xxcfo_appl_name   -- アプリケーション短縮名
                                      ,iv_name        => cv_msg_cfo_11131     -- メッセージコード
                                      )
             ,1
             ,5000
             );
...
--
    gv_je_category_shipment := 
      SUBSTRB(xxccp_common_pkg.get_msg(iv_application => cv_xxcfo_appl_name   -- アプリケーション短縮名
                                      ,iv_name        => cv_msg_cfo_11133     -- メッセージコード
                                      )
             ,1
             ,5000
             );
...
--
    ...
    WHEN global_lock_expt THEN
      ...
      ov_errmsg               := xxccp_common_pkg.get_msg(
        ...
        iv_token_value1       => gv_tbl_nm_wait_coop    -- 受払取引（出荷）未連携テーブル
      );
      ov_errbuf  := SUBSTRB(cv_pkg_name||cv_msg_cont||cv_prg_name||cv_msg_part||ov_errmsg,1,5000);
      ov_retcode := cv_status_error;
--
...
--
      ...
      FOR i IN 1 .. g_wait_coop_m_rec.COUNT LOOP
        ...
        IF ( g_data_tab(2)  = g_wait_coop_m_rec(i).order_header_id   -- 受注ヘッダアドオンID
         ...
         AND g_data_tab(80) = g_wait_coop_m_rec(i).record_type_code  -- 指示／実績区分
...
--
        ...
        BEGIN
          ...
          AND     gjh.period_name            = g_data_tab(91)        -- A-5で取得した会計期間
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          lv_errmsg := SUBSTRB( xxccp_common_pkg.get_msg( cv_xxcfo_appl_name    -- XXCFO
                                                         ...
                                                         ,gv_tbl_nm_wait_coop   -- 受払取引（出荷）未連携テーブル
...
--
    ...
    cv_shipping_sikyu_class_3 CONSTANT VARCHAR2(1)  := '3';        -- 出荷支給区分：3（倉替返品）
    ...
    cv_rec_type_cd_20         CONSTANT VARCHAR2(2)  := '20';       -- レコードタイプ：20（出荷実績）
...
--
    ...
    IS
      SELECT /*+ LEADING(xoha otta ooha xola oola wdd itp xmld) 
            ...
            ,xola.order_line_id                           AS order_line_id            -- 受注明細アドオンID
            ,ottt.name                                    AS ship_tran_type_name      -- 出庫形態名_出荷
            ,xoha.request_no                              AS request_no               -- 依頼No
            ,xoha.req_status                              AS req_status               -- ステータスコード
            ,xoha.notif_status                            AS notif_status             -- 通知ステータスコード
            ,xoha.head_sales_branch                       AS head_sales_branch_code   -- 管轄拠点コード
            ,xca2v_branch.party_name                      AS head_sales_branch_name   -- 管轄拠点名
            ,xoha.deliver_to                              AS deliver_to               -- 配送先コード
            ,xoha.result_deliver_to                       AS result_deliver_to        -- 配送先コード_実績
            ,xps2v_deli.party_site_name                   AS deliver_to_name          -- 配送先名
            ,xps2v_res_deli.party_site_name               AS result_deliver_to_name   -- 配送先名_実績
            ,xoha.customer_code                           AS customer_code            -- 顧客コード
            ,xca2v_cust.party_short_name                  AS customer_name            -- 顧客名
            ,xoha.deliver_from                            AS deliver_from             -- 出庫元コード
            ,mil.description                              AS deliver_from_name        -- 出庫元名
            ,TO_CHAR(xoha.schedule_ship_date
                    ,cv_date_format_ymd)                  AS schedule_ship_date       -- 出庫予定日
            ,TO_CHAR(xoha.shipped_date
                    ,cv_date_format_ymd)                  AS shipped_date             -- 出庫日
            ,TO_CHAR(xoha.schedule_arrival_date
                    ,cv_date_format_ymd)                  AS schedule_arrival_date    -- 着荷予定日
            ,TO_CHAR(xoha.arrival_date
                    ,cv_date_format_ymd)                  AS arrival_date             -- 着日
            ,flv_time_from.meaning                        AS time_from                -- 時間指定FROM
            ,flv_time_to.meaning                          AS time_to                  -- 時間指定TO
            ,xoha.delivery_no                             AS delivery_no              -- 配送No
            ,xoha.freight_charge_class                    AS freight_charge_class     -- 運賃区分
            ,xoha.freight_carrier_code                    AS freight_carrier_code     -- 運送業者コード
            ,xoha.result_freight_carrier_code             AS res_freight_carrier_code -- 運送業者コード_実績
            ...
            ,xoha.cust_po_number                          AS cust_po_number           -- 顧客発注番号
            ,xoha.collected_pallet_qty                    AS collected_pallet_qty     -- パレット回収枚数
            ,xoha.shipping_method_code                    AS shipping_method_code     -- 配送区分コード
            ,xoha.result_shipping_method_code             AS res_shipping_method_code -- 配送区分コード_実績
            ,flv_ship_methd.meaning                       AS shipping_method_name     -- 配送区分名
            ,flv_res_ship_methd.meaning                   AS res_shipping_method_name -- 配送区分名_実績
            ,xoha.mixed_no                                AS mixed_no                 -- 混載元No
            ,xoha.no_cont_freight_class                   AS no_cont_freight_class    -- 契約外運賃区分
            ,xoha.transfer_location_code                  AS transfer_location_code   -- 振替先コード
            ,xl2v.location_name                           AS transfer_location_name   -- 振替先名
            ,xoha.shipping_instructions                   AS description              -- 摘要
            ,xoha.confirm_request_class                   AS confirm_request_class    -- 物流担当確認依頼区
            ,xoha.weight_capacity_class                   AS wt_capa_class            -- 重量容積区分
            ,xola.request_item_code                       AS request_item_code        -- 品目コード
            ,xim2v_req.item_short_name                    AS request_item_name        -- 品目名称
            ,NVL(xola.uom_code
                ,xim2v_req.item_um)                       AS uom_code                 -- 単位
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN xola.pallet_quantity * -1
                       ELSE xola.pallet_quantity
                  END                                     AS pallet_quantity          -- パレット数
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN xola.layer_quantity * -1
                       ELSE xola.layer_quantity
                  END                                     AS layer_quantity           -- 段数
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN xola.case_quantity * -1
                       ELSE xola.case_quantity
                  END                                     AS case_quantity            -- ケース数
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
            ...
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                  ...
                  END                                     AS based_request_quantity   -- 拠点依頼数量
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
            ...
            ,TO_CHAR(xola.designated_production_date
                    ,cv_date_format_ymd)                  AS designated_product_date  -- 指定製造日
            ,ROUND(xola.weight  ,3)                       AS weight                   -- 合計重量
            ,ROUND(xola.capacity,0)                       AS capacity                 -- 合計容積
            ,xola.shipping_item_code                      AS shipping_item_code       -- 振替品目コード
            ,xim2v_ship.item_short_name                   AS shipping_item_name       -- 振替品目名称
            ,NVL(xoha.pallet_sum_quantity
                ,xoha.real_pallet_quantity)               AS real_pallet_quantity     -- パレット合計枚数
            ,xoha.based_weight                            AS based_weight             -- 基本重量
            ,xoha.based_capacity                          AS based_capacity           -- 基本容積
            ,ROUND(xoha.sum_weight  ,3)                   AS sum_weight               -- 合計重量（総合計）
            ,ROUND(xoha.sum_capacity,0)                   AS sum_capacity             -- 合計容積（総合計）
            ,xoha.loading_efficiency_weight               AS loading_efficiency_wgt   -- 重量積載効率
            ,xoha.loading_efficiency_capacity             AS loading_efficiency_capa  -- 容積積載効率
            ,itp.reason_code                              AS reason_code              -- 事由コード
            ,srct.reason_desc1                            AS reason_name              -- 事由名
            ,xcs.transaction_type                         AS carriers_scd_tran_type   -- 処理種別
            ,xcs.freight_charge_type                      AS freight_charge_type_code -- 運賃形態コード
            ,flv_freight_charge.meaning                   AS freight_charge_type_name -- 運賃形態名
            ,xcs.auto_process_type                        AS auto_process_type        -- 自動配車対象区分
            ,ottt_carr.name                               AS carr_tran_type_name      -- 出庫形態名_配車
            ,ROUND(xcs.sum_loading_weight  ,0)            AS sum_loading_weight       -- 積載重量合計
            ,ROUND(xcs.sum_loading_capacity,0)            AS sum_loading_capacity     -- 積載容積合計
            ,ROUND(xoha.sum_weight  ,3)                   AS sum_weight               -- 積載重量
            ,ROUND(xoha.sum_capacity,0)                   AS sum_capacity             -- 積載容積
            ,xoha.mixed_ratio                             AS mixed_ratio              -- 混載率
            ,xoha.slip_number                             AS slip_number              -- 送り状No
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN xoha.small_quantity * -1
                       ELSE xoha.small_quantity
                  END                                     AS small_quantity           -- 小口個数
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN xoha.label_quantity * -1
                       ELSE xoha.label_quantity
                  END                                     AS label_quantity           -- ラベル枚数
            ,xoha.mixed_sign                              AS mixed_sign               -- 混載記号
            ,xmld.record_type_code                        AS record_type_code         -- 指示／実績区分
            ...
            ,ilm.attribute6                               AS package_quantity         -- 在庫入数
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       ...
                       ELSE DECODE(xmld.record_type_code
                                  ...
                                  ,xmld.actual_quantity
                                  ,NULL)
                  END                                     AS indication_quantity      -- 指示数量
            ,CASE WHEN otta.order_category_code  = cv_order_retrun
                       THEN DECODE(xmld.record_type_code
                                  ,cv_rec_type_cd_20
                                  ,xmld.actual_quantity
                       ...
                       ELSE DECODE(xmld.record_type_code
                                  ...
                                  ,xmld.actual_quantity
                                  ,NULL)
                  END                                     AS actual_quantity          -- 実績数量
            ,ilm.lot_id                                   AS lot_id                   -- ロットID
            ,ilm.attribute7                               AS stock_price              -- 在庫単価
      ...
      FROM   oe_order_headers_all        ooha                     --受注ヘッダ(標準)
            ...
            ,oe_transaction_types_tl     ottt                     --受注タイプ(言語別)
            ,xxcmn_cust_accounts2_v      xca2v_branch             --顧客ビュー（管轄拠点）
            ,xxcmn_party_sites2_v        xps2v_deli               --パーティサイトビュー（出荷先）
            ,xxcmn_party_sites2_v        xps2v_res_deli           --パーティサイトビュー（出荷先_実績）
            ,xxcmn_cust_accounts2_v      xca2v_cust               --顧客ビュー（顧客）
            ...
            ,fnd_lookup_values           flv_ship_methd           --クイックコード（配送区分）
            ,fnd_lookup_values           flv_res_ship_methd       --クイックコード（配送区分＿実績）
            ,xxcmn_locations2_v          xl2v                     --事業所ビュー
            ,fnd_lookup_values           flv_wt_capa_class        --クイックコード（重量容積区分）
            ,xxcmn_item_mst2_v           xim2v_req                --OPM品目情報VIEW2（依頼品目）
            ,xxcmn_item_mst2_v           xim2v_ship               --OPM品目情報VIEW2（振替品目）
            ,xxwsh_carriers_schedule     xcs                      --配車配送計画（アドオン）
            ,fnd_lookup_values           flv_freight_charge       --クイックコード(運賃形態)
            ,oe_transaction_types_all    otta_carr                --受注タイプ（配車）
            ,oe_transaction_types_tl     ottt_carr                --受注タイプ(言語別) （配車）
      ...
      AND    xmld.document_type_code           = cv_document_type_10  --文書タイプ(アドオン)：10（出荷依頼）
      ...
      AND    itp.doc_type                      = ct_doc_type_omso               -- 文書タイプ：OMSO
      AND    itp.completed_ind                 = ct_completed_ind_1             -- 完了フラグ：1
      ...
      AND    NVL(xola.delete_flag ,cv_flag_n)  = cv_flag_n                 --明細削除フラグ：N
      AND    xoha.order_type_id                = otta.transaction_type_id
      AND    otta.attribute1                   = cv_shipping_sikyu_class_3 --出荷支給区分：3（倉替返品）
      AND    otta.attribute4                   = cv_adjs_class_1           --在庫調整区分：1（在庫調整以外）
      AND    otta.transaction_type_id          = ottt.transaction_type_id
      AND    ottt.language                     = cv_lang
      AND    xoha.head_sales_branch            = xca2v_branch.party_number
      ...
      AND    itp.doc_type                      = ct_doc_type_porc                       -- 文書タイプ：PORC
      AND    itp.completed_ind                 = ct_completed_ind_1                     -- 完了フラグ：1
      AND    itp.reason_code                   = srct.reason_code(+)
      AND    srct.language(+)                  = cv_lang
      AND    xoha.arrival_date                 BETWEEN TO_DATE(gv_period_name,cv_date_format_ym)
      ...
      SELECT /*+ LEADING(xoha otta ooha xola oola wdd itp xmld) 
            ...
            ,xca2v_branch.party_name                      AS head_sales_branch_name   -- 管轄拠点名
            ,xoha.deliver_to                              AS deliver_to               -- 配送先コード
            ,NVL(xoha.result_deliver_to,'0')              AS result_deliver_to        -- 配送先コード_実績
            ,xps2v_deli.party_site_name                   AS deliver_to_name          -- 配送先名
            ,xps2v_res_deli.party_site_name               AS result_deliver_to_name   -- 配送先名_実績
            ,xoha.customer_code                           AS customer_code            -- 顧客コード
            ,xca2v_cust.party_short_name                  AS customer_name            -- 顧客名
      ...
      AND    ottt_carr.language(+)             = cv_lang
      AND    xola.order_line_id                = xmld.mov_line_id
      AND    xmld.document_type_code           = cv_document_type_30  --文書タイプ(アドオン)：30（支給指示）
      AND    xmld.item_id                      = itp.item_id
      AND    xmld.lot_id                       = itp.lot_id
      AND    xmld.item_id                      = ilm.item_id(+)
      AND    xmld.lot_id                       = ilm.lot_id(+)
      ...
      AND    NVL(xola.delete_flag ,cv_flag_n)  = cv_flag_n                 --明細削除フラグ：N
      AND    xoha.order_type_id                = otta.transaction_type_id
      AND    otta.attribute4                  <> cv_adjs_class_2           --在庫調整区分：2
      AND    otta.transaction_type_id          = ottt.transaction_type_id
      AND    ottt.language                     = cv_lang
      AND    xoha.head_sales_branch            = xca2v_branch.party_number(+)
      AND    xoha.arrival_date                 BETWEEN NVL(xca2v_branch.start_date_active, xoha.arrival_date)
      ...
      AND    ottt_carr.language(+)             = cv_lang
      AND    xola.order_line_id                = xmld.mov_line_id
      AND    xmld.document_type_code           = cv_document_type_30  --文書タイプ(アドオン)：10（支給依頼）
      AND    xmld.item_id                      = itp.item_id
      AND    xmld.lot_id                       = itp.lot_id
      AND    xmld.item_id                      = ilm.item_id(+)
      AND    xmld.lot_id                       = ilm.lot_id(+)
...
--
    ...
    IS
      ...
      SELECT /*+ LEADING(xswc xoha otta ooha xola oola wdd itp xmld) 
            ...
            ,ilm.attribute7                               AS stock_price              -- 在庫単価
            ,gv_transfer_date                             AS interface_datetime       -- 連携日時
            ,xswc.period_name                             AS period_name              -- 会計期間
      ...
      FROM   oe_order_headers_all        ooha                     --受注ヘッダ(標準)
            ...
            ,ic_tran_pnd                 itp                      --OPM保留在庫トランザクション表
            ,sy_reas_cds_tl              srct                     --事由コード表(言語別）
            ,xxcfo_shipment_wait_coop    xswc                     --受払取引（出荷）未連携
      WHERE  ooha.org_id                       = gn_mfg_org_id
      AND    ooha.header_id                    = oola.header_id
      AND    ooha.header_id                    = xoha.header_id
      AND    xoha.latest_external_flag         = cv_flag_y                 --最新フラグ：Y
      ...
      AND    ottt_carr.language(+)             = cv_lang
      AND    xola.order_line_id                = xmld.mov_line_id
      AND    xmld.document_type_code           = cv_document_type_30  --文書タイプ(アドオン)：30（支給依頼）
      AND    xmld.item_id                      = itp.item_id
      AND    xmld.lot_id                       = itp.lot_id
      AND    xmld.item_id                      = ilm.item_id(+)
      AND    xmld.lot_id                       = ilm.lot_id(+)
    ...
    IF ( gv_exec_kbn = cv_exec_manual ) THEN
      ...
      LOOP
        FETCH get_manual_cur INTO g_data_tab(1)  -- 仕訳キー
                                 ,g_data_tab(2)  -- 受注ヘッダアドオンID
                                 ,g_data_tab(3)  -- 受注明細アドオンID
                                 ,g_data_tab(4)  -- 出庫形態名_出荷
                                 ,g_data_tab(5)  -- 依頼No
                                 ,g_data_tab(6)  -- ステータスコード
                                 ,g_data_tab(7)  -- 通知ステータスコード
                                 ,g_data_tab(8)  -- 管轄拠点コード
                                 ,g_data_tab(9)  -- 管轄拠点名
                                 ,g_data_tab(10) -- 配送先コード
                                 ,g_data_tab(11) -- 配送先コード_実績
                                 ,g_data_tab(12) -- 配送先名
                                 ...
                                 ,g_data_tab(20) -- 着荷予定日
                                 ,g_data_tab(21) -- 着日
                                 ,g_data_tab(22) -- 時間指定FROM
                                 ,g_data_tab(23) -- 時間指定TO
                                 ,g_data_tab(24) -- 配送No
                                 ,g_data_tab(25) -- 運賃区分
                                 ,g_data_tab(26) -- 運送業者コード
                                 ,g_data_tab(27) -- 運送業者コード_実績
                                 ,g_data_tab(28) -- 運送業者名
                                 ,g_data_tab(29) -- 運送業者名_実績
                                 ,g_data_tab(30) -- 顧客発注番号
                                 ,g_data_tab(31) -- パレット回収枚数
                                 ,g_data_tab(32) -- 配送区分コード
                                 ,g_data_tab(33) -- 配送区分コード_実績
                                 ,g_data_tab(34) -- 配送区分名
                                 ,g_data_tab(35) -- 配送区分名_実績
                                 ,g_data_tab(36) -- 混載元No
                                 ,g_data_tab(37) -- 契約外運賃区分
                                 ,g_data_tab(38) -- 振替先コード
                                 ,g_data_tab(39) -- 振替先名
                                 ,g_data_tab(40) -- 摘要
                                 ,g_data_tab(41) -- 物流担当確認依頼区分
                                 ,g_data_tab(42) -- 重量容積区分
                                 ,g_data_tab(43) -- 品目コード
                                 ,g_data_tab(44) -- 品目名称
                                 ,g_data_tab(45) -- 単位
                                 ,g_data_tab(46) -- パレット数
                                 ...
                                 ,g_data_tab(53) -- 合計重量
                                 ,g_data_tab(54) -- 合計容積
                                 ,g_data_tab(55) -- 振替品目コード
                                 ,g_data_tab(56) -- 振替品目名称
                                 ,g_data_tab(57) -- パレット合計枚数
                                 ,g_data_tab(58) -- 基本重量
                                 ,g_data_tab(59) -- 基本容積
                                 ,g_data_tab(60) -- 合計重量（総合計）
                                 ,g_data_tab(61) -- 合計容積（総合計）
                                 ,g_data_tab(62) -- 重量積載効率
                                 ,g_data_tab(63) -- 容積積載効率
                                 ,g_data_tab(64) -- 事由コード
                                 ,g_data_tab(65) -- 事由名
                                 ,g_data_tab(66) -- 処理種別
                                 ,g_data_tab(67) -- 運賃形態コード
                                 ,g_data_tab(68) -- 運賃形態名
                                 ,g_data_tab(69) -- 自動配車対象区分
                                 ,g_data_tab(70) -- 出庫形態名_配車
                                 ,g_data_tab(71) -- 積載重量合計
                                 ,g_data_tab(72) -- 積載容積合計
                                 ,g_data_tab(73) -- 積載重量
                                 ,g_data_tab(74) -- 積載容積
                                 ,g_data_tab(75) -- 混載率
                                 ,g_data_tab(76) -- 送り状No
  ...
  EXCEPTION
    ...
    WHEN get_data_expt THEN
      ...
      ov_errmsg               := xxccp_common_pkg.get_msg(
        ...
        iv_name               => cv_msg_cfo_10025,
        iv_token_name1        => cv_tkn_get_data,
        iv_token_value1       => gv_msg_shipment_info  -- 受払取引（出荷）情報
      );
      FND_FILE.PUT_LINE(
         which  => FND_FILE.OUTPUT
        ,buff   => ov_errmsg
...
--
      ...
      FOR i IN 1 .. g_row_id_tab.COUNT LOOP
        BEGIN
          DELETE FROM xxcfo_shipment_wait_coop xswc -- 受払取引（出荷）未連携テーブル
