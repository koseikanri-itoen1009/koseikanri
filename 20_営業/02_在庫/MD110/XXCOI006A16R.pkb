 ...
 *  2016/10/20    1.11  N.Watanabe       障害対応E_本稼動_13807
 *
 *****************************************************************************************/
--
--#######################  固定グローバル定数宣言部 START   #######################
...
-- == 2009/07/14 V1.4 Added START ===============================================================
  gd_target_date            DATE;
-- == 2016/10/20 V1.11 Added START =================================================================
  gn_process_cnt            NUMBER;                             -- 拠点別受払データ出力時の処理連番用
-- == 2016/10/20 V1.11 Added END   =================================================================
...
-- == 2015/03/03 V1.10 Modified END   ===============================================================
    ...
    -- ===============================
-- == 2016/10/20 V1.11 Added START ===============================================================
    -- 出力区分が60:専門店計、70:百貨店計、80:直営店計、90:全社計の場合
    IF (gv_param_output_kbn  IN (cv_output_kbn_60, cv_output_kbn_70, cv_output_kbn_80, cv_output_kbn_90)) THEN
-- == 2016/10/20 V1.11 Added END =================================================================
      -- カーソル判定
-- == 2016/10/20 V1.11 Del START =================================================================
...
-- == 2016/10/20 V1.11 Added START ===============================================================
    ...
    ELSIF (gv_param_output_kbn IN (cv_output_kbn_30, cv_output_kbn_40, cv_output_kbn_50)) THEN
      ...
      THEN
        -- 拠点別（商品部）、かつ、拠点が設定されていない場合
        lv_base_cur_kbn  :=  cv_cur_kbn_3;
        --
      END IF;
      --
      IF (lv_base_cur_kbn  = cv_cur_kbn_1) THEN      -- 拠点取得パターン１
        OPEN  svf_base_cur1;
        FETCH svf_base_cur1  INTO  svf_base_rec;
      --
      ELSIF (lv_base_cur_kbn  = cv_cur_kbn_2) THEN   -- 拠点取得パターン２
        OPEN  svf_base_cur2;
        FETCH svf_base_cur2  INTO  svf_base_rec;
      --
      ELSIF (lv_base_cur_kbn  = cv_cur_kbn_3) THEN   -- 拠点取得パターン３
        OPEN  svf_base_cur3;
      ...
      LOOP
        ...
        CLOSE svf_data_cur5;
        --
        IF (lv_base_cur_kbn  =  cv_cur_kbn_1)  THEN
          FETCH svf_base_cur1  INTO  svf_base_rec;
          EXIT  work_ins_base_loop  WHEN  svf_base_cur1%NOTFOUND;
        --
        ELSIF (lv_base_cur_kbn  =  cv_cur_kbn_2)  THEN
          FETCH svf_base_cur2  INTO  svf_base_rec;
          EXIT  work_ins_base_loop  WHEN  svf_base_cur2%NOTFOUND;
          --
        ELSIF (lv_base_cur_kbn  =  cv_cur_kbn_3)  THEN
          FETCH svf_base_cur3  INTO  svf_base_rec;
...
-- == 2016/10/20 V1.11 Del END   =================================================================
      ...
      ELSIF (svf_data_cur4%ISOPEN) THEN
         CLOSE svf_data_cur4;
-- == 2016/10/20 V1.11 Added START ===============================================================
      ELSIF (svf_data_cur5%ISOPEN) THEN
         CLOSE svf_data_cur5;
      END IF;
