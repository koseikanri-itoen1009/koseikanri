 ...
 *
 * Package Name     : INV_TXN_MANAGER_API
 * Description      : 取引インタフェースの処理の標準機能アドオン
 * MD.050           : 
 * Version          : 1.0
 *
 * Program List
 ...
 * ------------- ----- ---------------- -------------------------------------------------
 *  2009.10.08    1.0   K.Kanada         マイナス在庫許可='Y'の場合は
 *                                       有効在庫のチェック(INV_QUANTITY_TREE_PVT.create_tree)
 *                                       を通らないようにカスタマイズ
 *
 *****************************************************************************************/
--
...
-- Bug 2399354  The tree to be cleared prior to creating a tree to avoid
          ...
          IF (l_debug = 1) THEN
            inv_log_util.trace('<K.K l_progress_indicator> '||l_progress_indicator,l_procedure_name,9);
          END IF;
...
-- Add,Mod 2009.10.08 SCS K.Kanada Start
      BEGIN
        SELECT MP.NEGATIVE_INV_RECEIPT_CODE
        INTO   l_neg_inv_rcpt
        FROM   MTL_PARAMETERS  MP
