...
-- Ver.3.25 H.Futamura ADD START
    ln_bm1_cond_tax_amt            NUMBER         DEFAULT NULL;                 -- 【BM1】消費税_一時格納
    ln_bm2_cond_tax_amt            NUMBER         DEFAULT NULL;                 -- 【BM2】消費税_一時格納
    ln_bm3_cond_tax_amt            NUMBER         DEFAULT NULL;                 -- 【BM3】消費税_一時格納
    ln_bm1_csh_rcpt_dis_amt_tax    NUMBER         DEFAULT NULL;                 -- 【BM1】入金値引消費税_一時格納
    ln_bm2_csh_rcpt_dis_amt_tax    NUMBER         DEFAULT NULL;                 -- 【BM2】入金値引消費税_一時格納
    ln_bm3_csh_rcpt_dis_amt_tax    NUMBER         DEFAULT NULL;                 -- 【BM3】入金値引消費税_一時格納
    ln_electric_tax_amt            NUMBER         DEFAULT NULL;                 --  電気料消費税_一時格納
    ln_electric_tax_amt_var        NUMBER         DEFAULT NULL;                 --  変動電気料消費税_一時格納
    ln_bm1_amt_notax_tmp           NUMBER         DEFAULT NULL;                 -- 【BM1】VDBM(税抜)_消費税計算一時格納
    ln_bm2_amt_notax_tmp           NUMBER         DEFAULT NULL;                 -- 【BM2】VDBM(税抜)_消費税計算一時格納
    ln_bm3_amt_notax_tmp           NUMBER         DEFAULT NULL;                 -- 【BM3】VDBM(税抜)_消費税計算一時格納
    ln_bm1_csh_rcpt_dis_notax_tmp  NUMBER         DEFAULT NULL;                 -- 【BM1】入金値引(税抜)_消費税計算一時格納
    ln_bm2_csh_rcpt_dis_notax_tmp  NUMBER         DEFAULT NULL;                 -- 【BM2】入金値引(税抜)_消費税計算一時格納
    ln_bm3_csh_rcpt_dis_notax_tmp  NUMBER         DEFAULT NULL;                 -- 【BM3】入金値引(税抜)_消費税計算一時格納
    ln_electric_notax_amt_tmp      NUMBER         DEFAULT NULL;                 --  電気料消費税(税抜)_消費税計算一時格納
    ln_electric_notax_amt_var_tmp  NUMBER         DEFAULT NULL;                 --  変動電気料消費税(税抜)_消費税計算一時格納
...
--
    ...
    END IF;
    --==================================================
    -- 3.各VDBM(税込)、入金値引額(税込)が NULL 以外の場合、消費税額を算出します。
    --==================================================
-- Ver.3.20 N.Abe DEL START
--    -- BM1 VDBM(税抜)の設定
...
--      END IF;
--
      -- 端数処理区分が 'NEAREST':四捨五入の場合、電気料消費税の少数点以下を四捨五入
      IF( i_get_sales_data_rec.tax_rounding_rule = cv_tax_rounding_rule_nearest ) THEN
        ln_electric_tax_amt := ROUND( ln_electric_tax_amt );
      -- 端数処理区分が 'UP':切り上げの場合、電気料消費税の小数点以下を切り上げ
      ELSIF ( i_get_sales_data_rec.tax_rounding_rule = cv_tax_rounding_rule_up ) THEN
...
---- Ver.3.23 H.Futamura ADD END
      -- 電気料消費税に定額電気代消費税と変動電気代消費税を合算した値を設定
