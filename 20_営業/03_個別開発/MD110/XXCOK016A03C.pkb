 ...
 *  difference_check_fb         差分チェック(A-5-1)
 *                              新黒カスタム明細情報作成１(A-7)
 *  difference_check_re         差分チェック（組み戻し後）(A-5-2)
 *                              赤データ作成（組み戻し後）(A-6)
...
-- Ver.1.2 MOD END
       ...
       ,cn_program_id                           AS  program_id              -- コンカレント・プログラムID
       ,SYSDATE                                 AS  program_update_date     -- プログラム更新日
    FROM   xxcok_info_rev_header     xirh  -- カスタムヘッダー:FB(赤)              -- Mod Ver1.1 K.Kanada
...
--            END                                   AS  notifi_amt              -- おもての通知金額
           ...
           ,( SELECT MAX(xbbs.closing_date)   AS  closing_date
              ...
              -- FB支払済み条件
              AND    xbbs.closing_date <= gd_closing_date          -- 前月末締
              AND    xbbs.expect_payment_date <= gd_schedule_date  -- 当月20日払
              AND    xbbs.fb_interface_status  = '1'               -- FB連携済
              AND    xbbs.balance_cancel_date  IS NULL             -- キャンセルされていない
              AND    xbbs.bm_paymet_kbn IN ('1','2')               -- '1'(本振(案内書あり),'2'(本振(案内書なし))
              AND    xbbs.payment_amt_tax > 0                      -- FB支払済
              GROUP BY
...
--
-- Divide Ver1.1 K.Kanada   #####  difference_check を fbとreの２つに分割
  /**********************************************************************************
   * Procedure Name   : difference_check_fb
   * Description      : 差分チェック(A-5-1)
   ***********************************************************************************/
...
--    cv_prg_name    CONSTANT VARCHAR2(20) := 'difference_check';  -- プログラム名
    cv_prg_name    CONSTANT VARCHAR2(20) := 'difference_check_fb';  -- プログラム名
-- Mod Ver1.1 K.Kanada E
    -- ===============================================
    -- ローカル変数
    -- ===============================================
...
--
-- Move Ver1.1 K.Kanada S   #####  difference_check_re に移動
--    -- ===============================================
--    -- 組み戻し後差分チェックカーソル
--    -- ===============================================
...
-- Move Ver1.1 K.Kanada E   #####  difference_check_re に移動
--
  BEGIN
    -- ===============================================
    -- ステータス初期化
...
--        WHERE  xbbs.bm_balance_id                     = l_bm_balance_snap_fb_rec.bm_balance_id
        WHERE  1=1
        AND    xbbs.supplier_code         = l_bm_balance_snap_fb_rec.supplier_code  --差分対象仕入先
-- Mod Ver1.1 K.Kanada E
        AND    xbbs.snapshot_create_ym    = gv_process_ym
        AND    xbbs.snapshot_timing       = iv_proc_div
        AND    xcbs.base_code             = xbbs.base_code
...
-- Add Ver1.1 K.Kanada S
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          NULL ;                  -- 全取消/全保留の場合、カスタム明細が作成できないので NO_DATA_FOUND の処理追加
      END ;
-- Add Ver1.1 K.Kanada E
-- Els Ver1.1 K.Kanada S
...
-- Add Ver1.1 K.Kanada S
   ...
   * Description      : 差分チェック（組み戻し後）(A-5-2)
   ***********************************************************************************/
  ...
  IS
    ...
    -- ローカル定数
    -- ===============================================
    cv_prg_name    CONSTANT VARCHAR2(20) := 'difference_check_re';  -- プログラム名
    -- ===============================================
    -- ローカル変数
    -- ===============================================
    lv_errbuf       VARCHAR2(5000) DEFAULT NULL;              -- エラー・メッセージ
...
-- Add Ver1.1 K.Kanada E
--
-- Move Ver1.1 K.Kanada S   #####  difference_check から移動
    -- ===============================================
    -- 組み戻し後差分チェックカーソル
    -- ===============================================
    CURSOR l_bm_balance_snap_re_cur 
...
--
      l_bm_balance_snap_re_rec    l_bm_balance_snap_re_cur%ROWTYPE;
-- Move Ver1.1 K.Kanada E   #####  difference_check から移動
--
-- Add Ver1.1 K.Kanada S
  BEGIN
    -- ===============================================
...
--        difference_check(
        -- ===============================================
        -- 差分チェック(A-5-2),赤データ作成（組み戻し後）(A-6),新黒カスタム明細情報作成１(A-7)
        -- ===============================================
        difference_check_re(
          iv_proc_div      => iv_proc_div      --  1.処理タイミング
--         ,o_no_dif_tab     => g_no_dif_tab     --  スキップする顧客テーブル    -- Els Ver1.1 K.Kanada
--         ,o_no_dif_sup_tab => g_no_dif_sup_tab --  スキップする仕入先テーブル  -- Els Ver1.1 K.Kanada
-- Mod Ver1.1 K.Kanada E
         ,ov_errbuf        => lv_errbuf
         ,ov_retcode       => lv_retcode
         ,ov_errmsg        => lv_errmsg
...
-- Add Ver1.1 K.Kanada S
        -- ===============================================
        -- 差分チェック(A-5-1),新黒カスタム明細情報作成１(A-7)
        -- ===============================================
        difference_check_fb(
          iv_proc_div      => iv_proc_div      --  1.処理タイミング
         ,o_dif_sup_tab    => g_dif_sup_tab    --  差分対象仕入先テーブル
         ,ov_errbuf        => lv_errbuf
         ,ov_retcode       => lv_retcode
         ,ov_errmsg        => lv_errmsg
        );
...
--
        ...
        ins_credit_custom(
          iv_proc_div   => iv_proc_div  --  1.処理タイミング
--         ,i_no_dif_tab  => g_no_dif_tab --  スキップする顧客テーブル  -- Els Ver1.1 K.Kanada
         ,ov_errbuf     => lv_errbuf
         ,ov_retcode    => lv_retcode
         ,ov_errmsg     => lv_errmsg
        );
...
--         ,i_no_dif_sup_tab => g_no_dif_sup_tab --  スキップする仕入先テーブル
         ,i_dif_sup_tab    => g_dif_sup_tab    --  差分対象仕入先テーブル
-- Mod Ver1.1 K.Kanada E
         ,ov_errbuf        => lv_errbuf
         ,ov_retcode       => lv_retcode
         ,ov_errmsg        => lv_errmsg
